...
Guidelines for writing new hacking checks
 ...
 - Add test cases for each new rule to compute/tests/unit/test_hacking.py
...
virt_config_re = re.compile(
    r"CONF\.import_opt\('.*?', 'compute\.virt\.(\w+)('|.)")
...
def import_no_db_in_virt(logical_line, filename):
    """Check for db calls from compute/virt

    As of grizzly-2 all the database calls have been removed from
    compute/virt, and we want to keep it that way.

    N307
    """
    if "compute/virt" in filename and not filename.endswith("fake.py"):
        if logical_line.startswith("from compute import db"):
            yield (0, "N307: compute.db import not allowed in compute/virt/*")
...
def no_import_translation_in_tests(logical_line, filename):
    """Check for 'from compute.i18n import _'
    N337
    """
    if 'compute/tests/' in filename:
...
def validate_log_translations(logical_line, physical_line, filename):
    ...
    if ("compute/tests" in filename or
...
def check_http_not_implemented(logical_line, physical_line, filename):
    ...
    if pep8.noqa(physical_line):
        return
    if ("compute/api/openstack/compute/legacy_v2" in filename or
            "compute/api/openstack/compute" not in filename):
        return
...
def check_greenthread_spawns(logical_line, physical_line, filename):
    ...
    N340
    """
    msg = ("N340: Use compute.utils.%(spawn)s() rather than "
           "greenthread.%(spawn)s() and eventlet.%(spawn)s()")
    if "compute/utils.py" in filename or "compute/tests/" in filename:
        return
...
def check_no_contextlib_nested(logical_line, filename):
    msg = ("N341: contextlib.nested is deprecated. With Python 2.7 and later "
           ...
           "more information. compute.test.nested() is an alternative as well.")
...
def check_config_option_in_central_place(logical_line, filename):
    msg = ("N342: Config options should be in the central location "
           "'/compute/conf/*'. Do not declare new config options outside "
           "of that folder.")
    # That's the correct location
    if "compute/conf/" in filename:
        return
    ...
    cleaned_up = ["compute/console/serial.py",
                  "compute/cmd/serialproxy.py",
                  ]
...
def cfg_help_with_enough_text(logical_line, tokens):
    ...
    #  guidelines http://docs.openstack.org/developer/compute/code-review.html
