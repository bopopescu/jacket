...
"""Jacket common internal object model"""
...
from jacket.objects.cloud_config import fields as obj_fields
...
class JacketObjectRegistry(ovoo_base.VersionedObjectRegistry):
    def registration_hook(self, cls, index):
        # NOTE(danms): This is called when an object is registered,
        # and is responsible for maintaining compute.compute.$OBJECT
        # as the highest-versioned implementation of a given object.
        version = versionutils.convert_version_to_tuple(cls.VERSION)
        if not hasattr(cloud_config, cls.obj_name()):
            setattr(cloud_config, cls.obj_name(), cls)
        else:
            cur_version = versionutils.convert_version_to_tuple(
                getattr(cloud_config, cls.obj_name()).VERSION)
...
class JacketObject(ovoo_base.VersionedObject):
    ...
    # NOTE(danms): This has some minor change between the compute and o.vo
    ...
    # NOTE(danms): This is compute-specific
...
class JacketObjectDictCompat(ovoo_base.VersionedObjectDictCompat):
    def __iter__(self):
...
class JacketTimestampObject(object):
...
class JacketPersistentObject(object):
...
class JacketObjectSerializer(messaging.NoOpSerializer):
    """A JacketObject-aware Serializer.

    This implements the Oslo Serializer interface and provides the
    ability to serialize and deserialize JacketObject entities. Any service
    that needs to accept or return JacketObjects as arguments or result values
    ...
    def _process_object(self, context, objprim):
        try:
            objinst = JacketObject.obj_from_primitive(objprim, context=context)
...
def obj_to_primitive(obj):
    ...
    A JacketObject becomes a dict, and anything that implements ObjectListBase
    becomes a list.
    """
    if isinstance(obj, ObjectListBase):
        return [obj_to_primitive(x) for x in obj]
    elif isinstance(obj, JacketObject):
        result = {}
...
def obj_make_list(context, list_obj, item_cls, db_list, **extra_args):
    ...
    :param:item_cls: The JacketObject class of the compute within the list
