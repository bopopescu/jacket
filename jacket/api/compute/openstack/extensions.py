...
class ExtensionManager(object):
    ...
    See compute/tests/api/openstack/compute/extensions/foxinsocks.py or an
...
class ControllerExtension(object):
    """Extend core controllers of compute OpenStack API.

    Provide a way to extend existing compute OpenStack API core
    controllers.
    """
...
class ResourceExtension(object):
    """Add top level resources to the OpenStack API in compute."""
...
def core_authorizer(api_name, extension_name):
    def authorize(context, target=None, action=None):
        ...
        jacket.compute.policy.enforce(context, act, target)
    return authorize
        ...
        def wrapped(*args, **kwargs):
            try:
                ...
                msg = _('Unexpected API Error. Please report this at '
                    'http://bugs.launchpad.net/compute/ and attach the Nova '
                    'API log if possible.\n%s') % type(exc)
