...
from jacket.api.compute.openstack.compute.schemas import tenant_networks as schema
...
CONF = cfg.CONF
CONF.import_opt('enable_network_quota', 'compute.api.openstack.compute.'
                'legacy_v2.contrib.os_tenant_networks')
CONF.import_opt('use_neutron_default_nets', 'compute.api.openstack.compute.'
                'legacy_v2.contrib.os_tenant_networks')
CONF.import_opt('neutron_default_tenant_id', 'compute.api.openstack.compute.'
                'legacy_v2.contrib.os_tenant_networks')
CONF.import_opt('quota_networks', 'compute.api.openstack.compute.'
                'legacy_v2.contrib.os_tenant_networks')
...
class TenantNetworkController(wsgi.Controller):
    def __init__(self, network_api=None):
        self.network_api = jacket.compute.network.API(skip_policy_check=True)
...
def _sync_networks(context, project_id, session):
    ...
    ctx = ctx.elevated()
    networks = jacket.compute.network.api.API().get_all(ctx)
