...
def validate_policy(context, action):
    try:
        jacket.storage.policy.enforce_action(context, action)
        return True
...
def get_sort_params(params, default_key='created_at', default_dir='desc'):
    ...
    :param params: webob.multidict of request parameters (from
                   storage.api.storage.openstack.wsgi.Request.params)
...
def remove_version_from_href(href):
    ...
    Given: 'http://www.storage.com/v1.1/123'
    Returns: 'http://www.storage.com/123'

    Given: 'http://www.storage.com/v1.1'
    Returns: 'http://www.storage.com'
...
class ViewBuilder(object):
    ...
    def _get_next_link(self, request, identifier, collection_name):
        ...
        url = os.path.join(prefix,
                           request.environ["storage.context"].project_id,
                           collection_name)
