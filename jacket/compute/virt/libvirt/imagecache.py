...
http://wiki.openstack.org/compute-image-cache-management.
...
from jacket.compute.virt import imagecache
...
class ImageCacheManager(imagecache.ImageCacheManager):
    ...
    def _verify_checksum(self, img_id, base_file, create_if_missing=True):
        ...
        # Protect against other compute-computes performing checksums at the same
        ...
        if age < maxage:
            ...
            if remove_lock:
                try:
                    ...
                    # like compute-9e881789030568a317fad9daae82c5b1c65e0d4a
                    # or compute-03d8e206-6500-4d91-b47d-ee74897f9b4e
                    # according to the original file name
                    lockutils.remove_external_lock_file(lock_file,
                        lock_file_prefix='compute-', lock_path=self.lock_path)
                except OSError as e:
