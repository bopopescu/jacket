...
import jacket.compute.virt.firewall as base_firewall
...
class NWFilterFirewall(base_firewall.FirewallDriver):
    ...
    libvirt's nwfilter.
    all instances get a filter ("compute-base") applied. This filter
    ...
    def __init__(self, host, **kwargs):
        ...
        :param host: compute.virt.libvirt.host.Host instance
    ...
    def nova_no_nd_reflection_filter(self):
        ...
        Detection(DAD).
        """
        uuid = self._get_filter_uuid('compute-no-nd-reflection')
        return '''<filter name='compute-no-nd-reflection' chain='ipv6'>
                  <!-- no nd reflection -->
    ...
    def nova_dhcp_filter(self):
        ...
        """
        uuid = self._get_filter_uuid('compute-allow-dhcp-server')
        return '''<filter name='compute-allow-dhcp-server' chain='ipv4'>
                    <uuid>%s</uuid>
    ...
    def get_base_filter_list(self, instance, allow_dhcp):
        ...
        if pipelib.is_vpn_image(instance.image_ref):
            base_filter = 'compute-vpn'
        elif allow_dhcp:
            base_filter = 'compute-base'
        else:
            base_filter = 'compute-nodhcp'
        return [base_filter]
    ...
    def _ensure_static_filters(self):
        ...
        filter_set.append('compute-no-nd-reflection')
        self._define_filter(self._filter_container('compute-nodhcp', filter_set))
        filter_set.append('allow-dhcp-server')
        self._define_filter(self._filter_container('compute-base', filter_set))
        self._define_filter(self._filter_container('compute-vpn',
                                                   ['allow-dhcp-server']))
    ...
    def _instance_filter_name(instance, nic_id=None):
        if not nic_id:
            return 'compute-instance-%s' % (instance.name)
        return 'compute-instance-%s-%s' % (instance.name, nic_id)

    def instance_filter_exists(self, instance, network_info):
        """Check compute-instance-instance-xxx exists."""
        for vif in network_info:
            nic_id = vif['address'].replace(':', '')
            instance_filter_name = self._instance_filter_name(instance, nic_id)
            try:
...
class IptablesFirewallDriver(base_firewall.IptablesFirewallDriver):
    def __init__(self, execute=None, **kwargs):
        ...
        maps to an instance of the compute.virt.libvirt.host.Host
        class.
        """
