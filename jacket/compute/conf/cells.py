...
that runs a compute-api service, but no compute-compute services. Each
child cell should run all of the typical compute-* services in a regular
Compute cloud except for compute-api. You can think of cells as a normal
...
The compute-cells manager class. This class defines RPC methods that
the local cell may call. This class is NOT used for messages coming
from other cells. That communication is driver-specific.

Communication to other cells happens via the compute.cells.messaging module.
...
* compute_api_class: This option must be set to cells api driver
  for the top-level cell (compute.compute.cells_api.ComputeCellsAPI)
* quota_driver: Disable quota checking for the child cells.
  (compute.quota.NoopQuotaDriver)
"""),
...
On every run of the periodic task, compute cells manager will attempt to
...
compute-cells services do not attempt to sync the same instances in
lockstep.
...
"compute.cells.filters.all_filters" maps to all cells filters
included with compute. As of the Mitaka release the following
...
"compute.cells.weights.all_weighers" maps to all cell weighers
included with compute. As of the Mitaka release the following
