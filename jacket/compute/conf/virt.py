...
firewall_driver = cfg.StrOpt(
    'firewall_driver',
    help="""Firewall driver to use with ``compute-network`` service.

This option only applies when using the ``compute-network`` service. When using
...
* compute.virt.firewall.IptablesFirewallDriver
* compute.virt.firewall.NoopFirewallDriver
* compute.virt.libvirt.firewall.IptablesFirewallDriver
* [...]
...
* ``use_neutron``: This must be set to ``False`` to enable ``compute-network``
  networking
""")
...
* ``firewall_driver``: This must be set to
  ``compute.virt.libvirt.firewall.IptablesFirewallDriver`` to ensure the
...
injected_network_template = cfg.StrOpt(
    ...
    default=paths.basedir_def('compute/virt/interfaces.template'),
