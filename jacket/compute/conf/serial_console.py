...
In order to use this feature, the service ``compute-serialproxy`` needs to run.
...
base_url_opt = cfg.StrOpt('base_url',
        ...
        help="""
The URL an end user would use to connect to the ``compute-serialproxy`` service.

The ``compute-serialproxy`` service is called with this token enriched URL
...
* The IP address must be identical to the address to which the
  ``compute-serialproxy`` service is listening (see option ``serialproxy_host``
  in this section).
...
proxyclient_address_opt = cfg.StrOpt('proxyclient_address',
        default='127.0.0.1',
        help="""
The IP address to which proxy clients (like ``compute-serialproxy``) should
connect to get the serial console of an instance.

This is typically the IP address of the host of a ``compute-compute`` service.
...
serialproxy_host_opt = cfg.StrOpt('serialproxy_host',
        default='0.0.0.0',
        help="""
The IP address which is used by the ``compute-serialproxy`` service to listen
for incoming requests.

The ``compute-serialproxy`` service listens on this IP address for incoming
...
serialproxy_port_opt = cfg.IntOpt('serialproxy_port',
        ...
        max=65535,
        help="""
The port number which is used by the ``compute-serialproxy`` service to listen
for incoming requests.

The ``compute-serialproxy`` service listens on this port number for incoming
connection requests to instances which expose serial console.
