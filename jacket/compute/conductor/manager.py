...
"""Handles database requests from other compute services."""
...
class ConductorManager(manager.Manager):
    ...
    The methods in the base API for compute-conductor are various proxy operations
    performed on behalf of the compute-compute service running on compute nodes.
    Compute nodes are not allowed to directly access the database, so this set
    of methods allows them to get specific work done without locally accessing
    the database.

    The compute-conductor service also exposes an API in the 'compute_task'
...
class ComputeTaskManager(base.Base):
    ...
    This class presents an rpc API for compute-conductor under the 'compute_task'
