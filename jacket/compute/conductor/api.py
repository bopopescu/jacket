...
from jacket.compute.conductor import manager
from jacket.compute.conductor import rpcapi
...
class API(LocalAPI):
    ...
    def wait_until_ready(self, context, early_timeout=10, early_attempts=10):
        ...
        while True:
            ...
            # This may fail the first time around if compute-conductor wasn't
            # running when this service started.
            try:
                self.base_rpcapi.ping(context, '1.21 GigaWatts',
                                      timeout=timeout)
                if has_timedout:
                    LOG.info(_LI('compute-conductor connection '
                                 'established successfully'))
                break
            except messaging.MessagingTimeout:
                has_timedout = True
                LOG.warning(_LW('Timed out waiting for compute-conductor.  '
                                'Is it running? Or did this service start '
                                'before compute-conductor?  '
                                'Reattempting establishment of '
                                'compute-conductor connection...'))


class ComputeTaskAPI(object):
    """ComputeTask API that queues up compute tasks for compute-conductor."""
