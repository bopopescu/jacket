...
linux_net_opts = [
    cfg.MultiStrOpt('dhcpbridge_flagfile',
                    default=['/etc/compute/compute-dhcpbridge.conf'],
    ...
    cfg.StrOpt('dhcpbridge',
               default=paths.bindir_def('compute-dhcpbridge'),
               help='Location of compute-dhcpbridge'),
...
class IptablesTable(object):
    ...
    def add_chain(self, name, wrap=True):
        ...
        so if compute-compute creates a chain named 'OUTPUT', it'll actually
        end up named 'compute-compute-OUTPUT'.
...
class IptablesManager(object):
    ...
    First, compute-filter-top. It's added at the top of FORWARD and OUTPUT. Its
    name is not wrapped, so it's shared between the various compute workers. It's
    ...
    "local" which is jumped to from compute-filter-top.
    ...
    def __init__(self, execute=None):
        ...
        # Add a compute-filter-top chain. It's intended to be shared
        # among the various compute components. It sits at the very top
        # of FORWARD and OUTPUT.
        for tables in [self.ipv4, self.ipv6]:
            tables['filter'].add_chain('compute-filter-top', wrap=False)
            tables['filter'].add_rule('FORWARD', '-j compute-filter-top',
                                      wrap=False, top=True)
            tables['filter'].add_rule('OUTPUT', '-j compute-filter-top',
                                      wrap=False, top=True)

            tables['filter'].add_chain('local')
            tables['filter'].add_rule('compute-filter-top', '-j $local',
                                      wrap=False)
        ...
        # Add a compute-postrouting-bottom chain. It's intended to be shared
        # among the various compute components. We set it as the last chain
        # of POSTROUTING chain.
        self.ipv4['nat'].add_chain('compute-postrouting-bottom', wrap=False)
        self.ipv4['nat'].add_rule('POSTROUTING', '-j compute-postrouting-bottom',
                                  wrap=False)

        # We add a snat chain to the shared compute-postrouting-bottom chain
        # so that it's applied last.
        self.ipv4['nat'].add_chain('snat')
        self.ipv4['nat'].add_rule('compute-postrouting-bottom', '-j $snat',
                                  wrap=False)
    ...
    def _modify_rules(self, current_lines, table, table_name):
        ...
        if not current_lines:
            fake_table = ['#Generated by compute',
                          '*' + table_name, 'COMMIT',
                          '#Completed by compute']
        ...
        def _weed_out_removes(line):
            ...
            if line.startswith(':'):
                # it's a chain, for example, ":compute-billing - [0:0]"
...
def _dhcp_file(dev, kind):
    ...
    return os.path.abspath('%s/compute-%s.%s' % (CONF.networks_path,
                                              dev,
                                              kind))
...
def _ra_file(dev, kind):
    ...
    fileutils.ensure_tree(CONF.networks_path)
    return os.path.abspath('%s/compute-ra-%s.%s' % (CONF.networks_path,
                                              dev,
                                              kind))
