...
def require_context(f):
    ...
    :py:func:`jacket.compute.authorize_project_context` and
    :py:func:`jacket.compute.authorize_user_context`.
...
def action_event_start(context, values):
    ...
    action = _action_get_by_request_id(context, values['instance_uuid'],
                                       values['request_id'])
    # When compute-compute restarts, the context is generated again in
...
def _archive_deleted_rows_for_table(tablename, max_rows):
    ...
    """
    # NOTE(guochbo): There is a circular import, compute.db.sqlalchemy.utils
    # imports compute.db.sqlalchemy.api.
...
def pcidevice_online_data_migration(context, max_count):
    from jacket.objects.compute import pci_device as pci_dev_obj
