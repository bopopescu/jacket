...
def main():
    ...
    for volume_ref in volumes:
        try:
            ...
            jacket.storage.volume.utils.notify_about_volume_usage(
                admin_context,
                volume_ref,
        ...
        try:
            LOG.debug("Send notification for <snapshot_id: %(snapshot_id)s> "
                      ...
                      {'snapshot_id': snapshot_ref.id,
                       ...
                       'extra_info': extra_info})
            jacket.storage.volume.utils.notify_about_snapshot_usage(admin_context,
                                                            snapshot_ref,
                                                            'exists',
                                                            extra_info)
        except Exception as exc_msg:
            ...
            try:
                ...
                LOG.debug("Send create notification for "
                          ...
                          {'snapshot_id': snapshot_ref.id,
                           'project_id': snapshot_ref.project_id,
                           'extra_info': local_extra_info})
                jacket.storage.volume.utils.notify_about_snapshot_usage(
                    admin_context,
                    snapshot_ref,
                    'create.start', extra_usage_info=local_extra_info)
                jacket.storage.volume.utils.notify_about_snapshot_usage(
