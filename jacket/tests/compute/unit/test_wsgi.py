...
class TestLoaderNothingExists(test.NoDBTestCase):
    ...
    def test_relpath_config_not_found(self):
        ...
        self.assertRaises(
            jacket.compute.exception.ConfigNotFound,
            jacket.wsgi.compute.Loader,
        )

    def test_asbpath_config_not_found(self):
        self.flags(api_paste_config='/etc/compute/api-paste.ini')
        self.assertRaises(
            jacket.compute.exception.ConfigNotFound,
            jacket.wsgi.compute.Loader,
        )
...
document_root = /tmp
    ...
    def setUp(self):
        ...
        self.config.seek(0)
        self.config.flush()
        self.loader = jacket.wsgi.compute.Loader(self.config.name)
    ...
    def test_app_not_found(self):
        self.assertRaises(
            jacket.compute.exception.PasteAppNotFound,
...
class TestWSGIServer(test.NoDBTestCase):
    ...
    def test_socket_options_for_simple_server(self):
        ...
        server = jacket.wsgi.compute.Server("test_socket_options", None,
                                  host="127.0.0.1", port=0)
        server.start()
        sock = server._socket
    ...
    def test_server_pool_waitall(self):
        ...
        server = jacket.wsgi.compute.Server("test_server", None,
            host="127.0.0.1")
        server.start()
    ...
    def test_uri_length_limit(self):
        server = jacket.wsgi.compute.Server("test_uri_length_limit", None,
    ...
    def test_reset_pool_size_to_default(self):
        server = jacket.wsgi.compute.Server("test_resize", None,
            host="127.0.0.1", max_url_len=16384)
        server.start()
...
class TestWSGIServerWithSSL(test.NoDBTestCase):
    ...
    def test_ssl_server(self):
        ...
        fake_ssl_server = jacket.wsgi.compute.Server("fake_ssl", test_app,
                                           host="127.0.0.1", port=0,
                                           use_ssl=True)
    ...
    def test_two_servers(self):
        ...
        fake_server = jacket.wsgi.compute.Server("fake", test_app,
            host="127.0.0.1", port=0)
        fake_server.start()
    ...
    def test_app_using_ipv6_and_ssl(self):
        ...
        server = jacket.wsgi.compute.Server("fake_ssl",
                                  hello_world,
                                  host="::1",
                                  port=0,
                                  use_ssl=True)
