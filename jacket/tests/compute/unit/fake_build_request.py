...
def fake_db_req(**updates):
    ...
    db_build_request = {
            ...
            'security_groups': jsonutils.dumps(
                compute.SecurityGroupList().obj_to_primitive()),
            'config_drive': False,
            'key_name': None,
            'locked_by': None,
    ...
    for name, field in compute.BuildRequest.fields.items():
...
def fake_req_obj(context, db_req=None):
    if db_req is None:
        db_req = fake_db_req()
    req_obj = compute.BuildRequest(context)
    for field in req_obj.fields:
        ...
        if isinstance(req_obj.fields[field], fields.ObjectField):
            ...
            if field == 'request_spec':
                req_spec = compute.RequestSpec._from_db_object(context,
                        compute.RequestSpec(), value)
                req_obj.request_spec = req_spec
            elif field == 'info_cache':
                setattr(req_obj, field,
                        compute.InstanceInfoCache.obj_from_primitive(
                            jsonutils.loads(value)))
            elif field == 'security_groups':
                setattr(req_obj, field,
                        compute.SecurityGroupList.obj_from_primitive(
                            jsonutils.loads(value)))
