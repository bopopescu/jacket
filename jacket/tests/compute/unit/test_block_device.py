...
class TestBlockDeviceDict(test.NoDBTestCase):
    ...
    def test_init(self):
        ...
        self.stub_out('compute.block_device.BlockDeviceDict._fields',
                       set(['field1', 'field2']))
        self.stub_out('compute.block_device.BlockDeviceDict._db_only_fields',
                       set(['db_field1', 'db_field2']))
        self.stub_out('compute.block_device.BlockDeviceDict._validate',
                       fake_validate)
    ...
    def test_legacy_mapping_from_object_list(self):
        bdm1 = compute.BlockDeviceMapping()
        bdm1 = compute.BlockDeviceMapping._from_db_object(
            None, bdm1, fake_block_device.FakeDbBlockDeviceDict(
                self.new_mapping[0]))
        bdm2 = compute.BlockDeviceMapping()
        bdm2 = compute.BlockDeviceMapping._from_db_object(
            None, bdm2, fake_block_device.FakeDbBlockDeviceDict(
                self.new_mapping[1]))
        bdmlist = compute.BlockDeviceMappingList()
    ...
    def test_snapshot_from_bdm(self):
        for bdm in self.new_mapping:
            self._test_snapshot_from_bdm(compute.BlockDeviceMapping(**bdm))

    def test_snapshot_from_object(self):
        for bdm in self.new_mapping[:-1]:
            obj = compute.BlockDeviceMapping()
            obj = compute.BlockDeviceMapping._from_db_object(
