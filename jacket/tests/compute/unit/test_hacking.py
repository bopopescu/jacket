...
from jacket.compute.hacking import checks
from jacket.compute import test


class HackingTestCase(test.NoDBTestCase):
    """This class tests the hacking checks in compute.hacking.checks by passing
    ...
    def test_virt_driver_imports(self):
        ...
        self.assertEqual(expect, checks.import_no_virt_driver_import_deps(
            "from compute.virt.libvirt import utils as libvirt_utils",
            "./compute/virt/xenapi/driver.py"))

        self.assertEqual(expect, checks.import_no_virt_driver_import_deps(
            "import compute.virt.libvirt.utils as libvirt_utils",
            "./compute/virt/xenapi/driver.py"))

        self.assertIsNone(checks.import_no_virt_driver_import_deps(
            "from compute.virt.libvirt import utils as libvirt_utils",
            "./compute/virt/libvirt/driver.py"))

        self.assertIsNone(checks.import_no_virt_driver_import_deps(
            "import compute.virt.firewall",
            "./compute/virt/libvirt/firewall.py"))
    ...
    def test_virt_driver_config_vars(self):
        self.assertIsInstance(checks.import_no_virt_driver_config_deps(
            ...
            "'compute.virt.libvirt.driver', group='libvirt')",
            "./compute/virt/xenapi/driver.py"), tuple)

        self.assertIsNone(checks.import_no_virt_driver_config_deps(
            "CONF.import_opt('volume_drivers', "
            "'compute.virt.libvirt.driver', group='libvirt')",
            "./compute/virt/libvirt/volume.py"))
    ...
    def test_no_translate_debug_logs(self):
        self.assertEqual(len(list(checks.no_translate_debug_logs(
            "LOG.debug(_('foo'))", "compute/scheduler/foo.py"))), 1)

        self.assertEqual(len(list(checks.no_translate_debug_logs(
            "LOG.debug('foo')", "compute/scheduler/foo.py"))), 0)

        self.assertEqual(len(list(checks.no_translate_debug_logs(
            "LOG.info(_('foo'))", "compute/scheduler/foo.py"))), 0)

    def test_no_setting_conf_directly_in_tests(self):
        self.assertEqual(len(list(checks.no_setting_conf_directly_in_tests(
            "CONF.option = 1", "compute/tests/test_foo.py"))), 1)

        self.assertEqual(len(list(checks.no_setting_conf_directly_in_tests(
            "CONF.group.option = 1", "compute/tests/test_foo.py"))), 1)

        self.assertEqual(len(list(checks.no_setting_conf_directly_in_tests(
            "CONF.option = foo = 1", "compute/tests/test_foo.py"))), 1)

        # Shouldn't fail with comparisons
        self.assertEqual(len(list(checks.no_setting_conf_directly_in_tests(
            "CONF.option == 'foo'", "compute/tests/test_foo.py"))), 0)

        self.assertEqual(len(list(checks.no_setting_conf_directly_in_tests(
            "CONF.option != 1", "compute/tests/test_foo.py"))), 0)

        # Shouldn't fail since not in compute/tests/
        self.assertEqual(len(list(checks.no_setting_conf_directly_in_tests(
            "CONF.option = 1", "compute/compute/foo.py"))), 0)
    ...
    def test_check_http_not_implemented(self):
        code = """
               ...
               """
        filename = "compute/api/openstack/compute/v21/test.py"
        ...
        filename = "compute/api/openstack/compute/legacy_v2/test.py"
        self._assert_has_no_errors(code, checks.check_http_not_implemented,
                                   filename=filename)
    ...
    def test_check_greenthread_spawns(self):
        ...
        code = "compute.utils.spawn(func, arg1, kwarg1=kwarg1)"
        self._assert_has_no_errors(code, checks.check_greenthread_spawns)

        code = "compute.utils.spawn_n(func, arg1, kwarg1=kwarg1)"
        self._assert_has_no_errors(code, checks.check_greenthread_spawns)
