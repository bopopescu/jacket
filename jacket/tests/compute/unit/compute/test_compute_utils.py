...
def create_instance(context, user_id='fake', project_id='fake', params=None):
    ...
    info_cache = compute.InstanceInfoCache(network_info=net_info)
...
class ComputeValidateDeviceTestCase(test.NoDBTestCase):
    def setUp(self):
        ...
        if 'xen' in self.id():
            ...
            self.instance = compute.Instance(uuid=uuid.uuid4().hex,
...
class UsageInfoTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.flags(compute_driver='compute.virt.fake.FakeDriver',
                   network_manager='compute.network.manager.FlatManager')
        ...
        self.stubs.Set(jacket.tests.compute.unit.image.fake._FakeImageService,
                       'show', fake_show)
