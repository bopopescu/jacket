...
from jacket.compute.virt import event as virtevent
from jacket.compute.virt import fake as fake_driver
...
class ComputeManagerUnitTestCase(test.NoDBTestCase):
    ...
    def test_delete_instance_without_info_cache(self):
        ...
        quotas = mock.create_autospec(compute.Quotas, spec_set=True)
    ...
    def test_init_host(self):
        ...
        def _do_mock_calls(defer_iptables_apply):
            ...
            compute.instance_get_all_by_host(
    ...
    @mock.patch('compute.compute.InstanceList')
    ...
    def test_init_host_with_evacuated_instance(self, mock_save, mock_mig_get):
        ...
        migration = compute.Migration(instance_uuid=deleted_instance.uuid)
        ...
        self.compute.driver.init_host(host=our_host)
        context.get_admin_context().AndReturn(self.context)
        compute.instance_get_all_by_host(self.context, our_host,
                                    columns_to_join=['info_cache', 'metadata']
                                    ).AndReturn([])
        self.compute.init_virt_events()
    ...
    @mock.patch.object(compute.BlockDeviceMapping, 'destroy')
    @mock.patch.object(compute.BlockDeviceMappingList, 'get_by_instance_uuid')
    @mock.patch.object(compute.Instance, 'destroy')
    @mock.patch.object(compute.Instance, 'obj_load_attr')
    @mock.patch.object(compute.quotas.Quotas, 'commit')
    @mock.patch.object(compute.quotas.Quotas, 'reserve')
    @mock.patch.object(compute.quotas, 'ids_from_instance')
    def test_init_instance_complete_partial_deletion(
        ...
        # instance was deleted from compute.
    ...
    @mock.patch('compute.compute.manager.LOG')
    ...
    def test_init_instance_stuck_in_deleting(self):
        ...
        bdms = []
        quotas = compute.quotas.Quotas(self.context)
    ...
    def test_init_instance_resets_crashed_live_migration(self):
        ...
        with test.nested(
            mock.patch.object(instance, 'save'),
            mock.patch('compute.compute.utils.get_nw_info_for_instance',
                       return_value=network_model.NetworkInfo())
        ) as (save, get_nw_info):
            self.compute._init_instance(self.context, instance)
    ...
    def test_init_instance_sets_building_tasks_error_block_device(self):
        instance = compute.Instance(self.context)
    ...
    @mock.patch('compute.compute.manager.ComputeManager._get_power_state',
    ...
    @mock.patch('compute.compute.manager.ComputeManager.unpause_instance')
    ...
    def test_init_instance_deletes_error_deleting_instance(self):
        ...
        self.mox.StubOutWithMock(self.compute, '_delete_instance')
        self.mox.StubOutWithMock(instance, 'obj_load_attr')
        self.mox.StubOutWithMock(compute.quotas, 'ids_from_instance')
        self.mox.StubOutWithMock(self.compute, '_create_reservations')
        ...
        compute.BlockDeviceMappingList.get_by_instance_uuid(
                self.context, instance.uuid).AndReturn(bdms)
        compute.quotas.ids_from_instance(self.context, instance).AndReturn(
    ...
    @mock.patch('compute.context.RequestContext.elevated')
    @mock.patch('compute.compute.utils.get_nw_info_for_instance')
    @mock.patch(
        'compute.compute.manager.ComputeManager._get_instance_block_device_info')
    @mock.patch('compute.virt.driver.ComputeDriver.destroy')
    @mock.patch('compute.virt.fake.FakeDriver.get_volume_connector')
    ...
    @mock.patch('compute.virt.driver.ComputeDriver.list_instance_uuids')
    @mock.patch('compute.compute.api.instance_get_all_by_filters')
    ...
    def test_instance_usage_audit(self):
        instances = [compute.Instance(uuid=uuids.instance)]
        ...
        self.stubs.Set(compute.TaskLog, 'get', fake_task_log)
        self.stubs.Set(compute.InstanceList,
                       'get_active_by_window_joined', fake_get)
        self.stubs.Set(compute.TaskLog, 'begin_task', fake_task_log)
        self.stubs.Set(compute.TaskLog, 'end_task', fake_task_log)
    ...
    def test_query_driver_power_state_and_sync_pending_task(
        ...
        with mock.patch.object(self.compute.driver,
            ...
            db_instance = compute.Instance(uuid=uuids.db_instance,
    ...
    @mock.patch.object(compute.Migration, 'obj_as_admin')
    @mock.patch.object(compute.Migration, 'save')
    @mock.patch.object(compute.MigrationList, 'get_by_filters')
    ...
    def test_attach_interface_failure(self):
        ...
        f_instance = compute.Instance._from_db_object(self.context,
                                                      compute.Instance(),
                                                      db_instance)
    ...
    def test_swap_volume_volume_api_usage(self):
        ...
        self.stub_out('compute.compute.block_device_mapping_update',
                      fake_block_device_mapping_update)
        self.stub_out('compute.compute.instance_fault_create',
                      lambda x, y:
    ...
    def test_check_can_live_migrate_source(self, event_mock):
        ...
        instance = compute.Instance._from_db_object(
                self.context, compute.Instance(), db_instance)
    ...
    def _test_check_can_live_migrate_destination(self, event_mock,
        ...
        if do_raise:
            ...
            self.mox.StubOutWithMock(compute, 'instance_fault_create')
            compute.instance_fault_create(
    ...
    @mock.patch('compute.compute.manager.InstanceEvents._lock_name')
    def test_prepare_for_instance_event(self, lock_name_mock):
        inst_obj = compute.Instance(uuid=uuids.instance)
    ...
    def test_pop_instance_event(self, lock_name_mock):
        ...
        inst_obj = compute.Instance(uuid=uuids.instance)
        event_obj = compute.InstanceExternalEvent(name='network-vif-plugged',
                                                  tag=None)
    ...
    def test_process_instance_vif_deleted_event(self):
        ...
        info_cache = compute.InstanceInfoCache(network_info=nw_info,
                                               instance_uuid=uuids.instance)
        inst_obj = compute.Instance(id=3, uuid=uuids.instance,
    ...
    def test_external_instance_event(self):
        instances = [
            compute.Instance(id=1, uuid=uuids.instance_1),
            compute.Instance(id=2, uuid=uuids.instance_2),
            compute.Instance(id=3, uuid=uuids.instance_3)]
        events = [
            compute.InstanceExternalEvent(name='network-changed',
                                          tag='tag1',
                                          instance_uuid=uuids.instance_1),
            compute.InstanceExternalEvent(name='network-vif-plugged',
                                          instance_uuid=uuids.instance_2,
                                          tag='tag2'),
            compute.InstanceExternalEvent(name='network-vif-deleted',
    ...
    def test_cleanup_blocks_new_events(self):
        instance = compute.Instance(uuid=uuids.instance)
        self.compute.instance_events.cancel_all_events()
    ...
    @mock.patch('compute.compute.BlockDeviceMapping.get_by_volume_and_instance')
    @mock.patch('compute.compute.manager.ComputeManager._driver_detach_volume')
    @mock.patch('compute.compute.Instance._from_db_object')
    ...
    def _test_detach_volume(self, notify_inst_usage, detach,
        ...
        bdm = mock.MagicMock(spec=compute.BlockDeviceMapping)
        bdm.device_name = 'vdb'
        bdm_get.return_value = bdm
    ...
    def test_detach_volume_evacuate_legacy(self):
        ...
        before compute stashed the connector in connection_info. The connector
        sent to terminate_connection will still be for the local host in this
        case because compute does not have the info to get the connector for the
    ...
    def _test_rescue(self, clean_shutdown=True):
        ...
        rescue_image_meta = compute.ImageMeta.from_dict(
            {'id': 'fake', 'name': 'fake'})
        with test.nested(
    ...
    @mock.patch('compute.compute.manager.ComputeManager._get_power_state',
                return_value=power_state.RUNNING)
    @mock.patch.object(compute.Instance, 'save')
    @mock.patch('compute.utils.generate_password', return_value='fake-pass')
    ...
    @mock.patch('compute.compute.manager.ComputeManager._get_power_state',
                return_value=power_state.NOSTATE)
    @mock.patch('compute.compute.manager.ComputeManager._instance_update')
    @mock.patch.object(compute.Instance, 'save')
    @mock.patch.object(compute_utils, 'add_instance_fault_from_exc')
    ...
    def test_destroy_evacuated_instances(self):
        our_host = self.compute.host
        instance_1 = compute.Instance(self.context)
        ...
        instance_2 = compute.Instance(self.context)
        ...
        migration = compute.Migration(instance_uuid=instance_2.uuid)
    ...
    def test_rebuild_deleting(self):
        instance = compute.Instance(uuid='fake-uuid')
    ...
    @mock.patch.object(compute.InstanceList, 'get_by_host', return_value=[])
    @mock.patch.object(compute.BandwidthUsage, 'get_by_instance_uuid_and_mac')
    @mock.patch.object(compute, 'bw_usage_update')
    ...
    def test_reverts_task_state_instance_not_found(self):
        ...
        instance = compute.Instance(uuid=uuids.instance, task_state="FAKE")
    ...
    @mock.patch.object(compute.scheduler.client.SchedulerClient,
    ...
    def test_update_scheduler_instance_info(self, mock_update):
        ...
        # InstanceList
        self.assertIsInstance(args[2], compute.InstanceList)
    ...
    @mock.patch.object(compute.context.RequestContext, 'elevated')
    @mock.patch.object(compute.objects.InstanceList, 'get_by_host')
    @mock.patch.object(compute.scheduler.client.SchedulerClient,
                       'sync_instance_info')
    def test_sync_scheduler_instance_info(self, mock_sync, mock_get_by_host,
            mock_elevated):
        inst1 = compute.Instance(uuid=uuids.instance_1)
        inst2 = compute.Instance(uuid=uuids.instance_2)
        inst3 = compute.Instance(uuid=uuids.instance_3)
        exp_uuids = [inst.uuid for inst in [inst1, inst2, inst3]]
        mock_get_by_host.return_value = compute.InstanceList(
                compute=[inst1, inst2, inst3])
    ...
    def test_instance_update(self, mock_save):
        instance = compute.Instance(task_state=task_states.SCHEDULING,
    ...
    @mock.patch('compute.compute.manager.ComputeManager._delete_instance')
...
class ComputeManagerBuildInstanceTestCase(test.NoDBTestCase):
    ...
    def _instance_action_events(self):
        self.mox.StubOutWithMock(compute.InstanceActionEvent, 'event_start')
        self.mox.StubOutWithMock(compute.InstanceActionEvent,
    ...
    @mock.patch('compute.hooks._HOOKS')
    ...
    @mock.patch('compute.compute.manager.ComputeManager._build_and_run_instance')
    def test_build_and_run_instance_with_icehouse_requested_network(
        ...
        self.compute.build_and_run_instance(self.context, self.instance,
                ...
                injected_files=self.injected_files,
                admin_password=self.admin_pass,
                requested_networks=[compute.NetworkRequest(
    ...
    @mock.patch('compute.network.model.NetworkInfoAsyncWrapper.wait')
    @mock.patch(
        'compute.compute.manager.ComputeManager._build_networks_for_instance')
    @mock.patch('compute.compute.Instance.save')
    ...
    def test_access_ip_set_when_instance_set_to_active(self):
        ...
        @mock.patch.object(compute, 'instance_extra_update_by_uuid')
        @mock.patch.object(self.compute, '_notify_about_instance_usage')
...
class ComputeManagerMigrationTestCase(test.NoDBTestCase):
    def setUp(self):
        ...
        self.instance = fake_instance.fake_instance_obj(self.context,
                vm_state=vm_states.ACTIVE,
                expected_attrs=['metadata', 'system_metadata', 'info_cache'])
        self.migration = compute.Migration(context=self.context.elevated(),
    ...
    def test_finish_resize_failure(self):
        with test.nested(
            ...
            mock.patch.object(compute, 'instance_fault_create'),
            mock.patch.object(self.compute, '_instance_update'),
            mock.patch.object(self.instance, 'save'),
    ...
    def test_resize_instance_failure(self):
        ...
        with test.nested(
            ...
            mock.patch.object(compute.BlockDeviceMappingList,
                              'get_by_instance_uuid',
                              return_value=None),
            mock.patch.object(compute.Flavor,
                              'get_by_id',
                              return_value=None)
    ...
    def test_check_migrate_source_converts_object(self):
        # NOTE(danms): Make sure that we legacy-ify any data compute
    ...
    def test_live_migration_pause_vm_invalid_migration_state(
        ...
        instance = compute.Instance(id=1234, uuid=str(uuid.uuid4()))
        migration = compute.Migration()
    ...
    @mock.patch.object(compute.Migration, 'get_by_id')
    @mock.patch.object(compute.virt.fake.SmallFakeDriver, 'live_migration_abort')
    def test_live_migration_abort(self,
                                  mock_driver,
                                  mock_get_migration,
                                  mock_notify):
        instance = compute.Instance(id=123, uuid=uuids.instance)
    ...
    def test_live_migration_cleanup_flags_block_migrate_xenapi(self):
        migrate_data = compute.XenapiLiveMigrateData(block_migration=True)
        do_cleanup, destroy_disks = self.compute._live_migration_cleanup_flags(
            migrate_data)
        self.assertTrue(do_cleanup)
        self.assertTrue(destroy_disks)
    ...
    def test_live_migration_cleanup_flags_live_migrate_xenapi(self):
        migrate_data = compute.XenapiLiveMigrateData(block_migration=False)
        do_cleanup, destroy_disks = self.compute._live_migration_cleanup_flags(
            migrate_data)
        self.assertFalse(do_cleanup)
        self.assertFalse(destroy_disks)
...
class ComputeManagerInstanceUsageAuditTestCase(test.TestCase):
    ...
    @mock.patch('compute.compute.TaskLog')
    def test_deleted_instance(self, mock_task_log):
        ...
        flavor = compute.Flavor(admin_context, **fake_db_flavor)
        ...
        instance = compute.Instance(admin_context, **updates)
        instance.create()
        ...
        self.stub_out('compute.utils.last_completed_audit_period',
