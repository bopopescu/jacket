...
from jacket.tests.compute.unit.pci import fakes as pci_fakes
...
FAKE_VIRT_NUMA_TOPOLOGY = compute.NUMATopology(
        cells=[compute.NUMACell(id=0, cpuset=set([1, 2]), memory=3072,
                                cpu_usage=0, memory_usage=0, mempages=[],
                                siblings=[], pinned_cpus=set([])),
               compute.NUMACell(id=1, cpuset=set([3, 4]), memory=3072,
                                cpu_usage=0, memory_usage=0, mempages=[],
                                siblings=[], pinned_cpus=set([]))])
FAKE_VIRT_NUMA_TOPOLOGY_OVERHEAD = compute.NUMATopologyLimits(
...
class BaseTestCase(test.TestCase):
    ...
    def setUp(self, _mock_ext_mgr):
        ...
        self.stubs.Set(compute.InstanceList, 'get_by_host_and_node',
    ...
    def _create_compute_node_obj(self, context):
        ...
        compute_obj = compute.ComputeNode()
        compute_obj = compute.ComputeNode._from_db_object(
    ...
    def _fake_instance_obj(self, stash=True, flavor=None, **kwargs):
        ...
        if not isinstance(flavor, compute.Flavor):
            flavor = compute.Flavor(**flavor)

        instance_uuid = str(uuid.uuid1())
        instance = compute.Instance(context=self.context, uuid=instance_uuid,
                                    flavor=flavor)
        instance.update({
    ...
    def _fake_flavor_create(self, **kwargs):
        ...
        instance_type = compute.Flavor(**instance_type)
    ...
    def _fake_instance_get_by_host_and_node(self, context, host, nodename,
        ...
        return compute.InstanceList(
            compute=[i for i in self._instances.values() if i['host'] == host])
...
class MissingComputeNodeTestCase(BaseTestCase):
    def setUp(self):
        ...
        self.stub_out('compute.db.service_get_by_compute_host',
                self._fake_service_get_by_compute_host)
        self.stub_out('compute.db.compute_node_get_by_host_and_nodename',
                self._fake_compute_node_get_by_host_and_nodename)
        self.stub_out('compute.db.compute_node_create',
...
class BaseTrackerTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.stub_out('compute.db.compute_node_get_by_host_and_nodename',
                self._fake_compute_node_get_by_host_and_nodename)
        self.stub_out('compute.db.compute_node_update',
                self._fake_compute_node_update)
        self.stub_out('compute.db.compute_node_delete',
                self._fake_compute_node_delete)
        self.stub_out('compute.db.migration_update',
                self._fake_migration_update)
        self.stub_out('compute.db.migration_get_in_progress_by_host_and_node',
    ...
    def _assert(self, value, field, tracker=None):
        ...
        if field == 'numa_topology':
            self.assertEqualNUMAHostTopology(
                    value, compute.NUMATopology.obj_from_db_obj(x))
        else:
...
class InstanceClaimTestCase(BaseTrackerTestCase):
    def _instance_topology(self, mem):
        mem = mem * 1024
        return compute.InstanceNUMATopology(
    ...
    @mock.patch('compute.compute.InstancePCIRequests.get_by_instance_uuid',
                return_value=compute.InstancePCIRequests(requests=[]))
    ...
    def test_deleted_instances_with_migrations(self, mock_migration_list):
        migration = compute.Migration(context=self.context,
    ...
    def test_pair_instances_to_migrations(self):
        migrations = [compute.Migration(instance_uuid=uuidsentinel.instance1),
                      compute.Migration(instance_uuid=uuidsentinel.instance2)]
        instances = [compute.Instance(uuid=uuidsentinel.instance2),
                     compute.Instance(uuid=uuidsentinel.instance1)]
    ...
    @mock.patch('compute.compute.claims.Claim')
    @mock.patch('compute.compute.Instance.save')
    def test_claim_saves_numa_topology(self, mock_save, mock_claim):
        ...
        inst = compute.Instance(host=None, node=None, memory_mb=1024)
        inst.obj_reset_changes()
        numa = compute.InstanceNUMATopology()
...
class _MoveClaimTestCase(BaseTrackerTestCase):
    ...
    def test_existing_migration(self, save_mock, save_inst_mock):
        migration = compute.Migration(self.context, id=42,
...
class UpdateUsageFromMigrationsTestCase(BaseTrackerTestCase):
    ...
    def test_use_most_recent(self, mock_get_instance, mock_update_usage):
        instance = self._fake_instance_obj()
        mock_get_instance.return_value = instance
        migration_2002 = compute.Migration(
            id=2002,
            context=self.context,
            instance_uuid=instance.uuid,
            updated_at=datetime.datetime(2002, 1, 1, 0, 0, 0),
        )
        migration_2003 = compute.Migration(
            id=2003,
            context=self.context,
            instance_uuid=instance.uuid,
            updated_at=datetime.datetime(2003, 1, 1, 0, 0, 0),
        )
        migration_2001 = compute.Migration(
            id=2001,
            context=self.context,
            instance_uuid=instance.uuid,
