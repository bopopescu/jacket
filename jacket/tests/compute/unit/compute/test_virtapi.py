...
class FakeVirtAPITest(VirtAPIBaseTest):
    ...
    def assertExpected(self, method, *args, **kwargs):
        ...
        self.mox.StubOutWithMock(compute, method)
        ...
        if method in ('aggregate_metadata_add', 'aggregate_metadata_delete',
            ...
            # argument['id'], so expect that in the actual compute call
...
class ComputeVirtAPITest(VirtAPIBaseTest):
    ...
    def test_wait_for_instance_event(self):
        and_i_ran = ''
        event_1_tag = compute.InstanceExternalEvent.make_key(
            'event1')
        event_2_tag = compute.InstanceExternalEvent.make_key(
            'event2', 'tag')
        events = {
            'event1': event_1_tag,
