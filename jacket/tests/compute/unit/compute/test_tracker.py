...
_COMPUTE_NODE_FIXTURES = [
    compute.ComputeNode(
        ...
        host_ip='1.1.1.1',
        supported_hv_specs=[
            compute.HVSpec.from_list([arch.I686, hv_type.KVM, vm_mode.HVM])
        ],
        metrics=None,
...
_INSTANCE_TYPE_OBJ_FIXTURES = {
    1: compute.Flavor(id=1, flavorid='fakeid-1', name='fake1.small',
                      memory_mb=128, vcpus=1, root_gb=1,
                      ephemeral_gb=0, swap=0, rxtx_factor=0,
                      vcpu_weight=1, extra_specs={}),
    2: compute.Flavor(id=2, flavorid='fakeid-2', name='fake1.medium',
...
_INSTANCE_NUMA_TOPOLOGIES = {
    '2mb': compute.InstanceNUMATopology(cells=[
...
_NUMA_LIMIT_TOPOLOGIES = {
    '2mb': compute.NUMATopologyLimits(id=0,
                                      cpu_allocation_ratio=1.0,
                                      ram_allocation_ratio=1.0),
}

_NUMA_PAGE_TOPOLOGIES = {
    '2kb*8': compute.NUMAPagesTopology(size_kb=2, total=8, used=0)
}

_NUMA_HOST_TOPOLOGIES = {
    '2mb': compute.NUMATopology(cells=[
        compute.NUMACell(id=0, cpuset=set([1, 2]), memory=_2MB,
                         cpu_usage=0, memory_usage=0,
                         mempages=[_NUMA_PAGE_TOPOLOGIES['2kb*8']],
                         siblings=[], pinned_cpus=set([])),
        compute.NUMACell(id=1, cpuset=set([3, 4]), memory=_2MB,
                         cpu_usage=0, memory_usage=0,
                         mempages=[_NUMA_PAGE_TOPOLOGIES['2kb*8']],
                         siblings=[], pinned_cpus=set([]))]),
}
...
_MIGRATION_FIXTURES = {
    ...
    'source-and-dest': compute.Migration(
        id=3,
    ...
    'dest-only-evac': compute.Migration(
        id=4,
...
_MIGRATION_INSTANCE_FIXTURES = {
    # source-only
    'f15ecfb0-9bf6-42db-9837-706eb2c4bf08': compute.Instance(
        id=101,
        host=None,  # prevent RT trying to lazy-load this
        node=None,
    ...
    # dest-only
    'f6ed631a-8645-4b12-8e1e-2fff55795765': compute.Instance(
        id=102,
        host=None,  # prevent RT trying to lazy-load this
        node=None,
    ...
    # source-and-dest
    'f4f0bfea-fe7e-4264-b598-01cb13ef1997': compute.Instance(
        id=3,
        host=None,  # prevent RT trying to lazy-load this
        node=None,
    ...
    # dest-only-evac
    '077fb63a-bdc8-4330-90ef-f012082703dc': compute.Instance(
        id=102,
        host=None,  # prevent RT trying to lazy-load this
        node=None,
...
_MIGRATION_CONTEXT_FIXTURES = {
    'f4f0bfea-fe7e-4264-b598-01cb13ef1997': compute.MigrationContext(
        instance_uuid='f4f0bfea-fe7e-4264-b598-01cb13ef1997',
        migration_id=3,
        new_numa_topology=None,
        old_numa_topology=None),
    'c17741a5-6f3d-44a8-ade8-773dc8c29124': compute.MigrationContext(
        instance_uuid='c17741a5-6f3d-44a8-ade8-773dc8c29124',
        migration_id=3,
        new_numa_topology=None,
        old_numa_topology=None),
    'f15ecfb0-9bf6-42db-9837-706eb2c4bf08': compute.MigrationContext(
        instance_uuid='f15ecfb0-9bf6-42db-9837-706eb2c4bf08',
        migration_id=1,
        new_numa_topology=None,
        old_numa_topology=_INSTANCE_NUMA_TOPOLOGIES['2mb']),
    'f6ed631a-8645-4b12-8e1e-2fff55795765': compute.MigrationContext(
        instance_uuid='f6ed631a-8645-4b12-8e1e-2fff55795765',
        migration_id=2,
        new_numa_topology=_INSTANCE_NUMA_TOPOLOGIES['2mb'],
        old_numa_topology=None),
    '077fb63a-bdc8-4330-90ef-f012082703dc': compute.MigrationContext(
        instance_uuid='077fb63a-bdc8-4330-90ef-f012082703dc',
        migration_id=2,
        new_numa_topology=None,
        old_numa_topology=None),
...
def setup_rt(hostname, nodename, virt_resources=_VIRT_DRIVER_AVAIL_RESOURCES,
    ...
    with test.nested(
            mock.patch('compute.scheduler.client.SchedulerClient',
                       return_value=sched_client_mock),
            mock.patch('compute.rpc.get_notifier', return_value=notifier_mock)):
...
class TestUpdateAvailableResources(BaseTestCase):
    ...
    @mock.patch('compute.compute.ComputeNode.get_by_host_and_nodename')
    @mock.patch('compute.compute.MigrationList.get_in_progress_by_host_and_node')
    @mock.patch('compute.compute.InstanceList.get_by_host_and_node')
    def test_no_instances_no_migrations_no_reserved(self, get_mock, migr_mock,
        ...
        expected_resources.update({
            ...
            'free_ram_mb': 512,
            'memory_mb_used': 0,
            'pci_device_pools': compute.PciDevicePoolList(),
            'vcpus_used': 0,
            'hypervisor_type': 'fake',
            'local_gb_used': 0,
            'memory_mb': 512,
    ...
    def test_no_instances_source_migration(self, get_mock, get_inst_mock,
        ...
        # compute.Instance.get_by_uuid, so we have the migration return
    ...
    @mock.patch('compute.compute.MigrationContext.get_by_instance_uuid',
                return_value=None)
    @mock.patch('compute.compute.ComputeNode.get_by_host_and_nodename')
    @mock.patch('compute.compute.MigrationList.get_in_progress_by_host_and_node')
    @mock.patch('compute.compute.Instance.get_by_uuid')
...
class TestInitComputeNode(BaseTestCase):
    ...
    @mock.patch('compute.compute.ComputeNode.create')
    ...
    def test_compute_node_created_on_empty(self, get_mock, create_mock):
        ...
        expected_compute = compute.ComputeNode(
...
class TestInstanceClaim(BaseTestCase):
    ...
    def test_update_usage_with_claim(self, migr_mock, pci_mock):
        ...
        pci_mock.return_value = compute.InstancePCIRequests(requests=[])
    ...
    def test_update_usage_removed(self, migr_mock, pci_mock):
        ...
        expected_updated['pci_device_pools'] = compute.PciDevicePoolList()
    ...
    @mock.patch('compute.pci.stats.PciDeviceStats.support_requests',
                return_value=True)
    @mock.patch('compute.pci.manager.PciDevTracker.claim_instance')
    @mock.patch('compute.compute.InstancePCIRequests.get_by_instance_uuid')
    @mock.patch('compute.compute.MigrationList.get_in_progress_by_host_and_node')
    def test_claim_with_pci(self, migr_mock, pci_mock,
        ...
        pci_pools = compute.PciDevicePoolList()
        pci_manager_mock.return_value = pci_pools

        request = compute.InstancePCIRequest(count=1,
            spec=[{'vendor_id': 'v', 'product_id': 'p'}])
        pci_mock.return_value = compute.InstancePCIRequests(requests=[request])
    ...
    def test_claim_abort_context_manager(self, migr_mock, pci_mock):
        ...
        @mock.patch.object(self.instance, 'clear_numa_topology',
                           mock_clear_numa)
        @mock.patch.object(compute.Instance, 'obj_clone',
    ...
    def test_claim_numa(self, migr_mock, pci_mock):
        ...
        with mock.patch.object(self.rt, '_update') as update_mock:
            ...
            new_numa = compute.NUMATopology.obj_from_db_obj(new_numa)
...
class TestMoveClaim(BaseTestCase):
    def setUp(self):
        ...
        with test.nested(
            mock.patch('compute.compute.InstanceList.get_by_host_and_node'),
            mock.patch('compute.compute.MigrationList.'
                       'get_in_progress_by_host_and_node')
        ) as (inst_list_mock, migr_mock):
            inst_list_mock.return_value = compute.InstanceList(compute=[])
            migr_mock.return_value = compute.MigrationList(compute=[])
    ...
    def audit(self, rt, instances, migrations, migr_inst):
        self.inst_list_mock.return_value = \
                compute.InstanceList(compute=instances)
        self.migr_mock.return_value = \
                compute.MigrationList(compute=migrations)
    ...
    @mock.patch('compute.compute.Flavor.get_by_id')
    def test_claim(self, flavor_mock, pci_mock, inst_list_mock, inst_by_uuid,
        ...
        flavor_mock.return_value = compute.Flavor(**self.flavor)
        ...
        mig_ctxt_mock = mock.patch('compute.compute.MigrationContext',
    ...
    def test_claim_abort(self, pci_mock, inst_list_mock,
        ...
        with mock.patch('compute.compute.Instance.'
    ...
    def test_same_host(self, pci_mock, inst_list_mock, inst_by_uuid,
        ...
        with mock.patch('compute.compute.MigrationContext._destroy') as destroy_m:
            claim.abort()
            self.assertTrue(obj_base.obj_equal_prims(expected,
                                                     self.rt.compute_node))
