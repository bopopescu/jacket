...
class ClaimTestCase(test.NoDBTestCase):
    ...
    def setUp(self):
        ...
        self.empty_requests = compute.InstancePCIRequests(
            requests=[]
        )
    ...
    def _fake_instance_type(self, **kwargs):
        ...
        return compute.Flavor(**instance_type)
    ...
    def _fake_resources(self, values=None):
        resources = {
            ...
            'numa_topology': compute.NUMATopology(
                cells=[compute.NUMACell(id=1, cpuset=set([1, 2]), memory=512,
                                        memory_usage=0, cpu_usage=0,
                                        mempages=[], siblings=[],
                                        pinned_cpus=set([])),
                       compute.NUMACell(id=2, cpuset=set([3, 4]), memory=512,
                                        memory_usage=0, cpu_usage=0,
                                        mempages=[], siblings=[],
                                        pinned_cpus=set([]))]
                )._to_json()
    ...
    def test_numa_topology_no_limit(self):
        huge_instance = compute.InstanceNUMATopology(
    ...
    def test_numa_topology_fails(self):
        ...
        limit_topo = compute.NUMATopologyLimits(
    ...
    @mock.patch('compute.compute.InstancePCIRequests.get_by_instance')
    ...
    def test_numa_topology_with_pci_fail(self, mock_get_by_instance):
        ...
        request = compute.InstancePCIRequest(count=2,
            spec=[{'vendor_id': 'v', 'product_id': 'p'}])
        requests = compute.InstancePCIRequests(requests=[request])
        mock_get_by_instance.return_value = requests
...
class MoveClaimTestCase(ClaimTestCase):
    ...
    def _claim(self, limits=None, overhead=None, requests=None, **kwargs):
        ...
        @mock.patch('compute.compute.InstancePCIRequests.'
    ...
    def test_create_migration_context(self):
        ...
        migration = compute.Migration(context=self.context, id=42)
        claim.migration = migration
        fake_mig_context = mock.Mock(spec=compute.MigrationContext)

        @mock.patch('compute.db.instance_extra_get_by_instance_uuid',
                    return_value=None)
        @mock.patch('compute.compute.MigrationContext',
