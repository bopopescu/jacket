...
class PciDevTrackerTestCase(test.NoDBTestCase):
    ...
    def _create_pci_requests_object(self, mock_get, requests):
        ...
        for request in requests:
            pci_req_obj = compute.InstancePCIRequest(count=request['count'],
                                                     spec=request['spec'])
            pci_reqs.append(pci_req_obj)
        mock_get.return_value = compute.InstancePCIRequests(requests=pci_reqs)
    ...
    @mock.patch.object(compute.objects.PciDeviceList, 'get_by_compute_node')
    ...
    @mock.patch('compute.pci.whitelist.Whitelist.device_assignable',
                return_value=True)
    ...
    def test_pci_claim_instance_with_numa(self, mock_get):
        ...
        self._create_pci_requests_object(mock_get, pci_requests)
        self.inst.numa_topology = compute.InstanceNUMATopology(
    ...
    @mock.patch.object(compute.PciDevice, 'should_migrate_data',
                       return_value=False)
    def test_save(self, migrate_mock):
        self.stub_out(
                'compute.db.pci_device_update',
    ...
    def test_get_devs_object(self):
        def _fake_obj_load_attr(foo, attrname):
            if attrname == 'pci_devices':
                ...
                foo.pci_devices = compute.PciDeviceList()

        self.stub_out(
                'compute.compute.Instance.obj_load_attr',
                _fake_obj_load_attr)

        self.load_attr_called = False
        manager.get_instance_pci_devs(compute.Instance())
