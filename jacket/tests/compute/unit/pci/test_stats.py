...
from jacket.tests.compute.unit.pci import fakes
fake_pci_1 = {
    'compute_node_id': 1,
...
pci_requests = [compute.InstancePCIRequest(count=1,
                    spec=[{'vendor_id': 'v1'}]),
                compute.InstancePCIRequest(count=1,
                    spec=[{'vendor_id': 'v2'}])]


pci_requests_multiple = [compute.InstancePCIRequest(count=1,
                             spec=[{'vendor_id': 'v1'}]),
                         compute.InstancePCIRequest(count=3,
                          spec=[{'vendor_id': 'v2'}])]


class PciDeviceStatsTestCase(test.NoDBTestCase):
    def _create_fake_devs(self):
        self.fake_dev_1 = compute.PciDevice.create(None, fake_pci_1)
        self.fake_dev_2 = compute.PciDevice.create(None, fake_pci_2)
        self.fake_dev_3 = compute.PciDevice.create(None, fake_pci_3)
        self.fake_dev_4 = compute.PciDevice.create(None, fake_pci_4)
    ...
    def test_support_requests_numa(self):
        cells = [compute.NUMACell(id=0, cpuset=set(), memory=0),
                 compute.NUMACell(id=1, cpuset=set(), memory=0)]
    ...
    def _create_pci_devices(self):
        ...
        for dev in range(4):
            pci_dev = {'compute_node_id': 1,
                       ...
                       'parent_addr': None,
                       'numa_node': 0}
            self.pci_tagged_devices.append(compute.PciDevice.create(None,
                                                                    pci_dev))
        ...
        for dev in range(3):
            pci_dev = {'compute_node_id': 1,
                       ...
                       'parent_addr': None,
                       'numa_node': 0}
            self.pci_untagged_devices.append(compute.PciDevice.create(None,
                                                                      pci_dev))
...
class PciDeviceVFPFStatsTestCase(test.NoDBTestCase):
    ...
    def _create_pci_devices(self, vf_product_id=1515, pf_product_id=1528):
        ...
        for dev in range(2):
            pci_dev = {'compute_node_id': 1,
                       ...
                       'parent_addr': None,
                       'numa_node': 0}
            self.sriov_pf_devices.append(compute.PciDevice.create(None,
                                                                  pci_dev))
        ...
        for dev in range(8):
            pci_dev = {'compute_node_id': 1,
                       ...
                       'numa_node': 0}
            self.sriov_vf_devices.append(compute.PciDevice.create(None,
                                                                  pci_dev))
    ...
    def test_consume_VF_requests(self):
        with mock.patch.object(compute.PciDevice, 'get_by_dev_addr',
            ...
            self._create_pci_devices()
            pci_requests = [compute.InstancePCIRequest(count=2,
    ...
    def test_consume_VF_and_PF_requests_failed(self):
        with test.nested(
            ...
            mock.patch.object(compute.PciDeviceList, 'get_by_parent_address',
                               side_effect=self._fake_get_by_parent_address)):
            self._create_pci_devices()
            pci_requests = [compute.InstancePCIRequest(count=5,
                                spec=[{'product_id': '1515'}]),
                            compute.InstancePCIRequest(count=1,
                                spec=[{'product_id': '1528',
                                       'dev_type': 'type-PF'}])]
    ...
    def test_consume_VF_and_PF_same_prodict_id_failed(self):
        with test.nested(
            ...
            pci_requests = [compute.InstancePCIRequest(count=9,
                                spec=[{'product_id': '1515'}])]
            self.assertIsNone(self.pci_stats.consume_requests(pci_requests))
