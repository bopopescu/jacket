...
class _TestInstanceGroupObject(object):
    ...
    @mock.patch('compute.db.instance_group_get', return_value=_INST_GROUP_DB)
    def test_get_by_uuid(self, mock_db_get):
        obj = compute.InstanceGroup.get_by_uuid(mock.sentinel.ctx,
                                                       _DB_UUID)
    ...
    @mock.patch('compute.compute.utils.notify_about_server_group_update')
    @mock.patch('compute.db.instance_group_update')
    @mock.patch('compute.db.instance_group_get')
    ...
    def test_save_without_hosts(self, mock_db_get, mock_db_update,
        ...
        obj = compute.InstanceGroup.get_by_uuid(mock.sentinel.ctx, _DB_UUID)
    ...
    @mock.patch('compute.compute.utils.notify_about_server_group_update')
    @mock.patch('compute.db.instance_group_create', return_value=_INST_GROUP_DB)
    def test_create(self, mock_db_create, mock_notify):
        obj = compute.InstanceGroup(context=mock.sentinel.ctx)
        obj.uuid = _DB_UUID
    ...
    @mock.patch('compute.compute.utils.notify_about_server_group_update')
    @mock.patch('compute.db.instance_group_delete')
    ...
    def test_get_hosts(self, mock_get_db, mock_il_get):
        mock_il_get.return_value = [compute.Instance(host='host1'),
                                    compute.Instance(host='host2'),
                                    compute.Instance(host=None)]
        obj = compute.InstanceGroup.get_by_uuid(mock.sentinel.ctx, _DB_UUID)
        hosts = obj.get_hosts()
    ...
    def test_load_hosts(self, mock_get_by_filt):
        mock_get_by_filt.return_value = [compute.Instance(host='host1'),
                                         compute.Instance(host='host2')]

        obj = compute.InstanceGroup(mock.sentinel.ctx, members=['uuid1'])
    ...
    def test_load_anything_else_but_hosts(self):
        obj = compute.InstanceGroup(mock.sentinel.ctx)
...
class _TestInstanceGroupListObject(object):
    ...
    def test_get_by_name(self, mock_db_get):
        ...
        ig = compute.InstanceGroup.get_by_name(mock_ctx, 'f1')
        ...
        self.assertRaises(exception.InstanceGroupNotFound,
                          compute.InstanceGroup.get_by_name,
                          mock_ctx, 'unknown')

    @mock.patch('compute.compute.InstanceGroup.get_by_uuid')
    @mock.patch('compute.compute.InstanceGroup.get_by_name')
    def test_get_by_hint(self, mock_name, mock_uuid):
        compute.InstanceGroup.get_by_hint(mock.sentinel.ctx, _DB_UUID)
        mock_uuid.assert_called_once_with(mock.sentinel.ctx, _DB_UUID)
        compute.InstanceGroup.get_by_hint(mock.sentinel.ctx, 'name')
