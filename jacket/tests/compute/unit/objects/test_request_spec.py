...
from jacket.tests.compute.unit import fake_request_spec
...
class _TestRequestSpecObject(object):
    ...
    def test_image_meta_from_image_as_object(self):
        ...
        image_meta = compute.ImageMeta(name='foo')

        spec = compute.RequestSpec()
        spec._image_meta_from_image(image_meta)
        self.assertEqual(image_meta, spec.image)

    @mock.patch.object(compute.ImageMeta, 'from_dict')
    ...
    def test_to_legacy_image(self, obj_to_primitive):
        spec = compute.RequestSpec(image=compute.ImageMeta())
        fake_dict = mock.Mock()
    ...
    def test_to_legacy_image_with_none(self, obj_to_primitive):
        spec = compute.RequestSpec(image=None)
    ...
    def test_from_instance_as_object(self):
        ...
        instance.availability_zone = 'compute'
    ...
    def test_from_instance_with_numa_stuff(self):
        ...
        spec._from_instance(instance)
        self.assertIsInstance(spec.numa_topology, compute.InstanceNUMATopology)
        cells = spec.numa_topology.cells
        self.assertEqual(1, len(cells))
        self.assertIsInstance(cells[0], compute.InstanceNUMACell)
    ...
    def test_from_flavor_as_dict(self):
        ...
        spec = compute.RequestSpec(ctxt)

        spec._from_flavor(flavor_dict)
        self.assertIsInstance(spec.flavor, compute.Flavor)
        self.assertEqual({'id': 1}, spec.flavor.obj_get_changes())

    def test_to_legacy_instance(self):
        spec = compute.RequestSpec()
        spec.flavor = compute.Flavor(root_gb=10,
                                     ephemeral_gb=0,
                                     memory_mb=10,
                                     vcpus=1)
        ...
        spec.project_id = '1'
        spec.availability_zone = 'compute'
        ...
        self.assertEqual({'root_gb': 10,
                          ...
                          'pci_requests': None,
                          'project_id': '1',
                          'availability_zone': 'compute'}, instance)
    ...
    def test_from_retry(self):
        ...
        self.assertIsInstance(spec.retry, compute.SchedulerRetries)
        self.assertEqual(1, spec.retry.num_attempts)
        self.assertIsInstance(spec.retry.hosts, compute.ComputeNodeList)
    ...
    def test_populate_group_info(self):
        ...
        self.assertIsInstance(spec.instance_group, compute.InstanceGroup)
    ...
    def test_from_limits(self):
        ...
        self.assertIsInstance(spec.limits, compute.SchedulerLimits)
    ...
    @mock.patch.object(compute.SchedulerLimits, 'from_dict')
    def test_from_primitives(self, mock_limits):
        spec_dict = {'instance_type': compute.Flavor(),
                     'instance_properties': compute.Instance(
                         uuid=uuidutils.generate_uuid(),
                         numa_topology=None,
                         pci_requests=None,
                         project_id=1,
        ...
        ctxt = context.RequestContext('fake', 'fake')
        spec = compute.RequestSpec.from_primitives(ctxt, spec_dict, filt_props)
    ...
    def test_from_components(self):
        ...
        spec = compute.RequestSpec.from_components(ctxt, instance, image,
    ...
    @mock.patch('compute.compute.RequestSpec._populate_group_info')
    def test_from_components_with_instance_group(self, mock_pgi):
        ...
        instance_group = compute.InstanceGroup()

        compute.RequestSpec.from_components(ctxt, instance, image,
                flavor, instance.numa_topology, instance.pci_requests,
                filter_properties, instance_group, instance.availability_zone)
    ...
    def test_get_scheduler_hint(self):
        spec_obj = compute.RequestSpec(scheduler_hints={'foo_single': ['1'],
                                                        'foo_mul': ['1', '2']})
    ...
    @mock.patch.object(compute.RequestSpec, '_to_legacy_instance')
    @mock.patch.object(base, 'obj_to_primitive')
    def test_to_legacy_request_spec_dict(self, image_to_primitive,
        ...
        fake_flavor = compute.Flavor(root_gb=10,
                                     ephemeral_gb=0,
                                     memory_mb=512,
                                     vcpus=1)
        spec = compute.RequestSpec(num_instances=1,
                                   image=compute.ImageMeta(),
                                   # instance properties
                                   numa_topology=None,
                                   pci_requests=None,
                                   project_id=1,
    ...
    def test_to_legacy_filter_properties_dict(self):
        fake_numa_limits = compute.NUMATopologyLimits()
        fake_computes_obj = compute.ComputeNodeList(
            compute=[compute.ComputeNode(host='fake1',
                                         hypervisor_hostname='node1')])
        spec = compute.RequestSpec(
            ignore_hosts=['ignoredhost'],
            force_hosts=['fakehost'],
            force_nodes=['fakenode'],
            retry=compute.SchedulerRetries(num_attempts=1,
                                           hosts=fake_computes_obj),
            limits=compute.SchedulerLimits(numa_topology=fake_numa_limits,
                                           vcpu=1.0,
                                           disk_gb=10.0,
                                           memory_mb=8192.0),
            instance_group=compute.InstanceGroup(hosts=['fake1'],
                                                 policies=['affinity']),
    ...
    def test_to_legacy_filter_properties_dict_with_nullable_values(self):
        spec = compute.RequestSpec(force_hosts=None,
                                   force_nodes=None,
                                   retry=None,
                                   limits=None,
                                   instance_group=None,
    ...
    def test_get_by_instance_uuid(self, get_by_uuid):
        ...
        self.assertIsInstance(req_obj.image, compute.ImageMeta)
        self.assertIsInstance(req_obj.numa_topology,
                compute.InstanceNUMATopology)
        self.assertIsInstance(req_obj.pci_requests,
                compute.InstancePCIRequests)
        self.assertIsInstance(req_obj.flavor, compute.Flavor)
        self.assertIsInstance(req_obj.retry, compute.SchedulerRetries)
        self.assertIsInstance(req_obj.limits, compute.SchedulerLimits)
        self.assertIsInstance(req_obj.instance_group, compute.InstanceGroup)

    def _check_update_primitive(self, req_obj, changes):
        self.assertEqual(req_obj.instance_uuid, changes['instance_uuid'])
        serialized_obj = compute.RequestSpec.obj_from_primitive(
