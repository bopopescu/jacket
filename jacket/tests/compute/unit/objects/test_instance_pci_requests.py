...
class _TestInstancePCIRequests(object):
    @mock.patch('compute.db.instance_extra_get_by_instance_uuid')
    def test_get_by_instance_uuid(self, mock_get):
        mock_get.return_value = {
            'instance_uuid': FAKE_UUID,
            'pci_requests': jsonutils.dumps(fake_pci_requests),
        }
        requests = compute.InstancePCIRequests.get_by_instance_uuid(
    ...
    def test_get_by_instance_uuid_and_newness(self, mock_get):
        pcir = compute.InstancePCIRequests
        mock_get.return_value = compute.InstancePCIRequests(
            instance_uuid='fake-uuid',
            requests=[compute.InstancePCIRequest(count=1, is_new=False),
                      compute.InstancePCIRequest(count=2, is_new=True)])
    ...
    def test_get_by_instance_current(self, mock_get):
        instance = compute.Instance(uuid='fake-uuid',
                                    system_metadata={})
        compute.InstancePCIRequests.get_by_instance(self.context,
                                                    instance)
    ...
    def test_get_by_instance_legacy(self):
        ...
        requests = compute.InstancePCIRequests.get_by_instance(self.context,
                                                               instance)
        self.assertEqual(2, len(requests.requests))
    ...
    def test_new_compatibility(self):
        request = compute.InstancePCIRequest(is_new=False)
        self.assertFalse(request.new)

    def test_backport_1_0(self):
        requests = compute.InstancePCIRequests(
            requests=[compute.InstancePCIRequest(count=1,
                                                 request_id=FAKE_UUID),
                      compute.InstancePCIRequest(count=2,
                                                 request_id=FAKE_UUID)])
        primitive = requests.obj_to_primitive(target_version='1.0')
        backported = compute.InstancePCIRequests.obj_from_primitive(
            primitive)
    ...
    def test_obj_from_db(self):
        req = compute.InstancePCIRequests.obj_from_db(None, FAKE_UUID, None)
        self.assertEqual(FAKE_UUID, req.instance_uuid)
        self.assertEqual(0, len(req.requests))
        db_req = jsonutils.dumps(fake_pci_requests)
        req = compute.InstancePCIRequests.obj_from_db(None, FAKE_UUID, db_req)
        self.assertEqual(FAKE_UUID, req.instance_uuid)
    ...
    def test_from_request_spec_instance_props(self):
        ...
        result = compute.InstancePCIRequests.from_request_spec_instance_props(
                                                                        result)
