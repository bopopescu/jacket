...
from jacket.objects.compute import migration
...
class _TestMigrationObject(object):
    def test_get_by_id(self):
        ...
        self.mox.StubOutWithMock(compute, 'migration_get')
        compute.migration_get(ctxt, fake_migration['id']).AndReturn(fake_migration)
        self.mox.ReplayAll()
    ...
    def test_get_by_instance_and_status(self):
        ctxt = context.get_admin_context()
        fake_migration = fake_db_migration()
        self.mox.StubOutWithMock(compute, 'migration_get_by_instance_and_status')
        compute.migration_get_by_instance_and_status(ctxt,
                                                fake_migration['id'],
                                                'migrating'
    ...
    def test_create(self):
        ctxt = context.get_admin_context()
        fake_migration = fake_db_migration()
        self.mox.StubOutWithMock(compute, 'migration_create')
        compute.migration_create(ctxt, {'source_compute': 'foo',
                                   'migration_type': 'resize'}
                            ).AndReturn(fake_migration)
        self.mox.ReplayAll()
    ...
    def test_save(self):
        ctxt = context.get_admin_context()
        fake_migration = fake_db_migration()
        self.mox.StubOutWithMock(compute, 'migration_update')
        compute.migration_update(ctxt, 123, {'source_compute': 'foo'}
                            ).AndReturn(fake_migration)
        self.mox.ReplayAll()
        mig = migration.Migration(context=ctxt)
        mig.id = 123
    ...
    def test_instance(self):
        ...
        compute.instance_get_by_uuid(ctxt, fake_migration['instance_uuid'],
    ...
    def test_instance_setter(self):
        migration = compute.Migration(instance_uuid=uuidsentinel.instance)
        inst = compute.Instance(uuid=uuidsentinel.instance)
        with mock.patch('compute.compute.Instance.get_by_uuid') as mock_get:
    ...
    def test_get_unconfirmed_by_dest_compute(self):
        ...
        self.mox.StubOutWithMock(
            compute, 'migration_get_unconfirmed_by_dest_compute')
        compute.migration_get_unconfirmed_by_dest_compute(
    ...
    def test_get_in_progress_by_host_and_node(self):
        ...
        db_migrations = [fake_migration, dict(fake_migration, id=456)]
        self.mox.StubOutWithMock(
            compute, 'migration_get_in_progress_by_host_and_node')
        compute.migration_get_in_progress_by_host_and_node(
    ...
    def test_get_by_filters(self):
        ...
        db_migrations = [fake_migration, dict(fake_migration, id=456)]
        self.mox.StubOutWithMock(
            compute, 'migration_get_all_by_filters')
        filters = {'foo': 'bar'}
        compute.migration_get_all_by_filters(ctxt, filters).AndReturn(db_migrations)
        self.mox.ReplayAll()
    ...
    def test_migrate_unset_type_resize(self):
        mig = compute.Migration(old_instance_type_id=1,
    ...
    def test_get_by_id_and_instance(self, fake_get):
        ...
        migration = compute.Migration.get_by_id_and_instance(ctxt, '1', '1')
