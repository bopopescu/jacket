...
class _TestPciDeviceObject(object):
    ...
    def _create_fake_pci_device(self, ctxt=None):
        ...
        self.mox.StubOutWithMock(compute, 'pci_device_get_by_addr')
        compute.pci_device_get_by_addr(ctxt, 1, 'a').AndReturn(fake_db_dev)
        self.mox.ReplayAll()
    ...
    def test_get_by_dev_id(self):
        ctxt = context.get_admin_context()
        self.mox.StubOutWithMock(compute, 'pci_device_get_by_id')
        compute.pci_device_get_by_id(ctxt, 1).AndReturn(fake_db_dev)
        self.mox.ReplayAll()
    ...
    def test_save_empty_parent_addr(self):
        ...
        with mock.patch.object(compute, 'pci_device_update',
    ...
    def test_save(self):
        ...
        self.mox.StubOutWithMock(compute, 'pci_device_update')
        compute.pci_device_update(ctxt, 1, 'a',
    ...
    def test_save_no_extra_info(self):
        ...
        ctxt = context.get_admin_context()
        self.stub_out('compute.compute.pci_device_update', _fake_update)
        self.pci_device = pci_device.PciDevice.create(None, dev_dict)
    ...
    def test_save_removed(self):
        ...
        self.mox.StubOutWithMock(compute, 'pci_device_destroy')
        compute.pci_device_destroy(ctxt, 1, 'a')
        self.mox.ReplayAll()
        self.pci_device.save()
        self.assertEqual(self.pci_device.status,
    ...
    def test_save_deleted(self):
        ...
        def _fake_update(ctxt, node_id, addr, updates):
            self.called = True
        self.stub_out('compute.compute.pci_device_destroy', _fake_destroy)
        self.stub_out('compute.compute.pci_device_update', _fake_update)
    ...
    @mock.patch.object(compute.Service, 'get_minimum_version', return_value=4)
    ...
    @mock.patch.object(compute.Service, 'get_minimum_version', return_value=2)
    ...
    def test_pci_device_not_equivalent_with_not_pci_device(self):
        ...
        self.assertNotEqual(pci_device1, compute.PciDeviceList())
...
class _TestPciDeviceListObject(object):
    def test_get_by_compute_node(self):
        ctxt = context.get_admin_context()
        self.mox.StubOutWithMock(compute, 'pci_device_get_all_by_node')
        compute.pci_device_get_all_by_node(ctxt, 1).AndReturn(fake_pci_devs)
        self.mox.ReplayAll()
    ...
    def test_get_by_instance_uuid(self):
        ...
        fake_db_2 = dict(fake_db_dev, address='a2',
                         status=fields.PciDeviceStatus.ALLOCATED,
                         instance_uuid='1')
        self.mox.StubOutWithMock(compute, 'pci_device_get_all_by_instance_uuid')
        compute.pci_device_get_all_by_instance_uuid(ctxt, '1').AndReturn(
            [fake_db_1, fake_db_2])
        self.mox.ReplayAll()
...
class _TestSRIOVPciDeviceObject(object):
    def _create_pci_devices(self, vf_product_id=1515, pf_product_id=1528,
        ...
        for dev in range(num_pfs):
            pci_dev = {'compute_node_id': 1,
                       ...
                       'parent_addr': None,
                       'numa_node': 0}
            pci_dev_obj = compute.PciDevice.create(None, pci_dev)
    ...
    def test_claim_PF(self):
        self._create_fake_instance()
        with mock.patch.object(compute.PciDeviceList, 'get_by_parent_address',
    ...
    def test_claim_VF(self):
        self._create_fake_instance()
        with mock.patch.object(compute.PciDevice, 'get_by_dev_addr',
    ...
    def test_free_allocated_VF(self):
        self._create_fake_instance()
        with test.nested(
            mock.patch.object(compute.PciDevice, 'get_by_dev_addr',
                               side_effect=self._fake_pci_device_get_by_addr),
            mock.patch.object(compute.PciDeviceList, 'get_by_parent_address',
                               side_effect=self._fake_get_by_parent_address)):
            self._create_pci_devices()
