...
class _TestSecurityGroupRuleObject(object):
    def test_get_by_id(self):
        with mock.patch.object(compute, 'security_group_rule_get') as sgrg:
            sgrg.return_value = fake_rule
            rule = compute.SecurityGroupRule.get_by_id(
                self.context, 1)
    ...
    def test_get_by_security_group(self):
        ...
        with mock.patch.object(compute, stupid_method) as sgrgbsg:
            sgrgbsg.return_value = [rule]
            rules = (compute.SecurityGroupRuleList.
                     get_by_security_group(self.context, secgroup))
            self.assertEqual(1, len(rules))
            self.assertEqual(123, rules[0].grantee_group.id)

    @mock.patch.object(compute, 'security_group_rule_create',
                       return_value=fake_rule)
    def test_create(self, db_mock):
        rule = compute.SecurityGroupRule(context=self.context)
        rule.protocol = 'tcp'
        secgroup = compute.SecurityGroup()
        secgroup.id = 123
        parentgroup = compute.SecurityGroup()
        parentgroup.id = 223
...
class _TestSecurityGroupRuleListObject(object):
    @mock.patch('compute.compute.security_group_rule_get_by_instance')
    def test_get_by_instance(self, mock_get):
        mock_get.return_value = fake_rules
        instance = compute.Instance(uuid='fake-uuid')
        rules = compute.SecurityGroupRuleList.get_by_instance(self.context,
                                                              instance)
