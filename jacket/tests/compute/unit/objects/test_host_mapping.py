...
from jacket.objects.compute import host_mapping
from jacket.tests.compute.unit.objects import test_cell_mapping
...
class _TestHostMappingObject(object):
    ...
    def test_get_by_host(self, host_from_db):
        ...
        mapping_obj = compute.HostMapping().get_by_host(
    ...
    def test_from_db_object_no_cell_map(self):
        ...
        fake_cell_obj = compute.CellMapping(self.context, **fake_cell)

        mapping_obj = compute.HostMapping()._from_db_object(
                self.context, compute.HostMapping(), db_mapping)
        with mock.patch.object(
                host_mapping.HostMapping, '_get_cell_mapping') as mock_load:
    ...
    def test_create(self, create_in_db):
        ...
        fake_cell_obj = compute.CellMapping(self.context, **fake_cell)
        mapping_obj = compute.HostMapping(self.context)
    ...
    def test_save(self, save_in_db):
        ...
        fake_cell_obj = compute.CellMapping(self.context, **new_fake_cell)
        mapping_obj.cell_mapping = fake_cell_obj
