...
fake_obj_numa_topology = compute.InstanceNUMATopology(
...
class _TestInstanceNUMATopology(object):
    @mock.patch('compute.db.instance_extra_update_by_uuid')
    ...
    def _test_get_by_instance_uuid(self):
        numa_topology = compute.InstanceNUMATopology.get_by_instance_uuid(
        ...
        for obj_cell, topo_cell in zip(
            ...
            self.assertIsInstance(obj_cell, compute.InstanceNUMACell)
    ...
    def test_siblings(self):
        ...
        topo = compute.VirtCPUTopology(sockets=1, cores=3, threads=0)
        inst_cell = compute.InstanceNUMACell(
                cpuset=set([0, 1, 2]), cpu_topology=topo)
        self.assertEqual([], inst_cell.siblings)

        # One thread actually means no threads
        topo = compute.VirtCPUTopology(sockets=1, cores=3, threads=1)
        inst_cell = compute.InstanceNUMACell(
                cpuset=set([0, 1, 2]), cpu_topology=topo)
        self.assertEqual([], inst_cell.siblings)

        topo = compute.VirtCPUTopology(sockets=1, cores=2, threads=2)
        inst_cell = compute.InstanceNUMACell(
                cpuset=set([0, 1, 2, 3]), cpu_topology=topo)
        self.assertEqual([set([0, 1]), set([2, 3])], inst_cell.siblings)

        topo = compute.VirtCPUTopology(sockets=1, cores=1, threads=4)
        inst_cell = compute.InstanceNUMACell(
                cpuset=set([0, 1, 2, 3]), cpu_topology=topo)
        self.assertEqual([set([0, 1, 2, 3])], inst_cell.siblings)

    def test_pin(self):
        inst_cell = compute.InstanceNUMACell(cpuset=set([0, 1, 2, 3]),
                                             cpu_pinning=None)
        inst_cell.pin(0, 14)
