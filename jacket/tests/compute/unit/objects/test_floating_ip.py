...
from jacket.objects.compute import floating_ip
...
class _TestFloatingIPObject(object):
    ...
    @mock.patch('compute.db.floating_ip_get')
    ...
    @mock.patch('compute.db.floating_ip_get_by_address')
    ...
    @mock.patch('compute.db.floating_ip_fixed_ip_associate')
    ...
    @mock.patch('compute.db.floating_ip_deallocate')
    ...
    @mock.patch('compute.db.floating_ip_destroy')
    ...
    @mock.patch('compute.db.floating_ip_disassociate')
    ...
    @mock.patch('compute.db.floating_ip_update')
    ...
    def test_save_no_fixedip(self, update):
        ...
        floatingip = floating_ip.FloatingIP(context=self.context,
                                            id=123)
        floatingip.fixed_ip = compute.FixedIP(context=self.context,
                                              id=456)
        self.assertNotIn('fixed_ip', update.calls[1])

    @mock.patch('compute.db.floating_ip_get_all')
    ...
    @mock.patch('compute.db.floating_ip_get_all_by_host')
    ...
    @mock.patch('compute.db.floating_ip_get_all_by_project')
    ...
    @mock.patch('compute.db.floating_ip_get_by_fixed_address')
    ...
    @mock.patch('compute.db.floating_ip_get_by_fixed_ip_id')
    ...
    @mock.patch('compute.db.instance_floating_address_get_all')
    ...
    def test_make_ip_info(self):
        result = compute.FloatingIPList.make_ip_info('1.2.3.4', 'pool', 'eth0')
        self.assertEqual({'address': '1.2.3.4', 'pool': 'pool',
                          'interface': 'eth0'},
                         result)

    @mock.patch('compute.db.floating_ip_bulk_create')
    def test_bulk_create(self, create_mock):
        ...
        ips = [compute.FloatingIPList.make_ip_info('1.1.1.1', 'pool', 'eth0'),
               compute.FloatingIPList.make_ip_info('1.1.1.2', 'loop', 'eth1')]
        result = compute.FloatingIPList.create(None, ips)
        self.assertIs(result, None)
        result = compute.FloatingIPList.create(None, ips, want_result=True)
        self.assertEqual('1.1.1.1', str(result[0].address))
        self.assertEqual('1.1.1.2', str(result[1].address))

    @mock.patch('compute.db.floating_ip_bulk_destroy')
    def test_bulk_destroy(self, destroy_mock):
        ips = [{'address': '1.2.3.4'}, {'address': '4.5.6.7'}]
        compute.FloatingIPList.destroy(None, ips)
        destroy_mock.assert_called_once_with(None, ips)

    def test_backport_fixedip_1_1(self):
        floating = compute.FloatingIP()
        fixed = compute.FixedIP()
