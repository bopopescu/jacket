...
class _TestMonitorMetricObject(object):
    def test_monitor_metric_to_dict(self):
        obj = compute.MonitorMetric(name='cpu.frequency',
                                    value=1000,
                                    timestamp=_ts_now,
                                    source='compute.virt.libvirt.driver')
    ...
    def test_monitor_metric_perc_to_dict(self):
        ...
        obj = compute.MonitorMetric(name='cpu.percent',
                                    value=17,
                                    timestamp=_ts_now,
                                    source='compute.virt.libvirt.driver')
    ...
    def test_conversion_in_monitor_metric_list_from_json(self):
        ...
        metrics = compute.MonitorMetricList.from_json(
            jsonutils.dumps(spec_list))
        for metric, spec in zip(metrics, spec_list):
            exp = spec['value']
            if (spec['name'] in
                    compute.monitor_metric.FIELDS_REQUIRING_CONVERSION):
                exp = spec['value'] * 100
