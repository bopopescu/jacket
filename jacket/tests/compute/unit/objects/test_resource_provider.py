...
class _TestResourceProviderNoDB(object):
    ...
    @mock.patch('compute.compute.ResourceProvider._get_by_uuid_from_db',
                return_value=_RESOURCE_PROVIDER_DB)
    def test_object_get_by_uuid(self, mock_db_get):
        resource_provider_object = compute.ResourceProvider.get_by_uuid(
    ...
    @mock.patch('compute.compute.ResourceProvider._create_in_db',
                return_value=_RESOURCE_PROVIDER_DB)
    def test_create(self, mock_db_create):
        obj = compute.ResourceProvider(context=self.context,
                                       uuid=_RESOURCE_PROVIDER_UUID)
        obj.create()
...
class TestResourceProvider(test_objects._LocalTest):
    ...
    def test_create_in_db(self):
        ...
        db_rp = compute.ResourceProvider._create_in_db(
    ...
    def test_get_by_uuid_from_db(self):
        rp = compute.ResourceProvider(context=self.context,
                                      uuid=_RESOURCE_PROVIDER_UUID)
        rp.create()
        retrieved_rp = compute.ResourceProvider._get_by_uuid_from_db(
            self.context, _RESOURCE_PROVIDER_UUID)
        self.assertEqual(rp.uuid, retrieved_rp.uuid)

        self.assertRaises(exception.NotFound,
                          compute.ResourceProvider._get_by_uuid_from_db,
                          self.context,
                          uuids.missing)


class _TestInventoryNoDB(object):
    @mock.patch('compute.compute.Inventory._create_in_db',
                return_value=_INVENTORY_DB)
    def test_create(self, mock_db_create):
        rp = compute.ResourceProvider(id=_RESOURCE_PROVIDER_ID,
                                      uuid=_RESOURCE_PROVIDER_UUID)
        obj = compute.Inventory(context=self.context,
    ...
    @mock.patch('compute.compute.Inventory._update_in_db',
                return_value=_INVENTORY_DB)
    ...
    @mock.patch('compute.compute.InventoryList._get_all_by_resource_provider')
    def test_get_all_by_resource_provider(self, mock_get):
        ...
        objs = compute.InventoryList.get_all_by_resource_provider_uuid(
...
class TestInventory(test_objects._LocalTest):
    ...
    def test_update_in_db(self):
        ...
        compute.Inventory._update_in_db(self.context,
                                        db_inventory.id,
                                        {'total': 32})
        inventories = compute.InventoryList.\
    ...
    def test_update_in_db_fails_bad_id(self):
        db_rp, db_inventory = self._make_inventory()
        self.assertRaises(exception.NotFound,
                          compute.Inventory._update_in_db,
    ...
    def test_get_all_by_resource_provider_uuid(self):
        ...
        retrieved_inventories = (
            compute.InventoryList._get_all_by_resource_provider(
    ...
    def test_create_requires_resource_provider(self):
        ...
        inventory = compute.Inventory(context=self.context,
                                      **inventory_dict)
    ...
    def test_create_id_fails(self):
        inventory = compute.Inventory(self.context, **_INVENTORY_DB)
        self.assertRaises(exception.ObjectActionError, inventory.create)

    def test_save_without_id_fails(self):
        inventory_dict = dict(_INVENTORY_DB)
        inventory_dict.pop('id')
        inventory = compute.Inventory(self.context, **inventory_dict)
