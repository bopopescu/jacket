...
from jacket.objects.compute import instance
from jacket.objects.compute import instance_info_cache
from jacket.objects.compute import pci_device
from jacket.objects.compute import security_group
from jacket.compute import test
from jacket.tests.compute.unit import fake_instance
from jacket.tests.compute.unit.objects import test_instance_fault
from jacket.tests.compute.unit.objects import test_instance_info_cache
from jacket.tests.compute.unit.objects import test_instance_numa_topology
from jacket.tests.compute.unit.objects import test_instance_pci_requests
from jacket.tests.compute.unit.objects import test_migration_context as test_mig_ctxt
from jacket.tests.compute.unit.objects import test_objects
from jacket.tests.compute.unit.objects import test_security_group
...
class _TestInstanceObject(object):
    ...
    def test_datetime_deserialization(self):
        ...
        inst = compute.Instance(uuid=uuids.instance,
                                launched_at=red_letter_date)
        primitive = inst.obj_to_primitive()
        expected = {'nova_object.name': 'Instance',
                    'nova_object.namespace': 'compute',
        ...
        inst2 = compute.Instance.obj_from_primitive(primitive)
    ...
    def test_ip_deserialization(self):
        inst = compute.Instance(uuid=uuids.instance, access_ip_v4='1.2.3.4',
                                access_ip_v6='::1')
        primitive = inst.obj_to_primitive()
        expected = {'nova_object.name': 'Instance',
                    'nova_object.namespace': 'compute',
    ...
    def test_get_without_expected(self):
        ...
        compute.instance_get_by_uuid(self.context, 'uuid',
                                columns_to_join=[]
                                ).AndReturn(self.fake_instance)
        self.mox.ReplayAll()
        inst = compute.Instance.get_by_uuid(self.context, 'uuid',
                                             expected_attrs=[])
    ...
    def test_get_with_expected(self):
        self.mox.StubOutWithMock(compute, 'instance_get_by_uuid')
        self.mox.StubOutWithMock(compute, 'instance_fault_get_by_instance_uuids')
        self.mox.StubOutWithMock(
                compute, 'instance_extra_get_by_instance_uuid')
        ...
        fake_flavor = jsonutils.dumps(
            {'cur': compute.Flavor().obj_to_primitive(),
             'old': None, 'new': None})
        ...
        fake_service = {'created_at': None, 'updated_at': None,
                        ...
                        'host': 'fake-host', 'binary': 'compute-fake',
    ...
    def test_lazy_load_services_on_deleted_instance(self):
        ...
        instance = compute.Instance(self.context, uuid=uuids.instance,
                                    deleted=True)
        self.assertEqual(0, len(instance.services))

    def test_get_by_id(self):
        self.mox.StubOutWithMock(compute, 'instance_get')
        compute.instance_get(self.context, 'instid',
                        columns_to_join=['info_cache',
                                         'security_groups']
                        ).AndReturn(self.fake_instance)
        self.mox.ReplayAll()
        inst = compute.Instance.get_by_id(self.context, 'instid')
        self.assertEqual(self.fake_instance['uuid'], inst.uuid)

    def test_load(self):
        self.mox.StubOutWithMock(compute, 'instance_get_by_uuid')
        fake_uuid = self.fake_instance['uuid']
        compute.instance_get_by_uuid(self.context, fake_uuid,
                                columns_to_join=['info_cache',
                                                 'security_groups']
                                ).AndReturn(self.fake_instance)
        ...
        self.mox.ReplayAll()
        inst = compute.Instance.get_by_uuid(self.context, fake_uuid)
    ...
    def test_load_invalid(self):
        inst = compute.Instance(context=self.context, uuid=uuids.instance)
    ...
    def test_get_remote(self):
        ...
        compute.instance_get_by_uuid(self.context, uuids.instance,
                                columns_to_join=['info_cache',
                                                 'security_groups']
                                ).AndReturn(fake_instance)
        self.mox.ReplayAll()
        inst = compute.Instance.get_by_uuid(self.context, uuids.instance)
    ...
    def test_refresh_does_not_recurse(self):
        inst = compute.Instance(context=self.context, uuid=uuids.instance,
                                metadata={})
        inst_copy = compute.Instance()
        inst_copy.uuid = inst.uuid
        self.mox.StubOutWithMock(compute.Instance, 'get_by_uuid')
        compute.Instance.get_by_uuid(self.context, uuid=inst.uuid,
    ...
    def _save_test_helper(self, cell_type, save_kwargs):
        ...
        self.mox.StubOutWithMock(compute, 'instance_info_cache_update')
        ...
        if cell_type == 'api':
            ...
            cells_api_mock.instance_update_from_api(
                    self.context, mox.IsA(compute.Instance),
        ...
        inst = compute.Instance.get_by_uuid(self.context, old_ref['uuid'])
    ...
    def test_save_related_object_if_none(self):
        with mock.patch.object(compute.Instance, '_save_pci_requests'
                ) as save_mock:
            inst = compute.Instance()
            inst = compute.Instance._from_db_object(self.context, inst,
                    self.fake_instance)
            inst.pci_requests = None
            inst.save()
            self.assertTrue(save_mock.called)

    @mock.patch('compute.compute.instance_update_and_get_original')
    ...
    def test_save_does_not_refresh_pci_devices(self, mock_fdo, mock_update):
        ...
        inst = compute.Instance(context=self.context, id=123)
    ...
    @mock.patch('compute.compute.instance_extra_update_by_uuid')
    @mock.patch('compute.compute.instance_update_and_get_original')
    @mock.patch.object(instance.Instance, '_from_db_object')
    def test_save_updates_numa_topology(self, mock_fdo, mock_update,
            mock_extra_update):
        fake_obj_numa_topology = compute.InstanceNUMATopology(cells=[
            compute.InstanceNUMACell(id=0, cpuset=set([0]), memory=128),
            compute.InstanceNUMACell(id=1, cpuset=set([1]), memory=128)])
        ...
        # NOTE(sdague): the json representation of compute object for
    ...
    def test_save_migration_context_model(self, mock_update):
        ...
        self.assertIsInstance(
            compute.MigrationContext.obj_from_db_obj(
                actual_args[0][2]['migration_context']),
            compute.MigrationContext)
        mock_update.reset_mock()
    ...
    def test_save_flavor_skips_unchanged_flavors(self):
        ...
        with mock.patch('compute.compute.instance_extra_update_by_uuid') as mock_upd:
            inst.save()
    ...
    @mock.patch.object(compute, 'instance_update_and_get_original')
    def _test_skip_cells_sync_helper(self, mock_db_update, mock_update_at_top,
        ...
        # instance compute have no equality testing assert_called_once_with
        # doesn't work.
    ...
    def test_with_info_cache(self):
        ...
        compute.instance_info_cache_update(self.context, fake_uuid,
    ...
    def test_with_security_groups(self):
        ...
        self.mox.StubOutWithMock(compute, 'instance_get_by_uuid')
        self.mox.StubOutWithMock(compute, 'instance_update_and_get_original')
        self.mox.StubOutWithMock(compute, 'security_group_update')
        compute.instance_get_by_uuid(self.context, fake_uuid,
                                columns_to_join=['info_cache',
                                                 'security_groups']
                                ).AndReturn(fake_inst)
        compute.security_group_update(self.context, 1, {'description': 'changed'}
    ...
    @mock.patch('compute.compute.EC2Ids.get_by_instance')
    ...
    def test_with_ec2_ids(self, mock_get, mock_ec2):
        ...
        fake_ec2_ids = compute.EC2Ids(instance_id='fake-inst',
                                      ami_id='fake-ami')
    ...
    def test_with_image_meta(self, mock_get):
        ...
        self.assertIsInstance(image_meta, compute.ImageMeta)
    ...
    def test_iteritems_with_extra_attrs(self):
        self.stubs.Set(compute.Instance, 'name', 'foo')
        inst = compute.Instance(uuid=uuids.instance)
    ...
    def test_create_skip_scheduled_at(self):
        self.mox.StubOutWithMock(compute, 'instance_create')
        vals = {'host': 'foo-host',
                'memory_mb': 128,
        ...
        compute.instance_create(self.context, vals).AndReturn(fake_inst)
        self.mox.ReplayAll()
        inst = compute.Instance(context=self.context,
    ...
    def test_create(self):
        ...
        extras = {'vcpu_model': None,
                  'numa_topology': None,
                  'pci_requests': None}
        compute.instance_create(self.context, {'extra': extras}).AndReturn(
            self.fake_instance)
        self.mox.ReplayAll()
        inst = compute.Instance(context=self.context)
        inst.create()
    ...
    def test_create_with_values(self):
        inst1 = compute.Instance(context=self.context,
                                 user_id=self.context.user_id,
                                 project_id=self.context.project_id,
                                 host='foo-host')
        inst1.create()
        self.assertEqual('foo-host', inst1.host)
        inst2 = compute.Instance.get_by_uuid(self.context, inst1.uuid)
    ...
    def test_create_with_extras(self):
        inst = compute.Instance(context=self.context,
            ...
            pci_requests=compute.InstancePCIRequests(
                requests=[
                    compute.InstancePCIRequest(count=123,
                                               spec=[])]),
        ...
        got_numa_topo = compute.InstanceNUMATopology.get_by_instance_uuid(
            self.context, inst.uuid)
        self.assertEqual(inst.numa_topology.instance_uuid,
                         got_numa_topo.instance_uuid)
        got_pci_requests = compute.InstancePCIRequests.get_by_instance_uuid(
            self.context, inst.uuid)
        self.assertEqual(123, got_pci_requests.requests[0].count)
        vcpu_model = compute.VirtCPUModel.get_by_instance_uuid(
            self.context, inst.uuid)
    ...
    def test_create_with_special_things(self):
        ...
        compute.instance_create(self.context,
                           {'host': 'foo-host',
    ...
    def test_destroy_stubbed(self):
        ...
        fake_inst = fake_instance.fake_db_instance(deleted_at=deleted_at,
                                                   deleted=True)
        compute.instance_destroy(self.context, uuids.instance,
                            constraint=None).AndReturn(fake_inst)
        self.mox.ReplayAll()
        inst = compute.Instance(context=self.context, id=1,
    ...
    def test_destroy(self):
        values = {'user_id': self.context.user_id,
                  'project_id': self.context.project_id}
        db_inst = compute.instance_create(self.context, values)
        inst = compute.Instance(context=self.context, id=db_inst['id'],
                                 uuid=db_inst['uuid'])
        inst.destroy()
        self.assertRaises(exception.InstanceNotFound,
                          compute.instance_get_by_uuid, self.context,
                          db_inst['uuid'])
    ...
    def test_destroy_host_constraint(self):
        values = {'user_id': self.context.user_id,
                  ...
                  'host': 'foo'}
        db_inst = compute.instance_create(self.context, values)
        inst = compute.Instance.get_by_uuid(self.context, db_inst['uuid'])
        inst.host = None
        self.assertRaises(exception.ObjectActionError,
                          inst.destroy)

    @mock.patch.object(cells_rpcapi.CellsAPI, 'instance_destroy_at_top')
    @mock.patch.object(compute, 'instance_destroy')
    ...
    @mock.patch('compute.compute.InstancePCIRequests.get_by_instance_uuid')
    def test_get_with_pci_requests(self, mock_get):
        mock_get.return_value = compute.InstancePCIRequests()
        db_instance = compute.instance_create(self.context, {
            'user_id': self.context.user_id,
            'project_id': self.context.project_id})
    ...
    def test_get_flavor(self):
        ...
        inst = compute.Instance(flavor=db_flavor)
        self.assertEqual(db_flavor['flavorid'],
                         inst.get_flavor().flavorid)

    def test_get_flavor_namespace(self):
        db_flavor = flavors.get_default_flavor()
        inst = compute.Instance(old_flavor=db_flavor)
        self.assertEqual(db_flavor['flavorid'],
                         inst.get_flavor('old').flavorid)

    @mock.patch.object(compute, 'instance_metadata_delete')
    ...
    def test_load_ec2_ids_calls_handler(self):
        ...
        with mock.patch.object(inst, '_load_ec2_ids') as mock_load:
            def fake_load():
                inst.ec2_ids = compute.EC2Ids(instance_id='fake-inst',
                                              ami_id='fake-ami')
    ...
    def test_load_migration_context_no_data(self):
        inst = instance.Instance(context=self.context, uuid=uuids.instance)
        with mock.patch.object(
                compute.MigrationContext, 'get_by_instance_uuid') as mock_get:
    ...
    def test_apply_revert_migration_context(self):
        ...
        self.assertIsInstance(inst.numa_topology, compute.InstanceNUMATopology)
    ...
    def test_drop_migration_context(self):
        ...
        with mock.patch(
                'compute.compute.instance_extra_update_by_uuid') as update_extra:
    ...
    @mock.patch('compute.compute.instance_fault_get_by_instance_uuids')
    ...
    @mock.patch('compute.compute.SecurityGroupList.get_by_instance')
    def test_load_security_groups(self, mock_get):
        ...
        fake_secgroups = security_group.SecurityGroupList(compute=secgroups)
        mock_get.return_value = fake_secgroups
        inst = compute.Instance(context=self.context, uuid='fake')
        secgroups = inst.security_groups
        mock_get.assert_called_once_with(self.context, inst)
        self.assertEqual(fake_secgroups, secgroups)

    @mock.patch('compute.compute.PciDeviceList.get_by_instance_uuid')
    def test_load_pci_devices(self, mock_get):
        fake_pci_devices = pci_device.PciDeviceList()
        mock_get.return_value = fake_pci_devices
        inst = compute.Instance(context=self.context, uuid=uuids.pci_devices)
    ...
    def test_get_with_extras(self):
        pci_requests = compute.InstancePCIRequests(requests=[
            compute.InstancePCIRequest(count=123, spec=[])])
        inst = compute.Instance(context=self.context,
                                user_id=self.context.user_id,
                                project_id=self.context.project_id,
...
class TestInstanceObject(test_objects._LocalTest,
    ...
    def _test_save_objectfield_fk_constraint_fails(self, foreign_key,
        ...
        attrs = compute.instance.INSTANCE_OPTIONAL_ATTRS
...
class _TestInstanceListObject(object):
    ...
    def test_get_all_by_filters(self):
        ...
        self.mox.StubOutWithMock(compute, 'instance_get_all_by_filters')
        compute.instance_get_all_by_filters(self.context, {'foo': 'bar'}, 'uuid',
                                       'asc', limit=None, marker=None,
                                       columns_to_join=['metadata']
                                       ).AndReturn(fakes)
        self.mox.ReplayAll()
        inst_list = compute.InstanceList.get_by_filters(
        ...
        for i in range(0, len(fakes)):
            ...
            self.assertEqual(fakes[i]['uuid'], inst_list.compute[i].uuid)

    def test_get_all_by_filters_sorted(self):
        fakes = [self.fake_instance(1), self.fake_instance(2)]
        self.mox.StubOutWithMock(compute, 'instance_get_all_by_filters_sort')
        compute.instance_get_all_by_filters_sort(self.context, {'foo': 'bar'},
    ...
    @mock.patch.object(compute, 'instance_get_all_by_filters_sort')
    @mock.patch.object(compute, 'instance_get_all_by_filters')
    ...
    def test_get_all_by_filters_works_for_cleaned(self):
        ...
        self.mox.StubOutWithMock(compute, 'instance_get_all_by_filters')
        compute.instance_get_all_by_filters(self.context,
        ...
        self.assertEqual(fakes[1]['uuid'], inst_list.compute[0].uuid)

    def test_get_by_host(self):
        fakes = [self.fake_instance(1),
                 self.fake_instance(2)]
        self.mox.StubOutWithMock(compute, 'instance_get_all_by_host')
        compute.instance_get_all_by_host(self.context, 'foo',
                                    columns_to_join=None).AndReturn(fakes)
        self.mox.ReplayAll()
        inst_list = compute.InstanceList.get_by_host(self.context, 'foo')
        for i in range(0, len(fakes)):
            self.assertIsInstance(inst_list.objects[i], instance.Instance)
            self.assertEqual(fakes[i]['uuid'], inst_list.compute[i].uuid)
            self.assertEqual(self.context, inst_list.compute[i]._context)
        self.assertEqual(set(), inst_list.obj_what_changed())

    def test_get_by_host_and_node(self):
        fakes = [self.fake_instance(1),
                 self.fake_instance(2)]
        self.mox.StubOutWithMock(compute, 'instance_get_all_by_host_and_node')
        compute.instance_get_all_by_host_and_node(self.context, 'foo', 'bar',
                                             columns_to_join=None).AndReturn(
                                                 fakes)
        self.mox.ReplayAll()
        inst_list = compute.InstanceList.get_by_host_and_node(self.context,
                                                              'foo', 'bar')
        for i in range(0, len(fakes)):
            self.assertIsInstance(inst_list.objects[i], instance.Instance)
            self.assertEqual(fakes[i]['uuid'], inst_list.compute[i].uuid)
    ...
    def test_get_by_host_and_not_type(self):
        fakes = [self.fake_instance(1),
                 self.fake_instance(2)]
        self.mox.StubOutWithMock(compute, 'instance_get_all_by_host_and_not_type')
        compute.instance_get_all_by_host_and_not_type(self.context, 'foo',
                                                 type_id='bar').AndReturn(
                                                     fakes)
        self.mox.ReplayAll()
        inst_list = compute.InstanceList.get_by_host_and_not_type(
            self.context, 'foo', 'bar')
        for i in range(0, len(fakes)):
            self.assertIsInstance(inst_list.objects[i], instance.Instance)
            self.assertEqual(fakes[i]['uuid'], inst_list.compute[i].uuid)

    @mock.patch('compute.compute.instance._expected_cols')
    @mock.patch('compute.compute.instance_get_all')
    def test_get_all(self, mock_get_all, mock_exp):
        fakes = [self.fake_instance(1), self.fake_instance(2)]
        mock_get_all.return_value = fakes
        mock_exp.return_value = mock.sentinel.exp_att
        inst_list = compute.InstanceList.get_all(
    ...
    def test_get_hung_in_rebooting(self):
        fakes = [self.fake_instance(1),
                 self.fake_instance(2)]
        dt = utils.isotime()
        self.mox.StubOutWithMock(compute, 'instance_get_all_hung_in_rebooting')
        compute.instance_get_all_hung_in_rebooting(self.context, dt).AndReturn(
            fakes)
        self.mox.ReplayAll()
        inst_list = compute.InstanceList.get_hung_in_rebooting(self.context,
                                                               dt)
        for i in range(0, len(fakes)):
            self.assertIsInstance(inst_list.objects[i], instance.Instance)
            self.assertEqual(fakes[i]['uuid'], inst_list.compute[i].uuid)
    ...
    def test_get_active_by_window_joined(self):
        ...
        with mock.patch.object(compute, 'instance_get_active_by_window_joined',
                               fake_instance_get_active_by_window_joined):
            inst_list = compute.InstanceList.get_active_by_window_joined(
    ...
    def test_with_fault(self):
        ...
        self.mox.StubOutWithMock(compute, 'instance_get_all_by_host')
        self.mox.StubOutWithMock(compute, 'instance_fault_get_by_instance_uuids')
        compute.instance_get_all_by_host(self.context, 'host',
                                    columns_to_join=[]).AndReturn(fake_insts)
        compute.instance_fault_get_by_instance_uuids(
            self.context, [x['uuid'] for x in fake_insts]
            ).AndReturn(fake_faults)
        self.mox.ReplayAll()
        instances = compute.InstanceList.get_by_host(self.context, 'host',
                                                     expected_attrs=['fault'],
                                                     use_subordinate=False)
    ...
    def test_fill_faults(self):
        ...
        inst1 = compute.Instance(uuid=uuids.db_fault_1)
        inst2 = compute.Instance(uuid=uuids.db_fault_2)
        insts = [inst1, inst2]
        for inst in insts:
            inst.obj_reset_changes()
        db_faults = {
        ...
        compute.instance_fault_get_by_instance_uuids(self.context,
                                                [x.uuid for x in insts],
                                                ).AndReturn(db_faults)
        self.mox.ReplayAll()
        inst_list = compute.InstanceList()
    ...
    @mock.patch('compute.compute.instance.Instance.obj_make_compatible')
    def test_get_by_security_group(self, mock_compat):
        ...
        with mock.patch.object(compute, 'security_group_get') as sgg:
    ...
    def test_get_by_grantee_security_group_ids(self):
        ...
        with mock.patch.object(
            compute, 'instance_get_all_by_grantee_security_groups') as igabgsg:
