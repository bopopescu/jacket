...
from jacket.tests.compute.unit import fake_pci_device_pools as fake_pci
...
class _TestPciDevicePoolObject(object):
    ...
    def test_pci_pool_from_dict_not_distructive(self):
        ...
        compute.PciDevicePool.from_dict(test_dict)
        self.assertEqual(fake_pci.fake_pool_dict, test_dict)

    def test_pci_pool_from_dict(self):
        pool_obj = compute.PciDevicePool.from_dict(fake_pci.fake_pool_dict)
    ...
    def test_pci_pool_from_dict_bad_tags(self):
        ...
        self.assertRaises(ValueError,
                          compute.PciDevicePool.from_dict,
                          value=bad_dict)
    ...
    def test_pci_pool_from_dict_no_tags(self):
        ...
        dict_notag.pop('t1')
        dict_notag.pop('t2')
        pool_obj = compute.PciDevicePool.from_dict(dict_notag)
        self.assertEqual(pool_obj.tags, {})

    def test_pci_pool_to_dict(self):
        tags = {'t1': 'foo', 't2': 'bar'}
        pool_obj = compute.PciDevicePool(product_id='pid', tags=tags)
    ...
    def test_pci_pool_to_dict_no_tags(self):
        pool_obj = compute.PciDevicePool(product_id='pid', tags={})
        pool_dict = pool_obj.to_dict()
        self.assertEqual({'product_id': 'pid'}, pool_dict)

    def test_pci_pool_to_dict_with_tags_unset(self):
        pool_obj = compute.PciDevicePool(product_id='pid')
        pool_dict = pool_obj.to_dict()
        self.assertEqual({'product_id': 'pid'}, pool_dict)

    def test_obj_make_compatible(self):
        pool_obj = compute.PciDevicePool(product_id='pid', numa_node=1)
