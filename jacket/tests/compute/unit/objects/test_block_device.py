...
class _TestBlockDeviceMappingObject(object):
    ...
    def _test_save(self, cell_type=None, update_device_name=False):
        ...
        with test.nested(
            mock.patch.object(
                compute, 'block_device_mapping_update', return_value=fake_bdm),
            mock.patch.object(
                cells_rpcapi.CellsAPI, 'bdm_update_or_create_at_top')
        ) as (bdm_update_mock, cells_update_mock):
            bdm_object = compute.BlockDeviceMapping(context=self.context)
            bdm_object.id = 123
    ...
    def test_save_instance_changed(self):
        bdm_object = compute.BlockDeviceMapping(context=self.context)
        bdm_object.instance = compute.Instance()
        self.assertRaises(exception.ObjectActionError,
                          bdm_object.save)

    @mock.patch.object(compute, 'block_device_mapping_update', return_value=None)
    def test_save_not_found(self, bdm_update):
        bdm_object = compute.BlockDeviceMapping(context=self.context)
        bdm_object.id = 123
        self.assertRaises(exception.BDMNotFound, bdm_object.save)

    @mock.patch.object(compute, 'block_device_mapping_get_all_by_volume_id')
    def test_get_by_volume_id(self, get_by_vol_id):
        ...
        vol_bdm = compute.BlockDeviceMapping.get_by_volume_id(
    ...
    def test_get_by_volume_id_not_found(self, get_by_vol_id):
        ...
        self.assertRaises(exception.VolumeBDMNotFound,
                          compute.BlockDeviceMapping.get_by_volume_id,
                          self.context, 'fake-volume-id')
    ...
    def test_get_by_volume_returned_single(self, get_all):
        ...
        vol_bdm = compute.BlockDeviceMapping.get_by_volume(
                self.context, 'fake-volume-id')
    ...
    def test_get_by_instance_and_volume_id(self, mock_get):
        ...
        obj_bdm = compute.BlockDeviceMapping
    ...
    def _test_create_mocked(self, cell_type=None, update_or_create=False,
        ...
        with test.nested(
            mock.patch.object(
                    compute, 'block_device_mapping_create', return_value=fake_bdm),
            mock.patch.object(
                    compute, 'block_device_mapping_update_or_create',
            ...
            bdm = compute.BlockDeviceMapping(context=self.context, **values)
            if update_or_create:
    ...
    def _test_destroy_mocked(self, cell_type=None):
        ...
        if cell_type:
            ...
            self.flags(enable=False, group='cells')
        with test.nested(
            mock.patch.object(compute, 'block_device_mapping_destroy'),
...
class _TestBlockDeviceMappingListObject(object):
    ...
    @mock.patch.object(compute, 'block_device_mapping_get_all_by_instance_uuids')
    def test_bdms_by_instance_uuid(self, get_all_by_inst_uuids):
        ...
        bdms_by_uuid = compute.BlockDeviceMappingList.bdms_by_instance_uuid(
            self.context, ['fake-instance'])
        self.assertEqual(['fake-instance'], list(bdms_by_uuid.keys()))
        self.assertIsInstance(
            bdms_by_uuid['fake-instance'], compute.BlockDeviceMappingList)
        for faked, got in zip(fakes, bdms_by_uuid['fake-instance']):
            self.assertIsInstance(got, compute.BlockDeviceMapping)
    ...
    def test_get_by_instance_uuids(self, get_all_by_inst_uuids):
        fakes = [self.fake_bdm(123), self.fake_bdm(456)]
        get_all_by_inst_uuids.return_value = fakes
        bdm_list = compute.BlockDeviceMappingList.get_by_instance_uuids(
            self.context, ['fake-instance'])
    ...
    @mock.patch.object(compute, 'block_device_mapping_get_all_by_instance')
