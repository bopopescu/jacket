...
class TestNotificationBase(test.NoDBTestCase):
    ...
    def setUp(self):
        ...
        with mock.patch('compute.db.service_update') as mock_db_service_update:
            self.service_obj = compute.Service(context=mock.sentinel.context,
    ...
    def _verify_notification(self, mock_notifier, mock_context,
        ...
        mock_notifier.prepare.assert_called_once_with(
            publisher_id='compute-fake:fake-host')
    ...
    @mock.patch('compute.rpc.LEGACY_NOTIFIER')
    @mock.patch('compute.rpc.NOTIFIER')
