...
from jacket.objects.compute import aggregate
from jacket.objects.compute import fields
from jacket.objects.compute import service
from jacket.compute import test
from jacket.tests.compute.unit.objects import test_compute_node
...
class _TestServiceObject(object):
    ...
    def _test_query(self, db_method, obj_method, *args, **kwargs):
        self.mox.StubOutWithMock(compute, db_method)
    ...
    def test_create(self):
        self.mox.StubOutWithMock(compute, 'service_create')
        compute.service_create(self.context, {'host': 'fake-host',
    ...
    def test_save(self):
        self.mox.StubOutWithMock(compute, 'service_update')
        compute.service_update(self.context, 123,
                          {'host': 'fake-host',
                           'version': fake_service['version']}
                          ).AndReturn(fake_service)
        self.mox.ReplayAll()
    ...
    @mock.patch.object(compute, 'service_create',
    ...
    def _test_destroy(self):
        self.mox.StubOutWithMock(compute, 'service_destroy')
        compute.service_destroy(self.context, 123)
        self.mox.ReplayAll()
        service_obj = service.Service(context=self.context)
        service_obj.id = 123
        service_obj.destroy()
    ...
    def test_destroy(self):
        # The test harness needs compute.service_destroy to work,
        # so avoid leaving it broken here after we're done
        orig_service_destroy = compute.service_destroy
        try:
            self._test_destroy()
        finally:
            compute.service_destroy = orig_service_destroy

    def test_get_by_topic(self):
        self.mox.StubOutWithMock(compute, 'service_get_all_by_topic')
        compute.service_get_all_by_topic(self.context, 'fake-topic').AndReturn(
            [fake_service])
        self.mox.ReplayAll()
    ...
    @mock.patch('compute.compute.service_get_all_by_binary')
    ...
    def test_get_by_host(self):
        self.mox.StubOutWithMock(compute, 'service_get_all_by_host')
        compute.service_get_all_by_host(self.context, 'fake-host').AndReturn(
            [fake_service])
        self.mox.ReplayAll()
    ...
    def test_get_all(self):
        self.mox.StubOutWithMock(compute, 'service_get_all')
        compute.service_get_all(self.context, disabled=False).AndReturn(
            [fake_service])
        self.mox.ReplayAll()
    ...
    def test_get_all_with_az(self):
        ...
        self.mox.StubOutWithMock(aggregate.AggregateList,
                                 'get_by_metadata_key')
        compute.service_get_all(self.context, disabled=None).AndReturn(
    ...
    def test_compute_node(self):
        fake_compute_node = compute.ComputeNode._from_db_object(
            self.context, compute.ComputeNode(),
            test_compute_node.fake_compute_node)
        self.mox.StubOutWithMock(compute.ComputeNodeList, 'get_all_by_host')
        compute.ComputeNodeList.get_all_by_host(
    ...
    @mock.patch.object(compute.ComputeNodeList, 'get_all_by_host')
    def test_obj_make_compatible_for_compute_node(self, get_all_by_host):
        service_obj = compute.Service(context=self.context)
        fake_service_dict = fake_service.copy()
        fake_compute_obj = compute.ComputeNode(host=fake_service['host'],
    ...
    @mock.patch('compute.compute.service_get_minimum_version')
    def test_get_minimum_version_none(self, mock_get):
        mock_get.return_value = None
        self.assertEqual(0,
                         compute.Service.get_minimum_version(self.context,
                                                             'compute-compute'))
        mock_get.assert_called_once_with(self.context, 'compute-compute')
    ...
    @mock.patch('compute.compute.service_get_minimum_version')
    @mock.patch('compute.compute.service.LOG')
    def test_get_minimum_version_checks_binary(self, mock_log, mock_get):
        ...
        self.assertRaises(exception.ObjectActionError,
                         compute.Service.get_minimum_version,
                          self.context,
                          'compute')
    ...
    def test_get_minimum_version_with_caching(self, mock_get):
        compute.Service.enable_min_version_cache()
        mock_get.return_value = 123
        self.assertEqual(123,
                         compute.Service.get_minimum_version(self.context,
                                                             'compute-compute'))
        self.assertEqual({"compute-compute": 123},
                         compute.Service._MIN_VERSION_CACHE)
        self.assertEqual(123,
                         compute.Service.get_minimum_version(self.context,
                                                             'compute-compute'))
        mock_get.assert_called_once_with(self.context, 'compute-compute')
        compute.Service._SERVICE_VERSION_CACHING = False
        compute.Service.clear_min_version_cache()

    @mock.patch('compute.compute.service_get_minimum_version', return_value=2)
    def test_create_above_minimum(self, mock_get):
        with mock.patch('compute.compute.service.SERVICE_VERSION',
                        new=3):
            compute.Service(context=self.context,
                            binary='compute-compute').create()
...
class TestServiceVersion(test.TestCase):
    ...
    def test_version(self):
        ...
        self.assertEqual(
            ...
            'compute.compute.service.SERVICE_VERSION_HISTORY '
        ...
        self.assertEqual(
            ...
            'increment compute.compute.service.SERVICE_VERSION, and make sure it'
            'is equal to compute.compute.manager.ComputeManager.target.version.')
    ...
    def test_save_noop_with_only_version(self):
        o = compute.Service(context=self.ctxt, id=fake_service['id'])
        o.obj_reset_changes(['id'])
        self.assertEqual(set(['version']), o.obj_what_changed())
        with mock.patch('compute.compute.service_update') as mock_update:
            o.save()
...
class TestServiceStatusNotification(test.TestCase):
    ...
    @mock.patch('compute.compute.service.ServiceStatusNotification')
    ...
    @mock.patch('compute.compute.service_update')
    def test_service_update_with_notification(self, mock_db_service_update):
        service_obj = compute.Service(context=self.ctxt, id=fake_service['id'])
