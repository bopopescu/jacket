...
fake_cpu_model_feature_obj = compute.VirtCPUFeature(
    **fake_cpu_model_feature)
...
fake_vcpumodel_dict = {
    ...
    'topology': compute.VirtCPUTopology(sockets=1, cores=1, threads=1),
    'features': [fake_cpu_model_feature_obj],
    'mode': cpumodel.MODE_HOST_MODEL,
    'model': 'fake-model',
}
fake_vcpumodel = compute.VirtCPUModel(**fake_vcpumodel_dict)


class _TestVirtCPUFeatureObj(object):
    def test_policy_limitation(self):
        obj = compute.VirtCPUFeature()
...
class _TestVirtCPUModel(object):
    def test_create(self):
        model = compute.VirtCPUModel(**fake_vcpumodel_dict)
    ...
    def test_serialize(self):
        modelin = compute.VirtCPUModel(**fake_vcpumodel_dict)
        modelout = compute.VirtCPUModel.from_json(modelin.to_json())
