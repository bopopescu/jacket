...
class _TestAggregateObject(object):
    def test_get_by_id(self):
        self.mox.StubOutWithMock(compute, 'aggregate_get')
        compute.aggregate_get(self.context, 123).AndReturn(fake_aggregate)
        self.mox.ReplayAll()
        agg = aggregate.Aggregate.get_by_id(self.context, 123)
        self.compare_obj(agg, fake_aggregate, subs=SUBS)

    @mock.patch('compute.objects.Aggregate.save')
    @mock.patch('compute.compute.aggregate_get')
    ...
    def test_create(self):
        self.mox.StubOutWithMock(compute, 'aggregate_create')
        compute.aggregate_create(self.context, {'name': 'foo',
    ...
    def test_save(self):
        self.mox.StubOutWithMock(compute, 'aggregate_update')
        compute.aggregate_update(self.context, 123, {'name': 'baz'}).AndReturn(
            fake_aggregate)
        self.mox.ReplayAll()
        agg = aggregate.Aggregate(context=self.context)
        agg.id = 123
    ...
    def test_update_metadata(self):
        self.mox.StubOutWithMock(compute, 'aggregate_metadata_delete')
        self.mox.StubOutWithMock(compute, 'aggregate_metadata_add')
        compute.aggregate_metadata_delete(self.context, 123, 'todelete')
        compute.aggregate_metadata_add(self.context, 123, {'toadd': 'myval'})
        self.mox.ReplayAll()
    ...
    def test_destroy(self):
        self.mox.StubOutWithMock(compute, 'aggregate_delete')
        compute.aggregate_delete(self.context, 123)
        self.mox.ReplayAll()
        agg = aggregate.Aggregate(context=self.context)
        agg.id = 123
        agg.destroy()
    ...
    def test_add_host(self):
        self.mox.StubOutWithMock(compute, 'aggregate_host_add')
        compute.aggregate_host_add(self.context, 123, 'bar'
                              ).AndReturn({'host': 'bar'})
        self.mox.ReplayAll()
        agg = aggregate.Aggregate()
        agg.id = 123
    ...
    def test_delete_host(self):
        self.mox.StubOutWithMock(compute, 'aggregate_host_delete')
        compute.aggregate_host_delete(self.context, 123, 'foo')
        self.mox.ReplayAll()
        agg = aggregate.Aggregate()
        agg.id = 123
        agg.hosts = ['foo', 'bar']
    ...
    def test_get_all(self):
        self.mox.StubOutWithMock(compute, 'aggregate_get_all')
        compute.aggregate_get_all(self.context).AndReturn([fake_aggregate])
        self.mox.ReplayAll()
    ...
    def test_by_host(self):
        self.mox.StubOutWithMock(compute, 'aggregate_get_by_host')
        compute.aggregate_get_by_host(self.context, 'fake-host', key=None,
    ...
    @mock.patch('compute.compute.aggregate_get_by_metadata_key')
