...
from jacket.api.compute.openstack.compute import fping as fping_v21
from jacket.api.compute.openstack.compute.legacy_v2.contrib import fping
...
class FpingTestV21(test.TestCase):
    ...
    def setUp(self):
        ...
        self.stub_out("compute.db.instance_get_by_uuid",
                      return_server)
        self.stubs.Set(jacket.compute.utils, "execute",
                       execute)
    ...
    def test_fping_index_policy(self):
        ...
        req = fakes.HTTPRequest.blank(self._get_url() +
                                      "/os-fping?all_tenants=1")
        req.environ["compute.context"].is_admin = True
        res_dict = self.controller.index(req)
        self.assertIn("servers", res_dict)
    ...
    @mock.patch('compute.db.instance_get_by_uuid')
