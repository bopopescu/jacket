...
from jacket.api.compute.openstack.compute import hosts as os_hosts_v21
...
from jacket.tests.compute.unit import fake_hosts
...
def _create_instance(**kwargs):
    ...
    return compute.instance_create(ctxt, _create_instance_dict(**kwargs))
...
class FakeRequestWithNovaZone(object):
    environ = {"compute.context": context_maker.get_admin_context()}
    GET = {"zone": "compute"}
...
class HostTestCaseV21(test.TestCase):
    ...
    def _setup_stubs(self):
        ...
        self.stub_out('compute.compute.service_get_all',
                      stub_service_get_all)
        # Only hosts in our fake DB exist
        self.stub_out('compute.compute.service_get_by_host_and_binary',
    ...
    def test_host_status_bad_host(self):
        ...
        self.req.environ["compute.context"].is_admin = True
        dest = 'dummydest'
    ...
    def _create_compute_service(self):
        ...
        dic = {'host': 'dummy', 'binary': 'compute-compute', 'topic': 'compute',
               'report_count': 0}
        s_ref = compute.service_create(ctxt, dic)
        ...
        dic = {'service_id': s_ref['id'],
               ...
               'cpu_info': '', 'stats': ''}
        compute.compute_node_create(ctxt, dic)

        return compute.service_get(ctxt, s_ref['id'])
    ...
    def test_show_no_project(self):
        ...
        compute.service_destroy(ctxt, s_ref['id'])
    ...
    def test_show_works_correctly(self):
        ...
        for resource in result['host']:
            ...
            self.assertEqual(set(column), set(resource['resource'].keys()))
        compute.service_destroy(ctxt, s_ref['id'])
        compute.instance_destroy(ctxt, i_ref1['uuid'])
        compute.instance_destroy(ctxt, i_ref2['uuid'])
