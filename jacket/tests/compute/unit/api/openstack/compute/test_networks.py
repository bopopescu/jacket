...
from jacket.api.compute.openstack.compute.legacy_v2.contrib import networks_associate
from jacket.api.compute.openstack.compute.legacy_v2.contrib import os_networks \
        as networks
from jacket.api.compute.openstack.compute import networks as networks_v21
from jacket.api.compute.openstack.compute import networks_associate \
        as networks_associate_v21
from jacket.api.compute.openstack import extensions
import jacket.compute.context
from jacket.compute import exception
from jacket.compute.network import manager
from jacket.compute.network.neutronv2 import api as neutron
...
class FakeNetworkAPI(object):
    ...
    def _fake_db_network_get_all(self, context, project_only="allow_none"):
        ...
        objs = [compute.Network._from_db_object(context,
                                                compute.Network(),
                                                net)
                for net in nets]
        return compute.NetworkList(compute=objs)
        ...
        for network in self.networks:
            if network.get('uuid') == network_id:
                if 'injected' in network and network['injected'] is None:
                    ...
                    # When using compute-network, 'injected' value should be
                    # boolean because of the definition of compute.Network().
                ...
                return compute.Network._from_db_object(context,
                                                       compute.Network(),
                                                       network)
...
class NetworkCreateExceptionsTestV21(test.TestCase):
    ...
    def test_network_create_cidr_conflict(self):
        ...
        @staticmethod
        def get_all(context):
            ret = compute.NetworkList(context=context, compute=[])
            net = compute.Network(cidr='10.0.0.0/23')
            ret.objects.append(net)
            return ret

        self.stubs.Set(compute.NetworkList, 'get_all', get_all)
...
class NetworksTestV21(test.NoDBTestCase):
    ...
    def test_network_list_all_as_user(self):
        ...
        project_id = self.req.environ["compute.context"].project_id
        cxt = self.req.environ["jacket.compute"]
    ...
    @mock.patch('compute.tests.unit.api.openstack.compute.test_networks.'
