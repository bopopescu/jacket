...
from jacket.api.compute.openstack.compute import disk_config
from jacket.api.compute.openstack.compute import extension_info
from jacket.api.compute.openstack.compute import ips
from jacket.api.compute.openstack.compute import keypairs
from jacket.api.compute.openstack.compute.schemas import servers as servers_schema
from jacket.api.compute.openstack.compute import servers
from jacket.api.compute.openstack.compute import views
...
from jacket.compute import utils as nova_utils
...
def return_servers_empty(context, *args, **kwargs):
    return compute.InstanceList(compute=[])
...
def fake_instance_get_all_with_locked(context, list_locked, **kwargs):
    ...
    return compute.InstanceList(compute=obj_list)
...
class ControllerTest(test.TestCase):
    ...
    def setUp(self):
        ...
        self.stub_out('compute.compute.instance_update_and_get_original',
...
class ServersControllerTest(ControllerTest):
    ...
    def test_requested_networks_with_duplicate_networks(self):
        # duplicate networks are allowed only for compute neutron v2.0
        network = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa'
    ...
    def test_get_servers_with_bad_option(self):
        ...
        def fake_get_all(compute_self, context, search_opts=None,
            ...
            return instance_obj._make_instance_list(
                context, compute.InstanceList(), db_list, FIELDS)
    ...
    def test_get_servers_allows_flavor(self):
        ...
        def fake_get_all(compute_self, context, search_opts=None,
            ...
            return compute.InstanceList(
                compute=[fakes.stub_instance_obj(100, uuid=server_uuid)])
    ...
    def test_get_servers_with_bad_flavor(self):
        ...
        with mock.patch.object(compute_api.API, 'get_all') as mock_get:
            mock_get.return_value = compute.InstanceList(compute=[])
            servers = self.controller.index(req)['servers']
    ...
    def test_get_all_server_details_with_host(self):
        ...
        def return_servers_with_host(*args, **kwargs):
            return compute.InstanceList(
                compute=[fakes.stub_instance_obj(None,
                                                 id=i + 1,
                                                 user_id='fake',
                                                 project_id='fake',
                                                 host=i % 2,
...
class ServersControllerDeleteTest(ControllerTest):
    ...
    def test_delete_server_instance_if_not_launched(self):
        ...
        self.stub_out('compute.compute.instance_destroy', instance_destroy_mock)
...
class ServersControllerRebuildInstanceTest(ControllerTest):
    ...
    def setUp(self):
        ...
        self.useFixture(
            fixtures.MonkeyPatch('compute.api.openstack.compute.servers.'
...
class ServersControllerUpdateTest(ControllerTest):
    ...
    def test_update_server_not_found_on_update(self):
        ...
        self.stub_out('compute.compute.instance_update_and_get_original', fake_update)
        body = {'server': {'name': 'server_test'}}
        req = self._get_request(body)
        self.assertRaises(webob.exc.HTTPNotFound, self.controller.update,
                          req, FAKE_UUID, body=body)
...
class ServersControllerCreateTest(test.TestCase):
    ...
    def setUp(self):
        ...
        self.stub_out('compute.compute.project_get_networks', project_get_networks)
        self.stub_out('compute.compute.instance_create', instance_create)
        self.stub_out('compute.compute.instance_system_metadata_update', fake_method)
        self.stub_out('compute.compute.instance_get', instance_get)
        self.stub_out('compute.compute.instance_update', instance_update)
        self.stub_out('compute.compute.instance_update_and_get_original',
    ...
    def test_create_instance_private_flavor(self):
        ...
        compute.flavor_create(context.get_admin_context(), values)
    ...
    #
    #     self.stub_out('compute.compute.key_pair_get', key_pair_get)
    ...
    def test_create_instance_secgroup_leading_trailing_spaces_compat_mode(
        ...
        def fake_create(*args, **kwargs):
            ...
            return (compute.InstanceList(compute=[fakes.stub_instance_obj(
                self.req.environ['compute.context'])]), None)
    ...
    def test_create_instance_above_quota_server_group_members(self):
        ctxt = self.req.environ['compute.context']
        fake_group = compute.InstanceGroup(ctxt)
        ...
        def fake_count(context, name, group, user_id):
            ...
            self.assertEqual(user_id,
                             self.req.environ['compute.context'].user_id)
            return 10
        ...
        self.stub_out('compute.compute.instance_destroy', fake_instance_destroy)
    ...
    def test_create_instance_with_group_hint(self):
        ctxt = self.req.environ['compute.context']
        test_group = compute.InstanceGroup(ctxt)
        ...
        test_group = compute.InstanceGroup.get_by_uuid(ctxt, test_group.uuid)
    ...
    def test_create_instance_without_personality_should_get_empty_list(self):
        ...
        self.stub_out('compute.compute.api.API.create', create)
...
class ServersViewBuilderTest(test.TestCase):
    ...
    def test_build_server_detail_with_fault_admin(self):
        ...
        self.request.environ['compute.context'].is_admin = True
        output = self.view_builder.show(self.request, self.instance)
        self.assertThat(output['server']['fault'],
                        matchers.DictMatches(expected_fault))
...
class ServersAllExtensionsTestCase(test.TestCase):
    ...
    def test_update_missing_server(self):
        ...
        body = {'foo': {'a': 'b'}}
        req.body = jsonutils.dump_as_bytes(body)
        with mock.patch('compute.compute.Instance.save') as mock_save:
            res = req.get_response(self.app)
