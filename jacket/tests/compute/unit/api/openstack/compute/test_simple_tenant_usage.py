...
from jacket.api.compute.openstack.compute.legacy_v2.contrib import simple_tenant_usage as \
...
@mock.patch.object(compute, 'instance_get_active_by_window_joined',
...
class SimpleTenantUsageTestV21(test.TestCase):
    ...
    def _test_verify_index(self, start, stop):
        ...
        req.environ['compute.context'] = self.admin_context
    ...
    def _get_tenant_usages(self, detailed=''):
        ...
        orig_get_active_by_window_joined = (
            compute.InstanceList.get_active_by_window_joined)
        ...
        with mock.patch.object(compute.InstanceList,
    ...
    def _test_verify_show(self, start, stop):
        ...
        req = fakes.HTTPRequest.blank('?start=%s&end=%s' %
                    (start.isoformat(), stop.isoformat()))
        req.environ['compute.context'] = self.user_context
    ...
    def test_verify_show_cannot_view_other_tenant(self):
        req = fakes.HTTPRequest.blank('?start=%s&end=%s' %
                    (START.isoformat(), STOP.isoformat()))
        req.environ['compute.context'] = self.alt_user_context
...
class SimpleTenantUsageControllerTestV21(test.TestCase):
    ...
    def setUp(self):
        ...
        self.inst_obj = compute.Instance._from_db_object(
            self.context, compute.Instance(), self.baseinst)
        self.inst_obj.flavor = flavor

    @mock.patch('compute.compute.Instance.get_flavor',
    ...
    def test_get_flavor_from_deleted_with_notfound(self, fake_get_flavor):
        ...
        self.assertEqual(compute.Flavor, type(flavor))
