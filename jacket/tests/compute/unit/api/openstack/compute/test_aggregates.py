...
def _make_agg_obj(agg_dict):
    return compute.Aggregate(**agg_dict)


def _make_agg_list(agg_list):
    return compute.AggregateList(compute=[_make_agg_obj(a) for a in agg_list])
...
class AggregateTestCaseV21(test.NoDBTestCase):
    ...
    def _set_up(self):
        ...
        self.context = self.req.environ['compute.context']
    ...
    @mock.patch('compute.compute.api.AggregateAPI.create_aggregate')
    def test_create_with_none_availability_zone(self, mock_create_agg):
        mock_create_agg.return_value = compute.Aggregate(self.context,
                                                         name='test',
                                                         uuid=uuid.uuid4(),
                                                         hosts=[],
                                                         metadata={})
    ...
    def test_update_no_admin(self):
        body = {"aggregate": {"availability_zone": "compute"}}
        self.assertRaises(exception.PolicyNotAuthorized,
    ...
    @mock.patch('compute.compute.api.AggregateAPI.update_aggregate')
    def test_update_with_none_availability_zone(self, mock_update_agg):
        agg_id = uuid.uuid4()
        mock_update_agg.return_value = compute.Aggregate(self.context,
                                                         name='test',
                                                         uuid=agg_id,
                                                         hosts=[],
                                                         metadata={})
    ...
    def test_marshall_aggregate(self):
        ...
        agg = {'name': 'aggregate1',
               ...
               'metadata': {'foo': 'bar', 'availability_zone': 'compute'},
               'hosts': ['host1', 'host2']}
    ...
    def _assert_agg_data(self, expected, actual):
        self.assertTrue(obj_base.obj_equal_prims(expected, actual),
                        "The aggregate compute were not equal")
