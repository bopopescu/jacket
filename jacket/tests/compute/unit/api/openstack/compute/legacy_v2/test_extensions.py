...
class ExtensionTestCase(test.TestCase):
    def setUp(self):
        ...
        fox = ('compute.tests.unit.api.openstack.compute.legacy_v2.extensions.'
               'foxinsocks.Foxinsocks')
        if fox not in ext_list:
            ext_list.append(fox)
            self.flags(osapi_compute_extension=ext_list)
        self.fake_context = jacket.compute.context.RequestContext('fake', 'fake')
    ...
    def _test_extension_authorizer_throws_exception_if_policy_fails(self,
        ...
        target = {'project_id': '1234',
                  'user_id': '5678'}
        self.mox.StubOutWithMock(jacket.compute.policy, 'enforce')
        jacket.compute.policy.enforce(self.fake_context,
                            rule,
                            target).AndRaise(
