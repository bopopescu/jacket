...
from jacket.api.compute import auth as api_auth
...
from jacket.api.compute.openstack import auth
...
def stub_out_key_pair_funcs(stubs, have_key_pair=True, **kwargs):
    ...
    if have_key_pair:
        stubs.Set(jacket.compute.db, 'key_pair_get_all_by_user', key_pair)
        stubs.Set(jacket.compute.db, 'key_pair_get', one_key_pair)
    else:
        stubs.Set(jacket.compute.db, 'key_pair_get_all_by_user', no_key_pair)
...
def stub_out_rate_limiting(stubs):
    ...
    v2_limits = jacket.compute.api.openstack.compute.legacy_v2.limits
    ...
    def fake_reserve(context, **deltas):
        ...
        if requested > allowed:
            ...
            raise exc.OverQuota(overs=[resource], quotas=quotas,
                                usages=usages)
    test.stub_out('compute.quota.QUOTAS.reserve', fake_reserve)
...
def stub_out_nw_api(test, cls=None, private=None, publics=None):
    ...
    if cls is None:
        cls = Fake
    test.stub_out('compute.network.api.API', cls)
    fake_network.stub_out_nw_api_get_instance_nw_info(test)
...
class HTTPRequest(os_wsgi.Request):
    ...
    def blank(*args, **kwargs):
        ...
        out.environ['compute.context'] = FakeRequestContext('fake_user', 'fake',
...
class TestRouter(compute.Router):
...
class TestRouterV21(compute.Router):
    def __init__(self, controller, mapper=None):
        if not mapper:
            mapper = routes.Mapper()
        mapper.resource("test", "tests",
...
def fake_compute_get_all(num_servers=5, **kwargs):
    def _return_servers_objs(context, search_opts=None, limit=None,
        ...
        return base.obj_make_list(context, compute.InstanceList(),
                                  compute.Instance, db_insts,
...
def stub_instance_obj(ctxt, *args, **kwargs):
    ...
    expected = ['metadata', 'system_metadata', 'flavor',
                'info_cache', 'security_groups']
    inst = compute.Instance._from_db_object(ctxt, compute.Instance(),
                                            db_inst,
                                            expected_attrs=expected)
    inst.fault = None
