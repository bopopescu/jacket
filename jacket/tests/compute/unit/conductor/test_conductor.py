...
from jacket.tests.compute.unit import fake_utils
...
class ConductorTestCase(_BaseTestCase, test.TestCase):
    ...
    def test_reset(self):
        with mock.patch.object(compute.Service, 'clear_min_version_cache'
                               ) as mock_clear_cache:
...
class ConductorRPCAPITestCase(_BaseTestCase, test.TestCase):
    ...
    def setUp(self):
        ...
        self.conductor_service = self.start_service(
            'conductor', manager='compute.conductor.manager.ConductorManager')
...
class _BaseTaskTestCase(object):
    ...
    @mock.patch('compute.compute.Migration')
    def test_live_migrate(self, migobj):
        inst = fake_instance.fake_db_instance()
        inst_obj = compute.Instance._from_db_object(
    ...
    def _test_cold_migrate(self, clean_shutdown=True):
        ...
        scheduler_utils.build_request_spec(
            ...
            instance_type=mox.IsA(compute.Flavor)).AndReturn(request_spec)
    ...
    @mock.patch('compute.compute.Instance.refresh')
    def test_build_instances(self, mock_refresh):
        instance_type = flavors.get_default_flavor()
        instances = [compute.Instance(context=self.context,
                                      id=i,
                                      uuid=uuid.uuid4(),
    ...
    def test_unshelve_offload_instance_on_host_with_request_spec(self):
        ...
        @mock.patch.object(compute.RequestSpec, 'to_legacy_request_spec_dict')
        @mock.patch.object(compute.RequestSpec,
                           'to_legacy_filter_properties_dict')
        @mock.patch.object(compute.RequestSpec, 'reset_forced_destinations')
    ...
    def test_rebuild_instance_with_scheduler(self):
        ...
        with test.nested(
            ...
            mock.patch('compute.scheduler.utils.build_request_spec',
    ...
    @mock.patch.object(conductor_manager.compute_rpcapi.JacketAPI,
                       'rebuild_instance')
    @mock.patch.object(scheduler_utils, 'setup_instance_group')
    ...
    def test_rebuild_instance_evacuate_migration_record(self):
        ...
        with test.nested(
            ...
            mock.patch.object(self.conductor_manager.scheduler_client,
                              'select_destinations'),
            mock.patch.object(compute.Migration, 'get_by_instance_and_status',
    ...
    def test_rebuild_instance_with_request_spec(self):
        ...
        request_spec = {}
        filter_properties = {'ignore_hosts': [(inst_obj.host)]}
        fake_spec = compute.RequestSpec(ignore_hosts=[])
        augmented_spec = compute.RequestSpec(ignore_hosts=[inst_obj.host])
        rebuild_args, compute_args = self._prepare_rebuild_args(
            {'host': None, 'node': expected_node, 'limits': expected_limits,
...
class ConductorTaskTestCase(_BaseTaskTestCase, test_compute.BaseTestCase):
    ...
    def _test_migrate_server_deals_with_expected_exceptions(self, ex,
        ...
        inst_obj = compute.Instance._from_db_object(
            self.context, compute.Instance(), instance, [])
    ...
    def test_migrate_server_deals_with_invalidcpuinfo_exception(self):
        ...
        task = self.conductor._build_live_migrate_task(
            self.context, inst_obj, 'destination', 'block_migration',
            'disk_over_commit', mox.IsA(compute.Migration))
    ...
    def test_build_instances_instance_not_found(self):
        ...
        self.conductor_manager.compute_rpcapi.build_and_run_instance(
                ...
                requested_networks=None,
                security_groups='security_groups',
                block_device_mapping=mox.IsA(compute.BlockDeviceMappingList),
                node='node2', limits=[])
        self.mox.ReplayAll()
