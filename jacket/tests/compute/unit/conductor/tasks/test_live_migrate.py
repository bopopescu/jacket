...
class LiveMigrationTaskTestCase(test.NoDBTestCase):
    def setUp(self):
        ...
        self.migration = compute.Migration()
        self.fake_spec = compute.RequestSpec()
        self._generate_task()

    def _generate_task(self):
        self.task = live_migrate.LiveMigrationTask(self.context,
                                                   self.instance, self.destination, self.block_migration,
                                                   self.disk_over_commit, self.migration, compute_rpcapi.JacketAPI(),
    ...
    def test_check_instance_host_is_up(self):
        ...
        compute.Service.get_by_compute_host(self.context,
    ...
    def test_check_requested_destination(self):
        ...
        self.task.servicegroup_api.service_is_up("service").AndReturn(True)
        hypervisor_details = compute.ComputeNode(
            hypervisor_type="a",
    ...
    def test_check_requested_destination_fails_with_not_enough_memory(self):
        self.mox.StubOutWithMock(self.task, '_check_host_is_up')
        self.mox.StubOutWithMock(compute.ComputeNode,
                                 'get_first_node_by_host_for_old_compat')

        self.task._check_host_is_up(self.destination)
        compute.ComputeNode.get_first_node_by_host_for_old_compat(self.context,
            self.destination).AndReturn(
                compute.ComputeNode(free_ram_mb=513,
                                    memory_mb=1024,
    ...
    def test_find_destination_works(self):
        ...
        self.mox.StubOutWithMock(compute.RequestSpec,
    ...
    def test_find_destination_works_with_no_request_spec(self):
        task = live_migrate.LiveMigrationTask(
            ...
            compute_rpcapi.JacketAPI(), servicegroup.API(),
            scheduler_client.SchedulerClient(), request_spec=None)
        another_spec = compute.RequestSpec()
        self.instance.flavor = compute.Flavor()
    ...
    @mock.patch("compute.scheduler.utils.setup_instance_group")
    @mock.patch("compute.compute.RequestSpec.from_primitives")
