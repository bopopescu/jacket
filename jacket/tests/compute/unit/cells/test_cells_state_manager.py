...
def _create_fake_node(host, total_mem, total_disk, free_mem, free_disk):
    return compute.ComputeNode(host=host,
                               memory_mb=total_mem,
                               local_gb=total_disk,
...
def _fake_service_get_all_by_binary(cls, context, binary):
    def _node(host, total_mem, total_disk, free_mem, free_disk):
        ...
        return compute.Service(host=host,
                               disabled=False,
                               forced_down=False,
                               last_seen_up=now)
...
class TestCellsStateManager(test.NoDBTestCase):
    ...
    def setUp(self):
        ...
        self.stubs.Set(compute.ComputeNodeList, 'get_all',
                       _fake_compute_node_get_all)
        self.stubs.Set(compute.ServiceList, 'get_by_binary',
                       _fake_service_get_all_by_binary)
        self.stub_out('compute.db.flavor_get_all', _fake_instance_type_all)
        self.stub_out('compute.db.cell_get_all', _fake_cell_get_all)
    ...
    def test_filemanager_returned(self, mock_read_cached_file, mock_find_file):
        mock_find_file.return_value = "/etc/compute/cells.json"
...
class TestCellsStateManagerNodeDown(test.NoDBTestCase):
    def setUp(self):
        ...
        self.stub_out('compute.compute.ComputeNodeList.get_all',
                       _fake_compute_node_get_all)
        self.stub_out('compute.compute.ServiceList.get_by_binary',
