...
class CellsUtilsTestCase(test.NoDBTestCase):
    ...
    def _test_get_instances_pagination(self, mock_shuffle,
        ...
        instances0 = compute.instance._make_instance_list(fake_context,
                compute.InstanceList(),
                [fake_instance.fake_db_instance() for i in range(3)],
                expected_attrs=None)
        marker0 = instances0[-1]['uuid']
        instances1 = compute.instance._make_instance_list(fake_context,
                compute.InstanceList(),
                [fake_instance.fake_db_instance() for i in range(3)],
                expected_attrs=None)
    ...
    def test_add_cell_to_compute_node(self):
        fake_compute = compute.ComputeNode(id=1, host='fake')
        cell_path = 'fake_path'
    ...
    @mock.patch.object(compute.Service, 'obj_load_attr')
    def test_add_cell_to_service_no_compute_node(self, mock_get_by_id):
        fake_service = compute.Service(id=1, host='fake')
    ...
    def test_add_cell_to_service_with_compute_node(self):
        fake_service = compute.Service(id=1, host='fake')
        fake_service.compute_node = compute.ComputeNode(id=1, host='fake')
        cell_path = 'fake_path'
    ...
    def test_proxy_object_serializer_to_primitive(self):
        obj = compute.ComputeNode(id=1, host='fake')
