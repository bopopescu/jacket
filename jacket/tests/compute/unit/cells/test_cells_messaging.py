...
from jacket.objects.compute import fields as objects_fields
...
class CellsTargetedMethodsWithDatabaseTestCase(test.TestCase):
    ...
    def test_service_delete(self):
        fake_service = dict(id=42, host='fake_host', binary='compute-compute',
                            topic='compute')

        ctxt = self.ctxt.elevated()
        compute.service_create(ctxt, fake_service)

        self.src_msg_runner.service_delete(
            ctxt, self.tgt_cell_name, fake_service['id'])
        self.assertRaises(exception.ServiceNotFound,
                          compute.service_get, ctxt, fake_service['id'])
...
class CellsTargetedMethodsTestCase(test.NoDBTestCase):
    ...
    def test_run_compute_api_method_unknown_instance(self):
        ...
        compute.Instance.get_by_uuid(self.ctxt, instance.uuid,
    ...
    def test_service_get_by_compute_host(self):
        ...
        compute.Service.get_by_compute_host(self.ctxt,
    ...
    def test_service_update(self):
        ...
        fake_service = compute.Service(id=42, host='fake_host',
                                       binary='compute-compute',
                                       topic='compute')
        fake_service.compute_node = compute.ComputeNode(id=1, host='fake_host')
        self.mox.StubOutWithMock(compute.Service, 'get_by_args')
        self.mox.StubOutWithMock(compute.Service, 'save')
        compute.Service.get_by_args(
            self.ctxt, 'fake_host', 'compute-compute').AndReturn(fake_service)
        fake_service.save()
        self.mox.ReplayAll()
        ...
        result = response.value_or_raise()
        self.assertIsInstance(result, compute.Service)
    ...
    def test_proxy_rpc_to_manager_call(self):
        ...
        self.mox.StubOutWithMock(compute.Service, 'get_by_compute_host')
        compute.Service.get_by_compute_host(self.ctxt, fake_host_name)
    ...
    def test_compute_node_get(self):
        compute_id = 'fake-id'
        self.mox.StubOutWithMock(compute.ComputeNode, 'get_by_id')
        compute.ComputeNode.get_by_id(self.ctxt,
...
class CellsBroadcastMethodsTestCase(test.NoDBTestCase):
    ...
    def test_apply_expected_states_building(self):
        instance_info = compute.Instance(vm_state=vm_states.BUILDING)
    ...
    def test_apply_expected_states_resize_finish(self):
        instance_info = compute.Instance(task_state=task_states.RESIZE_FINISH)
    ...
    def _test_instance_update_at_top(self, exists=True):
        fake_uuid = fake_server_actions.FAKE_UUID
        fake_info_cache = compute.InstanceInfoCache(instance_uuid='fake-uuid')
        ...
        fake_instance = compute.Instance(**fake_attrs)
    ...
    def test_instance_destroy_at_top(self):
        fake_instance = compute.Instance(uuid='fake_uuid')

        with mock.patch.object(compute.Instance, 'destroy') as mock_destroy:
    ...
    def test_instance_destroy_at_top_incomplete_instance_obj(self):
        fake_instance = compute.Instance(uuid='fake_uuid')
        with mock.patch.object(compute.Instance, 'get_by_uuid') as mock_get:
            self.src_msg_runner.instance_destroy_at_top(self.ctxt,
                    fake_instance)
    ...
    def test_instance_fault_create_at_top(self):
        ...
        if_mock = mock.Mock(spec_set=compute.InstanceFault)
        ...
        with mock.patch.object(compute, 'InstanceFault') as if_obj_mock:
    ...
    def test_sync_instances(self):
        ...
        instance1 = compute.Instance(uuid='fake_uuid1', deleted=False)
        instance2 = compute.Instance(uuid='fake_uuid2', deleted=True)
    ...
    def test_service_get_all_with_disabled(self):
        ...
        self.mox.StubOutWithMock(compute.ServiceList, 'get_all')

        # Calls are made from grandchild-cell to api-cell
        compute.ServiceList.get_all(
    ...
    def test_compute_node_get_all(self):
        ...
        self.mox.StubOutWithMock(compute.ComputeNodeList, 'get_all')

        # Calls are made from grandchild-cell to api-cell
        compute.ComputeNodeList.get_all(mox.IgnoreArg()).AndReturn([4, 5])
        compute.ComputeNodeList.get_all(mox.IgnoreArg()).AndReturn([3])
        compute.ComputeNodeList.get_all(mox.IgnoreArg()).AndReturn([1, 2])
    ...
    def test_compute_node_get_all_with_hyp_match(self):
        ...
        self.mox.StubOutWithMock(compute.ComputeNodeList, 'get_by_hypervisor')

        # Calls are made from grandchild-cell to api-cell
        compute.ComputeNodeList.get_by_hypervisor(
    ...
    @mock.patch.object(compute.KeyPair, 'get_by_name',
...
class CellsPublicInterfacesTestCase(test.NoDBTestCase):
    ...
    def test_resize_instance(self, mock_message):
        instance = compute.Instance(cell_name='api-cell!child-cell')
        flavor = 'fake'
