...
from jacket.compute.scheduler.filters import numa_topology_filter
...
class TestNUMATopologyFilter(test.NoDBTestCase):
    ...
    def _get_spec_obj(self, numa_topology):
        image_meta = compute.ImageMeta(properties=compute.ImageMetaProps())
        spec_obj = compute.RequestSpec(numa_topology=numa_topology,
                                       pci_requests=None,
    ...
    def test_numa_topology_filter_pass(self):
        instance_topology = compute.InstanceNUMATopology(
            cells=[compute.InstanceNUMACell(id=0, cpuset=set([1]), memory=512),
                   compute.InstanceNUMACell(id=1, cpuset=set([3]), memory=512)
               ])
    ...
    def test_numa_topology_filter_fail_fit(self):
        instance_topology = compute.InstanceNUMATopology(
            cells=[compute.InstanceNUMACell(id=0, cpuset=set([1]), memory=512),
                   compute.InstanceNUMACell(id=1, cpuset=set([2]), memory=512),
                   compute.InstanceNUMACell(id=2, cpuset=set([3]), memory=512)
               ])
        spec_obj = self._get_spec_obj(numa_topology=instance_topology)
        host = fakes.FakeHostState('host1', 'node1',
                                   {'numa_topology': fakes.NUMA_TOPOLOGY,
    ...
    def test_numa_topology_filter_fail_memory(self):
        instance_topology = compute.InstanceNUMATopology(
            cells=[compute.InstanceNUMACell(id=0, cpuset=set([1]),
                                            memory=1024),
                   compute.InstanceNUMACell(id=1, cpuset=set([3]), memory=512)
               ])
        spec_obj = self._get_spec_obj(numa_topology=instance_topology)
    ...
    def test_numa_topology_filter_fail_cpu(self):
        instance_topology = compute.InstanceNUMATopology(
            cells=[compute.InstanceNUMACell(id=0, cpuset=set([1]), memory=512),
                   compute.InstanceNUMACell(id=1, cpuset=set([3, 4, 5]),
                                            memory=512)])
        spec_obj = self._get_spec_obj(numa_topology=instance_topology)
        host = fakes.FakeHostState('host1', 'node1',
                                   {'numa_topology': fakes.NUMA_TOPOLOGY,
    ...
    def _do_test_numa_topology_filter_cpu_policy(
        ...
        spec_obj = compute.RequestSpec(numa_topology=instance_topology,
                                       pci_requests=None,
                                       instance_uuid=str(uuid.uuid4()))
        ...
        for specs, props in itertools.product(extra_specs, image_props):
            ...
            fake_flavor = compute.Flavor(memory_mb=1024, extra_specs=specs)
            fake_image_props = compute.ImageMetaProps(**props)
            fake_image = compute.ImageMeta(properties=fake_image_props)
    ...
    def test_numa_topology_filter_pass_mempages(self):
        instance_topology = compute.InstanceNUMATopology(
            cells=[compute.InstanceNUMACell(id=0, cpuset=set([3]),
                                            memory=128, pagesize=4),
                   compute.InstanceNUMACell(id=1, cpuset=set([1]),
