...
from jacket.compute.scheduler.filters import core_filter
...
class TestCoreFilter(test.NoDBTestCase):
    ...
    def test_core_filter_passes(self):
        ...
        spec_obj = compute.RequestSpec(flavor=compute.Flavor(vcpus=1))
    ...
    def test_core_filter_single_instance_overcommit_fails(self):
        self.filt_cls = core_filter.CoreFilter()
        spec_obj = compute.RequestSpec(flavor=compute.Flavor(vcpus=2))
        host = fakes.FakeHostState('host1', 'node1',
    ...
    def test_aggregate_core_filter_value_error(self, agg_mock):
        ...
        spec_obj = compute.RequestSpec(
            context=mock.sentinel.ctx, flavor=compute.Flavor(vcpus=1))
        host = fakes.FakeHostState('host1', 'node1',
                {'vcpus_total': 4, 'vcpus_used': 7,
                 'cpu_allocation_ratio': 2})
