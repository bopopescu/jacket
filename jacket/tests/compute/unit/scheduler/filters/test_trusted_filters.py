...
from jacket.compute.scheduler.filters import trusted_filter
...
class TestTrustedFilter(test.NoDBTestCase):
    ...
    def setUp(self):
        ...
        fake_compute_nodes = [
            compute.ComputeNode(hypervisor_hostname='node1'),
        ]
        with mock.patch('compute.compute.ComputeNodeList.get_all') as mocked:
    ...
    def test_trusted_filter_combine_hosts(self, req_mock):
        fake_compute_nodes = [
            compute.ComputeNode(hypervisor_hostname='node1'),
            compute.ComputeNode(hypervisor_hostname='node2')
        ]
        with mock.patch('compute.compute.ComputeNodeList.get_all') as mocked:
            mocked.return_value = fake_compute_nodes
            self.filt_cls = trusted_filter.TrustedFilter()
