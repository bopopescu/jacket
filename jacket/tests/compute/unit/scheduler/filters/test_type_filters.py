...
from jacket.compute.scheduler.filters import type_filter
...
class TestTypeFilter(test.NoDBTestCase):
    ...
    def test_type_filter(self):
        ...
        spec_obj = compute.RequestSpec(
            ...
            flavor=compute.Flavor(id=target_id))
        # True since no instances on host
        self.assertTrue(self.filt_cls.host_passes(host, spec_obj))
        # Add an instance with the same instance_type_id
        inst1 = compute.Instance(uuid='aa', instance_type_id=target_id)
        ...
        inst2 = compute.Instance(uuid='bb', instance_type_id=diff_type)
    ...
    def test_aggregate_type_filter_no_metadata(self, agg_mock):
        ...
        spec_obj = compute.RequestSpec(
            ...
            flavor=compute.Flavor(name='fake1'))
        host = fakes.FakeHostState('fake_host', 'fake_node', {})
    ...
    def test_aggregate_type_filter_single_instance_type(self, agg_mock):
        ...
        spec_obj = compute.RequestSpec(
            context=mock.sentinel.ctx,
            flavor=compute.Flavor(name='fake1'))
        spec_obj2 = compute.RequestSpec(
            context=mock.sentinel.ctx,
            flavor=compute.Flavor(name='fake2'))
        host = fakes.FakeHostState('fake_host', 'fake_node', {})
    ...
    def test_aggregate_type_filter_multi_aggregate(self, agg_mock):
        ...
        spec_obj2 = compute.RequestSpec(
            context=mock.sentinel.ctx,
            flavor=compute.Flavor(name='fake2'))
        spec_obj3 = compute.RequestSpec(
            context=mock.sentinel.ctx,
            flavor=compute.Flavor(name='fake3'))
        host = fakes.FakeHostState('fake_host', 'fake_node', {})
