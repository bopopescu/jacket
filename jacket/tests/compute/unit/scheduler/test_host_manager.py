...
class HostManagerTestCase(test.NoDBTestCase):
    ...
    @mock.patch.object(compute.objects.InstanceList, 'get_by_filters')
    @mock.patch.object(compute.objects.ComputeNodeList, 'get_all')
    def test_init_instance_info_batches(self, mock_get_all,
                                        mock_get_by_filters):
        cn_list = compute.ComputeNodeList()
        for num in range(22):
            host_name = 'host_%s' % num
            cn_list.objects.append(compute.ComputeNode(host=host_name))
    ...
    def test_init_instance_info(self, mock_get_all,
                                mock_get_by_filters):
        cn1 = compute.ComputeNode(host='host1')
        cn2 = compute.ComputeNode(host='host2')
        inst1 = compute.Instance(host='host1', uuid='uuid1')
        inst2 = compute.Instance(host='host1', uuid='uuid2')
        inst3 = compute.Instance(host='host2', uuid='uuid3')
        mock_get_all.return_value = compute.ComputeNodeList(compute=[cn1, cn2])
        mock_get_by_filters.return_value = compute.InstanceList(
    ...
    @mock.patch.object(host_manager.HostManager, '_init_instance_info')
    @mock.patch.object(compute.AggregateList, 'get_all')
    ...
    def test_init_aggregates_one_agg_no_hosts(self, agg_get_all,
                                              mock_init_info):
        fake_agg = compute.Aggregate(id=1, hosts=[])
    ...
    def test_init_aggregates_one_agg_with_hosts(self, agg_get_all,
                                                mock_init_info):
        fake_agg = compute.Aggregate(id=1, hosts=['fake-host'])
        agg_get_all.return_value = [fake_agg]
        self.host_manager = host_manager.HostManager()
        self.assertEqual({1: fake_agg}, self.host_manager.aggs_by_id)
    ...
    def test_get_filtered_hosts(self):
        fake_properties = compute.RequestSpec(ignore_hosts=[],
    ...
    def test_get_filtered_hosts_with_ignore(self):
        fake_properties = compute.RequestSpec(
            instance_uuid='fake-uuid1',
    ...
    @mock.patch('compute.scheduler.host_manager.LOG')
    @mock.patch('compute.compute.ServiceList.get_by_binary')
    @mock.patch('compute.compute.ComputeNodeList.get_all')
    @mock.patch('compute.compute.InstanceList.get_by_host')
    def test_get_all_host_states(self, mock_get_by_host, mock_get_all,
        ...
        self.assertEqual(host_states_map[('host3', 'node3')].free_disk_mb,
                         3145728)
        self.assertThat(
                compute.NUMATopology.obj_from_db_obj(
    ...
    @mock.patch.object(compute.ComputeNodeList, 'get_all')
    @mock.patch.object(compute.ServiceList, 'get_by_binary')
    def test_get_all_host_states_with_no_aggs(self, svc_get_by_binary,
                                              cn_get_all, update_from_cn,
                                              mock_get_by_host):
        svc_get_by_binary.return_value = [compute.Service(host='fake')]
        cn_get_all.return_value = [
            compute.ComputeNode(host='fake', hypervisor_hostname='fake')]
        mock_get_by_host.return_value = compute.InstanceList()
    ...
    def test_get_all_host_states_with_matching_aggs(self, svc_get_by_binary,
        ...
        cn_get_all.return_value = [
            compute.ComputeNode(host='fake', hypervisor_hostname='fake')]
        mock_get_by_host.return_value = compute.InstanceList()
        fake_agg = compute.Aggregate(id=1)
        self.host_manager.host_aggregates_map = collections.defaultdict(
            set, {'fake': set([1])})
        self.host_manager.aggs_by_id = {1: fake_agg}
    ...
    def test_get_all_host_states_with_not_matching_aggs(self,
                                                        ...
                                                        mock_get_by_host):
        svc_get_by_binary.return_value = [compute.Service(host='fake'),
                                          compute.Service(host='other')]
        cn_get_all.return_value = [
            compute.ComputeNode(host='fake', hypervisor_hostname='fake'),
            compute.ComputeNode(host='other', hypervisor_hostname='other')]
        mock_get_by_host.return_value = compute.InstanceList()
        fake_agg = compute.Aggregate(id=1)
        self.host_manager.host_aggregates_map = collections.defaultdict(
            set, {'other': set([1])})
    ...
    def test_get_all_host_states_updated(self, mock_get_by_host,
        ...
        context = 'fake_context'
        hm = self.host_manager
        inst1 = compute.Instance(uuid='uuid1')
        cn1 = compute.ComputeNode(host='host1')
    ...
    def test_get_all_host_states_not_updated(self, mock_get_by_host,
        ...
        host_state = host_manager.HostState('host1', cn1)
        self.assertFalse(host_state.instances)
        mock_get_by_host.return_value = compute.InstanceList(compute=[inst1])
        host_state.update(
                inst_dict=hm._get_instance_info(context, cn1))
    ...
    def test_recreate_instance_info(self, mock_get_by_host):
        ...
        new_inst_list = compute.InstanceList(compute=[inst1, inst2])
    ...
    def test_update_instance_info(self):
        ...
        inst4 = fake_instance.fake_instance_obj('fake_context', uuid='ddd',
                                                host=host_name)
        update = compute.InstanceList(compute=[inst3, inst4])
    ...
    def test_update_instance_info_unknown_host(self):
        ...
        inst3 = fake_instance.fake_instance_obj('fake_context', uuid='ccc',
                                                host=bad_host)
        inst_list3 = compute.InstanceList(compute=[inst3])
...
class HostStateTestCase(test.NoDBTestCase):
    ...
    @mock.patch('compute.utils.synchronized',
    ...
    @mock.patch('compute.utils.synchronized',
                side_effect=lambda a: lambda f: lambda *args: f(*args))
    @mock.patch('compute.virt.hardware.get_host_numa_usage_from_instance')
    @mock.patch('compute.compute.Instance')
    @mock.patch('compute.virt.hardware.numa_fit_instance_to_host')
    @mock.patch('compute.virt.hardware.host_topology_and_format_from_host')
    def test_stat_consumption_from_instance(self, host_topo_mock,
                                            numa_fit_mock,
                                            instance_init_mock,
                                            numa_usage_mock,
                                            sync_mock):
        fake_numa_topology = compute.InstanceNUMATopology(
            cells=[compute.InstanceNUMACell()])
        fake_host_numa_topology = mock.Mock()
        fake_instance = compute.Instance(numa_topology=fake_numa_topology)
        ...
        spec_obj = compute.RequestSpec(
            ...
            flavor=compute.Flavor(root_gb=0, ephemeral_gb=0, memory_mb=0,
                                  vcpus=0),
        ...
        second_numa_topology = compute.InstanceNUMATopology(
            cells=[compute.InstanceNUMACell()])
        spec_obj = compute.RequestSpec(
            instance_uuid='fake-uuid',
            flavor=compute.Flavor(root_gb=0, ephemeral_gb=0, memory_mb=0,
    ...
    def test_stat_consumption_from_instance_pci(self):
        ...
        inst_topology = compute.InstanceNUMATopology(
                            cells = [compute.InstanceNUMACell(
                                                      cpuset=set([0]),
                                                      memory=512, id=0)])
        ...
        fake_requests = [{'request_id': 'fake_request1', 'count': 1,
                          'spec': [{'vendor_id': '8086'}]}]
        fake_requests_obj = compute.InstancePCIRequests(
                                requests=[compute.InstancePCIRequest(**r)
                                          for r in fake_requests],
                                instance_uuid='fake-uuid')
        req_spec = compute.RequestSpec(
            instance_uuid='fake-uuid',
            project_id='12345',
            numa_topology=inst_topology,
            pci_requests=fake_requests_obj,
            flavor=compute.Flavor(root_gb=0,
                                  ephemeral_gb=0,
                                  memory_mb=512,
                                  vcpus=1))
        host = host_manager.HostState("fakehost", "fakenode")
        ...
        host.pci_stats = pci_stats.PciDeviceStats(
                                      [compute.PciDevicePool(vendor_id='8086',
                                                             product_id='15ed',
                                                             numa_node=1,
                                                             count=1)])
