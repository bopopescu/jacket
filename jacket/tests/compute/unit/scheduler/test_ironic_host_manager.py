...
from jacket.compute.scheduler import ironic_host_manager
from jacket.compute import test
from jacket.tests.compute.unit.scheduler import ironic_fakes
...
class IronicHostManagerChangedNodesTestCase(test.NoDBTestCase):
    ...
    def setUp(self, mock_init_agg, mock_init_inst):
        ...
        self.host_manager = ironic_host_manager.IronicHostManager()
        ironic_driver = "compute.virt.ironic.driver.IronicDriver"
        supported_instances = [
            compute.HVSpec.from_list(["i386", "baremetal", "baremetal"])]
        self.compute_node = compute.ComputeNode(
    ...
    def test_consume_identical_instance_from_compute(self):
        ...
        spec_obj = compute.RequestSpec(
            flavor=compute.Flavor(root_gb=10, ephemeral_gb=0, memory_mb=1024,
                                  vcpus=1),
            uuid='fake-uuid')
    ...
    def test_consume_larger_instance_from_compute(self):
        ...
        self.assertIsNone(host.updated)
        spec_obj = compute.RequestSpec(
            flavor=compute.Flavor(root_gb=20, ephemeral_gb=0, memory_mb=2048,
                                  vcpus=2))
        host.consume_from_request(spec_obj)
    ...
    def test_consume_smaller_instance_from_compute(self):
        ...
        self.assertIsNone(host.updated)
        spec_obj = compute.RequestSpec(
            flavor=compute.Flavor(root_gb=5, ephemeral_gb=0, memory_mb=512,
                                  vcpus=1))
        host.consume_from_request(spec_obj)
