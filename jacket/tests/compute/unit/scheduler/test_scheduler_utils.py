...
class SchedulerUtilsTestCase(test.NoDBTestCase):
    ...
    def test_build_request_spec_without_image(self):
        ...
        instance_type = compute.Flavor(**test_flavor.fake_flavor)
    ...
    def test_set_vm_state_and_notify(self, mock_save, mock_add, mock_get):
        ...
        mock_add.assert_called_once_with(self.context, mock.ANY,
                                         exc_info, mock.ANY)
        self.assertIsInstance(mock_add.call_args[0][1], compute.Instance)
    ...
    def _get_group_details(self, group, policy=None):
        ...
        with test.nested(
            mock.patch.object(compute.InstanceGroup, 'get_by_instance_uuid',
                              return_value=group),
            mock.patch.object(compute.InstanceGroup, 'get_hosts',
                              return_value=['hostA']),
