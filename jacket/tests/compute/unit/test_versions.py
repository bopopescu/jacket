...
class VersionTestCase(test.NoDBTestCase):
    ...
    def test_version_string_with_package_is_good(self):
        ...
        self.stub_out('compute.version.version_info.version_string',
                lambda: '5.5.5.5')
        self.stub_out('compute.version.NOVA_PACKAGE', 'g9ec3421')
    ...
    def test_release_file(self):
        ...
        def fake_find_file(self, name):
            if name == "release":
                return "/etc/compute/release"
            return real_find_file(self, name)

        def fake_open(path, *args, **kwargs):
            if path == "/etc/compute/release":
                data = """[Nova]
