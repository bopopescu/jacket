...
test_service_opts = [
    cfg.StrOpt("fake_manager",
               default="compute.tests.unit.test_service.FakeManager",
...
class ServiceManagerTestCase(test.NoDBTestCase):
    ...
    def test_message_gets_to_manager(self):
        serv = service.Service('test',
                               'test',
                               'test',
                               'compute.tests.unit.test_service.FakeManager')
...
class ServiceTestCase(test.NoDBTestCase):
    ...
    def _service_start_mocks(self):
        self.mox.StubOutWithMock(compute.Service, 'create')
        self.mox.StubOutWithMock(compute.Service, 'get_by_host_and_binary')
        compute.Service.get_by_host_and_binary(mox.IgnoreArg(), self.host,
                                               self.binary)
        compute.Service.create()
    ...
    def _test_service_check_create_race(self, ex):
        ...
        compute.Service.get_by_host_and_binary(mox.IgnoreArg(), self.host,
                                               self.binary)
        compute.Service.create().AndRaise(ex)

        class TestException(Exception):
            pass

        compute.Service.get_by_host_and_binary(
    ...
    @mock.patch('compute.servicegroup.API')
    @mock.patch('compute.compute.service.Service.get_by_host_and_binary')
...
class TestWSGIService(test.NoDBTestCase):
    ...
    def setUp(self):
        ...
        self.stubs.Set(compute.Loader, "load_app", mox.MockAnything())

    @mock.patch('compute.compute.Service.get_by_host_and_binary')
    @mock.patch('compute.compute.Service.create')
