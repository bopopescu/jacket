...
CONF = cfg.CONF
CONF.import_opt('console_manager', 'compute.service')
CONF.import_opt('console_driver', 'compute.console.manager')
...
class ConsoleTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.flags(console_driver='compute.console.fake.FakeConsoleProxy',
                   stub_compute=True)
    ...
    def test_get_pool_creates_new_pool_if_needed(self):
        self.assertRaises(exception.NotFound,
                          compute.console_pool_get_by_host_type,
                          self.context,
                          self.host,
                          self.console.host,
        ...
        pool = self.console._get_pool_for_instance_host(self.context,
                                                           self.host)
        pool2 = compute.console_pool_get_by_host_type(self.context,
                              self.host,
                              self.console.host,
                              self.console.driver.console_type)
    ...
    def test_get_pool_does_not_create_new_pool_if_exists(self):
        ...
        new_pool = compute.console_pool_create(self.context, pool_info)
        pool = self.console._get_pool_for_instance_host(self.context,
                                                       'sometesthostname')
    ...
    def test_add_console(self):
        ...
        instance = compute.instance_get(self.context, instance['id'])
        pool = compute.console_pool_get_by_host_type(self.context,
...
class ConsoleAPITestCase(test.NoDBTestCase):
    ...
    def setUp(self):
        ...
        self.stub_out('compute.compute.console_get', _fake_db_console_get)

        def _fake_db_console_get_all_by_instance(_ctxt, _instance_uuid,
                                                 columns_to_join):
            return [self.fake_console]
        self.stub_out('compute.compute.console_get_all_by_instance',
    ...
    @mock.patch('compute.console.rpcapi.ConsoleAPI.remove_console')
    ...
    def test_create_console(self, mock_get_instance_by_uuid,
        ...
        mock_get_instance_by_uuid.return_value = compute.Instance(
            **self.fake_instance)
