...
class FixedIpCommandsTestCase(test.TestCase):
    ...
    def test_reserve(self):
        ...
        address = compute.fixed_ip_get_by_address(context.get_admin_context(),
                                             '192.168.0.100')
    ...
    def test_list_just_one_host(self):
        ...
        self.useFixture(fixtures.MonkeyPatch(
            'compute.compute.fixed_ip_get_by_host',
...
class NetworkCommandsTestCase(test.NoDBTestCase):
    ...
    def test_create(self):
        ...
        from jacket.compute.network import manager as net_manager
    ...
    def test_list(self):
        ...
        self.stub_out('compute.compute.network_get_all', fake_network_get_all)
        output = StringIO()
        sys.stdout = output
        self.commands.list()
    ...
    def test_delete(self):
        ...
        self.stub_out('compute.compute.network_get_by_uuid',
                      self.fake_network_get_by_uuid)

        def fake_network_delete_safe(context, network_id):
            self.assertTrue(context.to_dict()['is_admin'])
            self.assertEqual(network_id, self.fake_net['id'])
        self.stub_out('compute.compute.network_delete_safe', fake_network_delete_safe)
    ...
    def test_delete_by_cidr(self):
        ...
        self.fake_net['project_id'] = None
        self.fake_net['host'] = None
        self.stub_out('compute.compute.network_get_by_cidr',
    ...
    def _test_modify_base(self, update_value, project, host, dis_project=None,
        ...
        self.stub_out('compute.compute.network_get_by_cidr',
                      self.fake_network_get_by_cidr)
        self.stub_out('compute.compute.network_update', self.fake_network_update)
...
class VmCommandsTestCase(test.NoDBTestCase):
    def setUp(self):
        ...
        self.fake_flavor = compute.Flavor(**test_flavors.DEFAULT_FLAVORS[0])

    def test_list_without_host(self):
        output = StringIO()
        sys.stdout = output
        with mock.patch.object(compute.InstanceList, 'get_by_filters') as get:
            get.return_value = compute.InstanceList(
                compute=[fake_instance.fake_instance_obj(
    ...
    def test_list_with_host(self):
        output = StringIO()
        sys.stdout = output
        with mock.patch.object(compute.InstanceList, 'get_by_host') as get:
            get.return_value = compute.InstanceList(
                compute=[fake_instance.fake_instance_obj(
...
class DBCommandsTestCase(test.NoDBTestCase):
    ...
    @mock.patch.object(compute, 'archive_deleted_rows',
...
'''
    ...
    @mock.patch.object(compute, 'archive_deleted_rows', return_value={})
    ...
    @mock.patch('compute.context.get_admin_context')
...
class CellCommandsTestCase(test.NoDBTestCase):
    ...
    @mock.patch.object(compute, 'cell_create')
...
class CellV2CommandsTestCase(test.TestCase):
    ...
    def test_map_cell_and_hosts(self):
        ...
        for i in range(3):
            host = 'host%s' % i
            compute_node = compute.ComputeNode(ctxt, host=host, **values)
            compute_node.create()
        ...
        cell_mapping = compute.CellMapping.get_by_uuid(ctxt, cell_mapping_uuid)
        ...
        for i in range(3):
            host = 'host%s' % i
            host_mapping = compute.HostMapping.get_by_host(ctxt, host)
    ...
    def test_map_cell_and_hosts_partial_update(self):
        ...
        host_mapping = compute.HostMapping.get_by_host(ctxt, 'host2')
        self.assertEqual(cell_mapping.uuid, host_mapping.cell_mapping.uuid)
        # Verify the output
        output = sys.stdout.getvalue().strip()
        expected = ''
