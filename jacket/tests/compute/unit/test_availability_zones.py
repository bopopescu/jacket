...
class AvailabilityZoneTestCases(test.TestCase):
    ...
    def setUp(self):
        ...
        self.host = 'me'
        self.availability_zone = 'compute-test'
    ...
    def tearDown(self):
        compute.aggregate_delete(self.context, self.agg['id'])
        super(AvailabilityZoneTestCases, self).tearDown()

    def _create_az(self, agg_name, az_name):
        agg_meta = {'name': agg_name}
        agg = compute.aggregate_create(self.context, agg_meta)

        metadata = {'availability_zone': az_name}
        compute.aggregate_metadata_add(self.context, agg['id'], metadata)

        return agg

    def _update_az(self, aggregate, az_name):
        metadata = {'availability_zone': az_name}
        compute.aggregate_update(self.context, aggregate['id'], metadata)
    ...
    def _create_service_with_topic(self, topic, host, disabled=False):
        ...
        return compute.service_create(self.context, values)

    def _destroy_service(self, service):
        return compute.service_destroy(self.context, service['id'])

    def _add_to_aggregate(self, service, aggregate):
        return compute.aggregate_host_add(self.context,
                                     aggregate['id'], service['host'])

    def _delete_from_aggregate(self, service, aggregate):
        return compute.aggregate_host_delete(self.context,
                                        aggregate['id'], service['host'])
    ...
    def test_set_availability_zone_compute_service(self):
        ...
        service = self._create_service_with_topic('compute', self.host)
        services = compute.service_get_all(self.context)
    ...
    def test_get_availability_zones(self):
        ...
        #
        # zone         host        enabled
        # compute-test    host1       Yes
        # compute-test    host2       No
        # compute-test2   host3       Yes
        # compute-test3   host4       No
        # <default>    host5       No

        agg2 = self._create_az('agg-az2', 'compute-test2')
        agg3 = self._create_az('agg-az3', 'compute-test3')
        ...
        self.assertEqual(['compute-test', 'compute-test2'], zones)
        self.assertEqual(['compute-test3', 'compute'], not_zones)
        ...
        self.assertJsonEqual(zones,
                         [(u'compute-test2', set([u'host3'])),
                                 (u'compute-test', set([u'host1']))])
        self.assertJsonEqual(not_zones,
                         [(u'compute-test3', set([u'host4'])),
                                 (u'compute', set([u'host5']))])

    def test_get_instance_availability_zone_default_value(self):
        """Test get right availability zone by given an instance."""
        fake_inst = compute.Instance(host=self.host,
    ...
    def test_get_instance_availability_zone_from_aggregate(self):
        ...
        fake_inst = compute.Instance(host=host,
    ...
    def test_get_instance_availability_zone_no_host(self):
        ...
        fake_inst = compute.Instance(host=None, availability_zone='inst-az')
    ...
    def test_get_instance_availability_zone_no_host_no_az(self):
        ...
        fake_inst = compute.Instance(host=None, availability_zone=None)
