...
class NbdTestCase(test.NoDBTestCase):
    ...
    def test_inner_get_dev_works(self):
        ...
        self.useFixture(fixtures.MonkeyPatch('compute.utils.trycmd',
                                             self.fake_trycmd_creates_pid))
        self.useFixture(fixtures.MonkeyPatch('compute.utils.execute', _fake_noop))
    ...
    def test_get_dev_timeout(self):
        ...
        self.useFixture(fixtures.MonkeyPatch('compute.utils.trycmd',
                                             self.fake_trycmd_creates_pid))
        self.useFixture(fixtures.MonkeyPatch(('compute.virt.disk.mount.api.'
                                              'MAX_DEVICE_WAIT'), -10))
