...
from jacket.compute.virt.disk import api
...
class APITestCase(test.NoDBTestCase):
    def test_can_resize_need_fs_type_specified(self):
        ...
        self.useFixture(fixtures.MonkeyPatch(
                'compute.virt.disk.api.get_disk_size', _fake_get_disk_size))

        def fake_trycmd(*args, **kwargs):
            return '', 'broken'
        self.useFixture(fixtures.MonkeyPatch('compute.utils.trycmd', fake_trycmd))
        ...
        def fake_returns_nothing(*args, **kwargs):
            return ''
        self.useFixture(fixtures.MonkeyPatch(
                'compute.virt.disk.mount.nbd.NbdMount.get_dev',
                fake_returns_true))
        self.useFixture(fixtures.MonkeyPatch(
                'compute.virt.disk.mount.nbd.NbdMount.map_dev',
                fake_returns_true))
        self.useFixture(fixtures.MonkeyPatch(
                'compute.virt.disk.vfs.localfs.VFSLocalFS.get_image_fs',
    ...
    def test_get_file_extension_for_os_type_with_overrides(self):
        with mock.patch('compute.virt.disk.api._DEFAULT_MKFS_COMMAND',
                        'custom mkfs command'):
