...
from jacket.compute.virt.hyperv import vmops
...
class VMOpsTestCase(test_base.HyperVBaseTestCase):
    ...
    @mock.patch('compute.network.is_neutron')
    @mock.patch('compute.virt.hyperv.vmops.importutils.import_object')
    ...
    @mock.patch('compute.virt.hyperv.vmops.VMOps.destroy')
    @mock.patch('compute.virt.hyperv.vmops.VMOps.power_on')
    @mock.patch('compute.virt.hyperv.vmops.VMOps.attach_config_drive')
    @mock.patch('compute.virt.hyperv.vmops.VMOps._create_config_drive')
    @mock.patch('compute.virt.configdrive.required_by')
    @mock.patch('compute.virt.hyperv.vmops.VMOps.create_instance')
    @mock.patch('compute.virt.hyperv.vmops.VMOps.get_image_vm_generation')
    @mock.patch('compute.virt.hyperv.vmops.VMOps.create_ephemeral_vhd')
    @mock.patch('compute.virt.hyperv.vmops.VMOps._create_root_vhd')
    @mock.patch('compute.virt.hyperv.volumeops.VolumeOps.'
                'ebs_root_in_block_devices')
    @mock.patch('compute.virt.hyperv.vmops.VMOps._delete_disk_files')
    ...
    def test_get_image_vm_generation_default(self):
        image_meta = compute.ImageMeta.from_dict({"properties": {}})
    ...
    @mock.patch('compute.api.metadata.base.InstanceMetadata')
    @mock.patch('compute.virt.configdrive.ConfigDriveBuilder')
    ...
    @mock.patch('compute.virt.hyperv.vmops.VMOps._delete_disk_files')
    @mock.patch('compute.virt.hyperv.vmops.VMOps.power_off')
    ...
    @mock.patch("compute.virt.hyperv.vmops.VMOps._soft_shutdown")
    ...
    @mock.patch("compute.virt.hyperv.vmops.VMOps._wait_for_power_off")
    ...
    @mock.patch('compute.virt.hyperv.vmops.VMOps._set_vm_state')
    ...
    def test_get_console_output(self, fake_path_exists):
        ...
        with mock.patch('compute.virt.hyperv.vmops.open',
    ...
    def test_get_console_output_exception(self, fake_path_exists, fake_open):
        ...
        with mock.patch('compute.virt.hyperv.vmops.open', fake_open, create=True):
