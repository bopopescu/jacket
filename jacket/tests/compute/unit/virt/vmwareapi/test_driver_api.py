...
from jacket.tests.compute.unit import test_flavors
from jacket.tests.compute.unit import utils
from jacket.tests.compute.unit.virt.vmwareapi import fake as vmwareapi_fake
from jacket.tests.compute.unit.virt.vmwareapi import stubs
from jacket.tests.compute import uuidsentinel
from jacket.compute.virt import driver as v_driver
...
from jacket.compute.virt.vmwareapi import error_util
from jacket.compute.virt.vmwareapi import imagecache
from jacket.compute.virt.vmwareapi import images
from jacket.compute.virt.vmwareapi import vif
from jacket.compute.virt.vmwareapi import vim_util
from jacket.compute.virt.vmwareapi import vm_util
from jacket.compute.virt.vmwareapi import vmops
from jacket.compute.virt.vmwareapi import volumeops
...
class VMwareDriverStartupTestCase(test.NoDBTestCase):
    def _start_driver_with_flags(self, expected_exception_type, startup_flags):
        ...
        with mock.patch(
                'compute.virt.vmwareapi.driver.VMwareAPISession.__init__'):
            e = self.assertRaises(
    ...
    def test_start_driver_with_user_host_password(self):
        ...
        self._start_driver_with_flags(
                jacket.compute.exception.InvalidInput,
                dict(host_ip='ip', host_password='password',
...
class VMwareAPIVMTestCase(test.NoDBTestCase):
    ...
    def setUp(self, mock_register):
        ...
        image_ref = jacket.tests.compute.unit.image.fake.get_valid_image_id()
        (image_service, image_id) = glance.get_remote_image_service(
            self.context, image_ref)
        metadata = image_service.show(self.context, image_id)
        self.image = compute.ImageMeta.from_dict({
            'id': image_ref,
            'disk_format': 'vmdk',
    ...
    def test_login_retries(self):
        ...
        self.stub_out('compute.tests.unit.virt.vmwareapi.fake.FakeVim._login',
                      _fake_login)
        self.stub_out('compute.tests.unit.virt.vmwareapi.'
    ...
    def _create_instance(self, node=None, set_image_ref=True,
        ...
        values = {'name': 'fake_name',
                  ...
                  'flavor': compute.Flavor(**self.type_data),
                  'node': node,
    ...
    @mock.patch.object(jacket.compute.virt.vmwareapi.images.VMwareImage,
                       'from_image')
    ...
    def test_iso_disk_cdrom_attach(self):
        ...
        self.stub_out('compute.virt.vmwareapi.vmops._attach_cdrom_to_vm',
                       fake_attach_cdrom)
        self.image.disk_format = 'iso'
        self._create_vm()
    ...
    def _snapshot_delete_vm_snapshot_exception(self, exception, call_count=1):
        self._create_vm()
        fake_vm = vmwareapi_fake._get_objects("VirtualMachine").compute[0].obj
        snapshot_ref = vmwareapi_fake.ManagedObjectReference(
                               value="Snapshot-123",
                               name="VirtualMachineSnapshot")
    ...
    @mock.patch('compute.virt.vmwareapi.driver.VMwareVCDriver.detach_volume')
    ...
    def _rescue(self, config_drive=False):
        ...
        if config_drive:
            ...
            self.stub_out('compute.virt.vmwareapi.vmops._create_config_drive',
        ...
        info = self._get_info()
        self._check_vm_info(info, power_state.RUNNING)
        self.stub_out('compute.virt.vmwareapi.vm_util.power_on_instance',
                       fake_power_on_instance)
        self.stub_out('compute.virt.vmwareapi.volumeops.'
    ...
    def test_get_instance_diagnostics(self):
        ...
        expected = {'uptime': 0,
                    ...
                    'hypervisor_os': 'esxi',
                    'config_drive': 'False'}
        instance = compute.Instance(uuid=self.uuid,
                                    config_drive=False,
                                    system_metadata={},
                                    node=self.instance_node)
    ...
    @mock.patch.object(compute.block_device.BlockDeviceMappingList,
    ...
    def _override_time(self):
        ...
        self.stub_out('compute.virt.vmwareapi.imagecache.'
    ...
    @mock.patch('compute.virt.vmwareapi.ds_util.get_datastore')
