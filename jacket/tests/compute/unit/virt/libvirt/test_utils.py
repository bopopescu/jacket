...
class LibvirtUtilsTestCase(test.NoDBTestCase):
    ...
    @mock.patch('compute.virt.libvirt.volume.remotefs.SshDriver.copy_file')
    ...
    @mock.patch('compute.virt.libvirt.volume.remotefs.RsyncDriver.copy_file')
...
"""
        for i in range(0, 128):
            ...
            with mock.patch('compute.utils.execute',
...
cluster_size: 65536
disk size: 963434
backing file: /var/lib/compute/a328c7998805951a_2
"""
        mock_execute.return_value = (example_output, '')
        image_info = images.qemu_img_info(path)
        mock_execute.assert_called_once_with('env', 'LC_ALL=C', 'LANG=C',
        ...
        self.assertEqual(65536, image_info.cluster_size)
        self.assertEqual('/var/lib/compute/a328c7998805951a_2',
...
backing file: /var/lib/compute/a328c7998805951a_2 (actual path: /b/3a988059e51a_2)
"""
        mock_execute.return_value = (example_output, '')
        image_info = images.qemu_img_info(path)
        mock_execute.assert_called_once_with('env', 'LC_ALL=C', 'LANG=C',
...
"""
    ...
    def test_chown_for_id_maps(self, mock_execute):
        ...
        execute_args = ('compute-idmapshift', '-i',
    ...
    @mock.patch('compute.virt.images.fetch_to_raw')
    ...
    @mock.patch('compute.virt.images.fetch')
