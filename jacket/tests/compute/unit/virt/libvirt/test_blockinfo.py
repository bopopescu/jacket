...
from jacket.tests.compute.unit.virt import fakelibosinfo
...
class LibvirtBlockInfoTest(test.NoDBTestCase):
    ...
    def setUp(self):
        ...
        flavor = compute.Flavor(memory_mb=128,
                                root_gb=0,
                                name='m1.micro',
                                ephemeral_gb=0,
                                vcpus=1,
    ...
    @mock.patch('compute.virt.libvirt.blockinfo.has_disk_dev', return_value=True)
    ...
    def test_get_disk_mapping_simple_iso(self):
        ...
        instance_ref = compute.Instance(**self.test_instance)
        image_meta = compute.ImageMeta.from_dict({'disk_format': 'iso'})
    ...
    def test_get_device_name(self):
        bdm_obj = compute.BlockDeviceMapping(self.context,
    ...
    @mock.patch('compute.virt.libvirt.blockinfo.find_disk_dev_for_disk_bus',
                return_value='vda')
    @mock.patch('compute.virt.libvirt.blockinfo.get_disk_bus_for_disk_dev',
                return_value='virtio')
...
class DefaultDeviceNamesTestCase(test.NoDBTestCase):
    def setUp(self):
        ...
        self.instance = compute.Instance(
                ...
                root_device_name = '/dev/vda',
                system_metadata={})
        self.image_meta = compute.ImageMeta(
            disk_format='raw',
            properties=compute.ImageMetaProps())

        self.virt_type = 'kvm'
        self.flavor = compute.Flavor(swap=4)
        self.patchers = []
        self.patchers.append(mock.patch.object(self.instance, 'get_flavor',
                                               return_value=self.flavor))
        self.patchers.append(mock.patch(
                'compute.compute.block_device.BlockDeviceMapping.save'))
        for patcher in self.patchers:
            patcher.start()

        self.ephemerals = [compute.BlockDeviceMapping(
        ...
        self.swap = [compute.BlockDeviceMapping(
            self.context, **fake_block_device.FakeDbBlockDeviceDict(
        ...
        self.block_device_mapping = [
            compute.BlockDeviceMapping(self.context,
                **fake_block_device.FakeDbBlockDeviceDict(
                {'id': 3, 'instance_uuid': 'fake-instance',
                 'device_name': '/dev/vda',
                 'source_type': 'volume',
