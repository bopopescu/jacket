...
class IptablesFirewallTestCase(test.NoDBTestCase):
    ...
    def _create_instance_ref(self,
        ...
        inst = compute.Instance(
            ...
            instance_type_id=1)
        inst.info_cache = compute.InstanceInfoCache()
        inst.info_cache.deleted = False
        return inst

    @mock.patch.object(compute.InstanceList, "get_by_security_group_id")
    @mock.patch.object(compute.SecurityGroupRuleList, "get_by_instance")
    def test_static_filters(self, mock_secrule, mock_instlist):
        ...
        secgroup = compute.SecurityGroup(id=1,
                                         user_id='fake',
                                         project_id='fake',
                                         name='testgroup',
                                         description='test group')

        src_secgroup = compute.SecurityGroup(id=2,
                                             user_id='fake',
                                             project_id='fake',
                                             name='testsourcegroup',
                                             description='src group')

        r1 = compute.SecurityGroupRule(parent_group_id=secgroup['id'],
                                       protocol='icmp',
                                       from_port=-1,
                                       to_port=-1,
        ...
        r2 = compute.SecurityGroupRule(parent_group_id=secgroup['id'],
                                       protocol='icmp',
                                       from_port=8,
                                       to_port=-1,
                                       cidr='192.168.11.0/24',
        ...
        r3 = compute.SecurityGroupRule(parent_group_id=secgroup['id'],
                                       protocol='tcp',
                                       from_port=80,
                                       to_port=81,
        ...
        r4 = compute.SecurityGroupRule(parent_group_id=secgroup['id'],
                                       protocol='tcp',
                                       from_port=80,
                                       to_port=81,
                                       cidr=None,
        ...
        r5 = compute.SecurityGroupRule(parent_group_id=secgroup['id'],
                                       protocol=None,
                                       cidr=None,
                                       grantee_group=src_secgroup,
                                       group_id=src_secgroup['id'])

        secgroup_list = compute.SecurityGroupList()
        secgroup_list.objects.append(secgroup)
        src_secgroup_list = compute.SecurityGroupList()
        ...
        mock_secrule.return_value = compute.SecurityGroupRuleList(
            compute=[r1, r2, r3, r4, r5])

        def _fake_instlist(ctxt, id):
            if id == src_secgroup['id']:
                insts = compute.InstanceList()
    ...
    def test_multinic_iptables(self, mock_secrule):
        ...
        instance_ref.security_groups = compute.SecurityGroupList()
    ...
    def test_do_refresh_security_group_rules_instance_gone(self):
        instance1 = compute.Instance(None, id=1, uuid='fake-uuid1')
        instance2 = compute.Instance(None, id=2, uuid='fake-uuid2')
...
class NWFilterTestCase(test.NoDBTestCase):
    ...
    def test_creates_base_rule_first(self, mock_define):
        ...
        def _ensure_all_called(mac, allow_dhcp):
            instance_filter = 'compute-instance-%s-%s' % (instance_ref['name'],
    ...
    def test_multinic_base_filter_selection(self,
        ...
        assert_filterref(instance_ref, network_info[0],
                         expected=['compute-base'])
        assert_filterref(instance_ref, network_info[1],
                         expected=['compute-nodhcp'])
