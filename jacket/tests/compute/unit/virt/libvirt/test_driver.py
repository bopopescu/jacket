...
from jacket.tests.compute.unit.objects import test_vcpu_model
from jacket.tests.compute.unit.virt.libvirt import fake_imagebackend
from jacket.tests.compute.unit.virt.libvirt import fake_libvirt_utils
...
from jacket.compute.virt.disk import api as disk
from jacket.compute.virt import driver
from jacket.compute.virt import fake
from jacket.compute.virt import firewall as base_firewall
from jacket.compute.virt import hardware
from jacket.compute.virt.image import model as imgmodel
from jacket.compute.virt import images
from jacket.compute.virt.libvirt import blockinfo
from jacket.compute.virt.libvirt import config as vconfig
from jacket.compute.virt.libvirt import driver as libvirt_driver
from jacket.compute.virt.libvirt import firewall
from jacket.compute.virt.libvirt import guest as libvirt_guest
from jacket.compute.virt.libvirt import host
from jacket.compute.virt.libvirt import imagebackend
from jacket.compute.virt.libvirt.storage import dmcrypt
from jacket.compute.virt.libvirt.storage import lvm
from jacket.compute.virt.libvirt.storage import rbd_utils
from jacket.compute.virt.libvirt import utils as libvirt_utils
from jacket.compute.virt.libvirt.volume import volume as volume_drivers
...
class CacheConcurrencyTestCase(test.NoDBTestCase):
    def setUp(self):
        ...
        self.useFixture(fixtures.MonkeyPatch(
            'compute.virt.libvirt.imagebackend.libvirt_utils',
            fake_libvirt_utils))

    def _fake_instance(self, uuid):
        return compute.Instance(id=1, uuid=uuid)
...
def _create_test_instance():
    flavor = compute.Flavor(memory_mb=2048,
                            swap=0,
                            vcpu_weight=None,
                            root_gb=1,
                            id=2,
    ...
    return {
        ...
        'new_flavor': None,
        'old_flavor': None,
        'pci_devices': compute.PciDeviceList(),
        'numa_topology': None,
        'config_drive': None,
        'vm_mode': None,
        'kernel_id': None,
...
class LibvirtConnTestCase(test.NoDBTestCase):
    ...
    def setUp(self):
        ...
        self.useFixture(fixtures.MonkeyPatch(
            'compute.virt.libvirt.driver.libvirt_utils',
            fake_libvirt_utils))
        ...
        self.image_service = jacket.tests.compute.unit.image.fake.stub_out_image_service(
                self)
    ...
    def create_fake_libvirt_mock(self, **kwargs):
        ...
        # Creating mocks
        volume_driver = ['iscsi=compute.tests.unit.virt.libvirt.test_driver'
                         '.FakeVolumeDriver']
    ...
    def _create_service(self, **kwargs):
        ...
        return compute.Service(**service_ref)
    ...
    @mock.patch('compute.virt.libvirt.driver.LOG')
    ...
    @mock.patch('compute.utils.get_image_from_system_metadata')
    @mock.patch.object(host.Host,
                       'has_min_version', return_value=True)
    @mock.patch('compute.virt.libvirt.host.Host.get_guest')
    def test_set_admin_password(self, mock_get_guest, ver, mock_image):
        self.flags(virt_type='kvm', group='libvirt')
        instance = compute.Instance(**self.test_instance)
    ...
    @mock.patch.object(compute.Service, 'get_by_compute_host')
    ...
    def test_set_host_enabled_swallows_exceptions(self):
        ...
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), True)
        with mock.patch.object(compute, 'service_get_by_compute_host') as db_mock:
            # Make compute.service_get_by_compute_host raise NovaException; this
    ...
    @mock.patch.object(host.Host,
                       'has_min_version', return_value=True)
    @mock.patch.object(jacket.compute.virt.libvirt.guest.Guest, 'get_xml_desc')
    def test_detach_pci_devices(self, mocked_get_xml_desc, *args):
        ...
        pci_device = compute.PciDevice(**pci_device_info)
    ...
    def test_get_guest_config(self, time_mock):
        ...
        flavor = compute.Flavor(name='m1.small',
                                memory_mb=6,
                                vcpus=28,
                                root_gb=496,
                                ephemeral_gb=8128,
                                swap=33550336,
                                extra_specs={})
        instance_ref = compute.Instance(**test_instance)
        instance_ref.flavor = flavor
        image_meta = compute.ImageMeta.from_dict(self.test_image_meta)
    ...
    def test_get_guest_config_lxc(self):
        ...
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), True)
        instance_ref = compute.Instance(**self.test_instance)
        image_meta = compute.ImageMeta.from_dict(self.test_image_meta)
        cfg = drvr._get_guest_config(instance_ref,
                                     _fake_network_info(self, 1),
                                     image_meta, {'mapping': {}})
    ...
    def test_get_guest_config_numa_host_instance_fits(self, is_able):
        instance_ref = compute.Instance(**self.test_instance)
        image_meta = compute.ImageMeta.from_dict(self.test_image_meta)
        flavor = compute.Flavor(memory_mb=1, vcpus=2, root_gb=496,
    ...
    def test_get_guest_config_numa_host_instance_no_fit(self, is_able):
        instance_ref = compute.Instance(**self.test_instance)
        image_meta = compute.ImageMeta.from_dict(self.test_image_meta)
        flavor = compute.Flavor(memory_mb=4096, vcpus=4, root_gb=496,
                                ephemeral_gb=8128, swap=33550336, name='fake',
                                extra_specs={})
        instance_ref.flavor = flavor
    ...
    def test_get_guest_memory_backing_config_large_success(self, mock_version):
        host_topology = compute.NUMATopology(
            cells=[
                compute.NUMACell(
                    id=3, cpuset=set([1]), memory=1024, mempages=[
                        compute.NUMAPagesTopology(size_kb=4, total=2000,
                                                  used=0),
                        compute.NUMAPagesTopology(size_kb=2048, total=512,
                                                  used=0),
                        compute.NUMAPagesTopology(size_kb=1048576, total=0,
                                                  used=0),
                    ])])
        inst_topology = compute.InstanceNUMATopology(cells=[
    ...
    def test_get_guest_config_numa_host_instance_2pci_no_fit(self, is_able):
        ...
        pci_device2 = compute.PciDevice(**pci_device_info)
        with test.nested(
                mock.patch.object(
                    host.Host, "get_capabilities", return_value=caps),
                mock.patch.object(
    ...
    def _test_get_guest_config_numa_unsupported(self, fake_lib_version,
        ...
        instance_topology = compute.InstanceNUMATopology(
    ...
    def test_get_guest_config_numa_host_instance_fit_w_cpu_pinset(
        ...
        instance_ref = compute.Instance(**self.test_instance)
        image_meta = compute.ImageMeta.from_dict(self.test_image_meta)
        flavor = compute.Flavor(memory_mb=1024, vcpus=2, root_gb=496,
                                ephemeral_gb=8128, swap=33550336, name='fake',
                                extra_specs={})
        instance_ref.flavor = flavor
    ...
    def test_get_guest_config_non_numa_host_instance_topo(self, is_able):
        ...
        instance_ref.numa_topology = instance_topology
        image_meta = compute.ImageMeta.from_dict(self.test_image_meta)
        flavor = compute.Flavor(memory_mb=2048, vcpus=2, root_gb=496,
                                ephemeral_gb=8128, swap=33550336, name='fake',
                                extra_specs={})
        instance_ref.flavor = flavor
    ...
    def test_get_guest_config_numa_host_instance_topo(self, is_able):
        ...
        instance_ref.numa_topology = instance_topology
        image_meta = compute.ImageMeta.from_dict(self.test_image_meta)
        flavor = compute.Flavor(memory_mb=2048, vcpus=4, root_gb=496,
                                ephemeral_gb=8128, swap=33550336, name='fake',
                                extra_specs={})
        instance_ref.flavor = flavor
    ...
    def test_get_cpu_numa_config_from_instance(self):
        topology = compute.InstanceNUMATopology(cells=[
            compute.InstanceNUMACell(id=0, cpuset=set([1, 2]), memory=128),
            compute.InstanceNUMACell(id=1, cpuset=set([3, 4]), memory=128),
        ])
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), True)
    ...
    @mock.patch('compute.console.serial.acquire_port')
    @mock.patch('compute.virt.hardware.get_number_of_serial_ports',
                return_value=1)
    ...
    @mock.patch('compute.console.serial.release_port')
    ...
    def test_serial_console_release_port(
        ...
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), False)
        drvr._destroy(compute.Instance(**self.test_instance))
    ...
    @mock.patch('compute.virt.libvirt.storage.lvm.get_volume_size',
    ...
    @mock.patch('compute.virt.disk.api.teardown_container')
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver.get_info')
    @mock.patch('compute.virt.disk.api.setup_container')
    ...
    def test_video_driver_flavor_limit_not_set(self):
        ...
        with mock.patch.object(compute.Instance, 'save'):
    ...
    @mock.patch('compute.virt.libvirt.driver.os.path.exists')
    ...
    def _create_fake_service_compute(self):
        ...
        service_ref = compute.Service(**service_info)
        ...
        compute_ref = compute.ComputeNode(**compute_info)
    ...
    def test_get_guest_config_with_pci_passthrough_kvm(self):
        ...
        pci_device_info.update(compute_node_id=1,
                               ...
                               extra_info={})
        pci_device = compute.PciDevice(**pci_device_info)
        pci_list = compute.PciDeviceList()
    ...
    def _test_get_guest_config_ppc64(self, device_index):
        """Test for compute.virt.libvirt.driver.LibvirtDriver._get_guest_config.
        """
        self.flags(virt_type='kvm', group='libvirt')
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), True)
        instance_ref = compute.Instance(**self.test_instance)
    ...
    @mock.patch('compute.virt.libvirt.host.Host.get_online_cpus')
    ...
    @mock.patch('compute.virt.libvirt.host.Host.get_cpu_count')
    ...
    def test_create_snapshot_metadata(self):
        base = compute.ImageMeta.from_dict(
            {'disk_format': 'raw'})
        instance_data = {'kernel_id': 'kernel',
                    'project_id': 'prj_id',
                    'ramdisk_id': 'ram_id',
                    'os_type': None}
        instance = compute.Instance(**instance_data)
        img_fmt = 'raw'
    ...
    @mock.patch('compute.virt.libvirt.driver.LOG.debug')
    ...
    @mock.patch('compute.utils.get_image_from_system_metadata')
    @mock.patch('compute.virt.libvirt.blockinfo.get_info_from_bdm')
    @mock.patch('compute.virt.libvirt.host.Host.get_domain')
        ...
        with test.nested(
            ...
            for state in (power_state.RUNNING, power_state.PAUSED):
                ...
                mock_get_info.assert_called_with(
                    ...
                    test.MatchType(compute.ImageMeta),
                    bdm)
...
""", flags=flags)
  ...
  <devices>
    ...
    def _check_xml_and_container(self, instance):
        instance_ref = compute.Instance(**instance)
        image_meta = compute.ImageMeta.from_dict(self.test_image_meta)
    ...
    def _check_xml_and_uri(self, instance, mock_serial,
        ...
        for virt_type in hypervisors_to_check:
            ...
            with mock.patch('compute.virt.libvirt.driver.libvirt') as old_virt:
    ...
    @mock.patch.object(jacket.compute.virt.libvirt, 'config')
    ...
    @mock.patch.object(host.Host, 'compare_cpu')
    @mock.patch.object(jacket.compute.virt.libvirt.LibvirtDriver,
    ...
    def test_check_can_live_migrate_dest_cleanup_works_correctly(self):
        compute.Instance(**self.test_instance)
        dest_check_data = compute.LibvirtLiveMigrateData(
            filename="file",
            block_migration=True,
            disk_over_commit=False,
            disk_available_mb=1024)
    ...
    def test_check_can_live_migrate_source_block_migration(self):
        ...
        ret = drvr.check_can_live_migrate_source(self.context, instance,
                                                 dest_check_data)
        self.assertIsInstance(ret, compute.LibvirtLiveMigrateData)
    ...
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver.'
    ...
    def test_is_shared_block_storage_rbd(self):
        ...
        data = compute.LibvirtLiveMigrateData(image_type='rbd')
    ...
    def test_is_shared_block_storage_lvm(self):
        ...
        mock_get_instance_disk_info = mock.Mock()
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), False)
        data = compute.LibvirtLiveMigrateData(image_type='lvm',
                                              is_volume_backed=False,
                                              is_shared_instance_path=False)
        with mock.patch.object(drvr, 'get_instance_disk_info',
                               mock_get_instance_disk_info):
    ...
    def test_is_shared_block_storage_qcow2(self):
        ...
        mock_get_instance_disk_info = mock.Mock()
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), False)
        data = compute.LibvirtLiveMigrateData(image_type='qcow2',
                                              is_volume_backed=False,
                                              is_shared_instance_path=False)
        with mock.patch.object(drvr, 'get_instance_disk_info',
                               mock_get_instance_disk_info):
    ...
    def test_is_shared_block_storage_rbd_only_source(self):
        ...
        mock_get_instance_disk_info = mock.Mock()
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), False)
        data = compute.LibvirtLiveMigrateData(is_shared_instance_path=False,
                                              is_volume_backed=False)
        with mock.patch.object(drvr, 'get_instance_disk_info',
                               mock_get_instance_disk_info):
            drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), False)
    ...
    def test_is_shared_block_storage_volume_backed(self):
        ...
        data = compute.LibvirtLiveMigrateData(is_volume_backed=True,
                                              is_shared_instance_path=False)
    ...
    def test_live_migration_update_graphics_xml(self):
        ...
        instance_ref = compute.Instance(**instance_dict)
        ...
        migrate_data = compute.LibvirtLiveMigrateData(
    ...
    def test_live_migration_update_volume_xml(self):
        ...
        bdm = compute.LibvirtLiveMigrateBDMInfo(
    ...
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver._update_xml',
                return_value='')
    @mock.patch('compute.virt.libvirt.guest.Guest.get_xml_desc', return_value='')
    ...
    @mock.patch('os.path.exists', return_value=True)
    @mock.patch('compute.virt.libvirt.utils.get_instance_path_at_destination')
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver.destroy')
    ...
    def test_live_migration_copy_disk_paths(self, mock_xml, mock_version):
        ...
        <domain>
          ...
          <devices>
            <disk type="file">
               <source file="/var/lib/compute/instance/123/disk.root"/>
               <target dev="vda"/>
            </disk>
            <disk type="file">
               <source file="/var/lib/compute/instance/123/disk.shared"/>
               <target dev="vdb"/>
               <shareable/>
            </disk>
            <disk type="file">
               <source file="/var/lib/compute/instance/123/disk.config"/>
               <target dev="vdc"/>
               <readonly/>
            </disk>
            <disk type="block">
        ...
        self.assertEqual((["/var/lib/compute/instance/123/disk.root",
                          "/dev/mapper/somevol"], ['vda', 'vdd']), paths)

    @mock.patch.object(host.Host, "has_min_version", return_value=True)
    @mock.patch('compute.virt.driver.get_block_device_info')
    @mock.patch('compute.compute.BlockDeviceMappingList.get_by_instance_uuid')
    @mock.patch.object(fakelibvirt.Domain, "XMLDesc")
    def test_live_migration_copy_disk_paths_selective_block_migration(
        ...
        return_value = drvr._live_migration_copy_disk_paths(context, instance,
                                                            guest)
        expected = (['/var/lib/compute/instance/123/disk.root',
                     '/var/lib/compute/instance/123/disk.shared',
                     '/var/lib/compute/instance/123/disk.config'],
                    ['vda', 'vdb', 'vdc'])
    ...
    def test_live_migration_data_gb_block(self):
        ...
        def fake_stat(path):
            ...
            if path == "/var/lib/compute/instance/123/disk.root":
        ...
        disk_paths = ["/var/lib/compute/instance/123/disk.root",
                      "/dev/mapper/somevol"]
    ...
    @mock.patch.object(compute.Migration, "save")
    ...
    def _test_live_migration_monitoring(self,
        ...
        migration = compute.Migration(context=self.context, id=1)
        migrate_data = compute.LibvirtLiveMigrateData(
            migration=migration)
    ...
    def test_live_migration_main(self, mock_copy_disk_path, mock_running,
        ...
        guest = libvirt_guest.Guest(dom)
        migrate_data = compute.LibvirtLiveMigrateData(block_migration=True)
    ...
    @mock.patch('compute.virt.driver.block_device_info_get_mapping',
                return_value=())
    @mock.patch('compute.virt.configdrive.required_by',
                return_value=True)
    ...
    def test_pre_live_migration_vol_backed_works_correctly_mocked(self):
        ...
        with utils.tempdir() as tmpdir:
            ...
            class FakeNetworkInfo(object):
                def fixed_ips(self):
                    return ["test_ip_addr"]
            inst_ref = compute.Instance(**self.test_instance)
            c = context.get_admin_context()
            nw_info = FakeNetworkInfo()
            # Creating mocks
            self.mox.StubOutWithMock(drvr, "_connect_volume")
        ...
        with test.nested(
            ...
            for migrate_data in migrate_data_set:
                ...
                self.assertIsInstance(res,
                                      compute.LibvirtLiveMigrateData)
    ...
    def test_pre_live_migration_with_not_shared_instance_path(self):
        ...
        with test.nested(
            ...
            self.assertIsInstance(res, compute.LibvirtLiveMigrateData)
    ...
    def test_spawn_with_network_info(self):
        ...
        instance = compute.Instance(**instance_ref)
        image_meta = compute.ImageMeta.from_dict(self.test_image_meta)
    ...
    def test_start_lxc_from_volume(self):
        ...
        instance_ref['uuid'] = uuidutils.generate_uuid()
        inst_obj = compute.Instance(**instance_ref)
        image_meta = compute.ImageMeta.from_dict({})
    ...
    def test_spawn_with_pci_devices(self):
        ...
        instance_ref['image_ref'] = 'my_fake_image'
        instance = compute.Instance(**instance_ref)
        instance['pci_devices'] = compute.PciDeviceList(
            compute=[compute.PciDevice(address='0000:00:00.0')])
    ...
    def _test_create_image_plain(self, os_type='', filename='', mkfs=False):
        ...
        self.stubs.Set(jacket.compute.virt.libvirt.imagebackend.Backend, "image",
                       fake_image)
        ...
        self.stubs.Set(drvr, 'get_info', fake_get_info)
        if mkfs:
            self.stubs.Set(jacket.compute.virt.disk.api, '_MKFS_COMMAND',
    ...
    @mock.patch('compute.virt.libvirt.driver.imagecache')
    def test_create_image_initrd(self, mock_imagecache):
        ...
        with test.nested(
            mock.patch.object(jacket.compute.virt.libvirt.imagebackend.Backend,
              "image", fake_image),
    ...
    def _create_image_helper(self, callback, suffix=''):
        ...
        self.stubs.Set(jacket.compute.virt.configdrive.ConfigDriveBuilder,
    ...
    @mock.patch.object(jacket.compute.virt.libvirt.imagebackend.Image, 'cache',
    ...
    def test_create_ephemeral_specified_fs_not_valid(self):
        ...
        instance = compute.Instance(**instance_ref)
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), False)
        image_meta = compute.ImageMeta.from_dict({'disk_format': 'raw'})
        disk_info = blockinfo.get_disk_info(CONF.libvirt.virt_type,
                                            instance,
                                            image_meta)
    ...
    @mock.patch.object(libvirt_driver.LOG, 'warn')
    @mock.patch('compute.compute.utils.get_machine_ips')
    ...
    def test_immediate_delete(self, mock_save):
        ...
        instance = compute.Instance(self.context, **self.test_instance)
        drvr.destroy(self.context, instance, {})
        mock_save.assert_called_once_with()

    @mock.patch.object(compute.Instance, 'get_by_uuid')
    @mock.patch.object(compute.Instance, 'obj_load_attr', autospec=True)
    @mock.patch.object(compute.Instance, 'save', autospec=True)
    ...
    @mock.patch('compute.virt.libvirt.LibvirtDriver.get_info')
    @mock.patch('compute.virt.libvirt.LibvirtDriver._create_domain_and_network')
    @mock.patch('compute.virt.libvirt.LibvirtDriver._create_images_and_backing')
    @mock.patch('compute.virt.libvirt.LibvirtDriver._get_guest_xml')
    @mock.patch('compute.virt.libvirt.LibvirtDriver._get_instance_disk_info')
    @mock.patch('compute.virt.libvirt.blockinfo.get_disk_info')
    @mock.patch('compute.virt.libvirt.LibvirtDriver._destroy')
    ...
    @mock.patch('compute.pci.manager.get_instance_pci_devs')
    @mock.patch('compute.virt.libvirt.LibvirtDriver._prepare_pci_devices_for_use')
    @mock.patch('compute.virt.libvirt.LibvirtDriver._create_domain_and_network')
    @mock.patch('compute.virt.libvirt.LibvirtDriver._create_images_and_backing')
    @mock.patch('compute.virt.libvirt.LibvirtDriver._get_instance_disk_info')
    @mock.patch('compute.virt.libvirt.utils.write_to_file')
    @mock.patch('compute.virt.libvirt.utils.get_instance_path')
    @mock.patch('compute.virt.libvirt.LibvirtDriver._get_guest_config')
    @mock.patch('compute.virt.libvirt.blockinfo.get_disk_info')
    @mock.patch('compute.virt.libvirt.LibvirtDriver._destroy')
    def test_hard_reboot_does_not_call_glance_show(self,
        ...
        https://bugs.launchpad.net/compute/+bug/1339386
        """
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), False)
    ...
    def _test_detach_sriov_ports(self,
        ...
        expected_pci_device_obj = (
            compute.PciDevice(address=expeted_pci_slot, request_id=None))
        instance.pci_devices = compute.PciDeviceList()
    ...
    def test_detach_duplicate_mac_sriov_ports(self,
        ...
        instance.pci_devices.objects = [
            compute.PciDevice(address='0000:00:00.0', request_id=None),
            compute.PciDevice(address='0000:00:00.1', request_id=None)
        ]
    ...
    @mock.patch.object(host.Host, "list_instance_domains")
    @mock.patch.object(compute.BlockDeviceMappingList, "bdms_by_instance_uuid")
    @mock.patch.object(compute.InstanceList, "get_by_filters")
    ...
    @mock.patch('compute.virt.disk.api.clean_lxc_namespace')
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver.get_info')
    @mock.patch('compute.virt.disk.api.setup_container')
    @mock.patch('oslo_utils.fileutils.ensure_tree')
    @mock.patch.object(fake_libvirt_utils, 'get_instance_path')
    ...
    def test_create_domain_define_xml_fails(self):
        ...
        self.stubs.Set(jacket.compute.virt.libvirt.guest.LOG, 'error', fake_error)
    ...
    def test_create_domain_enable_hairpin_fails(self):
        ...
        self.mox.ReplayAll()
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), True)
        self.stubs.Set(jacket.compute.utils, 'execute', fake_execute)
        self.stubs.Set(
            jacket.compute.virt.libvirt.guest.Guest, 'get_interfaces',
            fake_get_interfaces)
    ...
    def _test_attach_detach_interface_get_config(self, method_name):
        ...
        fake_image_meta = compute.ImageMeta.from_dict(
        ...
        drvr.vif_driver.get_config(instance, network_info[0],
                                   mox.IsA(compute.ImageMeta),
                                   mox.IsA(compute.Flavor),
                                   CONF.libvirt.virt_type,
                                   drvr._host).\
                                   AndReturn(expected)
    ...
    def test_default_root_device_name(self):
        ...
        image_meta = compute.ImageMeta.from_dict({'id': 'fake'})
    ...
    @mock.patch.object(compute.BlockDeviceMapping, "save")
    def test_default_device_names_for_instance(self, save_mock):
        instance = compute.Instance(**self.test_instance)
        instance.root_device_name = '/dev/vda'
        ephemerals = [compute.BlockDeviceMapping(
                        **fake_block_device.AnonFakeDbBlockDeviceDict(
                            {'device_name': 'vdb',
                             'source_type': 'blank',
                             'volume_size': 2,
                             'destination_type': 'local'}))]
        swap = [compute.BlockDeviceMapping(
                        **fake_block_device.AnonFakeDbBlockDeviceDict(
                            {'device_name': 'vdg',
                             'source_type': 'blank',
                             'volume_size': 512,
    ...
    def test_get_device_name_for_instance_simple(self):
        new_bdm = compute.BlockDeviceMapping(
            context=context,
            source_type='volume', destination_type='volume',
    ...
    def test_create_propagates_exceptions(self):
        ...
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), False)
        instance = compute.Instance(id=1, uuid='fake-uuid',
    ...
    @mock.patch('compute.utils.is_neutron', return_value=False)
    def test_create_with_network_events_non_neutron(self, is_neutron):
        self._test_create_with_network_events()

    @mock.patch('compute.volume.encryptors.get_encryption_metadata')
    @mock.patch('compute.virt.libvirt.blockinfo.get_info_from_bdm')
    ...
    def test_get_guest_storage_config(self):
        ...
        instance = compute.Instance(**test_instance)
        image_meta = compute.ImageMeta.from_dict(self.test_image_meta)
    ...
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver._unplug_vifs')
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver._undefine_domain')
    def test_cleanup_pass_with_no_mount_device(self, undefine, unplug):
        ...
        fake_bdms = [{'connection_info': 'foo',
                     'mount_device': None}]
        with mock.patch('compute.virt.driver'
    ...
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver._disconnect_volume')
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver._swap_volume')
    @mock.patch('compute.virt.block_device.DriverVolumeBlockDevice.save')
    @mock.patch('compute.compute.block_device.BlockDeviceMapping.'
                'get_by_volume_and_instance')
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver._get_volume_config')
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver._connect_volume')
    @mock.patch('compute.virt.libvirt.host.Host.get_guest')
    def test_swap_volume_driver_bdm_save(self, get_guest,
        ...
        bdm = compute.BlockDeviceMapping(self.context,
            **fake_block_device.FakeDbBlockDeviceDict(
    ...
    def _test_live_snapshot(self, can_quiesce=False, require_quiesce=False):
        ...
        with test.nested(
            ...
            image_meta = compute.ImageMeta.from_dict(test_image_meta)
    ...
    def test_live_migration_abort(self, mock_abort):
        ...
        guest = libvirt_guest.Guest(dom)
        with mock.patch.object(jacket.compute.virt.libvirt.host.Host, 'get_guest',
                               return_value=guest):
    ...
    def test_get_guest_config_parallels_ct(self):
        ...
        instance_ref = compute.Instance(**ct_instance)
        image_meta = compute.ImageMeta.from_dict(self.test_image_meta)
    ...
    def test_get_guest_disk_config_rbd_older_config_drive_fall_back(self):
        ...
        disk_mapping = {'disk.config': {'bus': 'ide',
                                        'dev': 'hdd',
                                        'type': 'file'}}
        flavor = compute.Flavor(extra_specs={})
...
class HostStateTestCase(test.NoDBTestCase):
    ...
    pci_devices = [{
        ...
        "dev_type": fields.PciDeviceType.SRIOV_PF,
        "phys_function": None}]
    numa_topology = compute.NUMATopology(
                        cells=[compute.NUMACell(
    ...
    def test_update_status(self, mock_open):
        ...
        self.assertThat(compute.NUMATopology.obj_from_db_obj(
...
class LibvirtDriverTestCase(test.NoDBTestCase):
    """Test for compute.virt.libvirt.libvirt_driver.LibvirtDriver."""
    def setUp(self):
    ...
    def _create_instance(self, params=None):
        ...
        flavor = compute.Flavor(memory_mb=512,
                                swap=0,
                                vcpu_weight=None,
                                root_gb=10,
                                id=2,
        ...
        return compute.Instance(flavor=flavor,
    ...
    def test_migrate_disk_and_power_off_exception(self):
        """Test for compute.virt.libvirt.libvirt_driver.LivirtConnection
        ...
        flavor_obj = compute.Flavor(**flavor)
    ...
    @mock.patch('compute.virt.libvirt.utils.copy_image')
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver._destroy')
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver.get_host_ip_addr')
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver'
                '.get_instance_disk_info')
    ...
    def _test_migrate_disk_and_power_off_resize_check(self, expected_exc):
        """Test for compute.virt.libvirt.libvirt_driver.LibvirtConnection
        .migrate_disk_and_power_off.
        """
    ...
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver'
                '.get_instance_disk_info')
    @mock.patch('compute.virt.driver.block_device_info_get_ephemerals')
    ...
    def test_disk_size_from_instance_disk_info(self):
        ...
        inst = compute.Instance(**instance_data)
    ...
    @mock.patch('compute.virt.disk.api.extend')
    ...
    @mock.patch('compute.virt.disk.api.can_resize_image')
    @mock.patch('compute.virt.disk.api.is_image_extendable')
    @mock.patch('compute.virt.disk.api.extend')
    ...
    def _test_finish_migration(self, power_on, resize_instance=False):
        ...
        self.stubs.Set(jacket.compute.virt.libvirt.guest.Guest, 'enable_hairpin',
                       fake_enable_hairpin)
        self.stubs.Set(utils, 'execute', fake_execute)
    ...
    def test_lvm_disks(self, listlvs, exists):
        instance = compute.Instance(uuid='fake-uuid', id=1)
    ...
    @mock.patch('compute.virt.netutils.get_injected_network_template')
    @mock.patch('compute.virt.disk.api.inject_data')
    ...
    def test_detach_interface_device_not_found(self, mock_log):
        ...
        guest = mock.Mock(spec='compute.virt.libvirt.guest.Guest')
    ...
    @mock.patch(
        'compute.virt.configdrive.ConfigDriveBuilder.add_instance_metadata')
    @mock.patch('compute.virt.configdrive.ConfigDriveBuilder.make_drive')
    ...
    def test_delete_instance_files_resize(self, get_instance_path, exists,
        ...
        jacket.compute.utils.execute.side_effect = [Exception(), None]
        exists.side_effect = [False, False, True, False]
    ...
    def test_delete_instance_files_mv_failed(self, get_instance_path, exists,
        ...
        jacket.compute.utils.execute.side_effect = Exception()
    ...
    def test_delete_instance_files_concurrent(self, get_instance_path, exists,
        ...
        jacket.compute.utils.execute.side_effect = [Exception(), Exception(), None]
        exists.side_effect = [False, False, True, False]
  ...
  <target bus="virtio" dev="vdb"/>
    ...
    @mock.patch.object(host.Host, "get_guest")
    @mock.patch.object(dmcrypt, 'delete_volume')
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver.unfilter_instance')
    @mock.patch('compute.virt.libvirt.driver.LibvirtDriver._undefine_domain')
    @mock.patch.object(compute.Instance, 'save')
    ...
    def test_vcpu_model_to_config(self):
        ...
        feature = compute.VirtCPUFeature(policy=cpumodel.POLICY_REQUIRE,
                                         name='sse')
        feature_1 = compute.VirtCPUFeature(policy=cpumodel.POLICY_FORBID,
                                           name='aes')
        topo = compute.VirtCPUTopology(sockets=1, cores=2, threads=4)
        vcpu_model = compute.VirtCPUModel(mode=cpumodel.MODE_HOST_MODEL,
...
class LibvirtNonblockingTestCase(test.NoDBTestCase):
    ...
    def test_connection_to_primitive(self):
        # Test bug 962840.
        import jacket.compute.virt.libvirt.driver as libvirt_driver
...
class LibvirtVolumeSnapshotTestCase(test.NoDBTestCase):
    ...
    @mock.patch('compute.virt.block_device.DriverVolumeBlockDevice.'
    ...
    def test_volume_refresh_connection_info(self,
        ...
        instance = compute.Instance(**self.inst)
        ...
        fake_bdm = compute.BlockDeviceMapping(self.c, **fake_bdm)
    ...
    @mock.patch("compute.virt.libvirt.guest.Guest.is_active",
                 mock.Mock(return_value=False))
    @mock.patch('compute.virt.images.qemu_img_info',
...
class _BaseSnapshotTests(test.NoDBTestCase):
    def setUp(self):
        ...
        self.instance_ref = compute.Instance(**test_instance)
        self.instance_ref.info_cache = compute.InstanceInfoCache(
            network_info=None)
...
class LibvirtSnapshotTests(_BaseSnapshotTests):
    def test_ami(self):
        # Assign different image_ref from compute/images/fakes for testing ami
...
class LVMSnapshotTests(_BaseSnapshotTests):
    ...
    def _test_lvm_snapshot(self, disk_format, mock_volume_info,
                           mock_convert_image):
        self.flags(images_type='lvm',
                   images_volume_group='compute-vg', group='libvirt')

        self._test_snapshot(disk_format=disk_format)

        mock_volume_info.assert_has_calls([mock.call('/dev/compute-vg/lv')])
        mock_convert_image.assert_called_once_with(
            '/dev/compute-vg/lv', mock.ANY, 'raw', disk_format,
            run_as_root=True)
