...
from jacket.compute.tests import fixtures as nova_fixtures
from jacket.compute.tests.unit import fake_block_device
from jacket.compute.tests.unit.image import fake as fake_image
from jacket.compute.tests.unit import utils as test_utils
from jacket.compute.tests.unit.virt.libvirt import fake_libvirt_utils
...
from jacket.compute.virt import libvirt
...
class _FakeDriverBackendTestCase(object):
    def _setup_fakelibvirt(self):
        ...
        import jacket.compute.tests.unit.virt.libvirt.fake_imagebackend as fake_imagebackend
            fake_imagebackend
        import jacket.compute.tests.unit.virt.libvirt.fake_libvirt_utils as fake_libvirt_utils
            fake_libvirt_utils
        import jacket.compute.tests.unit.virt.libvirt.fakelibvirt as fakelibvirt

        import jacket.compute.tests.unit.virt.libvirt.fake_os_brick_connector as fake_os_brick_connector
            fake_os_brick_connector

        sys.modules['libvirt'] = fakelibvirt
        import jacket.compute.virt.libvirt.driver
        import jacket.compute.virt.libvirt.firewall
        import jacket.compute.virt.libvirt.host

        self.useFixture(fixtures.MonkeyPatch(
            'compute.virt.libvirt.driver.imagebackend',
            fake_imagebackend))
        self.useFixture(fixtures.MonkeyPatch(
            'compute.virt.libvirt.driver.libvirt',
            fakelibvirt))
        self.useFixture(fixtures.MonkeyPatch(
            'jacket.compute.libvirt_utils',
            fake_libvirt_utils))
        self.useFixture(fixtures.MonkeyPatch(
            'compute.virt.libvirt.host.libvirt',
            fakelibvirt))
        self.useFixture(fixtures.MonkeyPatch(
            'compute.virt.libvirt.imagebackend.libvirt_utils',
            fake_libvirt_utils))
        self.useFixture(fixtures.MonkeyPatch(
            'jacket.compute.libvirt',
            fakelibvirt))

        self.useFixture(fixtures.MonkeyPatch(
            'compute.virt.libvirt.driver.connector',
            fake_os_brick_connector))
        ...
        self.stubs.Set(jacket.compute.virt.libvirt.driver.LibvirtDriver,
                       '_get_instance_disk_info',
                       fake_get_instance_disk_info)

        self.stubs.Set(jacket.compute.virt.libvirt.driver.disk,
                       'extend', fake_extend)
        ...
        self.stubs.Set(jacket.compute.virt.libvirt.guest.Guest,
        ...
        self.stubs.Set(jacket.compute.virt.libvirt.driver.libvirt.Domain,
...
class VirtDriverLoaderTestCase(_FakeDriverBackendTestCase, test.TestCase):
    ...
    new_drivers = {
        'compute.virt.fake.FakeDriver': 'FakeDriver',
        'compute.virt.libvirt.LibvirtDriver': 'LibvirtDriver',
    ...
    def test_fail_to_load_new_drivers(self):
        self.flags(compute_driver='compute.virt.amiga')
...
class AbstractDriverTestCase(_VirtDriverTestCase, test.TestCase):
    def setUp(self):
        self.driver_module = "compute.virt.driver.ComputeDriver"
...
class FakeConnectionTestCase(_VirtDriverTestCase, test.TestCase):
    def setUp(self):
        self.driver_module = 'compute.virt.fake.FakeDriver'
...
class LibvirtConnTestCase(_VirtDriverTestCase, test.TestCase):
    ...
    def setUp(self):
        ...
        self.driver_module = 'compute.virt.libvirt.LibvirtDriver'
        super(LibvirtConnTestCase, self).setUp()
        self.stubs.Set(self.connection,
                       '_set_host_enabled', mock.MagicMock())
        self.useFixture(fixtures.MonkeyPatch(
            'compute.context.get_admin_context',
    ...
    def test_get_device_name_for_instance(self):
        self.skipTest("Tested by the compute.tests.unit.virt.libvirt suite")

    @catch_notimplementederror
    @mock.patch('compute.utils.get_image_from_system_metadata')
    @mock.patch("jacket.compute.Host.has_min_version")
