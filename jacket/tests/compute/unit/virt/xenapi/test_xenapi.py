...
from jacket.tests.compute.unit.db import fakes as db_fakes
...
from jacket.tests.compute.unit import fake_processutils
import jacket.tests.compute.unit.image.fake as fake_image
from jacket.tests.compute.unit import matchers
from jacket.tests.compute.unit.objects import test_aggregate
...
from jacket.compute.virt.xenapi import agent
...
from jacket.compute.virt.xenapi import fake as xenapi_fake
from jacket.compute.virt.xenapi import host
from jacket.compute.virt.xenapi.image import glance
...
from jacket.compute.virt.xenapi import vmops
from jacket.compute.virt.xenapi import volume_utils
...
CONF.import_opt('login_timeout', 'compute.virt.xenapi.client.session',
                group="xenserver")
...
def create_instance_with_system_metadata(context, instance_values):
    ...
    for k, v in instance_values.items():
        setattr(inst, k, v)
    inst.flavor = compute.Flavor.get_by_id(context,
                                           instance_values['instance_type_id'])
    inst.old_flavor = None
    inst.new_flavor = None
    inst.create()
    inst.pci_devices = compute.PciDeviceList(compute=[])
...
class XenAPIVolumeTestCase(stubs.XenAPITestBaseNoDB):
    ...
    def setUp(self):
        ...
        self.flags(firewall_driver='compute.virt.xenapi.firewall.'
...
class XenAPIVMTestCase(stubs.XenAPITestBase):
    ...
    def test_instance_not_exists(self):
        ...
        self.stubs.Set(compute.Instance, 'name', 'bar')
        instance = compute.Instance(uuid='fake-uuid')
    ...
    def check_vm_record(self, conn, instance_type_id, check_injection):
        flavor = compute.flavor_get(conn, instance_type_id)
    ...
    def _test_spawn(self, image_ref, kernel_id, ramdisk_id,
        ...
        if create_record:
            ...
            flavor = compute.Flavor.get_by_id(self.context,
                                              instance_type_id)
            ...
            instance = compute.Instance.get_by_id(self.context, instance_id,
...
"""
    ...
    def test_spawn_vlanmanager(self):
        self.flags(network_manager='compute.network.manager.VlanManager',
        ...
        # Instance 2 will use vlan network (see compute/fakes.py)
        ctxt = self.context.elevated()
        inst2 = self._create_instance(False, obj=True)
        networks = self.network.db.network_get_all(ctxt)
        with mock.patch('compute.compute.network.Network._from_db_object'):
        ...
        # consistent with bridge specified in compute compute
    ...
    @mock.patch('compute.compute.agent_build_get_by_triple')
    ...
    def _test_maintenance_mode(self, find_host, find_aggregate):
        ...
        def fake_aggregate_get(context, host, key):
            if find_aggregate:
                ...
                return []
        self.stub_out('compute.compute.aggregate_get_by_host',
                      fake_aggregate_get)
    ...
    def test_uuid_find(self):
        ...
        compute.instance_get_all_by_host(self.context, fake_inst['host'],
    ...
    def test_per_instance_usage_running(self):
        instance = self._create_instance(spawn=True)
        flavor = compute.Flavor.get_by_id(self.context, 3)
...
class XenAPIMigrateInstance(stubs.XenAPITestBase):
    ...
    def setUp(self):
        ...
        migration_values = {
            'source_compute': 'compute-compute',
            'dest_compute': 'compute-compute',
        ...
        self.migration = compute.migration_create(
    ...
    def _create_instance(self, **kw):
        ...
        values.update(kw)
        instance = compute.Instance(context=self.context, **values)
        instance.create()
        return instance
    ...
    def test_migrate_too_many_partitions_no_resize_down(self):
        instance = self._create_instance()
        xenapi_fake.create_vm(instance['name'], 'Running')
        flavor = compute.flavor_get_by_name(self.context, 'm1.small')
...
class XenAPIHostTestCase(stubs.XenAPITestBase):
    ...
    def test_set_enable_host_enable(self):
        _create_service_entries(self.context, values={'compute': ['fake-mini']})
        self._test_host_action_no_param(self.conn.set_host_enabled,
                                        True, 'enabled')
        service = compute.service_get_by_host_and_binary(self.context, 'fake-mini',
                                                    'compute-compute')
...
# compute.tests.unit.virt.test_libvirt.IPTablesFirewallDriverTestCase
...
class XenAPIDom0IptablesFirewallTestCase(stubs.XenAPITestBase):
    ...
    def _create_instance_ref(self):
        return compute.instance_create(self.context,
                                  {'user_id': self.user_id,
                                   'project_id': self.project_id,
                                   'instance_type_id': 1})
    ...
    def _create_test_security_group(self):
        ...
        secgroup = compute.security_group_create(admin_ctxt,
                                {'user_id': self.user_id,
                                 'project_id': self.project_id,
                                 'name': 'testgroup',
                                 'description': 'test group'})
        compute.security_group_rule_create(admin_ctxt,
    ...
    def test_static_filters(self):
        ...
        src_secgroup = compute.security_group_create(admin_ctxt,
                                                {'user_id': self.user_id,
                                                 'project_id': self.project_id,
        ...
        compute.instance_add_security_group(admin_ctxt, instance_ref['uuid'],
                                       secgroup['id'])
        compute.instance_add_security_group(admin_ctxt, src_instance_ref['uuid'],
                                       src_secgroup['id'])
        instance_ref = compute.instance_get(admin_ctxt, instance_ref['id'])
        src_instance_ref = compute.instance_get(admin_ctxt, src_instance_ref['id'])
        ...
        compute.instance_destroy(admin_ctxt, instance_ref['uuid'])
...
class XenAPIAggregateTestCase(stubs.XenAPITestBase):
    ...
    def setUp(self):
        ...
        self.aggr = compute.Aggregate(context=self.context, id=1,
                                      **values)
    ...
    def test_add_to_aggregate_for_first_host_sets_metadata(self):
        ...
        result = compute.aggregate_get(self.context, aggregate.id)
    ...
    def test_add_to_aggregate_first_host(self):
        ...
        aggregate = compute.Aggregate(context=self.context)
    ...
    def test_remove_host_from_aggregate_error(self):
        ...
        metadata = {pool_states.POOL_FLAG: "XenAPI",
                    pool_states.KEY: pool_states.ACTIVE}
        compute.aggregate_metadata_add(self.context, aggr.id, metadata)
...
class HypervisorPoolTestCase(test.NoDBTestCase):
    ...
    fake_aggregate = compute.Aggregate(**fake_aggregate)
...
class XenAPILiveMigrateTestCase(stubs.XenAPITestBaseNoDB):
    ...
    def test_live_migration(self):
        ...
        migrate_data = compute.XenapiLiveMigrateData(
