...
from jacket.compute.virt.xenapi import agent as xenapi_agent
...
class SpawnTestCase(VMOpsTestBase):
    ...
    def _test_spawn(self, name_label_param=None, block_device_info_param=None,
        ...
        if name_label is None:
            name_label = "dummy"
        image_meta = compute.ImageMeta.from_dict({"id": "image_id"})
        context = "context"
...
class CreateVMRecordTestCase(VMOpsTestBase):
    ...
    def test_create_vm_record_with_vm_device_id(self, mock_create_vm,
        ...
        context = "context"
        instance = compute.Instance(vm_mode="vm_mode", uuid="uuid123")
        name_label = "dummy"
        disk_image_type = "vhd"
        kernel_file = "kernel"
        ramdisk_file = "ram"
...
class LiveMigrateTestCase(VMOpsTestBase):
    ...
    @mock.patch.object(vmops.compute.AggregateList, 'get_by_host')
    def test_get_host_uuid_from_aggregate_no_aggr(self, mock_get_by_host):
        mock_get_by_host.return_value = compute.AggregateList(compute=[])
        context = "ctx"
        hostname = "other_host"
    ...
    def test_get_host_uuid_from_aggregate_bad_aggr(self, mock_get_by_host):
        context = "ctx"
        hostname = "other_host"
        fake_aggregate_obj = compute.Aggregate(hosts=['fake'],
                                               metadata={'this': 'that'})
        fake_aggr_list = compute.AggregateList(compute=[fake_aggregate_obj])
