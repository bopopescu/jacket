...
INSTANCE_NUMA_TOPOLOGY = compute.InstanceNUMATopology(
        cells=[compute.InstanceNUMACell(id=0, cpuset=set([1, 2]), memory=512),
              compute.InstanceNUMACell(id=1, cpuset=set([3, 4]), memory=512)])
...
def fake_spec_obj(remove_id=False):
    ...
    req_obj = compute.RequestSpec(ctxt)
    if not remove_id:
        req_obj.id = 42
    ...
    req_obj.retry = compute.SchedulerRetries()
    req_obj.limits = compute.SchedulerLimits()
    req_obj.instance_group = compute.InstanceGroup()
