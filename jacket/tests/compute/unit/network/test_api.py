...
from jacket.tests.compute.unit.objects import test_virtual_interface
...
class ApiTestCase(test.TestCase):
    ...
    @mock.patch('compute.compute.NetworkList.get_all')
    ...
    def test_get_all_liberal(self, mock_get_all):
        self.flags(network_manager='compute.network.manager.FlatDHCPManaager')
        mock_get_all.return_value = mock.sentinel.get_all
        self.assertEqual(mock.sentinel.get_all,
                         self.network_api.get_all(self.context))
        mock_get_all.assert_called_once_with(self.context,
    ...
    @mock.patch('compute.compute.Network.get_by_uuid')
    ...
    @mock.patch('compute.compute.Network.get_by_id')
    @mock.patch('compute.db.virtual_interface_get_by_instance')
    def test_get_vifs_by_instance(self, mock_get_by_instance,
        ...
        mock_get_by_instance.return_value = [
            dict(test_virtual_interface.fake_vif,
                 network_id=123)]
        mock_get_by_id.return_value = compute.Network()
        mock_get_by_id.return_value.uuid = mock.sentinel.network_uuid
        instance = compute.Instance(uuid=mock.sentinel.inst_uuid)
    ...
    @mock.patch('compute.compute.Network.get_by_id')
    @mock.patch('compute.db.virtual_interface_get_by_address')
    ...
    def test_allocate_for_instance_handles_macs_passed(self):
        # If a macs argument is supplied to the 'compute-network' API, it is just
        # ignored. This test checks that the call down to the rpcapi layer
        # doesn't pass macs down: compute-network doesn't support hypervisor
        ...
        instance = compute.Instance(id=1, uuid='uuid', project_id='project_id',
    ...
    def _do_test_associate_floating_ip(self, orig_instance_uuid):
        ...
        new_instance = compute.Instance(uuid=FAKE_UUID)
    ...
    @mock.patch('compute.compute.FloatingIP.get_by_id')
    ...
    @mock.patch('compute.compute.FloatingIP.get_pool_names')
    ...
    def _stub_migrate_instance_calls(self, method, multi_host, info):
        ...
        fake_instance = compute.Instance(
            uuid=uuid.uuid4().hex,
    ...
    def test_is_multi_host_instance_has_no_fixed_ip(self):
        ...
        instance = compute.Instance(uuid=FAKE_UUID)
    ...
    def _test_is_multi_host_network_has_no_project_id(self, is_multi_host,
        ...
        fip_get.return_value = [
            compute.FixedIP(instance_uuid=FAKE_UUID, network=network,
    ...
    @mock.patch('compute.compute.Network.get_by_uuid')
    @mock.patch('compute.compute.Network.disassociate')
    def test_network_disassociate_project(self, mock_disassociate, mock_get):
        net_obj = compute.Network(context=self.context, id=1)
    ...
    @mock.patch('compute.compute.Network.get_by_uuid')
    @mock.patch('compute.compute.Network.associate')
    ...
    @mock.patch('compute.compute.Network.get_by_uuid')
    @mock.patch('compute.compute.Network.save')
    ...
    def test_network_disassociate(self, mock_disassociate, mock_get):
        mock_get.return_value = compute.Network(context=self.context, id=123)
    ...
    def _test_refresh_cache(self, method, *args, **kwargs):
        ...
        with test.nested(
            ...
            mock.patch.object(compute.InstanceInfoCache, 'save'),
        ) as (
    ...
    @mock.patch('compute.db.fixed_ip_get_by_address')
    def test_get_fixed_ip_by_address(self, fip_get):
        ...
        fip = self.network_api.get_fixed_ip_by_address(self.context,
                                                       'fake-addr')
        self.assertIsInstance(fip, compute.FixedIP)

    @mock.patch('compute.compute.FixedIP.get_by_id')
    ...
    @mock.patch('compute.compute.FixedIP.get_by_floating_address')
    def test_get_instance_by_floating_address(self, mock_get_by_floating):
        mock_get_by_floating.return_value = compute.FixedIP(
    ...
    @mock.patch('compute.network.api.API.migrate_instance_start')
    ...
    @mock.patch('compute.network.api.API.migrate_instance_finish')
...
@mock.patch('compute.network.api.API')
@mock.patch('compute.db.instance_info_cache_update', return_value=fake_info_cache)
class TestUpdateInstanceCache(test.NoDBTestCase):
    def setUp(self):
        super(TestUpdateInstanceCache, self).setUp()
        self.context = context.get_admin_context()
        self.instance = compute.Instance(uuid=FAKE_UUID)
