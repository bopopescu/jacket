...
class LinuxNetworkUtilsTestCase(test.NoDBTestCase):
    def test_is_pid_cmdline_correct(self):
        ...
        with mock.patch.object(linux_net, 'open', fake_open, create=True):
            self.assertFalse(
                linux_net.is_pid_cmdline_correct(1, "compute-br100.conf"),
                "compute-br100.conf should not have been found")

        # Positive matching case
        fake_open = mock.mock_open(
            read_data=('/usr/sbin/dnsmasq '
                       '--dhcp-hostsfile='
                       '/opt/stack/data/compute/networks/compute-br100.conf'))
        with mock.patch.object(linux_net, 'open', fake_open, create=True):
            self.assertTrue(
                linux_net.is_pid_cmdline_correct(1, "compute-br100.conf"),
                'compute-br100.conf should have been found')
...
class LinuxNetworkTestCase(test.NoDBTestCase):
    ...
    def setUp(self):
        ...
        self.stub_out('compute.compute.virtual_interface_get_by_instance', get_vifs)
        self.stub_out('compute.compute.instance_get', get_instance)
        self.stub_out('compute.compute.network_get_associated_fixed_ips',
                      get_associated)
    ...
    def _get_fixedips(self, network, host=None):
        return compute.FixedIPList.get_by_network(self.context,
                                                  network,
                                                  host=host)
    ...
    def test_dhcp_opts_not_default_gateway_network(self):
        expected = "NW-0,3"
        fixedip = compute.FixedIPList.get_by_network(self.context,
                                                     {'id': 0})[0]
