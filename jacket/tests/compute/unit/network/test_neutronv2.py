...
class TestNeutronClient(test.NoDBTestCase):
    ...
    @mock.patch('compute.network.neutronv2.api._ADMIN_AUTH')
...
class TestNeutronv2Base(test.TestCase):
    ...
    def _fake_instance_object_with_info_cache(self, instance):
        ...
        instance = compute.Instance._from_db_object(self.context,
               compute.Instance(), fake_instance.fake_db_instance(**instance),
        ...
        if 'requested_networks' in kwargs:
            ...
            for n in nets:
                ordered_networks.append(
                    compute.NetworkRequest(network_id=n['id']))
...
class TestNeutronv2(TestNeutronv2Base):
    ...
    def test_get_instance_nw_info_remove_ports_from_neutron(self):
        ...
        # is also removed from the compute.
    ...
    def _fake_get_instance_nw_info_helper(self, network_cache,
        ...
        :param network_cache - data already in the compute network cache.
    ...
    def test_populate_neutron_extension_values_rxtx_factor(self):
        ...
        instance = compute.Instance(system_metadata={})
    ...
    def test_allocate_for_instance_accepts_only_portid(self):
        ...
        result = self._allocate_for_instance(
            requested_networks=compute.NetworkRequestList(
                compute=[compute.NetworkRequest(port_id='my_portid1')]))
        self.assertEqual(self.port_data1, result)

    @mock.patch('compute.network.neutronv2.api.API._unbind_ports')
    def test_allocate_for_instance_not_enough_macs_via_ports(self,
        ...
        requested_networks = compute.NetworkRequestList(
            compute = [
                compute.NetworkRequest(network_id=self.nets2[1]['id']),
                compute.NetworkRequest(port_id='my_portid1')])
    ...
    def test_allocate_for_instance_not_enough_macs(self, mock_unbind):
        ...
        requested_networks = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(network_id=self.nets2[1]['id']),
                     compute.NetworkRequest(network_id=self.nets2[0]['id'])])
        api = self._stub_allocate_for_instance(
            net_idx=2, requested_networks=requested_networks,
            macs=set(['my_mac2']),
    ...
    def test_allocate_for_instance_with_requested_non_available_network(self):
       ...
       """
        requested_networks = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(network_id=net['id'])
    ...
    def test_allocate_for_instance_with_invalid_network_id(self):
        requested_networks = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(network_id='invalid_id')])
    ...
    def test_allocate_for_instance_with_requested_networks_with_fixedip(self):
        # specify only first and last network
        requested_networks = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(network_id=self.nets1[0]['id'],
                                            address='10.0.1.0')])
    ...
    @mock.patch('compute.network.neutronv2.api.API._get_preexisting_port_ids')
    @mock.patch('compute.network.neutronv2.api.API._unbind_ports')
    ...
    def test_allocate_for_instance_port_not_found(self):
        ...
        requested_networks = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(port_id='invalid_id')])
        api = self._stub_allocate_for_instance(
            requested_networks=requested_networks,
            _break='pre_list_networks')
    ...
    def _deallocate_for_instance(self, number, requested_networks=None):
        ...
        # converted to handling instance compute.
        self.instance = fake_instance.fake_instance_obj(self.context,
                                                        **self.instance)
        api = neutronapi.API()
        port_data = number == 1 and self.port_data1 or self.port_data2
    ...
    def test_deallocate_for_instance_1_with_requested(self, mock_preexisting):
        mock_preexisting.return_value = []
        requested = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(network_id='fake-net',
                                            address='1.2.3.4',
                                            port_id='fake-port')])
    ...
    def test_validate_networks_duplicate_enable(self):
        ...
        requested_networks = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(network_id='my_netid1'),
                     compute.NetworkRequest(network_id='my_netid1')])
    ...
    def test_allocate_for_instance_requested_networks_duplicates_port(self):
        ...
        requested_networks = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(port_id=port['id'])
    ...
    def test_allocate_for_instance_requested_networks_duplicates_combo(self):
        ...
        requested_networks = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(network_id='my_netid2'),
                     compute.NetworkRequest(port_id=self.port_data1[0]['id']),
                     compute.NetworkRequest(network_id='my_netid2'),
                     compute.NetworkRequest(port_id=self.port_data3[0]['id'])])
        self._allocate_for_instance(net_idx=7,
                                    requested_networks=requested_networks)

    def test_validate_networks_not_specified(self):
        requested_networks = compute.NetworkRequestList(compute=[])
        self.moxed_client.list_networks(
    ...
    def test_validate_networks_port_in_use(self):
        requested_networks = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(port_id=self.port_data3[0]['id'])])
    ...
    def test_validate_networks_port_no_subnet_id(self):
        ...
        requested_networks = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(port_id=port_a['id'])])
    ...
    def test_validate_networks_no_subnet_id(self):
        requested_networks = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(network_id='his_netid4')])
        ids = ['his_netid4']
        self.moxed_client.list_networks(
            id=mox.SameElementsAs(ids)).AndReturn(
                {'networks': self.nets4})
    ...
    def test_validate_networks_ports_in_same_network_enable(self):
        ...
        requested_networks = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(port_id=port_a['id']),
                     compute.NetworkRequest(port_id=port_b['id'])])
    ...
    def test_validate_networks_with_ports_and_networks(self):
        # Test validation for a request for one instance needing
        # one port allocated via compute with another port being passed in.
        port_b = self.port_data2[1]
    ...
    def test_validate_networks_one_port_and_no_networks(self):
        ...
        port_b['device_owner'] = None
        requested_networks = compute.NetworkRequestList(
            compute=[compute.NetworkRequest(port_id=port_b['id'])])
        self.moxed_client.show_port(port_b['id']).AndReturn({'port': port_b})
        self.mox.ReplayAll()
        api = neutronapi.API()
        max_count = api.validate_networks(self.context,
    ...
    def test_associate_floating_ip(self):
        ...
        search_opts = {'device_owner': 'compute:compute',
    ...
    @mock.patch('compute.compute.Instance.get_by_uuid')
    ...
    def test_build_network_info_model(self):
        ...
        fake_inst = compute.Instance()
        fake_inst.project_id = 'fake'
        fake_inst.uuid = 'uuid'
        fake_inst.info_cache = compute.InstanceInfoCache()
        ...
        self.mox.ReplayAll()
        neutronapi.get_client('fake')
        fake_inst.info_cache = compute.InstanceInfoCache.new(
    ...
    @mock.patch('compute.network.neutronv2.api.API._nw_info_get_subnets')
    @mock.patch('compute.network.neutronv2.api.API._nw_info_get_ips')
    @mock.patch('compute.network.neutronv2.api.API._nw_info_build_network')
    @mock.patch('compute.network.neutronv2.api.API._get_preexisting_port_ids')
    @mock.patch('compute.network.neutronv2.api.API._gather_port_ids_and_networks')
    ...
    def test_get_all_empty_list_networks(self):
        ...
        self.assertIsInstance(networks, compute.NetworkList)
    ...
    def test_create_pci_requests_for_sriov_ports(self, mock_get_client,
        ...
        requested_networks = compute.NetworkRequestList(
            compute = [
                compute.NetworkRequest(port_id='my_portid1'),
                compute.NetworkRequest(network_id='net1'),
                compute.NetworkRequest(port_id='my_portid2'),
                compute.NetworkRequest(port_id='my_portid3'),
                compute.NetworkRequest(port_id='my_portid4')])
...
class TestNeutronv2WithMock(test.TestCase):
    ...
    def test_get_instance_nw_info_locks_per_instance(self, mock_lock):
        instance = compute.Instance(uuid=uuid.uuid4())
        api = neutronapi.API()
    ...
    @mock.patch('compute.network.neutronv2.api.LOG')
    def test_get_instance_nw_info_verify_duplicates_ignored(self, mock_log):
        ...
        instance = compute.Instance(uuid=instance_uuid,
    ...
    @mock.patch('compute.network.base_api.update_instance_cache_with_nw_info')
    ...
    @mock.patch('compute.network.neutronv2.api.API._get_preexisting_port_ids')
    @mock.patch('compute.network.neutronv2.api.API.'
    ...
    def test_get_all_networks(self):
        ...
        with mock.patch.object(client.Client, 'list_networks') as mock_list:
            ...
            self.assertIsInstance(net_objs, compute.NetworkList)
    ...
    @mock.patch('compute.network.neutronv2.api.compute_utils')
    ...
    @mock.patch('compute.network.neutronv2.api.API._has_port_binding_extension')
    @mock.patch('compute.network.neutronv2.api.get_client')
    ...
    @mock.patch('compute.network.neutronv2.api.API.get_instance_nw_info')
    @mock.patch('compute.network.neutronv2.api.excutils')
    @mock.patch('compute.network.neutronv2.api.API._delete_ports')
    @mock.patch('compute.network.neutronv2.api.API.'
    ...
    @mock.patch('compute.network.neutronv2.api.API._get_available_networks')
    @mock.patch('compute.network.neutronv2.api.get_client')
    def test_allocate_for_instance_unbind(self, mock_ntrn,
        ...
        nw_req = compute.NetworkRequestList(
            compute = [compute.NetworkRequest(port_id='fake-port1'),
                       compute.NetworkRequest(port_id='fake-port2'),
                       compute.NetworkRequest(port_id='fail-port')])
    ...
    @mock.patch('compute.network.neutronv2.api.API._process_requested_networks')
    @mock.patch('compute.network.neutronv2.api.API._has_port_binding_extension')
    @mock.patch('compute.network.neutronv2.api.API._get_available_networks')
    @mock.patch('compute.network.neutronv2.api.get_client')
    ...
    @mock.patch('compute.compute.network_request.utils')
    @mock.patch('compute.network.neutronv2.api.LOG')
    @mock.patch('compute.network.neutronv2.api.base_api')
    @mock.patch('compute.network.neutronv2.api.API._delete_ports')
    @mock.patch('compute.network.neutronv2.api.API._unbind_ports')
    @mock.patch('compute.network.neutronv2.api.API._get_preexisting_port_ids')
    @mock.patch('compute.network.neutronv2.api.get_client')
    def test_preexisting_deallocate_for_instance(self, mock_ntrn,
        ...
        nw_req = compute.NetworkRequestList(
            compute = [compute.NetworkRequest(network_id='net-1',
                                              address='192.168.0.3',
                                              port_id='port-1',
    ...
    @mock.patch('compute.network.neutronv2.api.API._show_port')
    @mock.patch('compute.network.neutronv2.api.API._has_port_binding_extension')
    @mock.patch('compute.network.neutronv2.api.get_client')
    def test_port_binding_failed_with_request(self, mock_ntrn,
        ...
        mock_show_port.return_value = {
                            ...
                            'binding:vif_type': 'binding_failed'}
        nw_req = compute.NetworkRequestList(
            compute = [compute.NetworkRequest(port_id='fake_id')])
    ...
    def test_no_security_groups_requested(
        ...
        nets = [
            ...
            {'id': 'net2',
             ...
             'subnets': ['mysubnid2'],
             'port_security_enabled': True}]
        onets = compute.NetworkRequestList(compute=[
            compute.NetworkRequest(network_id='net1'),
            compute.NetworkRequest(network_id='net2')])

        instance = compute.Instance(
            project_id=1, availability_zone='compute', uuid='uuid1')
        ...
        mock_create_port.assert_has_calls([
            mock.call(
                mock.ANY, instance,
                u'net1', {'port':
                          {'device_owner': u'compute:compute',
                           'device_id': 'uuid1'}},
                None, [], None, None),
            mock.call(
                mock.ANY, instance,
