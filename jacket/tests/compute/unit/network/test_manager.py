...
from jacket.compute.network import model as net_model
from jacket.objects import compute
from jacket.objects.compute import network as network_obj
...
from jacket.tests.compute.unit import fake_ldap
from jacket.tests.compute.unit import fake_network
from jacket.tests.compute.unit import matchers
from jacket.tests.compute.unit.objects import test_fixed_ip
from jacket.tests.compute.unit.objects import test_floating_ip
from jacket.tests.compute.unit.objects import test_network
...
class FlatNetworkTestCase(test.TestCase):
    ...
    def test_validate_networks(self):
        self.mox.StubOutWithMock(compute, 'fixed_ip_get_by_address')
        ...
        compute.fixed_ip_get_by_address(mox.IgnoreArg(),
                                   mox.IgnoreArg(),
    ...
    def test_validate_reserved(self):
        ...
        network = nets[0]
        self.assertEqual(4, compute.network_count_reserved_ips(context_admin,
                        network['id']))
    ...
    def test_validate_reserved_start_end(self):
        ...
        self.assertEqual(23, compute.network_count_reserved_ips(context_admin,
                        network['id']))
    ...
    @mock.patch('compute.compute.fixed_ip.FixedIPList.get_by_instance_uuid')
    def test_get_instance_nw_info(self, get):

        def make_ip(index):
            vif = compute.VirtualInterface(uuid=index, address=index)
            network = compute.Network(uuid=index,
                                      bridge=index,
                                      label=index,
                                      project_id=index,
                                      injected=False,
    ...
    @mock.patch.object(compute.Network, 'get_by_uuid')
    ...
    def test_allocate_fixed_ip_instance_dns(self):
        ...
        fip = compute.FixedIP(address='192.168.0.101')
        fip.save = mock.MagicMock()

        with mock.patch.object(compute.Instance, 'get_by_uuid',
                               return_value=instance),\
             mock.patch.object(compute.FixedIP, 'associate_pool',
                               return_value=fip):
    ...
    def test_get_networks_by_uuids_ordering(self):
        self.mox.StubOutWithMock(compute, 'network_get_all_by_uuids')

        requested_networks = ['bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb',
                              'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa']
        compute.network_get_all_by_uuids(mox.IgnoreArg(), mox.IgnoreArg(),
    ...
    @mock.patch('compute.compute.instance.Instance.get_by_uuid')
    @mock.patch('compute.compute.quotas.Quotas.reserve')
    @mock.patch('compute.compute.quotas.ids_from_instance')
    ...
    @mock.patch('compute.compute.fixed_ip.FixedIP.get_by_address')
    @mock.patch('compute.compute.quotas.Quotas.reserve')
    @mock.patch('compute.compute.quotas.ids_from_instance')
    def test_deallocate_calculates_quota_auth(self, util_method, reserve,
                                              get_by_address):
        inst = compute.Instance(uuid='fake-uuid')
        fip = compute.FixedIP(instance_uuid='fake-uuid',
    ...
    @mock.patch('compute.compute.instance.Instance.get_by_uuid')
    @mock.patch('compute.compute.virtual_interface.VirtualInterface'
                '.get_by_instance_and_network')
    @mock.patch('compute.compute.fixed_ip.FixedIP.disassociate')
    @mock.patch('compute.compute.fixed_ip.FixedIP.associate')
    @mock.patch('compute.compute.fixed_ip.FixedIP.save')
    ...
    @mock.patch('compute.compute.virtual_interface.VirtualInterface'
                '.get_by_instance_and_network')
    @mock.patch('compute.compute.fixed_ip.FixedIP.disassociate')
    @mock.patch('compute.compute.fixed_ip.FixedIP.associate_pool')
    @mock.patch('compute.network.manager.NetworkManager._add_virtual_interface')
    def test_allocate_fixed_ip_create_new_vifs(self,
        ...
        vif = compute.VirtualInterface(context,
                                       id=1000,
    ...
    @mock.patch('compute.compute.instance.Instance.get_by_uuid')
    @mock.patch.object(compute, 'virtual_interface_get_by_instance_and_network',
                       return_value=None)
    @mock.patch('compute.compute.fixed_ip.FixedIP')
    def test_allocate_fixed_ip_add_vif_fails(self, mock_fixedip,
        ...
        with mock.patch.object(self.network, 'quotas_cls', compute.QuotasNoOp):
...
class FlatDHCPNetworkTestCase(test.TestCase):
    ...
    @mock.patch('compute.compute.fixed_ip.FixedIP.get_by_id')
    @mock.patch('compute.compute.floating_ip.FloatingIPList.get_by_host')
    @mock.patch('compute.network.linux_net.iptables_manager._apply')
    def test_init_host_iptables_defer_apply(self, iptable_apply,
                                            floating_get_by_host,
                                            fixed_get_by_id):
        def get_by_id(context, fixed_ip_id, **kwargs):
            net = compute.Network(bridge='testbridge',
                                      cidr='192.168.1.0/24')
            if fixed_ip_id == 1:
                return compute.FixedIP(address='192.168.1.4',
                                            network=net)
            elif fixed_ip_id == 2:
                return compute.FixedIP(address='192.168.1.5',
                                            network=net)
        ...
        float1 = compute.FloatingIP(address='1.2.3.4', fixed_ip_id=1)
        float2 = compute.FloatingIP(address='1.2.3.5', fixed_ip_id=2)
...
class VlanNetworkTestCase(test.TestCase):
    ...
    def test_quota_driver_type(self):
        self.assertEqual(compute.QuotasNoOp,
                         self.network.quotas_cls)

    def test_vpn_allocate_fixed_ip(self):
        self.mox.StubOutWithMock(compute, 'fixed_ip_associate')
        self.mox.StubOutWithMock(compute, 'fixed_ip_update')
        self.mox.StubOutWithMock(compute,
                              'virtual_interface_get_by_instance_and_network')
        self.mox.StubOutWithMock(compute, 'instance_get_by_uuid')

        fixed = dict(test_fixed_ip.fake_fixed_ip,
                     address='192.168.0.1')
        compute.fixed_ip_associate(mox.IgnoreArg(),
                              mox.IgnoreArg(),
                              mox.IgnoreArg(),
        ...
        compute.virtual_interface_get_by_instance_and_network(mox.IgnoreArg(),
                mox.IgnoreArg(), mox.IgnoreArg()).AndReturn(vifs[0])
        compute.instance_get_by_uuid(mox.IgnoreArg(),
                                mox.IgnoreArg(),
        ...
        network = compute.Network._from_db_object(
            self.context, compute.Network(),
    ...
    def test_allocate_fixed_ip(self):
        self.stubs.Set(self.network,
                '_do_trigger_security_group_members_refresh_for_instance',
                lambda *a, **kw: None)
        self.mox.StubOutWithMock(compute, 'fixed_ip_associate_pool')
        self.mox.StubOutWithMock(compute,
                              'virtual_interface_get_by_instance_and_network')
        self.mox.StubOutWithMock(compute, 'instance_get_by_uuid')
        ...
        fixed = dict(test_fixed_ip.fake_fixed_ip,
                     address='192.168.0.1')
        compute.fixed_ip_associate_pool(mox.IgnoreArg(),
                                   mox.IgnoreArg(),
    ...
    @mock.patch('compute.network.manager.VlanManager._setup_network_on_host')
    @mock.patch('compute.network.manager.VlanManager.'
                '_validate_instance_zone_for_dns_domain')
    @mock.patch('compute.network.manager.VlanManager.'
                '_do_trigger_security_group_members_refresh_for_instance')
    @mock.patch('compute.network.manager.VlanManager._add_virtual_interface')
    @mock.patch('compute.compute.instance.Instance.get_by_uuid')
    @mock.patch('compute.compute.fixed_ip.FixedIP.associate')
    @mock.patch('compute.compute.VirtualInterface.get_by_instance_and_network')
    def test_allocate_fixed_ip_return_none(self, mock_get,
        ...
        vif = compute.VirtualInterface(self.context,
                                       id=1000,
                                       address='00:00:00:00:00:00',
                                       instance_uuid=instance.uuid,
                                       network_id=net['id'],
    ...
    def test_floating_ip_owned_by_project(self):
        ...
        floating_ip = compute.FloatingIP(address='10.0.0.1',
                                         project_id=None)
    ...
    @mock.patch('compute.compute.fixed_ip_get')
    ...
    @mock.patch('compute.compute.floating_ip_get_all_by_host')
    @mock.patch('compute.compute.fixed_ip_get')
    ...
    def test_add_fixed_ip_instance_without_vpn_requested_networks(self):
        self.stubs.Set(self.network,
                '_do_trigger_security_group_members_refresh_for_instance',
                lambda *a, **kw: None)
        self.mox.StubOutWithMock(compute, 'network_get')
        self.mox.StubOutWithMock(compute, 'fixed_ip_associate_pool')
        self.mox.StubOutWithMock(compute,
                              'virtual_interface_get_by_instance_and_network')
        self.mox.StubOutWithMock(compute, 'instance_get_by_uuid')
        ...
        compute.network_get(mox.IgnoreArg(),
                       mox.IgnoreArg(),
    ...
    @mock.patch('compute.compute.fixed_ip_get_by_address')
    @mock.patch('compute.compute.network_get')
    def test_ip_association_and_allocation_of_other_project(self, net_get,
        ...
        float_ip = compute.floating_ip_create(context1.elevated(),
                                         {'address': '1.2.3.4',
                                          'project_id': context1.project_id})

        float_addr = float_ip['address']

        instance = compute.instance_create(context1,
                                      {'project_id': 'project1'})

        fix_addr = compute.fixed_ip_associate_pool(context1.elevated(),
        ...
        compute.floating_ip_destroy(context1.elevated(), float_addr)
        compute.fixed_ip_disassociate(context1.elevated(), fix_addr)

    @mock.patch('compute.compute.fixed_ip_get_by_address')
    @mock.patch('compute.compute.network_get')
    @mock.patch('compute.compute.fixed_ip_update')
    def test_deallocate_fixed(self, fixed_update, net_get, fixed_get):
        """Verify that release is called properly.

        Ensures https://bugs.launchpad.net/compute/+bug/973442 doesn't return
        """
        net_get.return_value = dict(test_network.fake_network,
                                    **networks[1])
        ...
        self.stub_out('compute.compute.virtual_interface_get', vif_get)
        context1 = context.RequestContext('user', 'project1')
        ...
        fix_addr = compute.fixed_ip_associate_pool(elevated, 1, instance['uuid'])
        fixed_get.return_value = dict(test_fixed_ip.fake_fixed_ip,
    ...
    def _deallocate_fixed_with_dhcp(self, mock_dev_exists, fixed_update,
        ...
        with test.nested(
            mock.patch.object(compute, 'virtual_interface_get', vif_get),
            mock.patch.object(
                utils, 'execute',
            ...
            elevated = context1.elevated()
            fix_addr = compute.fixed_ip_associate_pool(elevated, 1,
                                                  instance['uuid'])
            fixed_get.return_value = dict(test_fixed_ip.fake_fixed_ip,
                                          address=fix_addr.address,
                                          instance_uuid=instance.uuid,
    ...
    @mock.patch('compute.network.linux_net.device_exists', return_value=True)
    def test_deallocate_fixed_with_dhcp(self, mock_dev_exists):
        self._deallocate_fixed_with_dhcp(mock_dev_exists)

    @mock.patch('compute.network.linux_net.device_exists', return_value=False)
    ...
    def test_deallocate_fixed_deleted(self):
        ...
        instance = compute.instance_create(context1,
                {'project_id': 'project1'})
        network = compute.network_create_safe(elevated, networks[0])

        _fix_addr = compute.fixed_ip_associate_pool(elevated, 1, instance['uuid'])
        fix_addr = _fix_addr.address
        compute.fixed_ip_update(elevated, fix_addr, {'deleted': 1})
        elevated.read_deleted = 'yes'
        delfixed = compute.fixed_ip_get_by_address(elevated, fix_addr)
        values = {'address': fix_addr,
                  'network_id': network.id,
                  'instance_uuid': delfixed['instance_uuid']}
        compute.fixed_ip_create(elevated, values)
    ...
    def test_deallocate_fixed_no_vif(self, fixed_update, net_get, fixed_get):
        ...
        Ensures https://bugs.launchpad.net/compute/+bug/968457 doesn't return
        """
        net_get.return_value = dict(test_network.fake_network,
                                    **networks[1])
    ...
    def test_fixed_ip_cleanup_fail(self, fixed_update, net_get, fixed_get):
        ...
        elevated = context1.elevated()
        fix_addr = compute.FixedIP.associate_pool(elevated, 1,
                                                  instance['uuid'])
...
class CommonNetworkTestCase(test.TestCase):
    ...
    def test_validate_instance_zone_for_dns_domain(self):
        ...
        self.stub_out('compute.compute.dnsdomain_get', dnsdomain_get)
    ...
    @mock.patch('compute.compute.instance_get')
    @mock.patch('compute.compute.fixed_ip_get_by_instance')
    def test_deallocate_for_instance_passes_host_info(self, fixed_get,
        ...
        compute.virtual_interface_delete_by_instance = lambda _x, _y: None
        ...
        manager.deallocate_for_instance(
            ctx, instance=compute.Instance._from_db_object(self.context,
                compute.Instance(), instance_get.return_value))
    ...
    def test_deallocate_for_instance_with_requested_networks(self):
        manager = fake_network.FakeNetworkManager()
        compute = manager.db
        compute.virtual_interface_delete_by_instance = mock.Mock()
        ctx = context.RequestContext('igonre', 'igonre')
    ...
    @mock.patch('compute.compute.fixed_ip_get_by_instance')
    @mock.patch('compute.compute.fixed_ip_disassociate')
    ...
    @mock.patch('compute.compute.network_get_all')
    ...
    @mock.patch('compute.compute.network_get')
    @mock.patch('compute.compute.fixed_ips_by_virtual_interface')
    ...
    @mock.patch('compute.compute.network_get_by_uuid')
    ...
    @mock.patch('compute.compute.network_get_by_uuid')
    @mock.patch('compute.compute.network_disassociate')
    ...
    def _test_init_host_dynamic_fixed_range(self, net_manager):
        ...
        # Stub out calls we don't want to really run, mock the compute
        ...
        self.stubs.Set(net_manager.l3driver, 'initialize_gateway',
                                             lambda *args: None)
        self.mox.StubOutWithMock(compute, 'network_get_all_by_host')
        fake_networks = [dict(test_network.fake_network, **n)
                         for n in networks]
        compute.network_get_all_by_host(mox.IgnoreArg(),
                                   mox.IgnoreArg()
        ...
        new_network = {'id': 2,
                       ...
                       'vpn_public_port': '22',
                       'vpn_private_address': '10.0.0.2'}
        new_network_obj = compute.Network._from_db_object(
            self.context, compute.Network(),
    ...
    @mock.patch('compute.compute.quotas.Quotas.rollback')
    @mock.patch('compute.compute.fixed_ip.FixedIP.get_by_address')
    @mock.patch('compute.network.manager.NetworkManager.'
                '_do_trigger_security_group_members_refresh_for_instance')
...
class RPCAllocateTestCase(test.NoDBTestCase):
    """Tests compute.network.manager.RPCAllocateFixedIP."""
    def setUp(self):
...
class AllocateTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        dns = 'compute.network.noop_dns_driver.NoopDNSDriver'
    ...
    def test_allocate_for_instance(self):
        ...
        compute.floating_ip_create(self.context,
                              {'address': address,
                               'pool': 'compute'})
        inst = compute.Instance(context=self.context)
        inst.host = HOST
        inst.display_name = HOST
        inst.instance_type_id = 1
        inst.uuid = FAKEUUID
        inst.create()
        networks = compute.network_get_all(self.context)
        for network in networks:
            compute.network_update(self.context, network['id'],
                              {'host': HOST})
    ...
    def test_allocate_for_instance_illegal_network(self):
        ...
        compute.network_update(self.context, networks[0]['id'],
                          {'project_id': self.user_context.project_id})
    ...
    def test_allocate_for_instance_with_mac(self):
        ...
        inst = compute.instance_create(self.context, {'host': HOST,
                                                 'display_name': HOST,
                                                 'instance_type_id': 1})
        networks = compute.network_get_all(self.context)
        for network in networks:
...
class FloatingIPTestCase(test.TestCase):
    """Tests compute.network.manager.FloatingIP."""
    ...
    @mock.patch('compute.compute.fixed_ip_get')
    @mock.patch('compute.compute.network_get')
    @mock.patch('compute.compute.instance_get_by_uuid')
    @mock.patch('compute.compute.service_get_by_host_and_binary')
    @mock.patch('compute.compute.floating_ip_get_by_address')
    ...
    def test_double_deallocation(self):
        instance_ref = compute.instance_create(self.context,
    ...
    def test_deallocate_floating_ip_quota_rollback(self):
        ...
        self.mox.StubOutWithMock(compute, 'floating_ip_deallocate')
        ...
        self.network._floating_ip_owned_by_project(self.context,
                                                   mox.IgnoreArg())
        compute.floating_ip_deallocate(mox.IgnoreArg(),
    ...
    def test_deallocation_deleted_instance(self):
        ...
        instance.create()
        network = compute.network_create_safe(self.context.elevated(), {
                'project_id': self.project_id,
                'host': CONF.host,
                'label': 'foo'})
        fixed = compute.fixed_ip_create(self.context, {'allocated': True,
    ...
    @mock.patch('compute.compute.fixed_ip_get')
    @mock.patch('compute.compute.floating_ip_get_by_address')
    @mock.patch('compute.compute.floating_ip_update')
    ...
    @mock.patch('compute.compute.FloatingIP.get_by_address')
    ...
    @mock.patch('compute.compute.FloatingIPList.get_by_project')
    ...
    @mock.patch('compute.compute.FloatingIPList.get_by_fixed_address')
    def test_get_floating_ips_by_fixed_address(self, mock_get):
        mock_get.return_value = [compute.FloatingIP(address='1.2.3.4'),
                                 compute.FloatingIP(address='5.6.7.8')]
        self.assertEqual(['1.2.3.4', '5.6.7.8'],
                         self.network.get_floating_ips_by_fixed_address(
                             self.context, mock.sentinel.address))
        mock_get.assert_called_once_with(self.context, mock.sentinel.address)

    @mock.patch('compute.compute.floating_ip_get_pools')
...
class InstanceDNSTestCase(test.TestCase):
    """Tests compute.network.manager instance DNS."""
    def setUp(self):
...
class LdapDNSTestCase(test.NoDBTestCase):
    """Tests compute.network.ldapdns.LdapDNS."""
    def setUp(self):
        super(LdapDNSTestCase, self).setUp()

        self.useFixture(fixtures.MonkeyPatch(
            'compute.network.ldapdns.ldap',
            fake_ldap))
        dns_class = 'compute.network.ldapdns.LdapDNS'
...
class NetworkManagerNoDBTestCase(test.NoDBTestCase):
    """Tests compute.network.manager.NetworkManager without a database."""
    ...
    @mock.patch.object(compute.FixedIP, 'get_by_address')
    def test_release_fixed_ip_not_associated(self, mock_fip_get_by_addr):
        ...
        # to an instance.
        fip = compute.FixedIP._from_db_object(
            self.context, compute.FixedIP(), fake_network.next_fixed_ip(1))
    ...
    @mock.patch.object(compute.FixedIP, 'get_by_address')
    @mock.patch.object(compute.VirtualInterface, 'get_by_address')
    def test_release_fixed_ip_mac_matches_associated_instance(self,
        ...
        vif = compute.VirtualInterface._from_db_object(
                    self.context, compute.VirtualInterface(), vif)
        fip = compute.FixedIP._from_db_object(
                    self.context, compute.FixedIP(), fip)
    ...
    @mock.patch.object(compute.FixedIP, 'get_by_address')
    @mock.patch.object(compute.VirtualInterface, 'get_by_id')
