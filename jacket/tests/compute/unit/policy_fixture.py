...
from jacket.tests.compute.unit import fake_policy
...
class RealPolicyFixture(fixtures.Fixture):
    ...
    def setUp(self):
        ...
        self.policy_file = paths.state_path_def('etc/compute/policy.json')
        self._prepare_policy()
        CONF.set_override('policy_file', self.policy_file, group='oslo_policy')
        jacket.compute.policy.reset()
        jacket.compute.policy.init()
        self.addCleanup(jacket.compute.policy.reset)

    def set_rules(self, rules):
        policy = jacket.compute.policy._ENFORCER
        policy.set_rules(oslo_policy.Rules.from_dict(rules))


class PolicyFixture(RealPolicyFixture):
    """Load a fake policy from compute.tests.unit.fake_policy
