...
from jacket.compute.network import minidns
...
def get_test_admin_context():
    return jacket.compute.context.get_admin_context()
...
def get_test_image_object(context, instance_ref):
    ...
    image_service, image_id = glance.get_remote_image_service(context,
                                                              image_ref)
    return compute.ImageMeta.from_dict(
...
def get_test_flavor(context=None, options=None):
    ...
    try:
        flavor_ref = jacket.db.compute.flavor_create(context, test_flavor)
    except (exception.FlavorExists, exception.FlavorIdExists):
        flavor_ref = jacket.db.compute.flavor_get_by_name(context, 'kinda.big')
    return flavor_ref
...
def get_test_instance(context=None, flavor=None, obj=False):
    ...
    if obj:
        instance = compute.Instance(context, **test_instance)
        instance.flavor = compute.Flavor.get_by_id(context, flavor['id'])
        instance.create()
    else:
        flavors.save_flavor_info(test_instance['system_metadata'], flavor, '')
        instance = jacket.db.compute.instance_create(context, test_instance)
    return instance
...
def coreutils_readlink_available():
    _out, err = jacket.compute.utils.trycmd('readlink', '-nm', '/')
    return err == ''
