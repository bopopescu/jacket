...
class FakeModel(dict):
    """Represent a model from the compute."""
...
class FakeNetworkManager(network_manager.NetworkManager):
    ...
    def __init__(self, stubs=None):
        self.db = self.FakeDB()
        if stubs:
            stubs.Set(vif_obj, 'compute', self.db)
...
def fake_get_instance_nw_info(test, num_networks=1, ips_per_vif=2,
    ...
    # test is an instance of compute.test.TestCase
    ...
    test.stub_out('compute.compute.fixed_ip_get_by_instance', fixed_ips_fake)
    test.stub_out('compute.compute.instance_info_cache_update', update_cache_fake)

    class FakeContext(jacket.compute.context.RequestContext):
        def is_admin(self):
            return True
...
def stub_out_nw_api_get_instance_nw_info(test, func=None,
    ...
    test.stub_out('compute.network.api.API.get_instance_nw_info', func)


def stub_out_network_cleanup(test):
    test.stub_out('compute.network.api.API.deallocate_for_instance',
...
def set_stub_network_methods(test):
    ...
    test.stub_out('compute.compute.manager.ComputeManager._allocate_network',
                  fake_async_networkinfo)
    test.stub_out('compute.compute.manager.ComputeManager._deallocate_network',
                  lambda *args, **kwargs: None)
...
def unset_stub_network_methods(test):
    ...
    if _real_functions:
        for name in _real_functions:
            test.stub_out('compute.compute.manager.ComputeManager.' + name,
...
def stub_compute_with_ips(stubs):
    ...
    stubs.Set(compute, 'pci_device_get_by_addr', fake_pci_device_get_by_addr)
...
def _get_instances_with_cached_ips(orig_func, *args, **kwargs):
    ...
    context = args[0]
    fake_device = compute.PciDevice.get_by_dev_addr(context, 1, 'a')
    ...
    def _info_cache_for(instance):
        ...
        if isinstance(instance, obj_base.NovaObject):
            _info_cache = compute.InstanceInfoCache(context)
            compute.InstanceInfoCache._from_db_object(context, _info_cache,
                                                      info_cache)
...
def _create_instances_with_cached_ips(orig_func, *args, **kwargs):
    ...
    for instance in instances:
        ...
        compute.instance_info_cache_update(args[1], instance['uuid'],
