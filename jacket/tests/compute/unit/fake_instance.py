...
def fake_db_instance(**updates):
    if 'instance_type' in updates:
        if isinstance(updates['instance_type'], compute.Flavor):
            flavor = updates['instance_type']
        else:
            flavor = compute.Flavor(**updates['instance_type'])
    ...
    for name, field in compute.Instance.fields.items():
...
def fake_instance_obj(context, obj_instance_class=None, **updates):
    if obj_instance_class is None:
        obj_instance_class = compute.Instance
    expected_attrs = updates.pop('expected_attrs', None)
    flavor = updates.pop('flavor', None)
    if not flavor:
        flavor = compute.Flavor(id=1, name='flavor1',
...
def fake_fault_obj(context, instance_uuid, code=404,
    ...
    return compute.InstanceFault._from_db_object(context,
                                                 compute.InstanceFault(),
                                                 fault)
