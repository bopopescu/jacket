...
from jacket.cmd.compute import baseproxy
from jacket.compute import config
from jacket.compute.console import websocketproxy
...
class BaseProxyTestCase(test.NoDBTestCase):
    ...
    @mock.patch('compute.console.websocketproxy.NovaWebSocketProxy.__init__',
                       return_value=None)
    @mock.patch('compute.console.websocketproxy.NovaWebSocketProxy.start_server')
    def test_proxy(self, mock_start, mock_init, mock_gmr, mock_log,
                   mock_exists):
        # Force verbose=False so something else testing compute.cmd.baseproxy
        # doesn't impact the call to mocked NovaWebSocketProxy.__init__.
        self.flags(verbose=False)
        baseproxy.proxy('0.0.0.0', '6080')
        mock_log.assert_called_once_with(baseproxy.CONF, 'compute')
