...
class ResourceProviderTestCase(test.NoDBTestCase):
    """Test resource-provider compute' lifecycles."""
    ...
    def test_create_resource_provider_requires_uuid(self):
        resource_provider = compute.ResourceProvider(
            context = self.context)
        self.assertRaises(exception.ObjectActionError,
                          resource_provider.create)

    def test_create_resource_provider(self):
        created_resource_provider = compute.ResourceProvider(
            context=self.context,
        ...
        retrieved_resource_provider = compute.ResourceProvider.get_by_uuid(
            self.context,
    ...
    def test_create_inventory_with_uncreated_provider(self):
        ...
        disk_inventory = compute.Inventory(
            context=self.context,
    ...
    def test_create_and_update_inventory(self):
        ...
        inventories = compute.InventoryList.get_all_by_resource_provider_uuid(
        ...
        reloaded_inventories = (
            compute.InventoryList.get_all_by_resource_provider_uuid(
            self.context, resource_provider.uuid))
