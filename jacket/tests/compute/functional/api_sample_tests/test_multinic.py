...
class MultinicSampleJsonTest(test_servers.ServersSampleBase):
    ...
    def _get_flags(self):
        ...
        f['osapi_compute_extension'].append(
            'compute.api.openstack.compute.contrib.multinic.Multinic')
        return f

    def _disable_instance_dns_manager(self):
        # NOTE(markmc): it looks like multinic and instance_dns_manager are
        #               incompatible. See:
        #               https://bugs.launchpad.net/compute/+bug/1213251
        self.flags(
            instance_dns_manager='compute.network.noop_dns_driver.NoopDNSDriver')
