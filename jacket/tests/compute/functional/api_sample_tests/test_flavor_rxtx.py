...
class FlavorRxtxJsonTest(api_sample_base.ApiSampleTestBaseV21):
    ...
    def _get_flags(self):
        ...
        f['osapi_compute_extension'].append(
            'compute.api.openstack.compute.contrib.flavor_rxtx.'
            'Flavor_rxtx')
        f['osapi_compute_extension'].append(
            'compute.api.openstack.compute.contrib.flavormanage.'
            'Flavormanage')
        f['osapi_compute_extension'].append(
            'compute.api.openstack.compute.contrib.flavor_disabled.'
            'Flavor_disabled')
        f['osapi_compute_extension'].append(
            'compute.api.openstack.compute.contrib.flavor_access.'
            'Flavor_access')
        f['osapi_compute_extension'].append(
            'compute.api.openstack.compute.contrib.flavorextradata.'
            'Flavorextradata')
        f['osapi_compute_extension'].append(
            'compute.api.openstack.compute.contrib.flavor_swap.'
            'Flavor_swap')
        return f
