...
class CertificatesSamplesJsonTest(api_sample_base.ApiSampleTestBaseV21):
    ...
    def _get_flags(self):
        ...
        f['osapi_compute_extension'].append(
            'compute.api.openstack.compute.contrib.certificates.Certificates')
        return f

    def setUp(self):
        super(CertificatesSamplesJsonTest, self).setUp()
        self.stub_out('compute.crypto.ensure_ca_filesystem',
                      fake_crypto.ensure_ca_filesystem)
        self.stub_out('compute.crypto.fetch_ca', fake_crypto.fetch_ca)
        self.stub_out('compute.crypto.generate_x509_cert',
