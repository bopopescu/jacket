...
class HypervisorsSampleJsonTests(api_sample_base.ApiSampleTestBaseV21):
    ...
    def _get_flags(self):
        ...
        f['osapi_compute_extension'].append(
            'compute.api.openstack.compute.contrib.hypervisors.Hypervisors')
        f['osapi_compute_extension'].append(
            'compute.api.openstack.compute.contrib.extended_hypervisors.'
            'Extended_hypervisors')
        f['osapi_compute_extension'].append(
            'compute.api.openstack.compute.contrib.hypervisor_status.'
            'Hypervisor_status')
        return f
    ...
    @mock.patch("compute.compute.api.HostAPI.instance_get_all_by_host")
    ...
    def test_hypervisors_uptime(self):
        ...
        self.stub_out('compute.compute.api.HostAPI.get_host_uptime',
...
@mock.patch("compute.servicegroup.API.service_is_up", return_value=True)
class HypervisorsCellsSampleJsonTests(api_sample_base.ApiSampleTestBaseV21):
    ...
    def test_hypervisor_uptime(self, mocks):
        fake_hypervisor = compute.ComputeNode(id=1, host='fake-mini',
        ...
        def fake_service_get_by_compute_host(self, context, host):
            return cells_utils.ServiceProxy(
                compute.Service(id=1, host='fake-mini', disabled=False,
                                disabled_reason=None),
                'cell1')

        self.stub_out(
            'compute.compute.cells_api.HostAPI.compute_node_get',
            fake_compute_node_get)
        self.stub_out(
            'compute.compute.cells_api.HostAPI.service_get_by_compute_host',
            fake_service_get_by_compute_host)
        self.stub_out(
            'compute.compute.cells_api.HostAPI.get_host_uptime',
            fake_get_host_uptime)
