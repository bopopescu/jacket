...
from jacket.tests.compute.functional.api import client as api_client
from jacket.tests.compute.unit.image import fake as fake_image
from jacket.tests.compute.unit import policy_fixture

CONF = cfg.CONF
CONF.import_opt('null_kernel', 'compute.compute.api')
...
class TestServerGet(test.TestCase):
    ...
    def test_boot_server_with_invalid_image_meta(self):
        ...
        properties but compute validates the image metadata against the
        compute.compute.arch.ALL values during the conversion to the ImageMeta
