...
from jacket.storage.volume.drivers.emc import emc_cli_fc
from jacket.storage.volume.drivers.emc import emc_cli_iscsi
from jacket.storage.volume.drivers.emc import emc_vnx_cli
from jacket.storage.zonemanager import fc_san_lookup_service as fc_service
...
class EMCVNXCLIDriverISCSITestCase(DriverTestCaseBase):
    ...
    @mock.patch("storage.volume.drivers.emc.emc_vnx_cli."
                "CommandLineHelper.create_lun_by_cmd",
                mock.Mock(return_value={'lun_id': 1}))
    @mock.patch(
        "storage.volume.drivers.emc.emc_vnx_cli.EMCVnxCliBase.get_lun_id",
        mock.Mock(
            side_effect=[1, 1]))
...
"""
    ...
    @mock.patch(
        ...
        mock.Mock(return_value=1))
    @mock.patch(
        "storage.volume.drivers.emc.emc_vnx_cli.CommandLineHelper." +
        "get_lun_by_name",
        mock.Mock(return_value={'lun_id': 1}))
    @mock.patch(
        "eventlet.event.Event.wait",
    ...
    @mock.patch(
        "storage.volume.volume_types.get_volume_type_extra_specs",
...
class EMCVNXCLIDriverFCTestCase(DriverTestCaseBase):
    ...
    @mock.patch(
        "storage.zonemanager.fc_san_lookup_service.FCSanLookupService." +
...
class EMCVNXCLIToggleSPTestCase(test.TestCase):
    ...
    def test_no_sp_toggle(self, time_mock):
        ...
        with mock.patch('storage.utils.execute') as mock_utils:
