...
from jacket.tests.storage.unit.backup import fake_service_with_verify as fake_service
...
class BaseBackupTest(test.TestCase):
    def setUp(self):
        ...
        paths = ['storage.volume.rpcapi.VolumeAPI.delete_snapshot',
                 'storage.volume.rpcapi.VolumeAPI.delete_volume',
                 'storage.volume.rpcapi.VolumeAPI.detach_volume',
                 'storage.volume.rpcapi.VolumeAPI.'
    ...
    def _create_backup_db_entry(self, volume_id=str(uuid.uuid4()),
        ...
        backup = storage.Backup(context=self.ctxt, **kwargs)
        backup.create()
        return backup
    ...
    def _create_volume_db_entry(self, display_name='test_volume',
        ...
        volume = storage.Volume(context=self.ctxt, **vol)
        volume.create()
        return volume.id
    ...
    def _create_snapshot_db_entry(self, display_name='test_snapshot',
        ...
        snapshot_obj = storage.Snapshot(context=self.ctxt, **kwargs)
        snapshot_obj.create()
        return snapshot_obj
    ...
    def _create_volume_attach(self, volume_id):
        ...
        storage.volume_attached(self.ctxt, attachment['id'], None, 'testhost',
                           '/dev/vd0')
    ...
    def _create_export_record_db_entry(self,
        ...
        if backup_id:
            kwargs['id'] = backup_id
        backup = storage.BackupImport(context=self.ctxt, **kwargs)
        backup.create()
        return backup
...
class BackupTestCase(BaseBackupTest):
    ...
    @mock.patch.object(storage.tests.unit.fake_driver.FakeISCSIDriver,
                       'set_initialized')
    @mock.patch.object(storage.tests.unit.fake_driver.FakeISCSIDriver,
                       'do_setup')
    @mock.patch.object(storage.tests.unit.fake_driver.FakeISCSIDriver,
                       'check_for_setup_error')
    @mock.patch('storage.context.get_admin_context')
    def test_init_host(self, mock_get_admin_context, mock_check, mock_setup,
        ...
        storage.volume_update(self.ctxt, vol1_id, {'status': 'backing-up'})
        vol2_id = self._create_volume_db_entry()
        self._create_volume_attach(vol2_id)
        storage.volume_update(self.ctxt, vol2_id, {'status': 'restoring-backup'})
        vol3_id = self._create_volume_db_entry()
        storage.volume_update(self.ctxt, vol3_id, {'status': 'available'})
        vol4_id = self._create_volume_db_entry()
        storage.volume_update(self.ctxt, vol4_id, {'status': 'backing-up'})
        temp_vol_id = self._create_volume_db_entry()
        storage.volume_update(self.ctxt, temp_vol_id, {'status': 'available'})
        vol5_id = self._create_volume_db_entry()
        storage.volume_update(self.ctxt, vol5_id, {'status': 'backing-up'})
        ...
        vol1 = storage.volume_get(self.ctxt, vol1_id)
        self.assertEqual('available', vol1['status'])
        vol2 = storage.volume_get(self.ctxt, vol2_id)
        self.assertEqual('error_restoring', vol2['status'])
        vol3 = storage.volume_get(self.ctxt, vol3_id)
        self.assertEqual('available', vol3['status'])
        vol4 = storage.volume_get(self.ctxt, vol4_id)
        self.assertEqual('available', vol4['status'])
        vol5 = storage.volume_get(self.ctxt, vol5_id)
        self.assertEqual('available', vol5['status'])

        backup1 = storage.backup_get(self.ctxt, backup1.id)
        self.assertEqual(fields.BackupStatus.ERROR, backup1['status'])
        backup2 = storage.backup_get(self.ctxt, backup2.id)
        ...
        temp_vol = storage.Volume.get_by_id(self.ctxt, temp_vol_id)
        self.volume_mocks['delete_volume'].assert_called_once_with(
            self.ctxt, temp_vol)
        self.assertTrue(self.volume_mocks['detach_volume'].called)

    @mock.patch('storage.storage.backup.BackupList.get_all_by_host')
    @mock.patch('storage.manager.SchedulerDependentManager._add_to_threadpool')
    def test_init_host_with_service_inithost_offload(self,
        ...
        vol1_id = self._create_volume_db_entry()
        storage.volume_update(self.ctxt, vol1_id, {'status': 'available'})
        backup1 = self._create_backup_db_entry(
            status=fields.BackupStatus.DELETING, volume_id=vol1_id)

        vol2_id = self._create_volume_db_entry()
        storage.volume_update(self.ctxt, vol2_id, {'status': 'available'})
        backup2 = self._create_backup_db_entry(
    ...
    @mock.patch('storage.rpc.LAST_RPC_VERSIONS', {'storage-backup': '1.3',
                                                 'storage-volume': '1.7'})
    @mock.patch('storage.rpc.LAST_OBJ_VERSIONS', {'storage-backup': '1.2',
                                                 'storage-volume': '1.3'})
    ...
    def test_cleanup_incomplete_backup_operations_with_exceptions(self):
        ...
        mock_backup_get_by_host = self.mock_object(
            storage.BackupList, 'get_all_by_host')
    ...
    def test_cleanup_one_backing_up_volume(self):
        ...
        volume_id = self._create_volume_db_entry(status='backing-up',
                                                 previous_status='available')
        volume = storage.volume_get(self.ctxt, volume_id)
    ...
    def test_cleanup_one_creating_backup(self):
        ...
        volume = storage.Volume.get_by_id(self.ctxt, vol1_id)
        self.assertEqual('available', volume.status)
    ...
    def test_cleanup_one_restoring_backup(self):
        ...
        vol1_id = self._create_volume_db_entry()
        storage.volume_update(self.ctxt, vol1_id, {'status': 'restoring-backup', })
    ...
    def test_cleanup_temp_volumes_snapshots_for_one_backup_volume_not_found(
        ...
        mock_volume_get = self.mock_object(storage, 'volume_get')
    ...
    def test_cleanup_temp_snapshot_for_one_backup_not_found(self):
        ...
        backup.destroy()
        storage.volume_destroy(self.ctxt, vol1_id)
    ...
    @mock.patch('storage.utils.brick_get_connector_properties')
    @mock.patch('storage.volume.rpcapi.VolumeAPI.get_backup_device')
    @mock.patch('storage.utils.temporary_chown')
    @mock.patch('six.moves.builtins.open')
    def test_create_backup(self, mock_open, mock_temporary_chown,
        ...
        vol = storage.Volume.get_by_id(self.ctxt, vol_id)
    ...
    @mock.patch('storage.volume.utils.notify_about_backup_usage')
    ...
    def test_restore_backup_with_bad_service(self):
        ...
        vol_id = self._create_volume_db_entry(status='restoring-backup',
                                              size=1)
        service = 'storage.tests.backup.bad_service'
        backup = self._create_backup_db_entry(
            status=fields.BackupStatus.RESTORING, volume_id=vol_id,
            service=service)
    ...
    def test_delete_backup(self):
        ...
        backup = storage.backup_get(ctxt_read_deleted, backup.id)
    ...
    def test_list_backup(self):
        project_id = str(uuid.uuid4())
        backups = storage.backup_get_all_by_project(self.ctxt, project_id)
    ...
    def test_backup_get_all_by_project_with_deleted(self):
        ...
        ctxt_read_deleted = context.get_admin_context('yes')
        backups = storage.backup_get_all_by_project(ctxt_read_deleted, project_id)
    ...
    def test_backup_get_all_by_host_with_deleted(self):
        ...
        Unless context.read_deleted is 'yes'
        """
        backups = storage.backup_get_all_by_host(self.ctxt, 'testhost')
        self.assertEqual(0, len(backups))
        ...
        ctxt_read_deleted = context.get_admin_context('yes')
        backups = storage.backup_get_all_by_host(ctxt_read_deleted, 'testhost')
        self.assertEqual(2, len(backups))

    def test_backup_manager_driver_name(self):
        """Test mapping between backup services and backup drivers."""
        self.override_config('backup_driver', "storage.backup.services.swift")
        backup_mgr = \
            importutils.import_object(CONF.backup_manager)
        self.assertEqual('storage.backup.drivers.swift',
    ...
    def test_import_record_with_verify_not_implemented(self):
        ...
        self.backup_mgr.import_record(self.ctxt,
                                      ...
                                      export['backup_url'],
                                      backup_hosts)
        backup = storage.backup_get(self.ctxt, imported_record.id)
        self.assertEqual(fields.BackupStatus.AVAILABLE, backup['status'])
        self.assertEqual(vol_size, backup['size'])
    ...
    def test_import_record_with_bad_service(self):
        ...
        export['backup_service'] = 'storage.tests.unit.backup.bad_service'
        imported_record = self._create_export_record_db_entry()
        ...
        BackupAPI_import = 'storage.backup.rpcapi.BackupAPI.import_record'
    ...
    @mock.patch('storage.backup.drivers.nfs.NFSBackupDriver.'
    ...
    def test_check_support_to_force_delete(self, mock_check_configuration,
        ...
        self.override_config('backup_driver', 'storage.backup.drivers.nfs')
        self.backup_mgr = importutils.import_object(CONF.backup_manager)
        result = self.backup_mgr.check_support_to_force_delete(self.ctxt)
...
class BackupTestCaseWithVerify(BaseBackupTest):
    ...
    def setUp(self):
        self.override_config(
            "backup_driver",
            "storage.tests.unit.backup.fake_service_with_verify")
    ...
    def test_backup_reset_status_to_available_invalid_backup(self):
        volume = storage.volume_create(self.ctxt, {'status': 'available',
                                              'host': 'test',
                                              'provider_location': '',
                                              'size': 1})
        backup = self._create_backup_db_entry(status=fields.BackupStatus.ERROR,
...
class BackupAPITestCase(BaseBackupTest):
    ...
    @mock.patch.object(storage, 'BackupList')
    ...
    @mock.patch.object(storage, 'backup_create',
    ...
    @mock.patch.object(api.API, '_get_available_backup_service_host',
                       return_value='fake_host')
    @mock.patch.object(storage.Backup, '__init__',
    ...
    def test_create_when_failed_to_new_backup_object(self, mock_new,
        ...
        # More detailed, in the try clause, backup = storage.Backup(...)
    ...
    @mock.patch('storage.backup.api.API._get_available_backup_service_host')
    @mock.patch('storage.backup.rpcapi.BackupAPI.restore_backup')
    def test_restore_volume(self,
        ...
        backup = storage.Backup.get_by_id(self.ctxt, backup.id)
