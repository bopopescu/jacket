...
patch_retry = mock.patch('storage.utils.retry', fake_retry)
patch_retry.start()
sys.modules['purestorage'] = mock.Mock()
from jacket.storage.volume.drivers import pure

# Only mock utils.retry for storage.volume.drivers.pure import
patch_retry.stop()

DRIVER_PATH = "storage.volume.drivers.pure"
...
REPLICATION_PROTECTION_GROUP = "storage-group"
...
VOLUME_PURITY_NAME = VOLUME['name'] + '-storage'
...
SNAPSHOT_PURITY_NAME = SRC_VOL["name"] + '-storage.' + SNAPSHOT["name"]
...
VOLUME_CONNECTIONS = [
    {"host": "h1", "name": VOLUME["name"] + "-storage"},
    {"host": "h2", "name": VOLUME["name"] + "-storage"},
]
TOTAL_CAPACITY = 50.0
USED_SPACE = 32.1
...
REPLICATED_VOLUME_SNAPS = [
    {
        "source": "array1:volume-%s-storage" % fake.volume_id,
        "serial": "BBA481C01639104E0001D5F7",
        "created": "2014-12-04T22:59:38Z",
        "name": "array1:storage-repl-pg.2.volume-%s-storage" % fake.volume_id,
        "size": 1048576
    },
    {
        "source": "array1:volume-%s-storage" % fake.volume2_id,
        "serial": "BBA481C01639104E0001D5F8",
        "created": "2014-12-04T22:59:38Z",
        "name": "array1:storage-repl-pg.2.volume-%s-storage" % fake.volume2_id,
        "size": 1048576
    },
    {
        "source": "array1:volume-%s-storage" % fake.volume3_id,
        "serial": "BBA481C01639104E0001D5F9",
        "created": "2014-12-04T22:59:38Z",
        "name": "array1:storage-repl-pg.2.volume-%s-storage" % fake.volume3_id,
        "size": 1048576
    }
]
...
class PureBaseVolumeDriverTestCase(PureBaseSharedDriverTestCase):
    ...
    @mock.patch(BASE_DRIVER_OBJ + '._generate_replication_retention')
    @mock.patch(BASE_DRIVER_OBJ + '._setup_replicated_pgroups')
    @mock.patch('storage.volume.volume_types.get_volume_type')
    def test_do_setup_replicated(self, mock_get_volume_type,
        ...
        calls = [
            mock.call(self.array, [self.array2], 'storage-group',
    ...
    def test_generate_purity_host_name(self):
        ...
        self.assertTrue(result.endswith("-storage"))
    ...
    def test_create_volume(self, mock_is_replicated_type):
        ...
        self.array.create_volume.assert_called_with(
            VOLUME["name"] + "-storage", 2 * units.Gi)
    ...
    def test_create_volume_with_cgroup(self, mock_is_replicated_type,
                                       mock_add_to_cgroup):
        vol_name = VOLUME_WITH_CGROUP["name"] + "-storage"
        mock_is_replicated_type.return_value = False
    ...
    def test_create_volume_from_snapshot(self, mock_is_replicated_type):
        vol_name = VOLUME["name"] + "-storage"
        snap_name = SNAPSHOT["volume_name"] + "-storage." + SNAPSHOT["name"]
        mock_is_replicated_type.return_value = False
    ...
    def test_create_volume_from_cgsnapshot(self, mock_is_replicated_type,
        ...
        snap_name = "consisgroup-4a2f7e3a-312a-40c5-96a8-536b8a0f" \
                    "e074-storage.4a2f7e3a-312a-40c5-96a8-536b8a0fe075."\
                    + vol_name
    ...
    def test_get_pgroup_name_from_id(self):
        ...
        expected_name = "consisgroup-%s-storage" % id
    ...
    def test_get_pgroup_snap_suffix(self):
        ...
        expected_suffix = "cgsnapshot-%s-storage" % cgsnap.id
    ...
    def test_get_pgroup_snap_name(self):
        ...
        expected_name = "consisgroup-%(cg)s-storage.cgsnapshot-%(snap)s-storage"\
    ...
    def test_get_pgroup_snap_name_from_snapshot(self):
        ...
        pgsnap_name_base = (
            'consisgroup-%s-storage.cgsnapshot-%s-storage.%s-storage')
    ...
    def _create_mock_cg(self):
        ...
        mock_group.cg_name = "consisgroup-" + mock_group.id + "-storage"
        return mock_group
    ...
    def test_update_consistencygroup(self):
        ...
        expected_addvollist = [vol['name'] + '-storage' for vol in add_vols]
        remove_vols = [
            {'name': 'vol4'},
            {'name': 'vol5'},
        ]
        expected_remvollist = [vol['name'] + '-storage' for vol in remove_vols]
    ...
    def test_create_volume_replicated(self, mock_get_volume_type):
        ...
        self.array.set_pgroup.assert_called_with(
            ...
            addvollist=[VOLUME["name"] + "-storage"])
...
class PureISCSIDriverTestCase(PureDriverTestCase):
    ...
    @mock.patch("storage.volume.utils.generate_password", autospec=True)
...
class PureFCDriverTestCase(PureDriverTestCase):
    ...
    def test_initialize_connection(self, mock_connection):
        ...
        mock_connection.return_value = {"vol": VOLUME["name"] + "-storage",
                                        "lun": 1,
                                        }
