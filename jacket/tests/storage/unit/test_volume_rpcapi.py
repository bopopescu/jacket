...
"""
Unit Tests for storage.volume.rpcapi
"""
import copy
...
class VolumeRpcAPITestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        vol['size'] = 1
        volume = storage.volume_create(self.context, vol)
        ...
        cgsnapshot = storage.CGSnapshot.get_by_id(self.context, cgsnapshot.id)
    ...
    def _test_volume_api(self, method, rpc_method, **kwargs):
        ...
        if 'volume' in expected_msg:
            ...
            if isinstance(volume, storage.Volume):
        ...
        for kwarg, value in self.fake_kwargs.items():
            if isinstance(value, storage.Snapshot):
    ...
    @mock.patch('oslo_messaging.RPCClient.can_send_version', return_value=True)
    @mock.patch('storage.quota.DbQuotaDriver.rollback')
