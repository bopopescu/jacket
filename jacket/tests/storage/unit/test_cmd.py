...
from jacket.cmd.storage import all as cinder_all
from jacket.cmd.storage import api as cinder_api
from jacket.cmd.storage import backup as cinder_backup
from jacket.cmd.storage import manage as cinder_manage
from jacket.cmd.storage import rtstool as cinder_rtstool
...
class TestCinderApiCmd(test.TestCase):
    """Unit test cases for python modules under storage/cmd."""

    def setUp(self):
        super(TestCinderApiCmd, self).setUp()
        sys.argv = ['storage-api']
        CONF(sys.argv[1:], project='storage', version=version.version_string())

    def tearDown(self):
        super(TestCinderApiCmd, self).tearDown()

    @mock.patch('storage.service.WSGIService')
    @mock.patch('storage.service.process_launcher')
    @mock.patch('storage.rpc.init')
    @mock.patch('storage.utils.monkey_patch')
    ...
    def test_main(self, log_setup, monkey_patch, rpc_init, process_launcher,
        ...
        self.assertEqual('storage', CONF.project)
        self.assertEqual(CONF.version, version.version_string())
        log_setup.assert_called_once_with(CONF, "storage")
...
class TestCinderBackupCmd(test.TestCase):
    ...
    def setUp(self):
        ...
        sys.argv = ['storage-backup']
        CONF(sys.argv[1:], project='storage', version=version.version_string())

    def tearDown(self):
        super(TestCinderBackupCmd, self).tearDown()

    @mock.patch('storage.service.wait')
    @mock.patch('storage.service.serve')
    @mock.patch('storage.service.Service.create')
    @mock.patch('storage.utils.monkey_patch')
    @mock.patch('oslo_log.log.setup')
    def test_main(self, log_setup, monkey_patch, service_create, service_serve,
        ...
        log_setup.assert_called_once_with(CONF, "storage")
        monkey_patch.assert_called_once_with()
        service_create.assert_called_once_with(binary='storage-backup')
...
class TestCinderAllCmd(test.TestCase):
    ...
    def test_main(self, log_setup, get_logger, monkey_patch, process_launcher,
        ...
        self.assertEqual(CONF.version, version.version_string())
        log_setup.assert_called_once_with(CONF, "storage")
        get_logger.assert_called_once_with('storage.all')
        monkey_patch.assert_called_once_with()
        rpc_init.assert_called_once_with(CONF)
        process_launcher.assert_called_once_with()
        wsgi_service.assert_called_once_with('osapi_volume')
        ...
        service_create.assert_has_calls([mock.call(binary='storage-scheduler'),
                                         mock.call(binary='storage-backup'),
                                         mock.call(binary='storage-volume')])
    ...
    def test_main_load_binary_exception(self, log_setup, get_logger,
        ...
        launcher.launch_service.assert_any_call(server,
                                                workers=server.workers)
        for binary in ['storage-volume', 'storage-scheduler', 'storage-backup']:
...
class TestCinderSchedulerCmd(test.TestCase):
    ...
    def test_main(self, log_setup, monkey_patch, service_create,
        ...
        log_setup.assert_called_once_with(CONF, "storage")
        monkey_patch.assert_called_once_with()
        service_create.assert_called_once_with(binary='storage-scheduler')
        service_serve.assert_called_once_with(server)
        service_wait.assert_called_once_with()
...
class TestCinderVolumeCmd(test.TestCase):
    ...
    def setUp(self):
        ...
        sys.argv = ['storage-volume']
        CONF(sys.argv[1:], project='storage', version=version.version_string())

    def tearDown(self):
        super(TestCinderVolumeCmd, self).tearDown()

    @mock.patch('storage.service.get_launcher')
    @mock.patch('storage.service.Service.create')
    @mock.patch('storage.utils.monkey_patch')
    @mock.patch('oslo_log.log.setup')
    def test_main(self, log_setup, monkey_patch, service_create,
        ...
        service_create.assert_called_once_with(binary='storage-volume')
        launcher.launch_service.assert_called_once_with(server)
        launcher.wait.assert_called_once_with()
...
class TestCinderManageCmd(test.TestCase):
    ...
    def setUp(self):
        ...
        sys.argv = ['storage-manage']
        CONF(sys.argv[1:], project='storage', version=version.version_string())

    def tearDown(self):
        super(TestCinderManageCmd, self).tearDown()

    @mock.patch('storage.db.migration.db_sync')
    ...
    @mock.patch('storage.version.version_string')
    ...
    @mock.patch('storage.objects.base.CinderObjectSerializer')
    @mock.patch('storage.rpc.get_client')
    @mock.patch('storage.rpc.init')
    @mock.patch('storage.rpc.initialized', return_value=False)
    ...
    @mock.patch('storage.db.sqlalchemy.api.volume_get')
    @mock.patch('storage.context.get_admin_context')
    @mock.patch('storage.rpc.get_client')
    @mock.patch('storage.rpc.init')
    ...
    @mock.patch('storage.db.volume_destroy')
    @mock.patch('storage.db.sqlalchemy.api.volume_get')
    @mock.patch('storage.context.get_admin_context')
    @mock.patch('storage.rpc.init')
    ...
    @mock.patch('storage.db.backup_get_all')
    @mock.patch('storage.context.get_admin_context')
    ...
    @mock.patch('storage.db.backup_update')
    @mock.patch('storage.db.backup_get_all_by_host')
    @mock.patch('storage.context.get_admin_context')
    ...
    @mock.patch('storage.utils.service_is_up')
    @mock.patch('storage.db.service_get_all')
    @mock.patch('storage.context.get_admin_context')
    ...
    def test_service_commands_list(self):
        ...
        for binary in ('volume', 'scheduler', 'backup'):
            service['binary'] = 'storage-%s' % binary
    ...
    def test_main(self, register_cli_opt, config_opts_call):
        ...
        self.assertTrue(register_cli_opt.called)
        config_opts_call.assert_called_once_with(
            sys.argv[1:], project='storage', version=version.version_string())
...
class TestCinderRtstoolCmd(test.TestCase):
    ...
    @mock.patch('storage.cmd.rtstool.restore_from_file')
    ...
    @mock.patch('storage.cmd.rtstool.usage')
    ...
    def _test_main_check_argv(self):
        with mock.patch('storage.cmd.rtstool.usage') as usage:
            usage.side_effect = SystemExit(1)
            sys.argv = ['storage-rtstool', 'create']
    ...
    @mock.patch('storage.cmd.rtstool.save_to_file')
    ...
    def test_main_create(self):
        with mock.patch('storage.cmd.rtstool.create') as create:
            sys.argv = ['storage-rtstool',
                        'create',
                        mock.sentinel.backing_device,
                        mock.sentinel.name,
    ...
    @mock.patch('storage.cmd.rtstool.create')
    ...
    def test_main_add_initiator(self):
        with mock.patch('storage.cmd.rtstool.add_initiator') as add_initiator:
            sys.argv = ['storage-rtstool',
                        'add-initiator',
                        mock.sentinel.target_iqn,
                        mock.sentinel.userid,
    ...
    def test_main_get_targets(self):
        with mock.patch('storage.cmd.rtstool.get_targets') as get_targets:
            sys.argv = ['storage-rtstool', 'get-targets']
    ...
    def test_main_delete(self):
        with mock.patch('storage.cmd.rtstool.delete') as delete:
            sys.argv = ['storage-rtstool', 'delete', mock.sentinel.iqn]
    ...
    def test_main_verify(self, mock_verify_rtslib):
        sys.argv = ['storage-rtstool', 'verify']
...
class TestCinderVolumeUsageAuditCmd(test.TestCase):
    ...
    def setUp(self):
        ...
        sys.argv = ['storage-volume-usage-audit']
        CONF(sys.argv[1:], project='storage', version=version.version_string())

    def tearDown(self):
        super(TestCinderVolumeUsageAuditCmd, self).tearDown()

    @mock.patch('storage.utils.last_completed_audit_period')
    @mock.patch('storage.rpc.init')
    @mock.patch('storage.version.version_string')
    @mock.patch('oslo_log.log.getLogger')
    @mock.patch('oslo_log.log.setup')
    ...
    def test_main_time_error(self, get_admin_context, log_setup, get_logger,
        ...
        self.assertEqual(CONF.version, version.version_string())
        log_setup.assert_called_once_with(CONF, "storage")
        get_logger.assert_called_once_with('storage')
        self.assertEqual(-1, exit.code)
        rpc_init.assert_called_once_with(CONF)
        last_completed_audit_period.assert_called_once_with()

    @mock.patch('storage.volume.utils.notify_about_volume_usage')
    @mock.patch('storage.db.volume_get_active_by_window')
    @mock.patch('storage.utils.last_completed_audit_period')
    @mock.patch('storage.rpc.init')
    @mock.patch('storage.version.version_string')
    @mock.patch('oslo_log.log.getLogger')
    ...
    @mock.patch('storage.volume.utils.notify_about_snapshot_usage')
    @mock.patch('storage.objects.snapshot.SnapshotList.get_active_by_window')
    @mock.patch('storage.volume.utils.notify_about_volume_usage')
    @mock.patch('storage.db.volume_get_active_by_window')
    @mock.patch('storage.utils.last_completed_audit_period')
    @mock.patch('storage.rpc.init')
