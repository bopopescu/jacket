...
from jacket.tests.storage.unit.backup import fake_swift_client
from jacket.tests.storage.unit.backup import fake_swift_client2
...
class BackupSwiftTestCase(test.TestCase):
    ...
    def _create_backup_db_entry(self,
        ...
        try:
            storage.volume_get(self.ctxt, volume_id)
    ...
    def setUp(self):
        ...
        notify_patcher = mock.patch(
            'storage.volume.utils.notify_about_backup_usage')
    ...
    def test_backup_uncompressed(self):
        ...
        backup = storage.Backup.get_by_id(self.ctxt, fake.backup_id)
    ...
    @mock.patch.object(storage, 'backup_update', wraps=storage.backup_update)
    ...
    @mock.patch('storage.backup.drivers.swift.SwiftBackupDriver.'
                '_send_progress_end')
    @mock.patch('storage.backup.drivers.swift.SwiftBackupDriver.'
    ...
    def test_backup_cmp_shafiles(self):
        ...
        service = swift_dr.SwiftBackupDriver(self.ctxt)
        self.volume_file.seek(0)
        deltabackup = storage.Backup.get_by_id(self.ctxt, fake.backup2_id)
    ...
    def test_restore_delta(self):
        ...
        with tempfile.NamedTemporaryFile() as restored_file:
            backup = storage.Backup.get_by_id(self.ctxt, fake.backup2_id)
