...
class TestSnapshot(test_objects.BaseObjectsTestCase):
    ...
    @mock.patch('storage.db.get_by_id', return_value=fake_db_snapshot)
    def test_get_by_id(self, snapshot_get):
        snapshot = storage.Snapshot.get_by_id(self.context, 1)
    ...
    def test_get_by_id_no_existing_id(self, model_query):
        ...
        self.assertRaises(exception.SnapshotNotFound,
                          storage.Snapshot.get_by_id, self.context, 123)
    ...
    @mock.patch('storage.db.snapshot_create', return_value=fake_db_snapshot)
    def test_create(self, snapshot_create):
        snapshot = storage.Snapshot(context=self.context)
        snapshot.create()
        self.assertEqual(fake_snapshot_obj['id'], snapshot.id)
        self.assertEqual(fake_snapshot_obj['volume_id'], snapshot.volume_id)

    @mock.patch('storage.db.snapshot_create')
    ...
    @mock.patch('storage.db.snapshot_update')
    def test_save(self, snapshot_update):
        snapshot = storage.Snapshot._from_db_object(
            self.context, storage.Snapshot(), fake_db_snapshot)
        snapshot.display_name = 'foobar'
        snapshot.save()
        snapshot_update.assert_called_once_with(self.context, snapshot.id,
                                                {'display_name': 'foobar'})

    @mock.patch('storage.db.snapshot_metadata_update',
    ...
    @mock.patch('storage.db.snapshot_destroy')
    def test_destroy(self, snapshot_destroy):
        snapshot = storage.Snapshot(context=self.context, id=fake.snapshot_id)
        snapshot.destroy()
        snapshot_destroy.assert_called_once_with(self.context,
                                                 fake.snapshot_id)

    @mock.patch('storage.db.snapshot_metadata_delete')
    def test_delete_metadata_key(self, snapshot_metadata_delete):
        snapshot = storage.Snapshot(self.context, id=fake.snapshot_id)
    ...
    def test_obj_fields(self):
        ...
        snapshot = storage.Snapshot(context=self.context, id=fake.volume_id,
                                    volume=volume)
    ...
    @mock.patch('storage.storage.volume.Volume.get_by_id')
    @mock.patch('storage.storage.cgsnapshot.CGSnapshot.get_by_id')
    ...
    @mock.patch('storage.db.snapshot_data_get_for_project')
    ...
    @mock.patch('storage.db.sqlalchemy.api.snapshot_get')
    def test_refresh(self, snapshot_get):
        ...
        snapshot = storage.Snapshot.get_by_id(self.context, fake.snapshot_id)
...
class TestSnapshotList(test_objects.BaseObjectsTestCase):
    @mock.patch('storage.storage.volume.Volume.get_by_id')
    @mock.patch('storage.db.snapshot_get_all', return_value=[fake_db_snapshot])
    def test_get_all(self, snapshot_get_all, volume_get_by_id):
        ...
        snapshots = storage.SnapshotList.get_all(
    ...
    @mock.patch('storage.db.snapshot_get_by_host',
                return_value=[fake_db_snapshot])
    def test_get_by_host(self, get_by_host, volume_get_by_id):
        fake_volume_obj = fake_volume.fake_volume_obj(self.context)
        volume_get_by_id.return_value = fake_volume_obj

        snapshots = storage.SnapshotList.get_by_host(
            self.context, 'fake-host')
        self.assertEqual(1, len(snapshots))
        TestSnapshot._compare(self, fake_snapshot_obj, snapshots[0])

    @mock.patch('storage.storage.volume.Volume.get_by_id')
    @mock.patch('storage.db.snapshot_get_all_by_project',
                return_value=[fake_db_snapshot])
    def test_get_all_by_project(self, get_all_by_project, volume_get_by_id):
        ...
        search_opts = mock.sentinel.search_opts
        snapshots = storage.SnapshotList.get_all_by_project(
    ...
    @mock.patch('storage.storage.volume.Volume.get_by_id')
    @mock.patch('storage.db.snapshot_get_all_for_volume',
                return_value=[fake_db_snapshot])
    ...
    @mock.patch('storage.storage.volume.Volume.get_by_id')
    @mock.patch('storage.db.snapshot_get_active_by_window',
                return_value=[fake_db_snapshot])
    ...
    @mock.patch('storage.storage.volume.Volume.get_by_id')
    @mock.patch('storage.db.snapshot_get_all_for_cgsnapshot',
                return_value=[fake_db_snapshot])
    ...
    @mock.patch('storage.storage.volume.Volume.get_by_id')
    @mock.patch('storage.db.snapshot_get_all')
