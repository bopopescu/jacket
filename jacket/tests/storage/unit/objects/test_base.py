...
from jacket.tests.storage.unit import fake_objects
...
class TestCinderObject(test_objects.BaseObjectsTestCase):
    ...
    def test_refresh(self):
        ...
        class MyTestObject(storage.base.CinderObject,
                           storage.base.CinderObjectDictCompat,
                           storage.base.CinderComparableObject):
        ...
        with mock.patch(
                'storage.storage.base.CinderObject.get_by_id') as get_by_id:
    ...
    def test_refresh_no_id_field(self):
        @objects.base.CinderObjectRegistry.register_if(False)
        class MyTestObjectNoId(storage.base.CinderObject,
                               storage.base.CinderObjectDictCompat,
                               storage.base.CinderComparableObject):
...
class TestCinderComparableObject(test_objects.BaseObjectsTestCase):
    def test_comparable_objects(self):
        ...
        class MyComparableObj(storage.base.CinderObject,
                              storage.base.CinderObjectDictCompat,
                              storage.base.CinderComparableObject):
...
class TestCinderObjectConditionalUpdate(test.TestCase):
    ...
    def _create_snapshot(self, volume):
        snapshot = storage.Snapshot(context=self.context, volume_id=volume.id)
        snapshot.create()
        return snapshot
    ...
    def test_default_conditional_fail_update_non_iterable_expected(self):
        ...
        volume = storage.Volume.get_by_id(self.context, volume_in_db.id)
        volume_in_db.size += 1
        volume_in_db.save()
    ...
    def test_conditional_update_negated_non_iterable_expected(self):
        ...
        self.assertTrue(volume.conditional_update(
            {'status': 'deleting', 'size': 2},
            {'status': storage.Not('in-use'), 'size': storage.Not(2)}))
    ...
    def test_conditional_update_negated_iterable_expected(self):
        ...
        self.assertTrue(volume.conditional_update(
            {'status': 'deleting', 'size': 20},
            {'status': storage.Not(('creating', 'in-use')), 'size': range(10)}))
    ...
    def test_conditional_update_fail_negated_iterable_expected(self):
        ...
        self.assertFalse(volume.conditional_update(
            {'status': 'error'},
            {'status': storage.Not(('available', 'in-use')), 'size': range(2, 10)}))
...
class TestCinderDictObject(test_objects.BaseObjectsTestCase):
    @objects.base.CinderObjectRegistry.register_if(False)
    class TestDictObject(storage.base.CinderObjectDictCompat,
                         storage.base.CinderObject):
...
@mock.patch('storage.storage.base.OBJ_VERSIONS', fake_objects.MyHistory())
class TestCinderObjectSerializer(test_objects.BaseObjectsTestCase):
    def setUp(self):
        ...
        self.parent = fake_objects.ParentObject(uuid=uuid.uuid4(),
                                                child=self.obj,
                                                scheduled_at=None)
        self.parent_list = fake_objects.ParentObjectList(storage=[self.parent])
    ...
    def test_serialize_init_pinned_has_manifest(self):
        ...
        objs_version = '1.5'
        serializer = storage.base.CinderObjectSerializer(objs_version)
    ...
    def test_serialize_entity_unknown_version(self):
        ...
        self.assertRaises(exception.CappedVersionUnknown,
                          storage.base.CinderObjectSerializer, '0.9')
