...
class TestVolume(test_objects.BaseObjectsTestCase):
    ...
    def test_get_by_id(self, volume_get):
        ...
        volume = storage.Volume.get_by_id(self.context, fake.volume_id)
    ...
    def test_get_by_id_no_existing_id(self, model_query):
        ...
        self.assertRaises(exception.VolumeNotFound,
                          storage.Volume.get_by_id, self.context, 123)

    @mock.patch('storage.db.volume_create')
    def test_create(self, volume_create):
        db_volume = fake_volume.fake_db_volume()
        volume_create.return_value = db_volume
        volume = storage.Volume(context=self.context)
        volume.create()
        self.assertEqual(db_volume['id'], volume.id)

    @mock.patch('storage.db.volume_update')
    def test_save(self, volume_update):
        db_volume = fake_volume.fake_db_volume()
        volume = storage.Volume._from_db_object(self.context,
                                                storage.Volume(), db_volume)
        volume.display_name = 'foobar'
        volume.save()
        volume_update.assert_called_once_with(self.context, volume.id,
                                              {'display_name': 'foobar'})

    @mock.patch('storage.db.volume_metadata_update',
    ...
    @mock.patch('storage.db.volume_admin_metadata_update',
                return_value={'key1': 'value1'})
    @mock.patch('storage.db.volume_update')
    def test_save_with_admin_metadata(self, volume_update,
        ...
        admin_context = context.RequestContext(self.user_id, self.project_id,
                                               is_admin=True)
        volume = storage.Volume._from_db_object(admin_context,
                                                storage.Volume(), db_volume)
        volume.admin_metadata = {'key1': 'value1'}
        volume.save()
    ...
    def test_save_with_consistencygroup(self):
        ...
        volume = storage.Volume._from_db_object(self.context,
                                                storage.Volume(), db_volume)
        volume.display_name = 'foobar'
        volume.consistencygroup = storage.ConsistencyGroup()
        self.assertRaises(exception.ObjectActionError, volume.save)
    ...
    def test_save_with_snapshots(self):
        ...
        volume = storage.Volume._from_db_object(self.context,
                                                storage.Volume(), db_volume)
        volume.display_name = 'foobar'
        volume.snapshots = storage.SnapshotList()
        self.assertRaises(exception.ObjectActionError, volume.save)
    ...
    def test_obj_fields(self):
        volume = storage.Volume(context=self.context, id=fake.volume_id,
    ...
    @mock.patch('storage.db.volume_metadata_delete')
    def test_delete_metadata_key(self, metadata_delete):
        volume = storage.Volume(self.context, id=fake.volume_id)
    ...
    @mock.patch('storage.db.volume_metadata_get')
    @mock.patch('storage.db.volume_glance_metadata_get')
    @mock.patch('storage.db.volume_admin_metadata_get')
    @mock.patch('storage.storage.volume_type.VolumeType.get_by_id')
    @mock.patch('storage.storage.volume_attachment.VolumeAttachmentList.'
                'get_all_by_volume_id')
    @mock.patch('storage.storage.consistencygroup.ConsistencyGroup.get_by_id')
    @mock.patch('storage.storage.snapshot.SnapshotList.get_all_for_volume')
    def test_obj_load_attr(self, mock_sl_get_all_for_volume, mock_cg_get_by_id,
                           ...
                           mock_metadata_get):
        volume = storage.Volume._from_db_object(
            self.context, storage.Volume(), fake_volume.fake_db_volume())
        ...
        volume2 = storage.Volume._from_db_object(
            self.context, storage.Volume(), fake2)
        volume_type = storage.VolumeType(context=self.context,
        ...
        va_objs = [storage.VolumeAttachment(context=self.context, id=i)
                   for i in [fake.object_id, fake.object2_id, fake.object3_id]]
        va_list = storage.VolumeAttachmentList(context=self.context,
                                               storage=va_objs)
        ...
        volume = storage.Volume._from_db_object(adm_context, storage.Volume(),
    ...
    def test_from_db_object_with_all_expected_attributes(self):
        ...
        volume = storage.Volume._from_db_object(self.context, storage.Volume(),
    ...
    @mock.patch('storage.db.volume_glance_metadata_get', return_value={})
    @mock.patch('storage.db.sqlalchemy.api.volume_get')
    ...
    @mock.patch('storage.db.volume_metadata_update', return_value={})
    @mock.patch('storage.db.volume_update')
    ...
    def test_finish_volume_migration(self, volume_update, metadata_update,
        ...
        if dest_vol_type_id:
            dest_volume_db['volume_type'] = fake_volume.fake_db_volume_type(
                id=dest_vol_type_id)
        expected_attrs = storage.Volume._get_expected_attrs(self.context)
        src_volume = storage.Volume._from_db_object(
            self.context, storage.Volume(), src_volume_db,
            expected_attrs=expected_attrs)
        dest_volume = storage.Volume._from_db_object(
            self.context, storage.Volume(), dest_volume_db,
            expected_attrs=expected_attrs)
...
class TestVolumeList(test_objects.BaseObjectsTestCase):
    @mock.patch('storage.db.volume_get_all')
    def test_get_all(self, volume_get_all):
        db_volume = fake_volume.fake_db_volume()
        volume_get_all.return_value = [db_volume]

        volumes = storage.VolumeList.get_all(self.context,
