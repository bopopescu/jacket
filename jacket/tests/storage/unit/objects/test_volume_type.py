...
class TestVolumeType(test_objects.BaseObjectsTestCase):
    ...
    @mock.patch('storage.db.sqlalchemy.api._volume_type_get_full')
    def test_get_by_id(self, volume_type_get):
        ...
        volume_type = storage.VolumeType.get_by_id(self.context,
                                                   fake.volume_type_id)
        self._compare(self, db_volume_type, volume_type)

    @mock.patch('storage.volume.volume_types.create')
    ...
    @mock.patch('storage.volume.volume_types.update')
    def test_save(self, volume_type_update):
        db_volume_type = fake_volume.fake_db_volume_type()
        volume_type = storage.VolumeType._from_db_object(self.context,
                                                         storage.VolumeType(),
                                                         db_volume_type)
    ...
    @mock.patch('storage.volume.volume_types.destroy')
...
class TestVolumeTypeList(test_objects.BaseObjectsTestCase):
    @mock.patch('storage.volume.volume_types.get_all_types')
    def test_get_all(self, get_all_types):
        db_volume_type = fake_volume.fake_db_volume_type()
        get_all_types.return_value = {db_volume_type['name']: db_volume_type}

        volume_types = storage.VolumeTypeList.get_all(self.context)
