...
class TestBackup(test_objects.BaseObjectsTestCase):
    ...
    @mock.patch('storage.db.get_by_id', return_value=fake_backup)
    def test_get_by_id(self, backup_get):
        backup = storage.Backup.get_by_id(self.context, fake.user_id)
    ...
    def test_get_by_id_no_existing_id(self, model_query):
        ...
        self.assertRaises(exception.BackupNotFound, storage.Backup.get_by_id,
                          self.context, 123)

    @mock.patch('storage.db.backup_create', return_value=fake_backup)
    def test_create(self, backup_create):
        backup = storage.Backup(context=self.context)
        backup.create()
    ...
    def test_save(self, backup_update):
        backup = storage.Backup._from_db_object(
            self.context, storage.Backup(), fake_backup)
        backup.display_name = 'foobar'
        backup.save()
        backup_update.assert_called_once_with(self.context, backup.id,
                                              {'display_name': 'foobar'})

    @mock.patch('storage.db.backup_destroy')
    def test_destroy(self, backup_destroy):
        backup = storage.Backup(context=self.context, id=fake.backup_id)
        backup.destroy()
    ...
    def test_import_record(self):
        ...
        imported_backup = storage.Backup.decode_record(export_string)
    ...
    @mock.patch('storage.db.sqlalchemy.api.backup_get')
    def test_refresh(self, backup_get):
        ...
        backup = storage.Backup.get_by_id(self.context, fake.backup_id)
...
class TestBackupList(test_objects.BaseObjectsTestCase):
    @mock.patch('storage.db.backup_get_all', return_value=[fake_backup])
    def test_get_all(self, backup_get_all):
        backups = storage.BackupList.get_all(self.context)
        self.assertEqual(1, len(backups))
        TestBackup._compare(self, fake_backup, backups[0])

    @mock.patch('storage.db.backup_get_all_by_project',
    ...
    def test_get_all_for_volume(self, get_all_by_host):
        ...
        backups = storage.BackupList.get_all_by_host(self.context,
                                                     fake_volume_obj.id)
        self.assertEqual(1, len(backups))
        TestBackup._compare(self, fake_backup, backups[0])
    ...
    def test_get_all_tenants(self, backup_get_all):
        ...
        backups = storage.BackupList.get_all(self.context, search_opts)
        self.assertEqual(1, len(backups))
        TestBackup._compare(self, fake_backup, backups[0])
