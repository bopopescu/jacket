...
import jacket.storage.volume.drivers.drbdmanagedrv as drv
...
class DrbdManageFakeDriver(object):
    ...
    def list_resources(self, res, serial, prop, req):
        ...
        if ('aux:storage-id' in prop and
                prop['aux:storage-id'].startswith("deadbeef")):
...
class DrbdManageIscsiTestCase(test.TestCase):
    ...
    def test_delete_volume(self):
        ...
        self.assertEqual(testvol['id'], dmd.odm.calls[0][3]["aux:storage-id"])
    ...
    def test_create_cloned_volume_larger_size(self):
        ...
        self.assertEqual(newvol['id'], dmd.odm.calls[9][3]["aux:storage-id"])
