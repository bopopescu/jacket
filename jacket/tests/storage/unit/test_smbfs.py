...
from jacket.storage.volume.drivers import smbfs
...
class SmbFsTestCase(test.TestCase):
    ...
    def test_get_volume_format_spec(self, volume_versioned_object,
        ...
        volume_type = storage.volume_type_create(
        ...
        volume = storage.volume_create(ctxt, fake_vol_dict)
        volume = storage.volume_get(ctxt, volume.id)

        if volume_versioned_object:
            volume = storage.Volume._from_db_object(ctxt, storage.Volume(),
                                                    volume)
