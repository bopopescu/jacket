...
class SSHPoolTestCase(test.TestCase):
    ...
    @mock.patch('storage.ssh_utils.CONF')
    ...
    def test_ssh_default_hosts_key_file(self, mock_isfile, mock_sshclient,
        ...
        mock_conf.ssh_hosts_key_file = '/var/lib/storage/ssh_known_hosts'
        ...
        self.assertEqual('/var/lib/storage/ssh_known_hosts', host_key_files)

        mock_ssh.load_host_keys.assert_called_once_with(
            '/var/lib/storage/ssh_known_hosts')
    ...
    def test_ssh_host_key_file_kwargs(self, mock_isfile, mock_sshclient,
        ...
        self.assertIn('/var/lib/storage/ssh_known_hosts', host_key_files)

        expected = [
            mock.call.load_host_keys('dummy_host_keyfile'),
            mock.call.load_host_keys('/var/lib/storage/ssh_known_hosts')]
    ...
    def test_create_default_known_hosts_file(self, mock_sshclient,
        ...
        CONF.state_path = '/var/lib/storage'
        CONF.ssh_hosts_key_file = '/var/lib/storage/ssh_known_hosts'

        default_file = '/var/lib/storage/ssh_known_hosts'
    ...
    @mock.patch.multiple('storage.ssh_utils.CONF',
                         strict_ssh_host_key_policy=True,
                         ssh_hosts_key_file='/var/lib/storage/ssh_known_hosts')
    @mock.patch('six.moves.builtins.open')
    @mock.patch('paramiko.SSHClient')
    @mock.patch('os.path.isfile', return_value=True)
