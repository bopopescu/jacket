...
from jacket.storage.volume.targets import tgt
...
class TestTgtAdmDriver(tf.TargetDriverFixture):
    ...
    def test_get_target(self):
        with mock.patch('storage.utils.execute',
    ...
    def test_create_iscsi_target_content(self):
        ...
        with mock.patch('storage.utils.execute', return_value=('', '')),\
                ...
                mock.patch.object(self.target, '_verify_backing_lun',
                                  side_effect=lambda x, y: True),\
                mock.patch('storage.volume.targets.tgt.open',
    ...
    def test_create_iscsi_target_already_exists(self):
        ...
        with mock.patch.object(self.target, '_get_target',
                ...
                mock.patch.object(self.target, '_verify_backing_lun',
                                  side_effect=lambda x, y: True),\
                mock.patch('storage.utils.execute', _fake_execute):
            self.assertEqual(
                1,
                self.target.create_iscsi_target(
                    self.test_vol,
