...
from jacket.api.storage.storage.contrib import volume_image_metadata
...
def fake_volume_get_all(*args, **kwargs):
    return storage.VolumeList(storage=[fake_volume_api_get()])


def fake_volume_get_all_empty(*args, **kwargs):
    return storage.VolumeList(storage=[])
...
class VolumeImageMetadataTest(test.TestCase):
    ...
    def _create_volume_and_glance_metadata(self):
        ...
        storage.volume_create(ctxt, {'id': 'fake', 'status': 'available',
                                'host': 'test', 'provider_location': '',
                                'size': 1})
        storage.volume_glance_metadata_create(ctxt, 'fake', 'image_id', 'someid')
        storage.volume_glance_metadata_create(ctxt, 'fake', 'image_name', 'fake')
        storage.volume_glance_metadata_create(ctxt, 'fake', 'kernel_id',
                                         'somekernel')
        storage.volume_glance_metadata_create(ctxt, 'fake', 'ramdisk_id',
                                         'someramdisk')
    ...
    def test_list_detail_volumes_with_limit(self):
        ...
        storage.volume_create(ctxt, {'id': 'fake', 'status': 'available',
                                'host': 'test', 'provider_location': '',
                                'size': 1})
        storage.volume_glance_metadata_create(ctxt, 'fake', 'key1', 'value1')
        storage.volume_glance_metadata_create(ctxt, 'fake', 'key2', 'value2')
    ...
    def test_create_image_metadata(self):
        ...
        self.stubs.Set(storage, 'volume_metadata_update',
    ...
    def test_delete(self):
        self.stubs.Set(storage, 'volume_metadata_delete',
