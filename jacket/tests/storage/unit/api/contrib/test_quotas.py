...
"""
Tests for storage.api.storage.contrib.quotas.py
"""
...
from jacket.tests.storage.unit import test_db_api
...
class QuotaSetsControllerTestBase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.req = mock.Mock()
        self.req.environ = {'storage.context': context.get_admin_context()}
        self.req.environ['storage.context'].is_admin = True
        self.req.params = {}

        self._create_project_hierarchy()
        self.req.environ['storage.context'].project_id = self.A.id

        get_patcher = mock.patch('storage.quota_utils.get_project_hierarchy',
                                 self._get_project)
        get_patcher.start()
        ...
        list_patcher = mock.patch('storage.quota_utils.get_all_projects',
                                  _list_projects)
        list_patcher.start()
...
class QuotaSetsControllerTest(QuotaSetsControllerTestBase):
    ...
    def test_show_not_authorized(self):
        self.req.environ['storage.context'].is_admin = False
        self.req.environ['storage.context'].user_id = 'bad_user'
        self.req.environ['storage.context'].project_id = 'bad_project'
    ...
    def test_show_non_admin_user(self):
        ...
        self.controller._get_quotas.assert_called_with(
            self.req.environ['storage.context'], 'foo', False)
    ...
    def test_update(self):
        ...
        body = make_body(gigabytes=storage.MAX_INT, tenant_id=None)
        result = self.controller.update(self.req, 'foo', body)
        self.assertDictMatch(body, result)
    ...
    @mock.patch(
        'storage.utils.validate_integer')
    ...
    def test_update_multi_value_with_bad_data(self):
        ...
        self.assertRaises(webob.exc.HTTPBadRequest, self.controller.update,
                          self.req, 'foo', body)
        # Verify that quota values are not updated in storage
    ...
    def test_update_bad_quota_limit(self):
        ...
        self.assertRaises(webob.exc.HTTPBadRequest, self.controller.update,
                          self.req, 'foo', body)
        body = {'quota_set': {'gigabytes': storage.MAX_INT + 1}}
        self.assertRaises(webob.exc.HTTPBadRequest, self.controller.update,
                          self.req, 'foo', body)

    def test_update_no_admin(self):
        self.req.environ['storage.context'].is_admin = False
        self.req.environ['storage.context'].project_id = 'foo'
        self.req.environ['storage.context'].user_id = 'foo_user'
    ...
    def _commit_quota_reservation(self):
        ...
        storage.reservation_commit(ctxt, res, 'foo')
        expected = {'project_id': 'foo',
                    'volumes': {'reserved': 0, 'in_use': 1},
                    'gigabytes': {'reserved': 0, 'in_use': 2},
                    }
        self.assertEqual(expected,
                         storage.quota_usage_get_all_by_project(ctxt, 'foo'))
    ...
    def test_subproject_show_not_using_nested_quotas(self):
        ...
        # see anyones quota
        self.req.environ['storage.context'].project_id = self.B.id
...
class QuotaSetControllerValidateNestedQuotaSetup(QuotaSetsControllerTestBase):
    ...
    def test_validate_nested_quotas_no_in_use_vols(self):
        ...
        self.req.environ['storage.context'].project_id = self.E.id
        quota['volumes'] = 1
    ...
    @mock.patch('storage.storage.quota_usage_get_all_by_project')
...
class QuotaSetsControllerNestedQuotasTest(QuotaSetsControllerTestBase):
    def setUp(self):
        ...
        patcher = mock.patch('storage.quota.VolumeTypeQuotaEngine._driver',
                             driver)
        patcher.start()
        self.addCleanup(patcher.stop)

    def test_subproject_defaults(self):
        context = self.req.environ['storage.context']
    ...
    def test_subproject_show_not_in_hierarchy_admin_context(self):
        ...
        self.project_by_id[E.id] = E
        self.req.environ['storage.context'].project_id = E.id
        result = self.controller.show(self.req, self.B.id)
        expected = make_subproject_body(tenant_id=self.B.id)
        self.assertDictMatch(expected, result)
    ...
    def test_project_allocated_considered_on_reserve(self):
        def _reserve(project_id):
            quotas.QUOTAS._driver.reserve(
                self.req.environ['storage.context'], quotas.QUOTAS.resources,
    ...
    def test_negative_child_limit_reserve_and_rollback(self):
        ...
        quotas.QUOTAS.rollback(self.req.environ['storage.context'], res,
                               self.D.id)
    ...
    @mock.patch('storage.storage.sqlalchemy.api._get_quota_usages')
    @mock.patch('storage.storage.quota_usage_get_all_by_project')
