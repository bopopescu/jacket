...
class VolumeActionsTest(test.TestCase):
    ...
    def setUp(self):
        ...
        for _meth in self._methods:
            self.api_patchers[_meth] = mock.patch('storage.volume.api.API.' +
                                                  _meth)
        ...
        self.get_patcher = mock.patch('storage.volume.api.API.get')
        self.mock_volume_get = self.get_patcher.start()
        self.addCleanup(self.get_patcher.stop)
        self.mock_volume_get.return_value = vol
        self.update_patcher = mock.patch('storage.volume.api.API.update')
        ...
        self.db_get_patcher = mock.patch('storage.db.sqlalchemy.api.volume_get')
...
class VolumeRetypeActionsTest(VolumeActionsTest):
    def setUp(self):
        ...
        self.retype_mocks = {}
        paths = ['storage.volume.volume_types.get_volume_type',
                 'storage.volume.volume_types.get_volume_type_by_name',
                 'storage.volume.qos_specs.get_qos_specs',
                 'storage.quota.QUOTAS.add_volume_type_opts',
                 'storage.quota.QUOTAS.reserve']
        for path in paths:
            name = path.split('.')[-1]
...
class VolumeImageActionsTest(test.TestCase):
    ...
    def test_copy_volume_to_image_without_core_prop(self):
        """Test glance_core_properties defined in storage.conf is empty."""
        id = 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee'
    ...
    def test_copy_volume_to_image_volume_type_none(
        ...
        volume_obj = storage.Volume._from_db_object(self.context,
                                                    storage.Volume(),
                                                    db_volume)
