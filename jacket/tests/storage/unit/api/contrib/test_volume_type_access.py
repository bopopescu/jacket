...
from jacket.api.storage.storage.contrib import volume_type_access as type_access
from jacket.api.storage.storage.v2 import types as types_api_v2
...
class VolumeTypeAccessTest(test.TestCase):
    ...
    def setUp(self):
        ...
        self.context = self.req.environ['storage.context']
        self.stubs.Set(storage, 'volume_type_get',
                       fake_volume_type_get)
        self.stubs.Set(storage, 'volume_type_get_all',
    ...
    def test_list_type_with_admin_default_proj1(self):
        ...
        req = fakes.HTTPRequest.blank('/v2/fake/types',
                                      use_admin_context=True)
        req.environ['storage.context'].project_id = PROJ1_UUID
    ...
    def test_list_type_with_admin_default_proj2(self):
        ...
        req = fakes.HTTPRequest.blank('/v2/fake/types',
                                      use_admin_context=True)
        req.environ['storage.context'].project_id = PROJ2_UUID
        result = self.type_controller_v2.index(req)
        self.assertVolumeTypeListEqual(expected['volume_types'],
                                       result['volume_types'])
    ...
    def test_add_project_access(self):
        ...
        self.stubs.Set(storage, 'volume_type_access_add',
    ...
    def test_remove_project_access_with_bad_access(self):
        def stub_remove_volume_type_access(context, type_id, project_id):
            raise exception.VolumeTypeAccessNotFound(volume_type_id=type_id,
                                                     project_id=project_id)
        self.stubs.Set(storage, 'volume_type_access_remove',
