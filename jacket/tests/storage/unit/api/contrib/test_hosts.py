...
SERVICE_LIST = [
    {'created_at': created_time, 'updated_at': curr_time,
     'host': 'test.host.1', 'topic': 'storage-volume', 'disabled': 0,
...
LIST_RESPONSE = [{'service-status': 'available', 'service': 'storage-volume',
                  'zone': 'storage', 'service-state': 'enabled',
                  'host_name': 'test.host.1', 'last-update': curr_time},
                 {'service-status': 'available', 'service': 'storage-volume',
                  'zone': 'storage', 'service-state': 'enabled',
                  'host_name': 'test.host.1', 'last-update': curr_time},
                 {'service-status': 'available', 'service': 'storage-volume',
                  'zone': 'storage', 'service-state': 'enabled',
                 ...
                 {'service-status': 'available', 'service': 'storage-volume',
                  'zone': 'storage', 'service-state': 'enabled',
                  'host_name': 'test.host.1', 'last-update': curr_time},
                 {'service-status': 'unavailable', 'service': 'storage-volume',
                  'zone': 'storage', 'service-state': 'enabled',
...
class HostTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.stubs.Set(storage, 'service_get_all',
    ...
    def test_list_hosts(self):
        ...
        cinder_hosts = os_hosts._list_hosts(self.req, 'storage-volume')
        expected = [host for host in LIST_RESPONSE
                    if host['service'] == 'storage-volume']
