...
class BackupsAPITestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.volume_api = jacket.storage.volume.API()
        self.backup_api = jacket.storage.backup.API()
    ...
    def _create_backup(volume_id=1,
        ...
        backup = storage.backup_create(context.get_admin_context(), backup)
        if not snapshot_id:
            storage.backup_update(context.get_admin_context(),
                             backup['id'],
    ...
    def _get_backup_attrib(backup_id, attrib_name):
        return storage.backup_get(context.get_admin_context(),
    ...
    def test_show_backup(self, backup_from_snapshot):
        ...
        if snapshot:
            snapshot.destroy()
        storage.backup_destroy(context.get_admin_context(), backup_id)
    ...
    def test_list_backups_json(self):
        ...
        storage.backup_destroy(context.get_admin_context(), backup_id3)
        storage.backup_destroy(context.get_admin_context(), backup_id2)
        storage.backup_destroy(context.get_admin_context(), backup_id1)
    ...
    def test_create_backup_snapshot_with_inconsistent_volume(self):
        ...
        self.addCleanup(storage.volume_destroy,
                        self.context.elevated(),
                        volume_id)
        self.addCleanup(storage.volume_destroy,
                        self.context.elevated(),
                        volume_id2)
        self.addCleanup(storage.snapshot_destroy,
                        self.context.elevated(),
                        snapshot_id)
        body = {"backup": {"display_name": "nightly001",
                           "display_description":
    ...
    def test_create_backup_WithOUT_enabled_backup_service(
        ...
        self.assertEqual('Service storage-backup could not be found.',
    ...
    def test_delete_delta_backup(self,
        ...
        storage.backup_destroy(context.get_admin_context(), delta_id)
        storage.backup_destroy(context.get_admin_context(), backup_id)
    ...
    def test_delete_backup_with_InvalidBackup2(self,
        ...
        storage.backup_destroy(context.get_admin_context(), delta_backup_id)
        storage.backup_destroy(context.get_admin_context(), backup_id)
    ...
    @mock.patch('storage.api.storage.API._get_available_backup_service_host')
    ...
    @mock.patch('storage.storage.service_get_all_by_topic')
    @mock.patch('storage.api.storage.API.create')
    def test_restore_backup_volume_id_unspecified(
        ...
        def fake_volume_api_create(context, size, name, description):
            ...
            return storage.volume_get(context, volume_id)
    ...
    @mock.patch('jacket.api.storage.API._get_available_backup_service_host')
    def test_restore_backup_name_volume_id_specified(
        ...
        self.assertEqual(backup_id, res_dict['restore']['backup_id'])
        self.assertEqual(volume_id, res_dict['restore']['volume_id'])
        restored_vol = storage.volume_get(self.context,
    ...
    @mock.patch('storage.backup.API.restore')
    ...
    @mock.patch('storage.API.restore')
    ...
    @mock.patch('jacket.storage.rpcapi.BackupAPI.restore_backup')
    @mock.patch('storage.api.storage.API._get_available_backup_service_host')
    ...
    @mock.patch('storage.api.storage.API._get_available_backup_service_host')
    @mock.patch('storage.backup.rpcapi.BackupAPI.export_record')
    ...
    @mock.patch('storage.backup.api.API._list_backup_hosts')
    @mock.patch('jacket.storage.rpcapi.BackupAPI.import_record')
    def test_import_record_volume_id_specified_json(self,
                                                    _mock_import_record_rpc,
                                                    _mock_list_services):
        utils.replace_obj_loader(self, storage.Backup)
        project_id = 'fake'
        backup_service = 'fake'
        ctx = context.RequestContext('admin', project_id, is_admin=True)
        backup = storage.Backup(ctx, id='id', user_id='user_id',
        ...
        db_backup = storage.Backup.get_by_id(ctx, 'id')
    ...
    @mock.patch('storage.api.storage.API._list_backup_hosts')
    @mock.patch('storage.rpcapi.BackupAPI.import_record')
    def test_import_record_volume_id_exists_deleted(self,
        ...
        backup = storage.Backup(ctx, id='id', user_id='original_user_id',
    ...
    @mock.patch('storage.backup.api.API._list_backup_hosts')
    @mock.patch('storage.backup.rpcapi.BackupAPI.import_record')
    ...
    def test_import_backup_with_existing_backup_record(self,
        ...
        backup_service = 'fake'
        backup = storage.Backup.get_by_id(ctx, backup_id)
        backup_url = backup.encode_record()
        _mock_list_services.return_value = ['no-match1', 'no-match2']
        req = webob.Request.blank('/v2/fake/backups/import_record')
        body = {'backup-record': {'backup_service': backup_service,
    ...
    def test_show_incremental_backup(self, backup_from_snapshot):
        ...
        storage.backup_destroy(context.get_admin_context(), child_backup_id)
        storage.backup_destroy(context.get_admin_context(), backup_id)
        storage.backup_destroy(context.get_admin_context(), parent_backup_id)
        if snapshot:
            snapshot.destroy()
        storage.volume_destroy(context.get_admin_context(), volume_id)
