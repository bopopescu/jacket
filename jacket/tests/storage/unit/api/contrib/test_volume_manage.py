...
def vt_get_volume_type_by_name(context, name):
    """Replacement for storage.volume.volume_types.get_volume_type_by_name.

    Overrides storage.volume.volume_types.get_volume_type_by_name to return
...
def vt_get_volume_type(context, vt_id):
    """Replacement for storage.volume.volume_types.get_volume_type.

    Overrides storage.volume.volume_types.get_volume_type to return the
...
def api_manage(*args, **kwargs):
    """Replacement for storage.volume.api.API.manage_existing.

    Overrides storage.volume.api.API.manage_existing to return some fake volume
...
class VolumeManageTest(test.TestCase):
    """Test cases for storage/api/contrib/volume_manage.py

    The API extension adds a POST /os-volume-manage API that is passed a storage
    host name, and a driver-specific reference parameter.  If everything
    is passed correctly, then the storage.volume.api.API.manage_existing method
    ...
    right, and calls down into storage.volume.api.API.manage_existing with the
    correct arguments.
    """
    ...
    @mock.patch('storage.volume.api.API.manage_existing', wraps=api_manage)
    @mock.patch(
    ...
    @mock.patch('storage.volume.api.API.manage_existing', api_manage)
    @mock.patch(
        'storage.api.storage.openstack.wsgi.Controller.validate_name_and_description')
    def test_manage_volume_volume_type_by_uuid(self, mock_validate):
        ...
        We wrap storage.volume.api.API.manage_existing so that managing is not
        actually attempted.
        """
        body = {'volume': {'host': 'host_ok',
                           'ref': 'fake_ref',
