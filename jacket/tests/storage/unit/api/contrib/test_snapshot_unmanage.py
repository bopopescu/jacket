...
def api_snapshot_get(self, context, snp_id):
    """Replacement for storage.volume.api.API.get_snapshot.

    We stub the storage.volume.api.API.get_snapshot method to check for the
...
@mock.patch('storage.volume.api.API.get_snapshot', api_snapshot_get)
class SnapshotUnmanageTest(test.TestCase):
    """Test cases for storage/api/contrib/snapshot_unmanage.py
    ...
    right, and calls down into storage.volume.api.API.delete_snapshot with the
    correct arguments.
    """
    ...
    @mock.patch('storage.db.conditional_update', return_value=1)
    @mock.patch('storage.db.snapshot_update')
    @mock.patch('storage.objects.Volume.get_by_id')
    @mock.patch('storage.volume.rpcapi.VolumeAPI.delete_snapshot')
