...
class VolumeTypeEncryptionTest(test.TestCase):
    ...
    def _create_type_and_encryption(self, volume_type, body=None):
        ...
        storage.volume_type_create(context.get_admin_context(), volume_type)
    ...
    def test_index(self):
        self.stubs.Set(storage, 'volume_type_encryption_get',
    ...
    def _create(self, cipher, control_location, key_size, provider):
        ...
        # check database
        encryption = storage.volume_type_encryption_get(context.get_admin_context(),
                                                   volume_type['id'])
    ...
    def test_create_volume_exists(self):
        ...
        volume_type = self._default_volume_type
        storage.volume_type_create(context.get_admin_context(), volume_type)
        storage.volume_create(context.get_admin_context(),
                         {'id': 'fake_id',
        ...
        self.assertEqual(expected, res_dict)
        storage.volume_destroy(context.get_admin_context(), 'fake_id')
        storage.volume_type_destroy(context.get_admin_context(), volume_type['id'])
    ...
    def test_delete_with_volume_in_use(self):
        ...
        # Delete the volumes
        storage.volume_destroy(context.get_admin_context(), 'fake_id')
        storage.volume_destroy(context.get_admin_context(), 'fake_id2')
