...
class VolumeApiTest(test.TestCase):
    def setUp(self):
        ...
        self.stubs.Set(storage, 'volume_get_all', stubs.stub_volume_get_all)
    ...
    def test_volume_create(self, mock_validate):
        ...
        self.stubs.Set(storage.sqlalchemy.api, '_volume_type_get_full',
    ...
    def test_volume_create_with_type(self, mock_validate):
        ...
        self.stubs.Set(volume_api.API, 'get_all',
                       lambda *args, **kwargs:
                       storage.VolumeList(storage=[vol_obj]))
        # NOTE(geguileo): This is required because common get_by_id method in
        # storage.storage.sqlalchemy.api caches the real get method.
        storage.sqlalchemy.api._GET_METHODS = {}
        self.stubs.Set(storage.sqlalchemy.api, '_volume_type_get_full',
                       stubs.stub_volume_type_get)
    ...
    @mock.patch.object(storage.sqlalchemy.api, '_volume_type_get_full',
                       autospec=True)
    ...
    def test_volume_create_with_image_ref_is_integer(self):
        ...
        self.ext_mgr.extensions = {'os-image-create': 'fake'}
        vol = self._vol_in_request_body(availability_zone="storage",
                                        image_ref=1234)
        body = {"volume": vol}
        req = fakes.HTTPRequest.blank('/v2/volumes')
    ...
    def test_volume_create_with_image_name_has_multiple(self):
        self.stubs.Set(storage, 'volume_get', stubs.stub_volume_get_db)
        self.stubs.Set(volume_api.API, "create", stubs.stub_volume_create)
        self.stubs.Set(fake_image._FakeImageService,
                       "detail",
                       stubs.stub_image_service_detail)
    ...
    def test_volume_update_with_admin_metadata(self, mock_validate):
        ...
        storage.volume_create(context.get_admin_context(), volume)
        storage.volume_admin_metadata_update(context.get_admin_context(), "1",
                                        {"readonly": "True",
                                         "invisible_key": "invisible_value"},
                                        False)
        values = {'volume_id': '1', }
        attachment = storage.volume_attach(context.get_admin_context(), values)
        storage.volume_attached(context.get_admin_context(),
                           attachment['id'], stubs.FAKE_UUID, None, '/')
        attach_tmp = storage.volume_attachment_get(context.get_admin_context(),
                                              attachment['id'])
        volume_tmp = storage.volume_get(context.get_admin_context(), '1')
        updates = {
            "name": "Updated Test Name",
        }
        body = {"volume": updates}
        ...
        req.environ['storage.context'] = admin_ctx
        res_dict = self.controller.update(req, '1', body)
        expected = self._expected_vol_from_controller(
    ...
    def test_volume_index_with_marker(self):
        ...
        self.stubs.Set(storage, 'volume_get_all_by_project',
    ...
    def _create_db_volumes(self, num_volumes):
        ...
        for vol in volumes:
            self.addCleanup(storage.volume_destroy, self.ctxt, vol.id)
        volumes.reverse()
        return volumes
    ...
    def test_volume_with_limit_zero(self):
        def stub_volume_get_all(context, marker, limit, **kwargs):
            return []
        self.stubs.Set(storage, 'volume_get_all', stub_volume_get_all)
    ...
    def test_volume_list_default_filters(self):
        ...
        self.stubs.Set(storage, 'volume_get_all', stub_volume_get_all2)
        ...
        self.stubs.Set(storage, 'volume_get_all', stub_volume_get_all3)
    ...
    def test_volume_show_no_attachments(self):
        ...
        self.stubs.Set(storage, 'volume_admin_metadata_get',
    ...
    def _test_get_volumes_by_name(self, get_all, display_name):
        req = mock.MagicMock()
        context = mock.Mock()
        req.environ = {'storage.context': context}
    ...
    @mock.patch('storage.volume.api.API.get_all')
    ...
    def test_get_volumes_filter_with_status(self, get_all):
        ...
        req.environ = {'storage.context': ctxt}
