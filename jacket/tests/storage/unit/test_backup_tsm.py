...
class TSMBackupSimulator(object):
    ...
    def _cmd_backup(self, **kwargs):
        ...
        ret_msg += ('Image backup of volume \'%s\'\n\n'
                    'Total number of storage inspected:  1\n'
                    % path)
        ...
        if self._next_cmd_error['backup'] == 'fail':
            ...
            ret_msg += 'Total number of storage backed up:  1'
    ...
    def _cmd_restore(self, **kwargs):
        ...
        if exists == 'OK':
            ret_msg += ('Total number of storage restored:  1\n'
                        'Total number of storage failed:  0')
            retcode = 0
        else:
            ret_msg += exists
            retcode = 12
    ...
    def _cmd_delete(self, **kwargs):
        ...
        if exists == 'OK':
            ret_msg += ('Total number of storage deleted:  1\n'
                        'Total number of storage failed:  0')
            retcode = 0
...
class BackupTSMTestCase(test.TestCase):
    ...
    def test_backup_image(self):
        ...
        with open(VOLUME_PATH, 'w+') as volume_file:
            ...
            backup1 = storage.Backup.get_by_id(self.ctxt, backup_id1)
            self.driver.backup(backup1, volume_file)
            backup2 = storage.Backup.get_by_id(self.ctxt, backup_id2)
            self.driver.backup(backup2, volume_file)

            # Create a backup that fails
            fail_back = storage.Backup.get_by_id(self.ctxt, backup_id3)
    ...
    def test_backup_file(self):
        ...
        with open(VOLUME_PATH, 'w+') as volume_file:
            # Create two backups of the volume
            backup1 = storage.Backup.get_by_id(self.ctxt, fake.backup_id)
            self.driver.backup(backup1, volume_file)
            backup2 = storage.Backup.get_by_id(self.ctxt, fake.backup2_id)
            self.driver.backup(backup2, volume_file)

            # Create a backup that fails
            self._create_backup_db_entry(fake.backup3_id, mode)
            fail_back = storage.Backup.get_by_id(self.ctxt, fake.backup3_id)
            self.sim.error_injection('backup', 'fail')
            self.assertRaises(exception.InvalidBackup,
                              self.driver.backup, fail_back, volume_file)
