...
from jacket.tests.storage.unit.backup import fake_service
...
class BackupMetadataAPITestCase(test.TestCase):
    ...
    def _add_metadata(self, vol_meta=False, vol_glance_meta=False):
        if vol_meta:
            ...
            storage.volume_metadata_update(self.ctxt, self.volume_id,
                                      {'fee': 'fi'}, False)
            storage.volume_metadata_update(self.ctxt, self.volume_id,
                                      {'fo': 'fum'}, False)
        ...
        if vol_glance_meta:
            ...
            storage.volume_glance_metadata_create(self.ctxt, self.volume_id,
                                             'disk_format', 'bare')
            storage.volume_glance_metadata_create(self.ctxt, self.volume_id,
                                             'container_type', 'ovf')
    ...
    def test_v1_restore_factory(self):
        ...
        vol = storage.volume_get(self.ctxt, self.volume_id)
    ...
    def test_restore_unencrypted_vol_to_different_volume_type(self):
        ...
        self.bak_meta_api._restore_vol_base_meta(
            ...
            fields)
        self.assertNotEqual(
            storage.volume_get(self.ctxt, vol1_id)['volume_type_id'],
            storage.volume_get(self.ctxt, vol2_id)['volume_type_id'])
    ...
    def test_restore_encrypted_vol_to_none_type_source_type_available(self):
        fields = ['encryption_key_id']
        container = {}
        storage.volume_type_create(self.ctxt, {'id': 'enc_vol_type_id',
                                          'name': 'enc_vol_type'})
        enc_vol_id = self._create_encrypted_volume_db_entry(str(uuid.uuid4()),
                                                            'enc_vol_type_id',
                                                            True)
        ...
        self.bak_meta_api._restore_vol_base_meta(
            ...
            fields)
        self.assertEqual(
            storage.volume_get(self.ctxt, undef_vol_id)['volume_type_id'],
            storage.volume_get(self.ctxt, enc_vol_id)['volume_type_id'])
