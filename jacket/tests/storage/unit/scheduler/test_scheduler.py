...
from jacket.storage.scheduler import driver
from jacket.storage.scheduler import filter_scheduler
from jacket.storage.scheduler import manager
...
class SchedulerManagerTestCase(test.TestCase):
    ...
    driver_cls_name = 'storage.scheduler.driver.Scheduler'
    ...
    @mock.patch('storage.volume.rpcapi.VolumeAPI.publish_service_capabilities')
    ...
    @mock.patch('storage.rpc.LAST_RPC_VERSIONS', {'storage-volume': '1.3'})
    @mock.patch('storage.rpc.LAST_OBJ_VERSIONS', {'storage-volume': '1.3'})
    ...
    @mock.patch('storage.scheduler.driver.Scheduler.'
    ...
    @mock.patch('storage.scheduler.driver.Scheduler.schedule_create_volume')
    ...
    @mock.patch('storage.scheduler.driver.Scheduler.schedule_create_volume')
    @mock.patch('storage.scheduler.driver.Scheduler.is_ready')
    @mock.patch('eventlet.sleep')
    ...
    @mock.patch('storage.storage.volume_get')
    @mock.patch('storage.scheduler.driver.Scheduler.host_passes_filters')
    @mock.patch('storage.storage.volume_update')
    ...
    @mock.patch('storage.storage.volume_update')
    @mock.patch('storage.storage.volume_attachment_get_used_by_volume_id')
    ...
    def test_create_consistencygroup_exceptions(self):
        with mock.patch.object(filter_scheduler.FilterScheduler,
            ...
            self.stubs.Set(storage, 'consistencygroup_update', mock.Mock())
            ...
            storage.consistencygroup_update.assert_called_once_with(
                self.context, group_id, {'status': (
                    fields.ConsistencyGroupStatus.ERROR)})

            mock_cg.reset_mock()
            LOG.exception.reset_mock()
            storage.consistencygroup_update.reset_mock()
...
class SchedulerTestCase(test.TestCase):
    ...
    @mock.patch('storage.scheduler.host_manager.HostManager.'
