...
from jacket.storage.volume.drivers.nexenta import iscsi
from jacket.storage.volume.drivers.nexenta import jsonrpc
from jacket.storage.volume.drivers.nexenta import nfs
from jacket.storage.volume.drivers.nexenta import utils
...
class TestNexentaISCSIDriver(test.TestCase):
    ...
    def test_create_volume(self):
        ...
        self.nms_mock.zvol.create.assert_called_with(
            'storage/%s' % self.TEST_VOLUME_REF['name'], '1G',
            self.cfg.nexenta_blocksize, self.cfg.nexenta_sparse)

    def test_delete_volume(self):
        self.nms_mock.zvol.get_child_props.return_value = (
            {'origin': 'storage/volume0@snapshot'})
        self.drv.delete_volume(self.TEST_VOLUME_REF)
        self.nms_mock.zvol.get_child_props.assert_called_with(
            'storage/volume1', 'origin')
        self.nms_mock.zvol.destroy.assert_called_with(
            'storage/volume1', '')

        self.nms_mock.zvol.get_child_props.assert_called_with(
            'storage/volume1', 'origin')
        self.nms_mock.zvol.destroy.assert_called_with('storage/volume1', '')
        self.drv.delete_volume(self.TEST_VOLUME_REF)

        self.nms_mock.zvol.get_child_props.assert_called_with(
            'storage/volume1', 'origin')
        self.nms_mock.zvol.get_child_props.return_value = (
            {'origin': 'storage/volume0@storage-clone-snapshot-1'})
        self.nms_mock.zvol.destroy.assert_called_with('storage/volume1', '')

        self.drv.delete_volume(self.TEST_VOLUME_REF)
        self.nms_mock.snapshot.destroy.assert_called_with(
            'storage/volume0@storage-clone-snapshot-1', '')
        self.nms_mock.volume.object_exists.assert_called_with('storage/volume0')
    ...
    def test_migrate_volume(self):
        ...
        host = {
            'capabilities': {
                'vendor_name': 'Nexenta',
                'location_info': 'NexentaISCSIDriver:1.1.1.1:storage',
                'free_capacity_gb': 1,
        ...
        snapshot = {
            ...
            'name': 'storage-migrate-snapshot-%s' % volume['id'],
        }
        volume_name = 'storage/%s' % volume['name']
        ...
        self.nms_mock.zvol.create_snapshot.assert_called_with(
            'storage/%s' % volume['name'], snapshot['name'], '')
        ...
        snapshot_name = 'storage/%(volume)s@%(snapshot)s' % {
            'volume': volume['name'],
            'snapshot': snapshot['name']
        }
    ...
    def test_remove_export(self):
        ...
        self.nms_mock.scsidisk.delete_lu.assert_called_with('storage/volume1')
...
class TestNexentaNfsDriver(test.TestCase):
    ...
    TEST_SHARES_CONFIG_FILE = '/etc/storage/nexenta-shares.conf'
