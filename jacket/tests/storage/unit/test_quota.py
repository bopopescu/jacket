...
import jacket.tests.storage.unit.image.fake
...
class QuotaIntegrationTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.volume_type = storage.volume_type_create(
            context.get_admin_context(),
            dict(name=self.volume_type_name))
        self.addCleanup(storage.volume_type_destroy, context.get_admin_context(),
                        self.volume_type['id'])
        ...
        # defaults.
        storage.quota_class_destroy_all_by_name(self.context, 'default')
        self.addCleanup(jacket.tests.storage.unit.image.fake.FakeImageService_reset)
    ...
    def _create_volume(self, size=1):
        ...
        volume = storage.Volume(context=self.context, **vol)
        volume.create()
        return volume

    def _create_snapshot(self, volume):
        snapshot = storage.Snapshot(self.context)
    ...
    def _create_backup(self, volume):
        ...
        return storage.backup_create(self.context, backup)

    def test_volume_size_limit_exceeds(self):
        resource = 'volumes_%s' % self.volume_type_name
        storage.quota_class_create(self.context, 'default', resource, 1)
        flag_args = {
            'quota_volumes': 10,
    ...
    def test_too_many_backups(self):
        ...
        with mock.patch.object(backup.API,
            ...
            self.assertRaises(exception.BackupLimitExceeded,
                              ...
                              False,
                              None)
            storage.backup_destroy(self.context, backup_ref['id'])
            storage.volume_destroy(self.context, vol_ref['id'])
    ...
    def test_too_many_combined_gigabytes(self):
        ...
        self.assertRaises(exception.QuotaError,
                          volume.API().create_snapshot,
                          self.context, vol_ref, '', '')
        usages = storage.quota_usage_get_all_by_project(self.context,
                                                   self.project_id)
    ...
    def test_backup_gb_quota_flag(self):
        ...
        with mock.patch.object(backup.API,
            ...
            storage.backup_destroy(self.context, backup_ref['id'])
            storage.backup_destroy(self.context, backup_ref2['id'])
            vol_ref.destroy()
            vol_ref2.destroy()

    def test_too_many_gigabytes_of_type(self):
        resource = 'gigabytes_%s' % self.volume_type_name
        storage.quota_class_create(self.context, 'default', resource, 10)
        flag_args = {
            'quota_volumes': 2000,
            'quota_gigabytes': 2000,
        }
...
class QuotaEngineTestCase(test.TestCase):
    ...
    def test_init_override_string(self):
        quota_obj = quota.QuotaEngine(
            quota_driver_class='storage.tests.unit.test_quota.FakeDriver')
...
class VolumeTypeQuotaEngineTestCase(test.TestCase):
    def test_default_resources(self):
        def fake_vtga(context, inactive=False, filters=None):
            return {}
        self.stubs.Set(storage, 'volume_type_get_all', fake_vtga)
    ...
    def test_volume_type_resources(self):
        ...
        vtype = storage.volume_type_create(ctx, {'name': 'type1'})
        vtype2 = storage.volume_type_create(ctx, {'name': 'type_2'})
        ...
        storage.volume_type_destroy(ctx, vtype['id'])
        storage.volume_type_destroy(ctx, vtype2['id'])
...
class DbQuotaDriverBaseTestCase(test.TestCase):
    ...
    def _stub_quota_class_get_default(self):
        ...
        self.stubs.Set(storage, 'quota_class_get_default', fake_qcgd)
    ...
    def _stub_quota_class_get_all_by_name(self):
        ...
        self.stubs.Set(storage, 'quota_class_get_all_by_name', fake_qcgabn)
    ...
    def _stub_allocated_get_all_by_project(self, allocated_quota=False):
        ...
        self.stubs.Set(storage, 'quota_allocated_get_all_by_project', fake_qagabp)
...
class DbQuotaDriverTestCase(DbQuotaDriverBaseTestCase):
    ...
    def _stub_get_by_project(self):
        ...
        self.stubs.Set(storage, 'quota_get_all_by_project', fake_qgabp)
        self.stubs.Set(storage, 'quota_usage_get_all_by_project', fake_qugabp)
    ...
    def _stub_quota_reserve(self):
        ...
        self.stubs.Set(storage, 'quota_reserve', fake_quota_reserve)
...
class NestedDbQuotaDriverBaseTestCase(DbQuotaDriverBaseTestCase):
    def setUp(self):
        ...
        keystone_patcher = mock.patch('storage.quota_utils._keystone_client',
                                      _keystone_mock)
...
class NestedQuotaValidation(NestedDbQuotaDriverBaseTestCase):
    def setUp(self):
        ...
        to_patch = [('storage.storage.quota_allocated_get_all_by_project',
                     self._fake_quota_allocated_get_all_by_project),
                    ('storage.storage.quota_get_all_by_project',
                     self._fake_quota_get_all_by_project),
                    ('storage.storage.quota_usage_get_all_by_project',
...
class QuotaReserveSqlAlchemyTestCase(test.TestCase):
    # storage.storage.sqlalchemy.api.quota_reserve is so complex it needs its
