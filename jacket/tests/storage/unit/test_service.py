...
test_service_opts = [
    cfg.StrOpt("fake_manager",
               default="storage.tests.unit.test_service.FakeManager",
...
class ServiceManagerTestCase(test.TestCase):
    ...
    def test_message_gets_to_manager(self):
        serv = service.Service('test',
                               'test',
                               'test',
                               'storage.tests.unit.test_service.FakeManager')
        serv.start()
    ...
    @mock.patch('storage.rpc.LAST_OBJ_VERSIONS', {'test': '1.5'})
    @mock.patch('storage.rpc.LAST_RPC_VERSIONS', {'test': '1.3'})
    def test_reset(self):
        serv = service.Service('test',
                               'test',
                               'test',
...
class ServiceTestCase(test.TestCase):
    ...
    def test_create(self):
        ...
        storage.Service.get_by_id(context.get_admin_context(), app.service_id)
    ...
    def test_report_state_newly_disconnected(self):
        service_ref = {'host': self.host,
                       ...
                       'id': 1}
        with mock.patch.object(storage.service, 'storage') as mock_db:
    ...
    def test_report_state_newly_connected(self):
        service_ref = {'host': self.host,
                       ...
                       'id': 1}
        with mock.patch.object(storage.service, 'storage') as mock_db,\
                mock.patch('storage.storage.sqlalchemy.api.get_by_id') as get_by_id:
            mock_db.service_get_by_args.side_effect = exception.NotFound()
            mock_db.service_create.return_value = service_ref
    ...
    def test_report_state_manager_not_working(self):
        service_ref = {'host': self.host,
                       ...
                       'availability_zone': 'nova',
                       'id': 1}
        with mock.patch('storage.storage') as mock_db:
    ...
    def test_service_with_long_report_interval(self):
        ...
        service.Service.create(
            binary="test_service",
            manager="storage.tests.unit.test_service.FakeManager")
    ...
    @mock.patch('storage.service.Service.report_state')
    @mock.patch('storage.service.Service.periodic_tasks')
...
class OSCompatibilityTestCase(test.TestCase):
    def _test_service_launcher(self, fake_os):
        # Note(lpetrut): The storage-volume service needs to be spawned
        ...
        with mock.patch('os.name', fake_os):
            with mock.patch('storage.service.process_launcher',
