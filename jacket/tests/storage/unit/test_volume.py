...
from jacket.tests.storage.unit.brick import fake_lvm
from jacket.tests.storage.unit import conf_fixture
from jacket.tests.storage.unit import fake_constants as fake
from jacket.tests.storage.unit import fake_driver
from jacket.tests.storage.unit import fake_snapshot
from jacket.tests.storage.unit import fake_volume
from jacket.tests.storage.unit.image import fake as fake_image
from jacket.tests.storage.unit.keymgr import fake as fake_keymgr
from jacket.tests.storage.unit import utils as tests_utils
from jacket.storage import utils
import jacket.storage.volume
...
from jacket.storage.volume.drivers import lvm
from jacket.storage.volume import manager as vol_manager
from jacket.storage.volume import rpcapi as volume_rpcapi
import jacket.storage.volume.targets.tgt
from jacket.storage.volume import utils as volutils
...
def create_snapshot(volume_id, size=1, metadata=None, ctxt=None,
    ...
    snap = storage.Snapshot(ctxt or context.get_admin_context())
...
class BaseVolumeTestCase(test.TestCase):
    ...
    def fake_get_all_volume_groups(obj, vg_name=None, no_suffix=True):
        return [{'name': 'storage-volumes',
                 'size': '5.00',
                 'available': '2.50',
                 'lv_count': '2',
                 'uuid': 'vR1JU3-FAKE-C4A9-PQFh-Mctm-9FwA-Xwzc1m'}]

    @mock.patch('storage.image.image_utils.TemporaryImages.fetch')
    @mock.patch('jacket.storage.flows.manager.create_volume.'
    ...
    def _create_volume_from_image(self, mock_clone_image_volume,
        ...
        try:
            ...
            # cleanup
            os.unlink(dst_path)
            volume = storage.Volume.get_by_id(self.context, volume.id)
...
class AvailabilityZoneTestCase(BaseVolumeTestCase):
    def test_list_availability_zones_cached(self):
        volume_api = jacket.storage.volume.api.API()
    ...
    def test_list_availability_zones_enabled_service(self):
        ...
        self.stubs.Set(storage, 'service_get_all_by_topic',
...
class VolumeTestCase(BaseVolumeTestCase):
    ...
    def setUp(self):
        ...
        self._clear_patch = mock.patch('storage.volume.utils.clear_volume',
                                       autospec=True)
    ...
    def test_init_host_clears_downloads(self):
        ...
        volume = storage.volume_get(context.get_admin_context(), volume_id)
    ...
    def test_init_host_clears_uploads_available_volume(self):
        ...
        volume = tests_utils.create_volume(self.context, status='uploading',
                                           size=0, host=CONF.host)
        self.volume.init_host()
        volume = storage.Volume.get_by_id(context.get_admin_context(),
                                          volume.id)
    ...
    def test_init_host_count_allocated_capacity(self):
        ...
        # here we do a storage.volume_get, and now the host has
        ...
        vol0 = storage.volume_get(context.get_admin_context(), vol0['id'])
    ...
    def test_init_host_sync_provider_info(self, mock_update):
        ...
        # initialize
        self.volume.init_host()
        # Grab volume and snapshot storage
        vol0_obj = storage.Volume.get_by_id(context.get_admin_context(),
                                            vol0.id)
        vol1_obj = storage.Volume.get_by_id(context.get_admin_context(),
                                            vol1.id)
        snap0_obj = storage.Snapshot.get_by_id(self.context, snap0.id)
        snap1_obj = storage.Snapshot.get_by_id(self.context, snap1.id)
    ...
    @mock.patch('storage.storage.service.Service.get_minimum_rpc_version')
    @mock.patch('storage.storage.service.Service.get_minimum_obj_version')
    @mock.patch('storage.rpc.LAST_RPC_VERSIONS', {'storage-scheduler': '1.3'})
    @mock.patch('storage.rpc.LAST_OBJ_VERSIONS', {'storage-scheduler': '1.3'})
    def test_reset(self, get_min_obj, get_min_rpc):
        ...
        get_min_obj.return_value = storage.base.OBJ_VERSIONS.get_current()
        vol_mgr.reset()
        ...
        with mock.patch.object(manager.driver,
            ...
            with mock.patch.object(manager.driver,
                ...
                with mock.patch.object(manager.driver,
                    ...
                    m_get_stats.return_value = {'name': 'storage-volumes'}
    ...
    def test_create_snapshot_fails_with_creating_status(self):
        ...
        snapshot_obj = storage.Snapshot.get_by_id(self.context, snap_id)
    ...
    def test_create_driver_not_initialized(self, reserve, commit, rollback):
        ...
        volume = storage.volume_get(context.get_admin_context(), volume_id)
        self.assertEqual("error", volume.status)
        storage.volume_destroy(context.get_admin_context(), volume_id)
    ...
    @mock.patch('storage.quota.QUOTAS.rollback', new=mock.Mock())
    @mock.patch('storage.quota.QUOTAS.commit', new=mock.Mock())
    @mock.patch('storage.quota.QUOTAS.reserve', return_value=['RESERVATION'])
    def test_create_delete_volume(self, _mock_reserve):
        ...
        self.assertDictMatch(expected, msg['payload'])
        self.assertEqual(volume_id, storage.volume_get(context.get_admin_context(),
                         volume_id).id)

        self.volume.delete_volume(self.context, volume_id)
        vol = storage.volume_get(context.get_admin_context(read_deleted='yes'),
                            volume_id)
    ...
    @mock.patch('storage.storage.volume_update')
    ...
    def test_delete_volume_metadata_with_metatype(self):
        ...
        self.assertEqual(test_meta2,
                         storage.volume_metadata_get(self.context, volume_id))
        ...
        rows = storage.volume_glance_metadata_get(self.context, volume_id)
        result = {}
        for row in rows:
    ...
    @mock.patch.object(jacket.storage.volume.api.API, 'list_availability_zones')
    ...
    @mock.patch('storage.quota.QUOTAS.rollback', new=mock.MagicMock())
    @mock.patch('storage.quota.QUOTAS.commit', new=mock.MagicMock())
    @mock.patch('storage.quota.QUOTAS.reserve', return_value=["RESERVATION"])
    def test_create_volume_with_volume_type(self, _mock_reserve):
        ...
        storage.volume_type_create(context.get_admin_context(),
                              {'name': vol_type, 'extra_specs': {}})

        db_vol_type = storage.volume_type_get_by_name(context.get_admin_context(),
                                                 vol_type)
    ...
    def test_create_volume_with_encrypted_volume_type(self):
        ...
        storage.volume_type_create(ctxt,
                              {'id': '61298380-0c12-11e3-bfd6-4b48424183be',
                               'name': 'LUKS'})
        storage.volume_type_encryption_create(
            ctxt,
        ...
        db_vol_type = storage.volume_type_get_by_name(ctxt, 'LUKS')
    ...
    def test_create_delete_volume_with_encrypted_volume_type(self):
        db_vol_type = storage.volume_type_create(
        ...
        storage.volume_update(self.context, volume['id'], {'status': 'available'})
        self.volume_api.delete(self.context, volume)

        volume = storage.volume_get(self.context, volume['id'])
        self.assertEqual('deleting', volume['status'])

        storage.volume_destroy(self.context, volume['id'])
        self.assertRaises(exception.NotFound,
                          storage.volume_get,
                          self.context,
                          volume['id'])
    ...
    def test_delete_busy_volume(self):
        ...
        self.mox.ReplayAll()
        self.volume.delete_volume(self.context, volume_id)
        volume_ref = storage.volume_get(context.get_admin_context(), volume_id)
    ...
    def test_delete_volume_in_error_extending(self):
        ...
        storage.volume_update(self.context, volume['id'],
                         {'status': 'error_extending'})
        self.volume.delete_volume(self.context, volume['id'])
        self.assertRaises(exception.NotFound, storage.volume_get,
                          self.context, volume['id'])

    @mock.patch.object(storage.sqlalchemy.api, 'volume_get',
    ...
    @mock.patch('jacket.storage.drivers.lvm.LVMVolumeDriver.'
    ...
    def test_create_volume_from_snapshot(self, mock_create_from_snap):
        ...
        snapshot_obj = storage.Snapshot.get_by_id(self.context, snapshot_id)
        ...
        self.assertEqual(snapshot_id,
                         storage.volume_get(context.get_admin_context(),
    ...
    @mock.patch('jacket.storage.flows.api.create_volume.get_flow')
    def test_create_volume_from_snapshot_with_types(self, _get_flow):
        ...
        foo_type = storage.volume_type_get_by_name(context.get_admin_context(),
                                              'foo')
        biz_type = storage.volume_type_get_by_name(context.get_admin_context(),
                                              'biz')
        ...
        with mock.patch.object(jacket.storage.volume.volume_types,
        ...
        storage.volume_type_destroy(context.get_admin_context(),
                               foo_type['id'])
        storage.volume_type_destroy(context.get_admin_context(),
                               biz_type['id'])

    @mock.patch('storage.volume.flows.api.create_volume.get_flow')
    ...
    @mock.patch('storage.flows.api.create_volume.get_flow')
    ...
    @mock.patch.object(jacket.storage.volume.drivers.lvm.LVMVolumeDriver,
                       'create_volume_from_snapshot')
    def test_create_volume_from_snapshot_check_locks(
        ...
        self.assertEqual(dst_vol_id, storage.volume_get(admin_ctxt, dst_vol_id).id)
        self.assertEqual(snap_id,
                         storage.volume_get(admin_ctxt, dst_vol_id).snapshot_id)
    ...
    def test_create_volume_from_volume_check_locks(self):
        ...
        self.assertEqual(src_vol_id,
                         storage.volume_get(admin_ctxt, dst_vol_id).source_volid)
    ...
    def test_create_volume_from_volume_delete_lock_taken(self):
        ...
        # volume is still in the storage since it was created by the test prior to
    ...
    def _raise_metadata_copy_failure(self, method, dst_vol_id, **kwargs):
        ...
        with mock.patch.object(storage, method) as mock_db:
        ...
        vol = storage.volume_get(self.context, dst_vol_id)
        self.assertEqual('error', vol['status'])

        # cleanup resource
        storage.volume_destroy(self.context, dst_vol_id)
    ...
    def test_create_volume_from_volume_with_glance_volume_metadata_none(
        ...
        storage.volume_update(self.context, src_vol['id'], {'bootable': True})
        ...
        self.assertRaises(exception.GlanceMetadataNotFound,
                          storage.volume_glance_metadata_copy_from_volume_to_volume,
                          self.context, src_vol_id, dst_vol['id'])

        # ensure that status of volume is 'available'
        vol = storage.volume_get(self.context, dst_vol['id'])
        self.assertEqual('available', vol['status'])

        # cleanup resource
        storage.volume_destroy(self.context, src_vol_id)
        storage.volume_destroy(self.context, dst_vol['id'])
    ...
    @mock.patch(
        'storage.driver.VolumeDriver.create_replica_test_volume')
    @mock.patch('storage.utils.execute')
    ...
    def test_create_volume_from_snapshot_with_glance_volume_metadata_none(
        ...
        volume = storage.volume_get(self.context, src_vol_id)
        ...
        self.assertRaises(exception.GlanceMetadataNotFound,
                          storage.volume_glance_metadata_copy_to_volume,
    ...
    @mock.patch(
        'jacket.storage.driver.VolumeDriver.create_replica_test_volume')
    def test_create_volume_from_srcreplica_with_glance_volume_metadata_none(
        ...
        volume_src = tests_utils.create_volume(self.context,
                                               **self.volume_params)
        self.volume.create_volume(self.context, volume_src['id'])
        storage.volume_update(self.context, volume_src['id'], {'bootable': True})

        volume = storage.volume_get(self.context, volume_src['id'])
        ...
        self.assertEqual('available',
                         storage.volume_get(self.context,
                                       volume_dst['id']).status)
        self.assertTrue(_create_replica_test.called)

        # cleanup resource
        storage.volume_destroy(self.context, volume_dst['id'])
        storage.volume_destroy(self.context, volume_src['id'])
    ...
    @mock.patch.object(jacket.storage.volume.targets.iscsi.ISCSITarget,
                       '_get_target_chap_auth')
    @mock.patch.object(storage, 'volume_admin_metadata_get')
    @mock.patch.object(storage.sqlalchemy.api, 'volume_get')
    @mock.patch.object(storage, 'volume_update')
    def test_initialize_connection_fetchqos(self,
        ...
        with mock.patch.object(jacket.storage.volume.volume_types,
            ...
            mock.patch.object(jacket.storage.tests.unit.fake_driver.FakeISCSIDriver,
    ...
    @mock.patch.object(storage, 'driver_initiator_data_get')
    @mock.patch.object(storage, 'driver_initiator_data_update')
    ...
    def test_run_attach_detach_volume_for_instance(self):
        ...
        attachment = self.volume.attach_volume(self.context, volume_id,
                                               instance_uuid, None,
                                               mountpoint, 'ro')
        vol = storage.volume_get(context.get_admin_context(), volume_id)
        ...
        vol = storage.volume_get(self.context, volume_id)
        self.assertEqual('available', vol['status'])
    ...
    def test_run_attach_detach_volume_with_attach_mode(self):
        ...
        volume = tests_utils.create_volume(self.context,
                                           ...
                                           **self.volume_params)
        volume_id = volume['id']
        storage.volume_update(self.context, volume_id, {'status': 'available', })
    ...
    def test_detach_volume_while_uploading_to_image_is_in_progress(self):
        ...
        storage.volume_update(self.context, volume_id, {'status': 'uploading'})
        # Call detach api
        self.volume.detach_volume(self.context, volume_id)
        vol = storage.volume_get(self.context, volume_id)
    ...
    def test_reserve_volume_success(self):
        ...
        jacket.storage.volume.api.API().reserve_volume(self.context, volume)
        volume_db = storage.volume_get(self.context, volume.id)
    ...
    def _test_reserve_volume_bad_status(self, status):
        volume = tests_utils.create_volume(self.context, status=status)
        self.assertRaises(exception.InvalidVolume,
                          jacket.storage.volume.api.API().reserve_volume,
                          self.context,
                          volume)
        storage.volume_destroy(self.context, volume.id)
    ...
    def test_unreserve_volume_success_in_use(self):
        ...
        jacket.storage.volume.api.API().unreserve_volume(self.context, volume)

        db_volume = storage.volume_get(self.context, volume.id)
    ...
    def test_create_delete_snapshot(self):
        ...
        self.assertEqual(
            snapshot_id, storage.Snapshot.get_by_id(self.context,
                                                    snapshot_id).id)
        msg = self.notifier.notifications[2]
        ...
        snap = storage.Snapshot.get_by_id(context.get_admin_context(
    ...
    @mock.patch.object(storage, 'snapshot_create',
                       side_effect=exception.InvalidSnapshot(
                           'Create snapshot in storage failed!'))
    def test_create_snapshot_failed_db_snapshot(self, mock_snapshot):
        """Test exception handling when create snapshot in storage failed."""
    ...
    def test_cannot_delete_volume_with_snapshots(self):
        ...
        self.volume.create_snapshot(self.context, volume['id'], snapshot)
        self.assertEqual(
            snapshot.id, storage.Snapshot.get_by_id(self.context,
                                                    snapshot.id).id)
    ...
    def test_create_snapshot_force(self):
        ...
        attachment = storage.volume_attach(self.context, values)
        storage.volume_attached(self.context, attachment['id'], instance_uuid,
                           None, '/dev/sda1')
        ...
        values = {'volume_id': volume['id'],
                  ...
                  'attach_status': 'attaching', }
        attachment = storage.volume_attach(self.context, values)
        storage.volume_attached(self.context, attachment['id'], None,
                           'fake_host', '/dev/sda1')
    ...
    def test_create_snapshot_from_bootable_volume(self):
        ...
        ctxt = context.get_admin_context()
        vol_glance_meta = storage.volume_glance_metadata_get(ctxt, volume_id)
        ...
        snap_glance_meta = storage.volume_snapshot_glance_metadata_get(
            ctxt, snap.id)
        ...
        # cleanup resource
        snap.destroy()
        storage.volume_destroy(ctxt, volume_id)
    ...
    def test_create_snapshot_from_bootable_volume_fail(self):
        ...
        with mock.patch.object(storage, 'volume_glance_metadata_copy_to_snapshot')\
                as mock_db:
            mock_db.side_effect = exception.MetadataCopyFailure(
                reason="Because of DB service down.")
            # create snapshot from bootable volume
    ...
    def test_create_snapshot_from_bootable_volume_with_volume_metadata_none(
        ...
        self.volume.create_volume(self.context, volume_id)
        # set bootable flag of volume to True
        storage.volume_update(self.context, volume_id, {'bootable': True})
        ...
        # cleanup resource
        snapshot.destroy()
        storage.volume_destroy(self.context, volume_id)

    def test_delete_busy_snapshot(self):
        """Test snapshot can be created and deleted."""

        self.volume.driver.vg = fake_lvm.FakeBrickLVM('storage-volumes',
                                                      False,
                                                      None,
                                                      'default')
        ...
        snapshot_id = snapshot.id
        self.volume.delete_snapshot(self.context, snapshot)
        snapshot_ref = storage.Snapshot.get_by_id(self.context, snapshot_id)
    ...
    @mock.patch('storage.drivers.lvm.LVMVolumeDriver.'
                'create_cloned_volume')
    @mock.patch('storage.quota.QUOTAS.rollback')
    @mock.patch('storage.quota.QUOTAS.commit')
    @mock.patch('storage.quota.QUOTAS.reserve', return_value=["RESERVATION"])
    ...
    def test_clone_image_volume_creation_failure(self, mock_reserve,
        ...
        with mock.patch.object(storage, 'Volume', side_effect=ValueError):
    ...
    def test_create_volume_from_image_exception(self):
        ...
        kwargs = {'display_description': 'Test Desc',
                  ...
                  'host': 'dummy'}
        volume = storage.Volume(context=self.context, **kwargs)
        volume.create()
    ...
    @mock.patch('storage.utils.brick_get_connector_properties')
    @mock.patch('storage.utils.brick_get_connector')
    @mock.patch('jacket.storage.driver.BaseVD.secure_file_operations_enabled')
    @mock.patch('jacket.storage.driver.BaseVD._detach_volume')
    def test_create_volume_from_image_unavailable(self, mock_detach,
        ...
        unbound_copy_method = jacket.storage.volume.driver.BaseVD.copy_image_to_volume
    ...
    def test_begin_roll_detaching_volume(self):
        ...
        instance_uuid = '12345678-1234-5678-1234-567812345678'
        volume = tests_utils.create_volume(self.context, **self.volume_params)
        attachment = storage.volume_attach(self.context,
                                      {'volume_id': volume['id'],
    ...
    def test_volume_api_update(self):
        ...
        vol = storage.volume_get(context.get_admin_context(), volume['id'])
    ...
    def test_volume_api_get_list_volumes_image_metadata(self):
        ...
        ctxt = context.get_admin_context()
        storage.volume_create(ctxt, {'id': 'fake1', 'status': 'available',
                                'host': 'test', 'provider_location': '',
                                'size': 1})
        storage.volume_glance_metadata_create(ctxt, 'fake1', 'key1', 'value1')
        storage.volume_glance_metadata_create(ctxt, 'fake1', 'key2', 'value2')
        storage.volume_create(ctxt, {'id': 'fake2', 'status': 'available',
                                'host': 'test', 'provider_location': '',
                                'size': 1})
        storage.volume_glance_metadata_create(ctxt, 'fake2', 'key3', 'value3')
        storage.volume_glance_metadata_create(ctxt, 'fake2', 'key4', 'value4')
        volume_api = jacket.storage.volume.api.API()
    ...
    def test_extend_volume(self, reserve):
        ...
        volume = storage.volume_get(context.get_admin_context(), volume['id'])
    ...
    def test_extend_volume_manager(self):
        ...
        with mock.patch.object(self.volume.driver,
            ...
            with mock.patch.object(QUOTAS, 'commit') as quotas_commit:
                ...
                self.volume.extend_volume(self.context, volume['id'], '4',
                                          fake_reservations)
                volume = storage.volume_get(context.get_admin_context(),
                                       volume['id'])
    ...
    def test_extend_volume_with_volume_type(self):
        ...
        storage.volume_type_create(elevated, {'name': 'type', 'extra_specs': {}})
        vol_type = storage.volume_type_get_by_name(elevated, 'type')

        volume_api = jacket.storage.volume.api.API()
        volume = volume_api.create(self.context, 100, 'name', 'description',
                                   volume_type=vol_type)
        try:
            usage = storage.quota_usage_get(elevated, project_id, 'gigabytes_type')
    ...
    @mock.patch('storage.api.storage.API.list_availability_zones',
    ...
    def test_create_volume_from_sourcevol_fail_wrong_az(self, _mock_laz):
        ...
        volume_src = storage.volume_get(self.context, volume_src['id'])
    ...
    def test_create_volume_from_sourcevol_with_glance_metadata(self):
        ...
        self.assertEqual('available',
                         storage.volume_get(context.get_admin_context(),
                                       volume_dst['id']).status)
        src_glancemeta = storage.volume_get(context.get_admin_context(),
                                       volume_src['id']).volume_glance_metadata
        dst_glancemeta = storage.volume_get(context.get_admin_context(),
    ...
    def test_create_volume_from_sourcevol_failed_clone(self):
        ...
        def fake_error_create_cloned_volume(volume, src_vref):
            storage.volume_update(self.context, src_vref['id'], {'status': 'error'})
    ...
    @mock.patch('jacket.storage.flows.common.make_pretty_name',
                new=mock.MagicMock())
    @mock.patch('storage.scheduler.rpcapi.SchedulerAPI.create_volume',
                return_value=None)
    @mock.patch('storage.flows.manager.create_volume.'
    ...
    def test_create_volume_raise_rescheduled_exception(self, mock_execute,
        ...
        volume = storage.volume_get(context.get_admin_context(), test_vol_id)
        self.assertEqual('creating', volume['status'])
...
class VolumeMigrationTestCase(BaseVolumeTestCase):
    ...
    def setUp(self):
        ...
        self._clear_patch = mock.patch('storage.utils.clear_volume',
                                       autospec=True)
        self._clear_patch.start()
        self.expected_status = 'available'
    ...
    def _fake_create_volume(self, ctxt, volume, host, req_spec, filters,
        ...
        return storage.volume_update(ctxt, volume['id'],
    ...
    @mock.patch('storage.compute.API')
    @mock.patch('jacket.storage.manager.VolumeManager.'
                'migrate_volume_completion')
    @mock.patch('storage.storage.sqlalchemy.api.volume_get')
    ...
    @mock.patch('storage.compute.API')
    @mock.patch('storage.manager.VolumeManager.'
                'migrate_volume_completion')
    @mock.patch('storage.storage.sqlalchemy.api.volume_get')
    ...
    def test_update_migrated_volume(self, volume_update):
        ...
        with mock.patch.object(self.volume.driver,
            ...
            # Reset the volume storage to their original value, since they
    ...
    def test_migrate_volume_generic_migrate_volume_completion_error(self):
        def fake_migrate_volume_completion(ctxt, volume_id, new_volume_id,
                                           error=False):
            storage.volume_update(ctxt, volume['id'],
    ...
    def _test_migrate_volume_completion(self, status='available',
        ...
        with mock.patch.object(self.volume, 'detach_volume') as \
            ...
            self.volume.migrate_volume_completion(self.context, old_volume.id,
                                                  new_volume.id)
            after_new_volume = storage.Volume.get_by_id(self.context,
                                                        new_volume.id)
            after_old_volume = storage.Volume.get_by_id(self.context,
                                                        old_volume.id)
            if status == 'in-use':
                mock_detach_volume.assert_called_with(self.context,
                                                      old_volume.id,
                                                      attachment_id)
                attachment = storage.volume_attachment_get_by_instance_uuid(
    ...
    def test_retype_setup_fail_volume_is_available(self):
        ...
        storage.volume_type_create(elevated, {'name': 'old', 'extra_specs': {}})
        old_vol_type = storage.volume_type_get_by_name(elevated, 'old')
        storage.volume_type_create(elevated, {'name': 'new', 'extra_specs': {}})
        new_vol_type = storage.volume_type_get_by_name(elevated, 'new')
        storage.quota_create(elevated, project_id, 'volumes_new', 0)

        volume = tests_utils.create_volume(self.context, size=1,
                                           host=CONF.host, status='available',
                                           volume_type_id=old_vol_type['id'])

        api = jacket.storage.volume.api.API()
        self.assertRaises(exception.VolumeLimitExceeded, api.retype,
                          self.context, volume, new_vol_type['id'])

        volume = storage.volume_get(elevated, volume.id)
        self.assertEqual(0, len(self.notifier.notifications),
                         self.notifier.notifications)
        self.assertEqual('available', volume['status'])
    ...
    def _retype_volume_exec(self, driver, snap=False, policy='on-demand',
        ...
        old_vol_type = storage.volume_type_get_by_name(elevated, 'old')
        storage.volume_type_create(elevated, {'name': 'new', 'extra_specs': {}})
        vol_type = storage.volume_type_get_by_name(elevated, 'new')
        storage.quota_create(elevated, project_id, 'volumes_new', 10)
        ...
        if reserve_vol_type_only:
            ...
            try:
                usage = storage.quota_usage_get(elevated, project_id, 'volumes')
                total_volumes_in_use = usage.in_use
                usage = storage.quota_usage_get(elevated, project_id, 'gigabytes')
        ...
        with mock.patch.object(self.volume.driver, 'retype') as _retype,\
                ...
                mock.patch.object(storage.sqlalchemy.api, 'volume_get') as mock_get:
        ...
        volume = storage.Volume.get_by_id(elevated, volume.id)
        try:
            usage = storage.quota_usage_get(elevated, project_id, 'volumes_new')
            volumes_in_use = usage.in_use
        except exception.QuotaUsageNotFound:
            volumes_in_use = 0
    ...
    def _test_delete_volume_in_migration(self, migration_status):
        ...
        volume = tests_utils.create_volume(self.context, **self.volume_params)
        volume = storage.volume_update(self.context, volume['id'],
                                  {'status': 'available',
...
class ConsistencyGroupTestCase(BaseVolumeTestCase):
    ...
    def test_create_delete_consistencygroup(self, fake_delete_cg,
        ...
        group = storage.ConsistencyGroup.get_by_id(self.context, group.id)
        self.assertEqual(0, len(self.notifier.notifications),
                         self.notifier.notifications)
        ...
        self.assertEqual(
            group.id,
            storage.ConsistencyGroup.get_by_id(context.get_admin_context(),
                                               group.id).id)

        self.volume.delete_consistencygroup(self.context, group)
        cg = storage.ConsistencyGroup.get_by_id(
        ...
        self.assertDictMatch(expected, msg['payload'])
        self.assertRaises(exception.NotFound,
                          storage.ConsistencyGroup.get_by_id,
                          self.context,
                          group.id)
    ...
    def test_update_consistencygroup(self, fake_update_cg,
        ...
        cg = storage.ConsistencyGroup.get_by_id(self.context, group.id)
        expected = {
            'status': fields.ConsistencyGroupStatus.AVAILABLE,
            'name': 'test_cg',
            'availability_zone': 'nova',
        ...
        self.assertDictMatch(expected, msg['payload'])
        cgvolumes = storage.volume_get_all_by_group(self.context, group.id)
    ...
    @mock.patch('storage.volume.drivers.lvm.LVMVolumeDriver.'
                'create_volume_from_snapshot')
    @mock.patch('jacket.storage.drivers.lvm.LVMVolumeDriver.'
                'create_cloned_volume')
    def test_create_consistencygroup_from_src(self,
        ...
        group2 = storage.ConsistencyGroup.get_by_id(self.context, group2.id)
        volume2 = tests_utils.create_volume(
            self.context,
        ...
        cg2 = storage.ConsistencyGroup.get_by_id(self.context, group2.id)
        expected = {
            'status': fields.ConsistencyGroupStatus.AVAILABLE,
            'name': 'test_cg',
            'availability_zone': 'nova',
        ...
        cg2 = storage.ConsistencyGroup.get_by_id(
        ...
        cg3 = storage.ConsistencyGroup.get_by_id(self.context, group3.id)
    ...
    def _create_cgsnapshot(self, group_id, volume_id, size='0'):
        ...
        cgsnap = storage.CGSnapshot(self.context)
        ...
        # Create a snapshot object
        snap = storage.Snapshot(context.get_admin_context())
        snap.volume_size = size
        snap.user_id = fake.user_id
        snap.project_id = fake.project_id
        snap.volume_id = volume_id
    ...
    @mock.patch('storage.driver.VolumeDriver.create_consistencygroup',
                autospec=True,
                return_value={'status': 'available'})
    @mock.patch('jacket.storage.driver.VolumeDriver.delete_consistencygroup',
                autospec=True,
                return_value=({'status': 'deleted'}, []))
    @mock.patch('jacket.storage.driver.VolumeDriver.create_cgsnapshot',
                autospec=True,
                return_value=({'status': 'available'}, []))
    @mock.patch('storage.driver.VolumeDriver.delete_cgsnapshot',
                autospec=True,
                return_value=({'status': 'deleted'}, []))
    def test_create_delete_cgsnapshot(self,
        ...
        cgsnap = storage.CGSnapshot.get_by_id(
    ...
    @mock.patch('storage.volume.driver.VolumeDriver.create_consistencygroup',
                return_value={'status': 'available'})
    @mock.patch('storage.driver.VolumeDriver.delete_consistencygroup',
                return_value=({'status': 'deleted'}, []))
    ...
    @mock.patch('jacket.storage.driver.VolumeDriver.create_consistencygroup',
                return_value={'status': 'available'})
    ...
    def test_create_volume_with_consistencygroup_invalid_type(self):
        ...
        vol_type = storage.volume_type_create(
            context.get_admin_context(),
            dict(name=conf_fixture.def_vol_type, extra_specs={})
        )
        db_vol_type = storage.volume_type_get(context.get_admin_context(),
                                         vol_type.id)
        cg = {
            'id': '1',
            'name': 'cg1',
        ...
        vol_api = jacket.storage.volume.api.API()
...
class CopyVolumeToImageTestCase(BaseVolumeTestCase):
    ...
    def setUp(self):
        ...
        self.volume_id = 1
        self.addCleanup(storage.volume_destroy, self.context, self.volume_id)
    ...
    def test_copy_volume_to_image_status_available(self):
        ...
        storage.volume_create(self.context, self.volume_attrs)
        ...
        volume = storage.volume_get(self.context, self.volume_id)
        self.assertEqual('available', volume['status'])
    ...
    def test_copy_volume_to_image_instance_deleted(self):
        ...
        # Storing unmocked storage api function reference here, because we have to
        # update volume status (set instance_uuid to None) before calling the
        # 'volume_update_status_based_on_attached_instance_id' storage api.
        unmocked_db_api = storage.volume_update_status_based_on_attachment
        ...
        def mock_volume_update_after_upload(context, volume_id):
            ...
            # set to None
            storage.volume_update(context, volume_id, {'instance_uuid': None})
            # Calling unmocked storage api
    ...
    def _test_copy_volume_to_image_with_image_volume(
        ...
        def fake_create(context, volume_id, **kwargs):
            storage.volume_update(context, volume_id, {'status': 'available'})
    ...
    def test_copy_volume_to_image_with_image_volume(self):
        ...
        self.assertTrue(image['locations'][0]['url'].startswith('storage://'))
...
class GetActiveByWindowTestCase(BaseVolumeTestCase):
    ...
    def test_volume_get_active_by_window(self):
        ...
        # Not in window
        storage.volume_create(self.ctx, self.db_attrs[0])

        # In - deleted in window
        storage.volume_create(self.ctx, self.db_attrs[1])

        # In - deleted after window
        storage.volume_create(self.ctx, self.db_attrs[2])

        # In - created in window
        storage.volume_create(self.context, self.db_attrs[3])

        # Not of window.
        storage.volume_create(self.context, self.db_attrs[4])

        volumes = storage.volume_get_active_by_window(
            self.context,
    ...
    def test_snapshot_get_active_by_window(self):
        ...
        storage.volume_create(self.context, {'id': fake.volume_id})
        for i in range(5):
            self.db_attrs[i]['volume_id'] = fake.volume_id

        # Not in window
        del self.db_attrs[0]['id']
        snap1 = storage.Snapshot(self.ctx, **self.db_attrs[0])
        snap1.create()

        # In - deleted in window
        del self.db_attrs[1]['id']
        snap2 = storage.Snapshot(self.ctx, **self.db_attrs[1])
        snap2.create()

        # In - deleted after window
        del self.db_attrs[2]['id']
        snap3 = storage.Snapshot(self.ctx, **self.db_attrs[2])
        snap3.create()

        # In - created in window
        del self.db_attrs[3]['id']
        snap4 = storage.Snapshot(self.ctx, **self.db_attrs[3])
        snap4.create()

        # Not of window.
        del self.db_attrs[4]['id']
        snap5 = storage.Snapshot(self.ctx, **self.db_attrs[4])
        snap5.create()

        snapshots = storage.SnapshotList.get_active_by_window(
            self.context,
            datetime.datetime(1, 3, 1, 1, 1, 1),
...
class DriverTestCase(test.TestCase):
    ...
    driver_name = "jacket.storage.driver.FakeBaseDriver"
    ...
    def _detach_volume(self, volume_id_list):
        ...
        for volume_id in volume_id_list:
            storage.volume_detached(self.context, volume_id)
            self.volume.delete_volume(self.context, volume_id)


class GenericVolumeDriverTestCase(DriverTestCase):
    """Test case for VolumeDriver."""
    driver_name = "storage.tests.unit.fake_driver.LoggingVolumeDriver"
    ...
    def test_backup_volume_available(self, mock_volume_get,
        ...
        backup = tests_utils.create_backup(self.context,
                                           vol['id'])
        backup_obj = storage.Backup.get_by_id(self.context, backup.id)
        properties = {}
    ...
    def test_restore_backup(self,
        ...
        root_helper = 'sudo storage-rootwrap /etc/storage/rootwrap.conf'
    ...
    def test_get_backup_device_available(self):
        ...
        volume = storage.Volume.get_by_id(self.context, vol.id)
    ...
    @mock.patch.object(jacket.storage.volume.manager.VolumeManager, '_attach_volume')
    @mock.patch.object(jacket.storage.volume.manager.VolumeManager, '_detach_volume')
    ...
    def test_copy_volume_data_mgr(self,
        ...
        # cleanup resource
        storage.volume_destroy(self.context, src_vol['id'])
        storage.volume_destroy(self.context, dest_vol['id'])


@ddt.ddt
class LVMVolumeDriverTestCase(DriverTestCase):
    """Test case for VolumeDriver"""
    driver_name = "storage.drivers.lvm.LVMVolumeDriver"
    ...
    def test_check_for_setup_error(self):
        ...
        vg_obj = fake_lvm.FakeBrickLVM('storage-volumes',
                                       False,
                                       None,
                                       'default')
        ...
        storage.backup_create(self.context, backup)
    ...
    def test_update_migrated_volume(self):
        ...
        fake_vg = fake_lvm.FakeBrickLVM('storage-volumes', False,
                                        None, 'default')
    ...
    @mock.patch.object(jacket.storage.volume.utils, 'get_all_volume_groups',
                       return_value=[{'name': 'storage-volumes'}])
    @mock.patch('storage.brick.local_dev.lvm.LVM.update_volume_group_info')
    @mock.patch('storage.brick.local_dev.lvm.LVM.get_all_physical_volumes')
    @mock.patch('storage.brick.local_dev.lvm.LVM.supports_thin_provisioning',
                return_value=True)
    ...
    @mock.patch.object(jacket.storage.volume.utils, 'get_all_volume_groups',
                       return_value=[{'name': 'storage-volumes'}])
    @mock.patch.object(jacket.storage.brick.local_dev.lvm.LVM, 'get_volumes',
                       return_value=[])
    @mock.patch('storage.brick.local_dev.lvm.LVM.update_volume_group_info')
    @mock.patch('storage.brick.local_dev.lvm.LVM.get_all_physical_volumes')
    @mock.patch('storage.brick.local_dev.lvm.LVM.supports_thin_provisioning',
    ...
    @mock.patch('storage.brick.local_dev.lvm.LVM.update_volume_group_info')
    @mock.patch('storage.brick.local_dev.lvm.LVM.get_all_physical_volumes')
    @mock.patch('storage.brick.local_dev.lvm.LVM.get_volume')
    @mock.patch('storage.brick.local_dev.lvm.LVM.supports_thin_provisioning',
                return_value=False)
    ...
    def test_create_cloned_volume_by_thin_snapshot(self, mock_extend):
        self.configuration.lvm_type = 'thin'
        fake_vg = mock.Mock(fake_lvm.FakeBrickLVM('storage-volumes', False,
                                                  None, 'default'))
        lvm_driver = lvm.LVMVolumeDriver(configuration=self.configuration,
                                         vg_obj=fake_vg,
                                         storage=storage)
    ...
    def test_lvm_migrate_volume_same_volume_group(self, vgs):
        ...
        capabilities = {'location_info': 'LVMVolumeDriver:%s:'
                        'storage-volumes:default:0' % hostname}
        host = {'capabilities': capabilities}
        vol = {'name': 'test', 'id': 1, 'size': 1, 'status': 'available'}
        self.volume.driver.vg = fake_lvm.FakeBrickLVM('storage-volumes',
                                                      False,
    ...
    def test_lvm_migrate_volume_volume_copy_error(self, vgs, copy_volume,
        ...
        host = {'capabilities': capabilities}
        vol = {'name': 'test', 'id': 1, 'size': 1, 'status': 'available'}
        self.volume.driver.vg = fake_lvm.FakeBrickLVM('storage-volumes-old',
                                                      False, None, 'default')
        self.assertRaises(processutils.ProcessExecutionError,
                          self.volume.driver.migrate_volume, self.context,
                          vol, host)
    ...
    def test_lvm_volume_group_missing(self):
        hostname = socket.gethostname()
        capabilities = {'location_info': 'LVMVolumeDriver:%s:'
                        'storage-volumes-3:default:0' % hostname}
        host = {'capabilities': capabilities}
        vol = {'name': 'test', 'id': 1, 'size': 1, 'status': 'available'}

        def get_all_volume_groups():
            return [{'name': 'storage-volumes-2'}]
    ...
    def test_lvm_migrate_volume_proceed(self):
        hostname = socket.gethostname()
        capabilities = {'location_info': 'LVMVolumeDriver:%s:'
                        'storage-volumes-2:default:0' % hostname}
        host = {'capabilities': capabilities}
        ...
        with mock.patch.object(brick_lvm.LVM, 'get_all_physical_volumes',
            ...
            mock_copy.assert_called_once_with(
                '/dev/mapper/storage--volumes-testvol',
                '/dev/mapper/storage--volumes--2-testvol',
                2048,
                '1M',
    ...
    def test_lvm_migrate_volume_proceed_with_thin(self):
        ...
        with mock.patch.object(brick_lvm.LVM, 'get_all_physical_volumes',
            ...
            lvm_driver.vg = fake_lvm.FakeBrickLVM('storage-volumes',
                                                  False,
                                                  None,
                                                  'default')
            lvm_driver._sparse_copy_volume = True
    ...
    def test_lvm_max_over_subscription_ratio(self,
        ...
        fake_vg = mock.Mock(fake_lvm.FakeBrickLVM('storage-volumes', False,
                                                  None, 'default'))
        lvm_driver = lvm.LVMVolumeDriver(configuration=configuration,
                                         vg_obj=fake_vg, storage=storage)
...
class ISCSITestCase(DriverTestCase):
    ...
    def _attach_volume(self):
        ...
        for index in range(3):
            vol = {}
            vol['size'] = 0
            vol_ref = storage.volume_create(self.context, vol)
            self.volume.create_volume(self.context, vol_ref['id'])
            vol_ref = storage.volume_get(self.context, vol_ref['id'])

            # each volume has a different mountpoint
            mountpoint = "/dev/sd" + chr((ord('b') + index))
            instance_uuid = '12345678-1234-5678-1234-567812345678'
            storage.volume_attached(self.context, vol_ref['id'], instance_uuid,
                               mountpoint)
    ...
    def test_do_iscsi_discovery(self):
        self.configuration = conf.Configuration(None)
        iscsi_driver = \
            jacket.storage.volume.targets.tgt.TgtAdm(
    ...
    def test_get_iscsi_properties(self):
        volume = {"provider_location": '',
                  ...
                  "attached_mode": "rw"}
        iscsi_driver = \
            jacket.storage.volume.targets.tgt.TgtAdm(configuration=self.configuration)
    ...
    @mock.patch('storage.brick.local_dev.lvm.LVM.get_lvm_version',
    ...
    def test_get_volume_stats(self, _mock_get_version):
        ...
        self.volume.driver.vg = brick_lvm.LVM('storage-volumes', 'sudo')
        ...
        lvm_driver = lvm.LVMVolumeDriver(configuration=self.configuration,
                                         ...
                                         vg_obj=vg_obj)
        lvm_driver.check_for_setup_error()
        lvm_driver.vg = brick_lvm.LVM('storage-volumes', 'sudo')
...
class FibreChannelTestCase(DriverTestCase):
    ...
    driver_name = "storage.driver.FibreChannelDriver"
...
class VolumePolicyTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        jacket.storage.policy.init()

        self.context = context.get_admin_context()

    def test_check_policy(self):
        self.mox.StubOutWithMock(jacket.storage.policy, 'enforce')
        target = {
            'project_id': self.context.project_id,
            'user_id': self.context.user_id,
        }
        jacket.storage.policy.enforce(self.context, 'volume:attach', target)
        self.mox.ReplayAll()
        jacket.storage.volume.api.check_policy(self.context, 'attach')
    ...
    def test_check_policy_with_target(self):
        ...
        jacket.storage.policy.enforce(self.context, 'volume:attach', target)
        self.mox.ReplayAll()
        jacket.storage.volume.api.check_policy(self.context, 'attach', {'id': 2})
...
class ImageVolumeCacheTestCase(BaseVolumeTestCase):
    ...
    def test_delete_image_volume(self):
        ...
        storage.image_volume_cache_create(self.context,
                                     volume['host'],
                                     image_id,
                                     ...
                                     volume['size'])
        volume_api.delete(self.context, volume)
        entry = storage.image_volume_cache_get_by_volume_id(self.context,
                                                       volume['id'])
