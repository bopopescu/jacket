...
def check_policy(context, action, target_obj=None):
    ...
    jacket.storage.policy.enforce(context, _action, target)
...
class API(base.Base):
    ...
    def get_all(self, context, marker=None, limit=None, sort_keys=None,
        ...
        if context.is_admin and allTenants:
            ...
            volumes = storage.VolumeList.get_all(context, marker, limit,
                                                 sort_keys=sort_keys,
                                                 sort_dirs=sort_dirs,
                                                 filters=filters,
                                                 offset=offset)
    ...
    def get_snapshot(self, context, snapshot_id):
        ...
        # FIXME(jdg): The storage don't have the storage name entries
    ...
    def create_snapshot_in_db(self, context,
        ...
        try:
            ...
            snapshot = storage.Snapshot(context=context, **kwargs)
            snapshot.create()
    ...
    def create_snapshots_in_db(self, context,
        ...
        try:
            for options in options_list:
                snapshot = storage.Snapshot(context=context, **options)
                snapshot.create()
    ...
    def migrate_volume(self, context, volume, host, force_host_copy,
        ...
        snaps = storage.SnapshotList.get_all_for_volume(context, volume.id)
        if snaps:
