...
class LioAdm(iscsi.ISCSITarget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # FIXME(jdg): modify executor to use the storage-rtstool
    ...
    def _get_target(self, iqn):
        (out, err) = self._execute('storage-rtstool',
                                   'get-targets',
                                   run_as_root=True)
        lines = out.split('\n')
        for line in lines:
    ...
    def initialize_connection(self, volume, connector):
        ...
        try:
            self._execute('storage-rtstool', 'add-initiator',
                          volume_iqn,
                          auth_user,
                          auth_pass,
                          connector['initiator'],
    ...
    def terminate_connection(self, volume, connector, **kwargs):
        ...
        try:
            self._execute('storage-rtstool', 'delete-initiator',
                          volume_iqn,
                          connector['initiator'],
                          run_as_root=True)
        except putils.ProcessExecutionError:
