...
class V7000Common(object):
    ...
    def _create_lun_snapshot(self, snapshot):
        """Create a new storage snapshot on a volume.
        ...
        :param snapshot:  storage snapshot object provided by the Manager
    ...
    def _delete_lun_snapshot(self, snapshot):
        """Delete the specified storage snapshot.
    ...
    def _create_volume_from_snapshot(self, snapshot, volume):
        ...
        :param snapshot:  storage snapshot object provided by the Manager
        :param volume:  storage volume to be created
        """
    ...
    def _create_lun_from_lun(self, src_vol, dest_vol):
        ...
        :param src_vol:  storage volume to clone
        :param dest_vol:  storage volume to be created
        """
        pool = None
        result = None
    ...
    def _ensure_snapshot_resource_area(self, volume_id):
        ...
        Exceptions:
            VolumeBackendAPIException: if storage volume does not exist
        ...
        if not self.vmem_mg.snapshot.lun_has_a_snapshot_resource(
           lun=volume_id):
            ...
            #   500MB to 2G                50%
            #   >= 2G                      20%
            # Note: storage volume.size is in GB, vmemclient wants MB.
    ...
    def _ensure_snapshot_policy(self, volume_id):
        """Ensure concerto snapshot policy exists on storage volume.
    ...
    def _compress_snapshot_id(self, cinder_snap_id):
        """Compress storage snapshot ID so it fits in backend.
    ...
    def _get_snapshot_from_lun_snapshots(
        ...
        """Locate backend snapshot dict associated with storage snapshot id.
    ...
    def _wait_for_lun_or_snap_copy(self, src_vol_id, dest_vdev_id=None,
        ...
        :param src_vol_id:  storage volume ID of source volume
