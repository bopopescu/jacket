...
class HGSTDriver(driver.VolumeDriver):
    """This is the Class to set in storage.conf (volume_driver).
    ...
    @lockutils.synchronized('devices', 'storage-hgst-')
    ...
    def check_for_setup_error(self):
        ...
        # Verify vgc-cluster exists and is executable by storage user
        try:
        ...
        try:
            ...
            msg = _("hgst_group %(grp)s and hgst_user %(usr)s must map to "
                    "valid users/groups in storage.conf") % {
        ...
        try:
            ...
            msg = _("hgst_space_mode must be an octal/int in storage.conf")
            raise exception.VolumeDriverException(message=msg)
        ...
        try:
            ...
            self._log_cli_err(err)
            msg = _("hgst_net %(net)s specified in storage.conf not found "
                    "in cluster") % {
        ...
        if (sl is None) or (six.text_type(sl) == ""):
            msg = _("hgst_storage_servers must be defined in storage.conf")
            raise exception.VolumeDriverException(message=msg)
        servers = sl.split(",")
