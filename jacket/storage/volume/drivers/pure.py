...
REPLICATION_CG_NAME = "storage-group"
...
class PureBaseVolumeDriver(san.SanDriver):
    ...
    def _get_vol_name(volume):
        ...
        return volume["name"] + "-storage"

    @staticmethod
    def _get_snap_name(snapshot):
        """Return the name of the snapshot that Purity will use."""
        return "%s-storage.%s" % (snapshot["volume_name"], snapshot["name"])

    @staticmethod
    def _get_pgroup_name_from_id(id):
        return "consisgroup-%s-storage" % id

    @staticmethod
    def _get_pgroup_snap_suffix(cgsnapshot):
        return "cgsnapshot-%s-storage" % cgsnapshot.id
    ...
    def _get_pgroup_snap_name_from_snapshot(self, snapshot):
        ...
        pg_vol_snap_name = "%(group_snap)s.%(volume_name)s-storage" % {
    ...
    def _generate_purity_host_name(name):
        ...
        name = name.lstrip("-")
        return "{name}-{uuid}-storage".format(name=name, uuid=uuid.uuid4().hex)
