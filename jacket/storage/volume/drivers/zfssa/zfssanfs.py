...
from jacket.storage.volume.drivers.zfssa import zfssarest
...
ZFSSA_OPTS = [
    ...
    cfg.StrOpt('zfssa_cache_directory', default='os-storage-cache',
...
class ZFSSANFSDriver(nfs.NfsDriver):
    ...
    def do_setup(self, context):
        ...
        for cred in credentials:
            if not getattr(lcfg, cred, None):
                exception_msg = _('%s not set in storage.conf') % cred
    ...
    def _create_snapshot_name(self):
        ...
        return ('storage-zfssa-nfs-snapshot-%s' %
    ...
    def _verify_volume_to_manage(self, name, vol_props):
        ...
        if vol_props['cinder_managed'] == "":
            err_msg = (_("Unknown if the volume: %s to be managed is "
                         ...
                         "Alternatively, Set the value of storage config "
    ...
    def unmanage(self, volume):
        """Remove an existing volume from storage management.
