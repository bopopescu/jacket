...
from jacket.storage.volume.drivers.emc import emc_vmax_fast
from jacket.storage.volume.drivers.emc import emc_vmax_https
from jacket.storage.volume.drivers.emc import emc_vmax_masking
from jacket.storage.volume.drivers.emc import emc_vmax_provision
from jacket.storage.volume.drivers.emc import emc_vmax_provision_v3
from jacket.storage.volume.drivers.emc import emc_vmax_utils
...
CINDER_EMC_CONFIG_FILE = '/etc/storage/cinder_emc_config.xml'
CINDER_EMC_CONFIG_FILE_PREFIX = '/etc/storage/cinder_emc_config_'
...
emc_opts = [
    cfg.StrOpt('cinder_emc_config_file',
               ...
               help='use this file for storage emc plugin '
                    'config data'), ]
...
class EMCVMAXCommon(object):
    ...
    def migrate_volume(self, ctxt, volume, host, new_type=None):
        ...
        LOG.warning(_LW("The VMAX plugin only supports Retype. "
                        ...
                        "storage --os-volume-api-version 2 retype <volumeId> "
    ...
    def manage_existing(self, volume, external_ref):
        ...
        for sgInstanceName in sgInstanceNames:
            ...
            if mvInstanceName:
                exceptionMessage = (_(
                    "Unable to import volume %(deviceId)s to storage. "
        ...
        LOG.debug("Storage pool of existing volume: %(volPool)s, "
                  "Storage pool currently managed by storage: %(cinderPool)s.",
                  {'volPool': volumePoolName,
                   'cinderPool': cinderPoolName})
        if volumePoolName != cinderPoolName:
            exceptionMessage = (_(
                "Unable to import volume %(deviceId)s to storage. The external "
                "volume is not in the pool managed by current storage host.")
                % {'deviceId': deviceId})
            LOG.error(exceptionMessage)
            raise exception.VolumeBackendAPIException(
                data=exceptionMessage)
