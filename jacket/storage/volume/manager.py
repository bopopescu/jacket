...
:volume_topic:  What :mod:`rpc` topic to listen to (default: `storage-volume`).
:volume_manager:  The module name of a class derived from
                  :class:`manager.Manager` (default:
                  :class:`storage.volume.manager.Manager`).
:volume_driver:  Used by :class:`Manager`.  Defaults to
                 :class:`storage.volume.drivers.lvm.LVMVolumeDriver`.
...
from jacket.storage.volume import configuration as config
...
class VolumeManager(manager.Manager):
        ...
        try:
            ...
            if not rescheduled:
                if not vol_ref:
                    ...
                    vol_ref = storage.Volume.get_by_id(context, volume.id)
    ...
    def _clone_image_volume(self, ctx, volume, image_meta):
        ...
        try:
            ...
            image_volume = storage.Volume(context=ctx, **new_vol_values)
            image_volume.create()
        except Exception as ex:
    ...
    def _migrate_volume_generic(self, ctxt, volume, host, new_type_id):
        ...
        new_volume = storage.Volume.get_by_id(ctxt, new_volume.id)
        tries = 0
    ...
    def migrate_volume_completion(self, ctxt, volume_id, new_volume_id,
        ...
        if volume is None or new_volume is None:
            ...
            new_volume = storage.Volume.get_by_id(ctxt, new_volume_id)
    ...
    def retype(self, ctxt, volume_id, new_type_id, host,
        ...
        if not retyped:
            ...
            snaps = storage.SnapshotList.get_all_for_volume(context,
                                                            volume.id)
            if snaps:
                _retype_error(context, volume, old_reservations,
                              new_reservations, status_update)
        ...
        try:
            ...
            if snapshots_model_update:
                for snap_model in snapshots_model_update:
                    ...
                    # NOTE(xyang): snapshots is a list of snapshot storage.
        ...
        for snapshot in snapshots:
            ...
            if vol_ref.bootable:
                try:
                    ...
                    # after cgsnapshot-storage bugs are fixed
    ...
    def update_migrated_volume(self, ctxt, volume, new_volume,
        ...
        # FIXME(thangp): Remove this in v2.0 of RPC API.
        if (not isinstance(volume, storage.Volume) or
                not isinstance(new_volume, storage.Volume)):
            volume = storage.Volume.get_by_id(ctxt, volume['id'])
            new_volume = storage.Volume.get_by_id(ctxt, new_volume['id'])
    ...
    def failover_host(self, context,
        ...
        service = storage.Service.get_by_args(
            ...
            svc_host,
            'storage-volume')
        volumes = storage.VolumeList.get_all_by_host(context, self.host)
        ...
        for update in volume_update_list:
            # Response must include an id key: {volume_id: <storage-uuid>}
            ...
            #  provider_id
            #  replication_status
            vobj = storage.Volume.get_by_id(context, update['volume_id'])
