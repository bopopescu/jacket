...
volume_opts = [
    ...
    cfg.BoolOpt('use_multipath_for_image_xfer',
                default=False,
                help='Do we attach/detach volumes in storage using multipath '
    ...
    cfg.BoolOpt('image_upload_use_cinder_backend',
                ...
                help='If set to True, upload-to-image in raw format will '
                     ...
                     'content. The storage backend and locations support '
...
class BaseVD(object):
    ...
    def init_capabilities(self):
        ...
        This properties part is consisted of storage standard
        ...
        # Set storage standard capabilities
    ...
    def get_filter_function(self):
        ...
        in storage.conf. If nothing is specified in storage.conf, then try to
    ...
    def _get_backup_volume_temp_volume(self, context, backup):
        ...
        back it up.
        """
        volume = storage.Volume.get_by_id(context, backup.volume_id)
        snapshot = None
        if backup.snapshot_id:
            snapshot = storage.Snapshot.get_by_id(context, backup.snapshot_id)
    ...
    def _create_temp_snapshot(self, context, volume):
        ...
        temp_snap_ref = storage.Snapshot(context=context, **kwargs)
    ...
    def initialize_connection(self, volume, connector, initiator_data=None):
        ...
        storage with key-value pairs that have been saved for this initiator by
    ...
    def failover_host(self, context, volumes, secondary_id=None):
        ...
        *Key things to consider (attaching failed-over volumes):
        ...
        :param context: security context
        :param volumes: list of volume storage, in case the driver needs
...
class ManageableVD(object):
    ...
    def manage_existing(self, volume, existing_ref):
        ...
        that the driver should somehow associate with the newly-created storage
        volume structure.
        ...
        1. Rename the backend storage object so that it matches the,
           ...
           storage volume and the associated backend storage object.
...
class ManageableSnapshotsVD(object):
    ...
    def manage_existing_snapshot(self, snapshot, existing_ref):
        ...
        1. Rename the backend storage object so that it matches the
           ...
           storage snapshot and the associated backend storage object.
