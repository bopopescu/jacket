...
def check_already_managed_volume(storage, vol_name):
    """Check storage storage for already managed volume.

    :param storage: database api parameter
    :param vol_name: volume name parameter
    :returns: bool -- return True, if storage entry with specified
                      volume name exist, otherwise return False
    """
    vol_id = _extract_id(vol_name)
    try:
        if vol_id and uuid.UUID(vol_id, version=4):
            storage.volume_get(context.get_admin_context(), vol_id)
            return True
