...
Guidelines for writing new hacking checks
 ...
 - Add test cases for each new rule to
   storage/tests/test_hacking.py

"""

# NOTE(thangp): Ignore N323 pep8 error caused by importing storage objects
UNDERSCORE_IMPORT_FILES = ['storage/objects/__init__.py']
...
def check_no_print_statements(logical_line, filename, noqa):
    # The files in storage/cmd do need to use 'print()' so
    # we don't need to check those files.  Other exemptions
    # should use '# noqa' to avoid failing here.
    if "storage/cmd" not in filename and not noqa:
...
def no_test_log(logical_line, filename, noqa):
    if "storage/tests" not in filename or noqa:
        return
    # Skip the "integrated" tests for now
    if "storage/tests/unit/integrated" in filename:
        return
